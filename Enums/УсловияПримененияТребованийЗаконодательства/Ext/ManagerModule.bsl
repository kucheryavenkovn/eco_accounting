...
Функция УсловиеВыполняется(Условие, Период, Периодичность, Организация) Экспорт
	...
	Если Условие = РегистрируютсяКадровыеМероприятия Тогда
	...
	Если Условие = ЕстьЛьготыПоТранспортномуЗемельномуНалогу Тогда
		Возврат ЕстьЛьготыПоТранспортномуНалогу(Организация, Период, Периодичность)
			ИЛИ ЕстьЛьготыПоЗемельномуНалогу(Организация, Период, Периодичность);
	КонецЕсли;
	...
	ИначеЕсли Условие = АкцизыФормаНефтяноеСырье Тогда
	...
	Если Условие = НекоммерческиеОрганизации Тогда
...
Функция ЕстьЛьготыПоТранспортномуНалогу(Организация, Период, Периодичность)
...
Функция ЕстьЛьготыПоЗемельномуНалогу(Организация, Период, Периодичность)
	...
	|	И (НЕ РегистрацияЗемельныхУчастков.НалоговаяЛьготаПоНалоговойБазе В (ЗНАЧЕНИЕ(Перечисление.ВидНалоговойЛьготыПоНалоговойБазеПоЗемельномуНалогу.ПустаяССылка), ЗНАЧЕНИЕ(Перечисление.ВидНалоговойЛьготыПоНалоговойБазеПоЗемельномуНалогу.НеПрименяется))
	|			ИЛИ РегистрацияЗемельныхУчастков.НеОблагаемаяНалогомСумма > 0
	|			ИЛИ РегистрацияЗемельныхУчастков.СниженнаяНалоговаяСтавка > 0
	|			ИЛИ РегистрацияЗемельныхУчастков.ПроцентУменьшенияСуммыНалога > 0
