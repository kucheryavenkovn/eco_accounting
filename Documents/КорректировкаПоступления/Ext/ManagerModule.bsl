...
Функция ТекстЗапросаВременныеТаблицыДокументаРасчетыВВалюте(НомераТаблиц, Реквизиты)
	...
	Если Реквизиты.ЕстьТовары Тогда
		...
		|	ТаблицаТовары.СпособУчетаНДС КАК СпособУчетаНДС,
		|	ТаблицаТовары.ОтражениеВУСН КАК ОтражениеВУСН,
		|	ТаблицаПосчитанныхРублевыхСумм.СуммаБезНДСРуб / ВЫБОР
		|		КОГДА ТаблицаТовары.Количество = 0
		|			ТОГДА 1
		|		ИНАЧЕ ТаблицаТовары.Количество
		|	КОНЕЦ КАК ЦенаБезНДС,
		...
		|	ТаблицаТовары.ЕстьВДокументеПоступления КАК ЕстьВДокументеПоступления,
		|	ВЫБОР
		|		КОГДА ТаблицаПосчитанныхРублевыхСумм.СуммаБезНДСРубДоИзменения <> 0
		|			ТОГДА ТаблицаПосчитанныхРублевыхСумм.СуммаБезНДСРубДоИзменения / ВЫБОР
		|					КОГДА ТаблицаТовары.КоличествоДоИзменения = 0
		|						ТОГДА 1
		|					ИНАЧЕ ТаблицаТовары.КоличествоДоИзменения
		|				КОНЕЦ
		|		ИНАЧЕ ТаблицаПосчитанныхРублевыхСумм.СуммаБезНДСРубДоКорректировки / ВЫБОР
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого Строка Из СтрокиДокумента Цикл
			...
			Если КурсВалюты <> Неопределено Тогда
				Если Строка.СуществуютЗаписиРублевыхСумм 
