...
Функция ПечатьАктовОбОказанииУслуг()
	...
	Пока Выборка.Следующий() Цикл
		...
		Руководители = ОбщегоНазначения.ОтветственныеЛица(Выборка.Организация, Выборка.Дата, Подразделение);
		
		Если ЗначениеЗаполнено(Исполнитель) И Исполнитель <> Руководители.Руководитель Тогда 
			ДанныеПодотчетногоЛица = ОбщегоНазначения.ДанныеФизЛица(Выборка.Организация, Исполнитель, Выборка.Дата);
			Если ЗначениеЗаполнено(ДанныеПодотчетногоЛица.Должность) Тогда 
				ДолжностьИНазваниеОрганизации = "" + ДанныеПодотчетногоЛица.Должность + " " + ПредставлениеПоставщика;
			Иначе
		...
		Иначе 
			ЗаместительПоПриказу = Руководители.РуководительПредставление;
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			Если УчетНДС.ПолучитьВерсиюПостановления(Дата) = 2 Тогда
				ТабДокументСФ = ОбъектСФ.ПечатьСчетаФактуры1137(ДанныеДляПечати);
			Иначе
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь, НепосредственнаяПечать = Ложь) Экспорт
	...
	ИначеЕсли ИмяМакета = "СчетаФактуры" Тогда
		...
		Если ТабДокумент.ВысотаТаблицы = 0 Тогда
			Возврат;
		КонецЕсли;
	ИначеЕсли ИмяМакета = "СФ_ТОРГ12_Счет" Тогда
		
		Для каждого Строка из ЭтотОбъект.Контрагенты цикл
			Если Строка.ВыданСФ = Истина Тогда
				ТабДокумент = Строка;
				...
				//УниверсальныеМеханизмы.НапечататьДокумент(Строка.СчетНаОплату, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект, ЭтотОбъект.Метаданные().Представление()), НепосредственнаяПечать);
				Возврат;
		КонецЕсли;
	КонецЦикла;
		
	КонецЕсли;
	//Условие печать по флажкам
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект, ЭтотОбъект.Метаданные().Представление()), НепосредственнаяПечать);
			//УниверсальныеМеханизмы.НапечататьДокумент(Строка.СчетНаОплату, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект, ЭтотОбъект.Метаданные().Представление()), НепосредственнаяПечать);
...
Процедура ДвиженияРегистровПодсистемыНДС(СтруктураШапкиДокумента, ТаблицаНачислений, ТаблицаВзаиморасчетов, Отказ, Заголовок)
	...
	Если СтруктураШапкиДокумента.ОрганизацияПрименяетУСН
		ИЛИ СтруктураШапкиДокумента.ОрганизацияПрименяетОсобыйПорядокНалогообложения Тогда
	...
	Если СтруктураШапкиДокумента.УчитыватьНДС Тогда
		...
		УчетНДСФормированиеДвижений.СформироватьДвиженияПоРегиструНДСПродажи(СтруктураШапкиДокумента, ТаблицаНачислений, Движения, Отказ,,,, ТаблицаВзаиморасчетовДляНДС);
...
Процедура ДвиженияПоРегистрам(СтруктураШапкиДокумента, ТаблицаНачислений, Отказ, Заголовок)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНачислений Цикл
		...
		КопияСтруктурыШапкиДокумента = Новый Структура("Ссылка,ВидДокумента,Дата,Организация,ПодразделениеОрганизации,СпособЗачетаАвансов,
										|ВалютаРегламентированногоУчета,ОтражатьВНалоговомУчете,ПрименениеПБУ18,
										|ОрганизацияПрименяетУСН,ОрганизацияПрименяетОсобыйПорядокНалогообложения,
										|ОтражатьВНалоговомУчетеУСНПатент,ОтражатьВНалоговомУчетеУСНДоходы,ОтражатьВНалоговомУчетеУСНПатент");
	...
	ДоходыИРасходыПредпринимателя.ЗачетОплатыПокупателя(ЭтотОбъект, СтруктураШапкиДокумента, Движения.ИПМПЗОтгруженные.Выгрузить(), ТаблицаВзаиморасчетовСводная);
	// }УчетДоходовИРасходовИП
...
Процедура ДвиженияПоРегистрамУСНРегл(СтруктураШапкиДокумента, ТаблицаВзаиморасчетов, Отказ, Заголовок)
	...
	Если НЕ СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСН
		И НЕ СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСНДоходы
		И НЕ СтруктураШапкиДокумента.ОтражатьВНалоговомУчетеУСНПатент Тогда
