...
Процедура ДеревоОтчетовПередУдалением(Элемент, Отказ)
	...
	Для каждого ВыделеннаяСтрока Из Элементы.ДеревоОтчетов.ВыделенныеСтроки Цикл
		...
		Если ПорядокПроверкиЗаполнения = "ВыдаватьОшибку" Тогда
		...
		Для каждого ПодчиненнаяСтрока Из ПодчиненныеСтроки Цикл
			Если ЗначениеЗаполнено(ПодчиненнаяСтрока.ПроверкаЗаполнения) Тогда
				ПорядокПроверкиЗаполнения = ПодчиненнаяСтрока.ПроверкаЗаполнения;
				Если ПорядокПроверкиЗаполнения = "ВыдаватьОшибку" Тогда
	...
	Если ОбязательныеСтроки.Количество() > 0 Тогда
		Если ОбязательныеСтроки.Количество() = 1 Тогда
			НачалоТекстаПредупреждения = СтрШаблон(
				НСтр("ru = 'По требованию %1 отчет ""%2"" представляется обязательно.'"),
				Объект.Банк,
				ПолноеПредставлениеОтчета(ОбязательныеСтроки[0]));
		ИначеЕсли ОбязательныеСтроки.Количество() = 2 Тогда
			НачалоТекстаПредупреждения = СтрШаблон(
				НСтр("ru = 'По требованию %1 отчеты ""%2"" и ""%3"" представляются обязательно.'"),
				Объект.Банк,
				ПолноеПредставлениеОтчета(ОбязательныеСтроки[0]),
		...
		Иначе
			...
			НачалоТекстаПредупреждения = СтрШаблон(
				НСтр("ru = 'По требованию %1 отчет ""%2"" и еще %3 представляются обязательно.'"),
				Объект.Банк,
				ПолноеПредставлениеОтчета(ОбязательныеСтроки[0]),
				СтрокаПрописью);
		КонецЕсли;
		...
		ТекстПредупреждения = СтрШаблон(
			НСтр("ru = '%1
						|
						|В тех случаях, когда запрашиваемые отчеты не применимы для %2, необходимо выбрать пункт ""Данные отсутствуют"" в контекстном меню гиперссылки Заполнить/Выбрать.'"),
...
Процедура ВыполнитьМассовуюОперацию(ИмяОперации, ДополнительныеПараметры = Неопределено)
	...
	ИначеЕсли Модифицированность Или ДеревоОтчетовИзменено Тогда
		...
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да, ИмяОперации);
		Возврат;
...
Процедура ЗагрузитьПроверка(ПомещаемыеФайлы, ОтказОтПомещенияФайла, ДополнительныеПараметры) Экспорт
	...
	Если ПомещаемыеФайлы.Файл = Неопределено Тогда // свериться с протоколом в архиве, разархивировав на сервере
		Возврат;
	КонецЕсли;
...
Процедура СформироватьЗаголовок()
	...
	Заголовок = СтрШаблон(НСтр("ru = 'Отчет для %1 за %2 (%3)'"),
		Объект.Банк,
		ЗаполнениеФинОтчетностиВБанкиКлиентСервер.ПредставлениеПериодаОтчета(НачалоМесяца(Объект.Дата), КонецМесяца(Объект.Дата)),
		Объект.Организация);
...
Функция РезультатФоновогоЗадания(Результат, СохранитьАдресРезультата = Ложь)
	...
	Если Не РезультатДляКлиента.Выполнено Тогда
		...
		Если РезультатДляКлиента.ОтчетностьПоКлючамСтрок.Количество() = 0 Тогда
...
Функция ПараметрыОтбораКриптографии(ПараметрыПодписания)
	...
	ПараметрыОтбора = УниверсальныйОбменСБанкамиКлиентСервер.ПараметрыОтбораСертификата(
						ПараметрыПодписания.Сервис, ПараметрыПодписания.Организация);
	...
	РеквизитыСубъекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СубъектОтбора,
							"Наименование,ИНН,ЮридическоеФизическоеЛицо", Истина);
	ПараметрыОтбора.Вставить("ЮридическоеФизическоеЛицо", РеквизитыСубъекта.ЮридическоеФизическоеЛицо);
	ПараметрыОтбора.Вставить("ИНН", РеквизитыСубъекта.ИНН);
	
	ПараметрыОтбора.ПредставлениеОтбора = СтрШаблон(НСтр("ru = '%1 (ИНН %2)'"),
		РеквизитыСубъекта.Наименование, СокрЛП(РеквизитыСубъекта.ИНН));
...
Функция ПараметрыВыбораФайла(ИдентификаторСтрокиДерева)
	...
	Если Результат = Неопределено Тогда
		...
		Результат = ЗаполнениеФинОтчетностиВБанки.НовыеПараметрыВыбораФайла();
		Результат.Расширение = "." + ЗаполнениеФинОтчетностиВБанкиКлиентСервер.РасширениеТабличногоДокумента();
		Результат.ФильтрДиалогаВыбора = НСтр("ru = 'Книга Excel (*.xls; *.xlsx)|*.xls; *.xlsx|Документ Adobe PDF (*.pdf)|*.pdf|
		...
		Результат.МаксРазмер = 0; // Используем ограничение по умолчанию, установленное БСП.
	КонецЕсли;
...
Процедура ЗаполнитьДобавленныеКолонкиЭкономическиеСубъекты(Форма)
	...
	Для каждого СтрокаТаблицы Из Объект.ЭкономическиеСубъекты Цикл
		СтрокаТаблицы.Наименование = СформироватьНаименованиеСубъекта(
...
&НаКлиентеНаСервереБезКонтекста
Функция СформироватьНаименованиеСубъекта(Субъект, ВключатьОбособленныеПодразделения)
...
Функция ДобавитьНовыйЭкономическийСубъект(Субъект, ВключатьОбособленныеПодразделения)
	...
	НоваяСтрока.Наименование       = СформироватьНаименованиеСубъекта(Субъект, ВключатьОбособленныеПодразделения);
...
Процедура ЗаполнитьДеревоОтчетов(СтрокаСубъекта)
	...
	Для каждого СтрокаВидаОтчетов Из ОписаниеВидовОтчетов Цикл
		...
		Если ГруппаОтчета <> Неопределено Тогда
			ГруппаВидаОтчета = ГруппаОтчета.Строки.Добавить();
		Иначе
			ГруппаВидаОтчета = ПолноеДеревоОтчетов.Строки.Добавить();
		КонецЕсли;
		...
		Для каждого СтрокаОтчета Из СтрокаВидаОтчетов.ПодготовленныеОтчеты Цикл
			...
			// 3. Идентификатор строки комплекта (наименование отчета, как он называется в требованиях банка - для произвольных отчетов/файлов)
			Если ЭтоПроизвольныйОтчет И ЗначениеЗаполнено(СтрокаОтчета.ИдентификаторСтрокиКомплекта) Тогда
				Если ГруппаПоИдентификаторуНастройки = Неопределено
					ИЛИ ГруппаПоИдентификаторуНастройки.ИдентификаторСтрокиКомплекта <> СтрокаОтчета.ИдентификаторСтрокиКомплекта Тогда
					ГруппаПоИдентификаторуНастройки = ГруппаВидаОтчета.Строки.Найти(
						СтрокаОтчета.ИдентификаторСтрокиКомплекта, "ИдентификаторСтрокиКомплекта");
				КонецЕсли;
				...
				Если ГруппаПоИдентификаторуНастройки = Неопределено Тогда
					...
					ГруппаПоИдентификаторуНастройки.ИдентификаторСтрокиКомплекта = СтрокаОтчета.ИдентификаторСтрокиКомплекта;
				КонецЕсли;
			Иначе
...
Процедура ПеренестиВДеревоОтчетовНаФорме(СтрокиПолногоДерева, РодительскаяСтрокаДереваНаФорме)
	...
	Для каждого СтрокаПолногоДерева Из СтрокиПолногоДерева Цикл
		...
		Если СтрокаПолногоДерева.Строки.Количество() > 1 ИЛИ СтрокаПолногоДерева.Строки.Количество() = 0 Тогда
...
Процедура СформироватьПредставлениеОтчета(СтрокаДереваОтчетов, НастройкиКомплекта, ОписаниеВидовОтчетов)
	...
	Если ЗначениеЗаполнено(СтрокаДереваОтчетов.ИдентификаторСтрокиКомплекта) Тогда
		Если ПолеУказаноВРодительскойСтроке(СтрокаДереваОтчетов, "ИдентификаторСтрокиКомплекта", СтрокаДереваОтчетов.ИдентификаторСтрокиКомплекта) Тогда
	...
	Если Подстроки.Количество() = 0 Тогда
		Если НЕ ЗначениеЗаполнено(СтрокаДереваОтчетов.ВидОтчета) Тогда
			// Это верхнеуровневая группа из разных видов отчетов, ее представление оставляем как есть.
			Возврат;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(СтрокаДереваОтчетов.ДатаНачала) И ЗначениеЗаполнено(СтрокаДереваОтчетов.ДатаОкончания) Тогда
		...
		Иначе
			...
			Если СтрокаВидаОтчета <> Неопределено И СтрокаВидаОтчета.ОтчетНаДату Тогда
				Подстроки.Добавить(СтрШаблон(НСтр("ru = 'на %1'"),
					Формат(СтрокаДереваОтчетов.ДатаОкончания, "ДФ=dd.MM.yyyy")));
			Иначе
				Подстроки.Добавить(СтрШаблон(НСтр("ru = 'за %1'"),
	...
	Если ЭтоПроизвольныйФайл Тогда
...
Функция ОтчетЗаполнен(СтрокаДереваОтчетов)
	...
	Возврат СтрокаДереваОтчетов.ДанныеОтсутствуют // Явно указано, что данных нет и не будет.
		ИЛИ ЗначениеЗаполнено(СтрокаДереваОтчетов.ОтчетСсылка) // Указан регламентированный отчет.
...
Процедура ПроверитьСостояниеСдачиЧерез1СОтчетность(ИдентификаторИлиСтрокаДереваОтчетов)
	...
	// Для регламентированных отчетов определяем статус сдачи через 1С-Отчетность.
	Если ЗаполнениеФинОтчетностиВБанкиКлиентСервер.ЭтоОтчетБРО(СтрокаДереваОтчетов.ВидОтчета)
		...
		СтрокаДереваОтчетов.СданЧерез1СОтчетность = СостояниеРеглОтчета.Отправлено И СостояниеРеглОтчета.Сдано;
...
&НаКлиентеНаСервереБезКонтекста
Функция ДействиеДанныеОтсутствуют()
...
Процедура СформироватьДоступныеДействия(Форма, СтрокаДереваОтчетов)
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДереваОтчетов.КлючСтрокиОтчета) Тогда
		// Это группа отчетов.
		СтрокаДереваОтчетов.ДействиеСОтчетом         = "";
		СтрокаДереваОтчетов.ДействиеПодписать        = "";
		СтрокаДереваОтчетов.ДоступноДействиеОчистить = Ложь;

	ИначеЕсли СтрокаДереваОтчетов.ДанныеОтсутствуют Тогда
		СтрокаДереваОтчетов.ДействиеСОтчетом         = ДействиеДанныеОтсутствуют();
		СтрокаДереваОтчетов.ДействиеПодписать        = "";
	...
	ИначеЕсли СтрокаДереваОтчетов.ОтчетЗаполнен Тогда
		...
		Если ЗначениеЗаполнено(СтрокаДереваОтчетов.ОтчетСсылка) Тогда
			ОтчетностьПодписана = СтрокаДереваОтчетов.СданЧерез1СОтчетность // Регламентированный отчет сдан с помощью 1С-Отчетности, есть отметка ФНС о приеме.
		...
		Иначе
		 	ОтчетностьПодписана = СтрокаДереваОтчетов.ФайлОтчетаСодержитЭП // В присоединенном ФайлеОтчета содержится архив, внутри которого есть, в том числе, файлы подписи.
				Или СтрокаДереваОтчетов.ПодписанЭП; // Электронная подпись прикреплена к присоединенному ФайлуОтчета в регистре БСП ЭлектронныеПодписи.
		КонецЕсли;
		СтрокаДереваОтчетов.ДействиеПодписать = ?(ОтчетностьПодписана, ДействиеПодписано(), ДействиеПодписать());
...
Процедура ОбработатьДействиеСОтчетом(СтрокаДереваОтчетов, ИмяДействия)
	...
	ИначеЕсли ИмяДействия = ДействиеЗаполнить()
		ИЛИ ИмяДействия = ДействиеВыбрать()
		ИЛИ ИмяДействия = ДействиеДанныеОтсутствуют() Тогда
...
Процедура ОбработатьДействиеОписание(СтрокаДереваОтчетов)
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыФормы.ПодробноеОписание) Тогда
		Возврат;
	КонецЕсли;
	
	ОткрытьФорму("Документ.ФинОтчетВБанк.Форма.ФормаОписаниеОтчета", ПараметрыФормы, ЭтотОбъект);
...
Функция ПараметрыФормыОписания(ИдентификаторСтроки)
	...
	Результат.Вставить("Банк",              Объект.Банк);
	...
	Если СтрокаНастройки <> Неопределено Тогда
		Результат.Заголовок         = СтрокаНастройки.КраткоеПредставление;
		Результат.ПодробноеОписание = СтрокаНастройки.ПодробноеПредставление;
		Результат.ИмяФайлаШаблона   = СтрокаНастройки.ИмяФайлаШаблона;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Результат.Заголовок) И ЗначениеЗаполнено(СтрокаДереваОтчетов.ВидОтчета) Тогда
		ОписаниеВидовОтчетов = ЗаполнениеФинОтчетностиВБанки.ОписаниеВидовОтчетов();
		СтрокаВидаОтчета = НайтиОписаниеПоВидуОтчета(СтрокаДереваОтчетов.ВидОтчета, ОписаниеВидовОтчетов);
		Если СтрокаВидаОтчета <> Неопределено Тогда
			Результат.Заголовок = СтрокаВидаОтчета.ПредставлениеОтчета;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбработатьДействиеВыбратьОтчетБРО(СтрокаДереваОтчетов)
	...
	СписокВариантов.Добавить("ДанныеОтсутствуют", НСтр("ru = 'Данные отсутствуют'"));
...
Процедура ОбработатьДействиеВыбратьОтчетБРО_ПослеВыбораВарианта(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ВыбранныйЭлемент.Значение = "ДанныеОтсутствуют" Тогда
		
		ОчиститьДанныеСтрокиДерева(ДополнительныеПараметры.ИдентификаторСтроки, Истина);
...
Функция НесоответствияДляВсехОтчетов(РезультатЗакрытия, СтрокаОтчета, СтрокаСубъекта)
	...
	Если СтрокаСубъекта.Субъект <> РезультатЗакрытия.Организация Или ОбособленныеИзменились Тогда
	...
	Если ЗначениеЗаполнено(СтрокаОтчета.ДатаНачала) И ЗначениеЗаполнено(СтрокаОтчета.ДатаОкончания) Тогда
		Если РезультатЗакрытия.Свойство("Период") Тогда
...
Процедура ПослеЗакрытияОтчета(РезультатЗакрытия, ДополнительныеПараметры)
	...
	ЗаписатьОтчетИСохранитьРезультатВПрисоединенныеФайлы(РезультатЗакрытия.Результат, СтрокаОтчета.КлючСтрокиОтчета);
	
КонецПроцедуры

&НаСервере
Процедура ЗаписатьОтчетИСохранитьРезультатВПрисоединенныеФайлы(Результат, КлючСтрокиОтчета)
	
	АдресХранилища = ПоместитьВоВременноеХранилище(Результат, УникальныйИдентификатор);
	
	СохранитьОтчетВПрисоединенныйФайл(КлючСтрокиОтчета, АдресХранилища);
...
Процедура ОткрытьОтчетВаловаяПрибыль(СтрокаДереваОтчетов)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗакрытияВаловаяПрибыль", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ОткрытьОтчетВедомостьАмортизацииОС(СтрокаДереваОтчетов)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗакрытияВедомостьАмортизацииОС", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ОткрытьОтчетРасшифровкаЗадолженности(СтрокаДереваОтчетов)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗакрытияРасшифровкаЗадолженности", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ОткрытьОтчетКассоваяКнига(СтрокаДереваОтчетов)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗакрытияКассоваяКнига", ЭтотОбъект, ДополнительныеПараметры);
...
Функция ПараметрыФормыКнигаУчетаДоходовИРасходовПредпринимателя(ИдентификаторСтроки)
	...
	ПараметрыОтчетаОсновные = ЗаполнениеФинОтчетностиВБанки.ПараметрыОтчетаКнигаУчетаДоходовИРасходовПредпринимателя(
		НастройкиОтчета);
	ЗаполнитьОбщиеПараметрыОтчета(ПараметрыОтчетаОсновные, СтрокаДереваОтчетов);
	
	Документы.ФинОтчетВБанк.ЗаполнитьПараметрыПоКлючу(
		Объект.ПараметрыОтчетов, ПараметрыОтчетаОсновные, СтрокаДереваОтчетов.КлючСтрокиОтчета);
	
	ПараметрыОтчетаОсновные.Вставить("СписокТаблиц"); // чтобы не заполнялась Таблица значений, не поддерживаемая на Клиенте
	ПараметрыОтчета = Отчеты.КнигаУчетаДоходовИРасходовПредпринимателя.ЗаполнитьПараметрыОтчета(ПараметрыОтчетаОсновные);
	Документы.ФинОтчетВБанк.ПроверитьУстановитьРасширение(
		ПараметрыОтчета, СтрокаДереваОтчетов.ИдентификаторСтрокиКомплекта, НастройкиКомплекта);
...
Процедура ОткрытьДиалогВыбораФайлаСДиска(СтрокаДереваОтчетов = Неопределено)
	...
	ПараметрыДобавления.Вставить("МаксимальныйРазмер",  ПараметрыВыбораФайла.МаксРазмер / 1024);
...
&НаСервере
Функция СохранитьОтчетВПрисоединенныйФайл(КлючСтрокиОтчета, АдресХранилища)
	...
	ИмяФайлаБезРасширения = Документы.ФинОтчетВБанк.ИмяФайлаБезРасширенияПоШаблону(
		РеквизитыСубъекта.ИНН,
		РеквизитыСубъекта.КПП,
		СтрокаВидаОтчетов.ШаблонИмениФайла,
		СтрокаОтчета.УточнениеНаименования,
		СтрокаОтчета.ДатаНачала,
		СтрокаОтчета.ДатаОкончания);
...
Процедура ОткрытьДиалогВыбораПрисоединенногоФайла(СтрокаДереваОтчетов = Неопределено)
	...
	Иначе
		ИдентификаторСтроки = -1; // Признак того, что необходимо добавить новую строку в дерево после выбора файла.
		ЗаголовокДиалога    = НСтр("ru = 'присоединенный файл'");	
	КонецЕсли;
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ВыборПрисоединенногоФайлаЗавершение", ЭтотОбъект, ДополнительныеПараметры);
	
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("Заголовок", СтрШаблон(НСтр("ru='Выберите %1'"), ЗаголовокДиалога));
	ПараметрыФормы.Вставить("МножественныйВыбор", Истина);
	ПараметрыФормы.Вставить("МаксимальныйРазмер", ПараметрыВыбораФайла.МаксРазмер);
	ПараметрыФормы.Вставить("ФильтрДиалогаВыбора", ПараметрыВыбораФайла.ФильтрДиалогаВыбора);
	
	ОткрытьФорму("Документ.ФинОтчетВБанк.Форма.ФормаВыборПрисоединенногоФайла", ПараметрыФормы, ЭтотОбъект,,,, ОписаниеОповещения);
...
Процедура ОбработкаОповещенияИзменениеПрисоединенногоФайла(ИзмененныйФайл)
	...
	Для каждого СтрокаДерева Из СтрокиДерева Цикл
		Если СтрокаДерева.ФайлОтчета = ИзмененныйФайл Тогда
...
Процедура ПодписатьФинОтчет(ДополнительныеПараметры)
	...
	Если Не ЗначениеЗаполнено(ПараметрыОтбора.Организация) Тогда
		ФинОтчетностьВБанкиКлиент.
			ПоказатьОшибку(НСтр("ru = 'Субъект, подписывающий отчетность, не определен'"), ЭтотОбъект);
		Возврат;
	КонецЕсли;

	ПараметрыКриптографии = Новый Структура;
	ПараметрыКриптографии.Вставить("ПараметрыОтбора", ПараметрыОтбора);
	ПараметрыКриптографии.Вставить("Заголовок",
		СтрШаблон(НСтр("ru = 'Сертификат для %1'"), ПараметрыОтбора.ПредставлениеОтбора));
	...
	УниверсальныйОбменСБанкамиКлиент.ПараметрыКриптографии(
		ПараметрыПодписания.Сервис,
		ПараметрыПодписания.Организация,
		ОповещениеОВыбореСертификата,
		ЭтотОбъект,
...
Функция ПараметрыПодписания()
	...
	Возврат Документы.ФинОтчетВБанк.ПараметрыПодписания(Объект, КлючСтрокиСубъекта, НастройкиКомплекта());
...
Функция СостояниеПередВыгрузкой()
	...
	Для каждого Отчет Из Объект.Отчетность Цикл
		...
		Если Не Отчет.ДанныеОтсутствуют
		   И Не ЗначениеЗаполнено(Отчет.ОтчетСсылка)
...
Процедура ВыгрузитьФинОтчет(КраткоеПредставление)
	...
	ПараметрыВыгрузки.Вставить("ФинОтчет",             Объект.Ссылка);
	ПараметрыВыгрузки.Вставить("КраткоеПредставление", КраткоеПредставление);
	
	// Строка - XDTO-пакет ФинОтчетностьВБанки, тип ФорматыСохраненияОтчетов - в каком формате выгружать регл. отчеты.
	// Для оптимизации значение будет заполнено уже на сервере перед запуском фонового задания.
	ПараметрыВыгрузки.Вставить("ФорматыСохранения", Новый Соответствие);
	
	ДлительнаяОперация = НачатьВыгрузкуВАрхив(УникальныйИдентификатор, ПараметрыВыгрузки);
...
&НаСервере
Функция НачатьВыгрузкуВАрхив(Знач УникальныйИдентификаторФормы, Знач ПараметрыВыгрузки)
	
	// Получаем из настроек как выгружать регламентированные отчеты.
	ПараметрыВыгрузки.ФорматыСохранения = Документы.ФинОтчетВБанк.ФорматыСохраненияОтчетов(НастройкиКомплекта());
