...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для каждого Тип Из Типы Цикл
		Ссылка = Новый(Тип);
		МетаданныеТипа = Ссылка.Метаданные();
		...
		Ссылка = Новый(ТипВладельца);
		МетаданныеВладельца = Ссылка.Метаданные();
		...
		Если Метаданные.Документы.Содержит(МетаданныеВладельца)
			И НЕ Метаданные.ЖурналыДокументов.ЖурналОпераций.РегистрируемыеДокументы.Содержит(МетаданныеВладельца) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОтобразитьПрисоединенныеФайлы(Владелец, Имя)
	...
	ЭтоДоговор = МетаданныеВладельца = Метаданные.Справочники.ДоговорыКонтрагентов;
	ЭтоОрганизация = МетаданныеВладельца = Метаданные.Справочники.Организации;
	...
	Если НЕ ЕстьОрганизация Тогда
		...
		ТестЗапроса = СтрЗаменить(ТекстЗапроса,
			"ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК Организации ПО Файлы.ВладелецФайла.Организация = Организации.Ссылка",
			"");
	КонецЕсли;
