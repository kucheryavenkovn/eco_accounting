...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") Тогда
		...
		//++ЭЛИАС 20170512 
	ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.АР_СчетНаАренду") Тогда
		ЗаполнитьНоменклатуруПоСчетуНаСервере(ВыбранноеЗначение);
		//--ЭЛИАС 20170512	
	КонецЕсли;
...
Процедура ДобавитьНоменклатуруПоСчетуНаОплату(Команда)
	...
	ВыборФормыДляВыбораСчета(ПараметрыФормы)
	//--ЭЛИАС 20170512
...
Процедура ЗаполнитьНоменклатуруПоСчетуНаСервере(СчетНаОплату)
	...
	Если ПерезаполнятьТоварыУслугиПоСчету
		...
		АвансыПоСтавкамНДС.Колонки.Добавить("СчетНаОплату", Новый ОписаниеТипов("ДокументСсылка.СчетНаОплатуПокупателю, ДокументСсылка.АР_СчетНаАренду"));
		//--ЭЛИАС 20170512
...
//++ЭЛИАС 20170512 
&НаКлиенте
Процедура ВыборФормыДляВыбораСчета(ПараметрыФормы)
	
	Оповещение = Новый ОписаниеОповещения("ВыборФормыДляВыбораСчетаЗавершение", ЭтаФорма, ПараметрыФормы);
	Список = Новый СписокЗначений;
	Список.Добавить("СчетНаОплатуПокупателю", "Счет на оплату");
	Список.Добавить("АР_СчетНаАренду", "Счет на аренду");
	Список.ПоказатьВыборЭлемента(Оповещение, "Выберите вид документа");
	
КонецПроцедуры //--ЭЛИАС 20170512

//++ЭЛИАС 20170512 
&НаКлиенте
Процедура ВыборФормыДляВыбораСчетаЗавершение(РезультатВыбора, ПараметрыФормы) Экспорт
	
	Если НЕ РезультатВыбора = Неопределено Тогда
		ОткрытьФорму("Документ." + РезультатВыбора.Значение + ".ФормаВыбора", ПараметрыФормы, ЭтотОбъект);
	КонецЕсли;
