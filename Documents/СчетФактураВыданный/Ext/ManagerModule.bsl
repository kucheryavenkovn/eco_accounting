...
//++ЭЛИАС 20120706
Функция ДокументОснованияОтноситсяКАренде(ДокументОснование) Экспорт
	
	Результат = Ложь;
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты") Тогда
		Результат = Истина;
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.АР_НачислениеПени") Тогда
		Результат = Истина;
...
Функция НоменклатураСчетаНаОплату(СчетНаОплату) Экспорт
	...
	|	СчетНаАренду.Ссылка
	|ИЗ
	|	Документ.АР_СчетНаАренду КАК СчетНаАренду
	|ГДЕ
	|	СчетНаАренду.Ссылка = &СчетНаОплату
	//--ЭЛИАС 20120706
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетНаОплату";
...
Функция ПолучитьКодВидаОперации(СтруктураПараметров, КодВидаОперацииОснования = Неопределено) Экспорт
	...
	ИначеЕсли ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаРеализацию Тогда
		...
		Для Каждого СтрокаТабличнойЧасти ИЗ ДокументыОснования Цикл
			...
			ИначеЕсли ВерсияКодовВидовОпераций >= 3 Тогда
				...
				Если ТипОснования = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
					...
					//++ЭЛИАС 20150415  
				ИначеЕсли ТипОснования = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты") 
					ИЛИ ТипОснования = Тип("ДокументСсылка.АР_НачислениеПени") Тогда
					МассивДокументовРеализации.Добавить(СтрокаТабличнойЧасти);
					//--ЭЛИАС 20150415
		...
		Если МассивДокументовРеализации.Количество() > 0 Тогда
			...
			|ГДЕ
			|	АР_НачислениеАренднойПлатыСостав.Ссылка В(&ДокументыОснования)
			|	И НЕ АР_НачислениеАренднойПлатыСостав.Контрагент = Значение(Справочник.Контрагенты.ПустаяСсылка)
			|	И НЕ АР_НачислениеАренднойПлатыСостав.ДоговорКонтрагента = Значение(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			|	АР_НачислениеПениСостав.Ссылка,
			|	1,
			|	0
			|ИЗ
			|	Документ.АР_НачислениеПени.Состав КАК АР_НачислениеПениСостав
			|ГДЕ
			|	АР_НачислениеПениСостав.Ссылка В(&ДокументыОснования)
			|	И НЕ АР_НачислениеПениСостав.Контрагент = Значение(Справочник.Контрагенты.ПустаяСсылка)
			|	И НЕ АР_НачислениеПениСостав.ДоговорКонтрагента = Значение(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
			//--ЭЛИАС 20150415
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
...
Функция ПодготовитьПараметрыЗаполненияАванс(ДокументОснование, ДокументСсылка, ДоговорКонтрагента = Неопределено, Отказ) Экспорт
	...
	КонецЕсли;
	//++ЭЛИАС 20170530 
	Запрос.Текст = Запрос.Текст + ?(ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПоступлениеНаРасчетныйСчет") 
		ИЛИ ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер"), ТекстЗапросаНоменклатураАвансыПоАренде(НомераТаблиц), "");
	//--ЭЛИАС 20170530
	...
	Если НомераТаблиц.Свойство("НоменклатураАвансыПоАренде") Тогда
		НоменклатураАвансыПоАренде = Результат[НомераТаблиц.НоменклатураАвансыПоАренде].Выгрузить();
		Для Каждого Стр Из НоменклатураАвансыПоАренде Цикл
			Если ЗначениеЗаполнено(Стр.Номенклатура) Тогда
				ВидПериода = ?(ЗначениеЗаполнено(Стр.СчетНаОплату), Стр.СчетНаОплату.ВидПериода, Перечисления.Периодичность.Месяц);
				ПериодАренды = ?(ЗначениеЗаполнено(Стр.ПериодАренды), Стр.ПериодАренды, Стр.Дата);
				Стр.Содержание = РаботаСНоменклатуройКлиентСервер.СодержаниеУслуги(Стр.Содержание, Стр.Номенклатура.ПериодичностьУслуги, 
					ПериодАренды, Истина, ВидПериода);
			КонецЕсли;
		КонецЦикла;
	Иначе
		НоменклатураАвансыПоАренде = Неопределено;
	КонецЕсли;
	
	Если НЕ НоменклатураАвансыПоАренде = Неопределено И НоменклатураАвансыПоАренде.Количество() > 0 Тогда
		Если НЕ НоменклатураАвансыПоАренде.Итог("Сумма") = АвансыПоСчетамНаОплату.Итог("Сумма") Тогда
			
			Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
				АвансыПоСчетамНаОплатуКопия = АвансыПоСчетамНаОплату.Скопировать(Новый Структура("ДоговорКонтрагента", ДоговорКонтрагента));
			Иначе
				АвансыПоСчетамНаОплатуКопия = АвансыПоСчетамНаОплату.Скопировать();
			КонецЕсли;
				
			Для Каждого СтрокаАвансаПоАренде Из НоменклатураАвансыПоАренде Цикл
				Отбор = Новый Структура("Номенклатура, СчетНаОплату, СтавкаНДС", 
					СтрокаАвансаПоАренде.Номенклатура, СтрокаАвансаПоАренде.СчетНаОплату, СтрокаАвансаПоАренде.СтавкаНДС);
				УдаляемыеСтроки = НоменклатураСчетовНаОплату.НайтиСтроки(Отбор);
				Если УдаляемыеСтроки.Количество() = 0 Тогда
					Если Отбор.СтавкаНДС = Перечисления.СтавкиНДС.НДС18_118 Тогда
						Отбор.СтавкаНДС = Перечисления.СтавкиНДС.НДС18;
					ИначеЕсли Отбор.СтавкаНДС = Перечисления.СтавкиНДС.НДС10_110 Тогда
						Отбор.СтавкаНДС = Перечисления.СтавкиНДС.НДС10;
					КонецЕсли;
					УдаляемыеСтроки = НоменклатураСчетовНаОплату.НайтиСтроки(Отбор);
				КонецЕсли;
				СуммаАвансаАренды = СтрокаАвансаПоАренде.Сумма;
				Для Каждого Стр Из УдаляемыеСтроки Цикл
					ТекСумма = Мин(СуммаАвансаАренды, Стр.Сумма);
					Если ТекСумма = Стр.Сумма Тогда
						НоменклатураСчетовНаОплату.Удалить(Стр);
					Иначе
						Стр.Сумма = Стр.Сумма - ТекСумма;
					КонецЕсли;
					СуммаАвансаАренды = СуммаАвансаАренды - ТекСумма;
				КонецЦикла;
				Отбор.Удалить("Номенклатура");
				Отбор.Вставить("ДоговорКонтрагента", СтрокаАвансаПоАренде.ДоговорКонтрагента);
				СуммаКорр = СтрокаАвансаПоАренде.Сумма;
				Если НЕ ЗначениеЗаполнено(Отбор.СчетНаОплату) Тогда
					Отбор.Удалить("СчетНаОплату");
				КонецЕсли;
				СтрокиДляКорректировки = АвансыПоСчетамНаОплатуКопия.НайтиСтроки(Отбор);
				Если СтрокиДляКорректировки.Количество() = 0 Тогда
					Если Отбор.СтавкаНДС = Перечисления.СтавкиНДС.НДС20 Тогда
						Отбор.СтавкаНДС = Перечисления.СтавкиНДС.НДС20_120;
					ИначеЕсли Отбор.СтавкаНДС = Перечисления.СтавкиНДС.НДС18 Тогда
						Отбор.СтавкаНДС = Перечисления.СтавкиНДС.НДС18_118;
					ИначеЕсли Отбор.СтавкаНДС = Перечисления.СтавкиНДС.НДС10 Тогда
						Отбор.СтавкаНДС = Перечисления.СтавкиНДС.НДС10_110;
					КонецЕсли;
					СтрокиДляКорректировки = АвансыПоСчетамНаОплатуКопия.НайтиСтроки(Отбор);
				КонецЕсли;
				Для Каждого СтрокаКорректировки Из СтрокиДляКорректировки Цикл
					ТекСумма = Мин(СтрокаКорректировки.Сумма, СуммаКорр);
					СтрокаКорректировки.Сумма = СтрокаКорректировки.Сумма - ТекСумма;
					СуммаКорр = СуммаКорр - ТекСумма;
					Если СтрокаКорректировки.Сумма = 0 Тогда
						АвансыПоСчетамНаОплатуКопия.Удалить(СтрокаКорректировки);
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
			
			НоменклатураСчетаФактуры = РаспределитьСуммыАвансовПоНоменклатуреСчетовНаОплату(
				АвансыПоСчетамНаОплатуКопия, НоменклатураСчетовНаОплату, ВалютаРеглУчета);
				
		Иначе
			НоменклатураСчетаФактуры = НоменклатураСчетовНаОплату.СкопироватьКолонки();		
		КонецЕсли;
		
		НоменклатураСчетовНаОплатуНовая = НоменклатураСчетовНаОплату.СкопироватьКолонки();
		Для Каждого СтрокаАвансаПоАренде Из НоменклатураАвансыПоАренде Цикл
			Стр = НоменклатураСчетовНаОплатуНовая.Добавить();
			ЗаполнитьЗначенияСвойств(Стр, СтрокаАвансаПоАренде);
			...
			ИначеЕсли Стр.СтавкаНДС = Перечисления.СтавкиНДС.НДС10 Тогда
				Стр.СтавкаНДС = Перечисления.СтавкиНДС.НДС10_110;
			КонецЕсли;
		КонецЦикла;
		Для Каждого Строк Из НоменклатураСчетаФактуры Цикл
			Стр = НоменклатураСчетовНаОплатуНовая.Добавить();
			ЗаполнитьЗначенияСвойств(Стр, Строк);
		КонецЦикла;
		НоменклатураСчетаФактуры = НоменклатураСчетовНаОплатуНовая.Скопировать();
		
	Иначе	
		Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
			АвансыПоСчетамНаОплату = АвансыПоСчетамНаОплату.Скопировать(Новый Структура("ДоговорКонтрагента", ДоговорКонтрагента));
		КонецЕсли;
		НоменклатураСчетаФактуры = РаспределитьСуммыАвансовПоНоменклатуреСчетовНаОплату(
			АвансыПоСчетамНаОплату, НоменклатураСчетовНаОплату, ВалютаРеглУчета);
	КонецЕсли;
	// Поднимем предварительную оплату в начало таблицы
	СтрокиПредвОплаты = НоменклатураСчетаФактуры.НайтиСтроки(Новый Структура("Номенклатура", Справочники.Номенклатура.ПустаяСсылка()));
	Для Каждого Стр Из СтрокиПредвОплаты Цикл
		Индекс = НоменклатураСчетаФактуры.Индекс(Стр);
		НоменклатураСчетаФактуры.Сдвинуть(Стр, -1 * Индекс);
	КонецЦикла;
	//--ЭЛИАС 20170531
...
Функция ТекстЗапросаДанныеПлатежа(НомераТаблиц)
	...
	|	И РасшифровкаПлатежа.СчетНаОплату <> ЗНАЧЕНИЕ(Документ.АР_СчетНаАренду.ПустаяСсылка) //++ЭЛИАС 20120706
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетНаОплату
	|;
...
Функция ТекстЗапросаНоменклатураСчетовНаОплату(НомераТаблиц)
	...
	|ИЗ
	|	Документ.АР_СчетНаАренду.Состав КАК ТаблицаУслуги
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СчетаНаОплату КАК СчетаНаОплату
	|		ПО ТаблицаУслуги.Ссылка = СчетаНаОплату.СчетНаОплату";
	//--ЭЛИАС 20120706
...
//++ЭЛИАС 20170530 
Функция ТекстЗапросаНоменклатураАвансыПоАренде(НомераТаблиц)

	НомераТаблиц.Вставить("АвансыПоАренде", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("НоменклатураАвансыПоАренде", НомераТаблиц.Количество());
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ВзаиморасчетыПоАрендеОбороты.Регистратор КАК Регистратор,
	|	ВзаиморасчетыПоАрендеОбороты.ДокументРасчетов КАК ДокументРасчетов,
	|	ВзаиморасчетыПоАрендеОбороты.СуммаРасход КАК СуммаАванса,
	|	ВзаиморасчетыПоАрендеОбороты.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ВзаиморасчетыПоАрендеОбороты.УслугаАренды КАК УслугаАренды,
	|	ВзаиморасчетыПоАрендеОбороты.ОбъектАренды КАК ОбъектАренды,
	|	ВзаиморасчетыПоАрендеОбороты.ПериодАренды КАК ПериодАренды,
	|	ВзаиморасчетыПоАрендеОбороты.СчетНаАренду КАК СчетНаАренду
	...
	|				ИЛИ ДоговорКонтрагента = &ДоговорКонтрагента) КАК ВзаиморасчетыПоАрендеОбороты
	|ГДЕ
	|	ВзаиморасчетыПоАрендеОбороты.Регистратор = &ДокументОснование
	|	И ВзаиморасчетыПоАрендеОбороты.Регистратор = ВзаиморасчетыПоАрендеОбороты.ДокументРасчетов
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РасшифровкаПлатежа.Дата КАК Дата,
	|	РасшифровкаПлатежа.АР_ПериодАренды КАК ПериодАренды,
	|	РасшифровкаПлатежа.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	РасшифровкаПлатежа.АР_СчетНаАренду КАК СчетНаОплату,
	|	3 КАК НомерТабЧасти,
	|	РасшифровкаПлатежа.НомерСтроки КАК НомерСтроки,
	|	РасшифровкаПлатежа.АР_УслугаАренды КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА РасшифровкаПлатежа.АР_УслугаАренды.НаименованиеПолное = """"
	|			ТОГДА РасшифровкаПлатежа.АР_УслугаАренды.Наименование
	|		ИНАЧЕ РасшифровкаПлатежа.АР_УслугаАренды.НаименованиеПолное
	|	КОНЕЦ КАК Содержание,
	|	АвансыПоАренде.СуммаАванса КАК Сумма,
	|	РасшифровкаПлатежа.СтавкаНДС КАК СтавкаНДС,
	|	ВЫРАЗИТЬ(АвансыПоАренде.СуммаАванса * РасшифровкаПлатежа.СуммаНДС / ВЫБОР
	|			КОГДА РасшифровкаПлатежа.Сумма = 0
	|				ТОГДА 1
	|			ИНАЧЕ РасшифровкаПлатежа.Сумма
	|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК СуммаНДС
	|ИЗ
	|	РасшифровкаПлатежа КАК РасшифровкаПлатежа
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ АвансыПоАренде КАК АвансыПоАренде
	|		ПО РасшифровкаПлатежа.ДоговорКонтрагента = АвансыПоАренде.ДоговорКонтрагента
	|			И РасшифровкаПлатежа.АР_УслугаАренды = АвансыПоАренде.УслугаАренды
	|			И (НЕ РасшифровкаПлатежа.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоОбъектам
	|				ИЛИ РасшифровкаПлатежа.АР_ОбъектАренды = АвансыПоАренде.ОбъектАренды)
	|			И (НЕ РасшифровкаПлатежа.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоПериодам
	|				ИЛИ РасшифровкаПлатежа.АР_ПериодАренды = АвансыПоАренде.ПериодАренды)
	|			И (НЕ(РасшифровкаПлатежа.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПост
	|						ИЛИ РасшифровкаПлатежа.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПерем)
	|				ИЛИ РасшифровкаПлатежа.АР_СчетНаАренду = АвансыПоАренде.СчетНаАренду)
	|			И (НЕ РасшифровкаПлатежа.АР_УслугаАренды = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.СписокФорм    = "ФормаДокументаНаРеализацию,ФормаДокументаКорректировочный";
	//--ЭЛИАС 20140909
	...
	Если ПравоДоступа("Использование", Метаданные.Обработки.ПечатьУПД) Тогда
		...
		КомандаПечати.СписокФорм    = "ФормаДокументаНаРеализацию";
		//--ЭЛИАС 20171020
	КонецЕсли;
...
Функция ПодготовитьДанныеКорректировочнойСправки(СтруктураПараметров)
	...
	|ВЫБРАТЬ
	|	НДСЗаписиКнигиПокупок.Регистратор,
	|	-(НДСЗаписиКнигиПокупок.НДС + НДСЗаписиКнигиПокупок.СуммаБезНДС)
	|ИЗ
	|	РегистрНакопления.НДСЗаписиКнигиПокупок КАК НДСЗаписиКнигиПокупок
	|ГДЕ
	|	НДСЗаписиКнигиПокупок.СчетФактура В
