...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбработкаНовостейПереопределяемый.КонтекстныеНовости_ПриСозданииНаСервере(
		ЭтаФорма,
		"БП.Документ.АР_ДополнительноеСоглашение",
		"ФормаДокумента",
		НСтр("ru='Новости: Дополнительное соглашение'"),
...
Процедура ЗаполнитьТекущимиДаннымиНаСервере()
	...
	ОбъектЗначение = РеквизитФормыВЗначение("Объект");
	ОбъектЗначение.ЗаполнитьТекущимиДанными();
	ЗначениеВРеквизитФормы(ОбъектЗначение,"Объект");
	СрокДействияСоглашения = ЗначениеЗаполнено(Объект.СрокДействия);
	ЗаполнитьСуммыПостояннойЧасти();
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьСтавки(ИмяТабличнойЧасти)
	
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Ставки можно заполнить только после записи документа!'"));
		Возврат;
	КонецЕсли;
	
	Если Объект.Проведен Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Необходимо отменить проведение документа для заполнения ставок!'"));
		Возврат;
	КонецЕсли;
	...
	Для каждого СтрокаТЧ Из Объект[ИмяТабличнойЧасти] Цикл
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Услуга) Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Объект.Ссылка, ИмяТабличнойЧасти +"[" + Строка(СтрокаТЧ.НомерСтроки - 1) + "].Услуга", "Объект", Отказ);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(СтрокаТЧ.ОбъектАренды) Тогда
			ТекстСообщения = "Необходимо выбрать объект аренды в текущей строке";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Объект.Ссылка, ИмяТабличнойЧасти + "[" + Строка(СтрокаТЧ.НомерСтроки - 1) + "].ОбъектАренды", "Объект", Отказ);
		КонецЕсли;
	КонецЦикла;
	...
	ЕстьЗаполненнаяСтавка = Ложь;
	Для каждого СтрокаТЧ Из Объект[ИмяТабличнойЧасти] Цикл
		Если ЗначениеЗаполнено(СтрокаТЧ.Ставка) Тогда
			ЕстьЗаполненнаяСтавка = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ЕстьЗаполненнаяСтавка Тогда
		Оповещение = Новый ОписаниеОповещения("СпроситьОПерезаполненииЦен", ЭтотОбъект, ИмяТабличнойЧасти);
		ПоказатьВопрос(Оповещение, "Указанные в документе ставки будут перезаполнены. Продолжить?", РежимДиалогаВопрос.ДаНет);
	Иначе
		ЗаполнитьСтавкиЗавершение(ИмяТабличнойЧасти);
	КонецЕсли;
...
&НаКлиенте
Процедура СпроситьОПерезаполненииЦен(РезультатВопроса, ИмяТабличнойЧасти) Экспорт
...
&НаКлиенте
Процедура ЗаполнитьСтавкиЗавершение(ИмяТабличнойЧасти)
	
	ЗаполнитьСтавкиНаСервере(ИмяТабличнойЧасти="ПостояннаяЧастьАренднойПлаты");
	ТекущиеДанные = Элементы[ИмяТабличнойЧасти].ТекущиеДанные;
	Если Не ТекущиеДанные = Неопределено Тогда
		ЭлементФормыСтавка = ?(ИмяТабличнойЧасти="ПостояннаяЧастьАренднойПлаты", Элементы.ПостояннаяЧастьАренднойПлатыСтавка, Элементы.ПеременнаяЧастьАренднойПлатыСтавка);
		НастроитьЭлементСтавки(ЭлементФормыСтавка, ТекущиеДанные.Ставка);
		СформироватьНадписьГрафикаСтавок(ТекущиеДанные.Ставка);
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьСтавкиНаСервере(ПостояннаяЧасть)
	
	ДопСоглашение = РеквизитФормыВЗначение("Объект");
	ДопСоглашение.ЗаполнитьСтавки(ПостояннаяЧасть);
	ЗначениеВРеквизитФормы(ДопСоглашение, "Объект");
	ЗаполнитьСуммыПостояннойЧасти();
...
Процедура УстановитьВидимость()
	...
	Элементы.СрокДействия.Видимость = СрокДействияСоглашения = 1;
	Элементы.КомментарийСрокДействия.Видимость = СрокДействияСоглашения = 1;
...
&НаКлиенте
Процедура ЗаполнитьСтавкиПостояннаяЧасть(Команда)
	
	ЗаполнитьСтавки("ПостояннаяЧастьАренднойПлаты");
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьСтавкиПеременнаяЧасть(Команда)
	
	ЗаполнитьСтавки("ПеременнаяЧастьАренднойПлаты");
...
Процедура ЗаполнитьКолонку(Команда)
	...
	ПараметрыФормы = Новый Структура("ИмяДокумента", "АР_ДополнительноеСоглашение");
...
&НаКлиенте
Процедура ЗаполнитьСчетчики(Команда)
	
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Счетчики можно заполнить только после записи документа!'"));
		Возврат;
	КонецЕсли;
	
	Если Объект.Проведен Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Необходимо отменить проведение документа для заполнения счетчиков!'"));
		Возврат;
	КонецЕсли;
	...
	Для каждого СтрокаТЧ Из Объект.ПеременнаяЧастьАренднойПлаты Цикл
		Если Не ЗначениеЗаполнено(СтрокаТЧ.Услуга) Тогда
			ТекстСообщения = "Необходимо выбрать услугу в текущей строке";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Объект.Ссылка, "ПеременнаяЧастьАренднойПлаты[" + Строка(СтрокаТЧ.НомерСтроки - 1) + "].Услуга", "Объект", Отказ);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(СтрокаТЧ.ОбъектАренды) Тогда
			ТекстСообщения = "Необходимо выбрать объект аренды в текущей строке";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Объект.Ссылка, "ПеременнаяЧастьАренднойПлаты[" + Строка(СтрокаТЧ.НомерСтроки - 1) + "].ОбъектАренды", "Объект", Отказ);
		КонецЕсли;
	КонецЦикла;
	...
	ЕстьЗаполненныйСчетчик = Ложь;
	Для каждого СтрокаТЧ Из Объект.ПеременнаяЧастьАренднойПлаты Цикл
		Если ЗначениеЗаполнено(СтрокаТЧ.Счетчик) Тогда
			ЕстьЗаполненныйСчетчик = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если ЕстьЗаполненныйСчетчик Тогда
		Оповещение = Новый ОписаниеОповещения("ЗаполнитьСчетчикиПродолжение", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, "Указанные в документе счетчики будут перезаполнены. Продолжить?", РежимДиалогаВопрос.ДаНет);
	Иначе
		ЗаполнитьСчетчикиНаСервере();
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьСчетчикиПродолжение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
&НаСервере
Процедура ЗаполнитьСчетчикиНаСервере()
	
	ДопСоглашение = РеквизитФормыВЗначение("Объект");
	ДопСоглашение.ЗаполнитьСчетчики();
	ЗначениеВРеквизитФормы(ДопСоглашение, "Объект");
