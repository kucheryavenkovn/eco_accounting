...
Процедура ДополнитьТаблицуВалютнойСуммой(ТаблицаПоДаннымОрганизации, СтруктураПараметров)
	...
	Если СтруктураПараметров.ВалютаДокумента = СтруктураПараметров.ВалютаРегламентированногоУчета 
		...
		ТаблицаПоДаннымОрганизации.Свернуть(
			"Дата,ДатаВходящегоДокумента,Документ,Представление,Договор,Валюта,ЭтоУниверсальныйДокумент,ЭтоКорректировочныйДокумент",
	...
	Иначе
		ТаблицаПоДаннымОрганизации.Свернуть(
			"Дата,ДатаВходящегоДокумента,Документ,Представление,Договор,ЭтоУниверсальныйДокумент,ЭтоКорректировочныйДокумент",
			"Дебет, Кредит");
	КонецЕсли;
...
Функция ТекстЗапросаДляФормированияПечатнойФормыПисьмаОбОплатеЗадолженности()
	...
	|	АктСверкиВзаиморасчетовПоДаннымОрганизации.Ссылка.Дата КАК ДатаАктаСверки,
	|	АктСверкиВзаиморасчетовПоДаннымОрганизации.Ссылка.Организация КАК Организация,
	|	АктСверкиВзаиморасчетовПоДаннымОрганизации.Ссылка.ПредставительКонтрагента КАК ПредставительКонтрагента,
	|	АктСверкиВзаиморасчетовПоДаннымОрганизации.Ссылка.ПредставительОрганизации КАК ПредставительОрганизации,
	...
	|		ПО АктСверкиВзаиморасчетовПоДаннымОрганизации.Ссылка.Организация.ОсновнойБанковскийСчет = БанковскиеСчета.Ссылка
	|ГДЕ
	|	АктСверкиВзаиморасчетовПоДаннымОрганизации.Ссылка В(&Документы)
	|
	|СГРУППИРОВАТЬ ПО
	|	АктСверкиВзаиморасчетовПоДаннымОрганизации.Ссылка,
	|	АктСверкиВзаиморасчетовПоДаннымОрганизации.Ссылка.Контрагент,
	|	АктСверкиВзаиморасчетовПоДаннымОрганизации.Ссылка.Дата,
...
Функция ПечатьПисьмаОбОплатеЗадолженности(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		СуммаЗадолженности = Выборка.Дебет - Выборка.Кредит;
