...
Функция УчетПеремещенияТоваровИзвлечьСтруктуруXML(ДатаЗаполнения)
	...
	Если ДатаЗаполнения >= '20200701' Тогда
		Попытка
			...
			|	РегламентированныеОтчеты.Ссылка КАК Количество
			|ИЗ
			|	Справочник.РегламентированныеОтчеты КАК РегламентированныеОтчеты
			|ГДЕ
			|	РегламентированныеОтчеты.ВнешнийОтчетИспользовать = ИСТИНА
			|	И РегламентированныеОтчеты.ИсточникОтчета = ""РегламентированныйОтчетСтатистикаФормаУчетаПеремещенияТоваровТС""";
			
			ВнешнийОтчетИспользовать = ?(Запрос.Выполнить().Выбрать().Количество() > 0, "Внешний", "");
			ИмяФормы = ВнешнийОтчетИспользовать + "Отчет.РегламентированныйОтчетСтатистикаФормаУчетаПеремещенияТоваровТС"
						+ ".Форма.Форма";
			Макет = РегламентированнаяОтчетностьВызовСервера.ОбъектОтчета(ИмяФормы).ПолучитьМакет("СхемаВыгрузкиXML2020Кв1");
		Исключение
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Для выгрузки документа необходимо подключить внешний
					|отчет статистической формы учета перемещения товаров'"));
			Возврат Неопределено;
		КонецПопытки;
