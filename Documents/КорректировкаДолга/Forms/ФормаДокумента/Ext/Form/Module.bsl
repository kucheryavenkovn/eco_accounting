...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	Если НЕ Объект.ВидОперации = СтарыйВидОперации Тогда
		...
		Если НЕ Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкаДолга.Прочее") Тогда
			...
			Объект.АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеДт.Очистить(); //++ЭЛИАС 20120705
			Объект.АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеКт.Очистить(); //++ЭЛИАС 20120705
...
Процедура СчетДтПриИзменении(Элемент)
	...
	УправлениеФормой(); //++ЭЛИАС 20120705
...
Процедура УстановитьУсловноеОформление()
	...
	//++ЭЛИАС 20150225  
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ДополнительнаяАналитикаДтКурсВзаиморасчетов");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ДополнительнаяАналитикаДтКратностьВзаиморасчетов");
	//--ЭЛИАС 20150225
	...
	//++ЭЛИАС 20150225  
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ДополнительнаяАналитикаКтКурсВзаиморасчетов");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ДополнительнаяАналитикаКтКратностьВзаиморасчетов");
	//--ЭЛИАС 20150225
	...
	//++ЭЛИАС 20150225  
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ДополнительнаяАналитикаДтСуммаНУ");
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "ДополнительнаяАналитикаКтСуммаНУ");
	//--ЭЛИАС 20150225
...
Процедура УправлениеФормой()
	...
	//++ЭЛИАС 20120622
	Элементы.ГруппаДополнительнаяАналитикаДт.Видимость = Объект.АР_КорректировкаДолгаПоАренде;
	Элементы.ГруппаДополнительнаяАналитикаКт.Видимость = Объект.АР_КорректировкаДолгаПоАренде;
	Элементы.ГруппаДополнительнаяАналитикаДтВспомогательная.Видимость = Объект.АР_КорректировкаДолгаПоАренде;
	Элементы.ДополнительнаяАналитикаСчетДт.Видимость = Объект.АР_КорректировкаДолгаПоАренде И НЕ (Объект.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.СписаниеЗадолженности);
	Элементы.ДополнительнаяАналитикаСчетКт.Видимость = Объект.АР_КорректировкаДолгаПоАренде И НЕ (Объект.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.СписаниеЗадолженности);
	
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкаДолга.Прочее") Тогда
		
		Если ЗначениеЗаполнено(Объект.СчетДт) Тогда
			ПараметрыДокумента = БухгалтерскийУчетКлиентСервер.ДанныеУстановкиПараметровСубконто(
				Объект, ПараметрыУстановкиСвойствСубконтоДт(ЭтотОбъект));
			Договор = Неопределено;
			Если ПараметрыДокумента.Свойство("ДоговорКонтрагента", Договор) И ЗначениеЗаполнено(Договор) Тогда
				ДанныеДоговораДт = АР_ОбщиеПроцедуры.ЗначенияРеквизитовОбъекта(Договор, "АР_ВестиВзаиморасчетыПоОбъектам, АР_ВестиВзаиморасчетыПоУслугам,
						|АР_ВестиВзаиморасчетыПоПериодам, АР_ВестиВзаиморасчетыПоСчетамПост, АР_ВестиВзаиморасчетыПоСчетамПерем, АР_АрендныйДоговор");
				ВидимостьДт = ДанныеДоговораДт.АР_АрендныйДоговор;
				ВестиВзаиморасчетыПоСчетамДт = ДанныеДоговораДт.АР_ВестиВзаиморасчетыПоСчетамПост ИЛИ ДанныеДоговораДт.АР_ВестиВзаиморасчетыПоСчетамПерем;		
			ИначеЕсли ПараметрыДокумента.Свойство("ДоговорКонтрагента", Договор) Тогда
				ВидимостьДт = Ложь;
				ВестиВзаиморасчетыПоСчетамДт = Ложь;
			Иначе
				ВидимостьДт = Ложь;
				ВестиВзаиморасчетыПоСчетамДт = Ложь;
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(Объект.СчетКт) Тогда
			ПараметрыДокумента = БухгалтерскийУчетКлиентСервер.ДанныеУстановкиПараметровСубконто(
				Объект, ПараметрыУстановкиСвойствСубконтоКт(ЭтотОбъект));
			Договор = Неопределено;
			Если ПараметрыДокумента.Свойство("ДоговорКонтрагента", Договор) И ЗначениеЗаполнено(Договор) Тогда
				ДанныеДоговораКт = АР_ОбщиеПроцедуры.ЗначенияРеквизитовОбъекта(Договор, "АР_ВестиВзаиморасчетыПоОбъектам, АР_ВестиВзаиморасчетыПоУслугам,
						|АР_ВестиВзаиморасчетыПоПериодам, АР_ВестиВзаиморасчетыПоСчетамПост, АР_ВестиВзаиморасчетыПоСчетамПерем, АР_АрендныйДоговор");
				ВидимостьКт = ДанныеДоговораКт.АР_АрендныйДоговор;
				ВестиВзаиморасчетыПоСчетамКт = ДанныеДоговораКт.АР_ВестиВзаиморасчетыПоСчетамПост ИЛИ ДанныеДоговораКт.АР_ВестиВзаиморасчетыПоСчетамПерем;		
			ИначеЕсли ПараметрыДокумента.Свойство("ДоговорКонтрагента", Договор) Тогда
				ВидимостьКт = Ложь;
				ВестиВзаиморасчетыПоСчетамКт = Ложь;
			Иначе
				ВидимостьКт = Ложь;
				ВестиВзаиморасчетыПоСчетамКт = Ложь;
			КонецЕсли;
		...
		ЭтоПереносЗадолжности = (Объект.СчетДт <> Объект.СчетКт) 
								ИЛИ ( Объект.СчетДт = Объект.СчетКт
									И Объект.СубконтоДт1 = Объект.СубконтоКт1
									И Объект.СубконтоДт2 = Объект.СубконтоКт2
									И Объект.СубконтоДт3 = Объект.СубконтоКт3
								);
		ВидимостьДт = ВидимостьДт И ЭтоПереносЗадолжности;
		ВидимостьКт = ВидимостьКт И ЭтоПереносЗадолжности;
		
		Элементы.АР_ОбъектАрендыДт.Видимость = ВидимостьДт И ДанныеДоговораДт.АР_ВестиВзаиморасчетыПоОбъектам;
		Элементы.АР_ПериодАрендыДт.Видимость = ВидимостьДт И ДанныеДоговораДт.АР_ВестиВзаиморасчетыПоПериодам;
		Элементы.АР_СчетНаАрендуДт.Видимость = ВидимостьДт И ВестиВзаиморасчетыПоСчетамДт;
		Элементы.АР_УслугаАрендыДт.Видимость = ВидимостьДт И ДанныеДоговораДт.АР_ВестиВзаиморасчетыПоУслугам;
		Элементы.АР_ПениДт.Видимость 		 = ВидимостьДт;
		Элементы.АР_ОбъектАрендыКт.Видимость = ВидимостьКт И ДанныеДоговораКт.АР_ВестиВзаиморасчетыПоОбъектам И Не Объект.АР_ДепозитКт;
		Элементы.АР_ПериодАрендыКт.Видимость = ВидимостьКт И ДанныеДоговораКт.АР_ВестиВзаиморасчетыПоПериодам И Не Объект.АР_ДепозитКт;
		Элементы.АР_СчетНаАрендуКт.Видимость = ВидимостьКт И ВестиВзаиморасчетыПоСчетамКт И Не Объект.АР_ДепозитКт;
		Элементы.АР_УслугаАрендыКт.Видимость = ВидимостьКт И ДанныеДоговораКт.АР_ВестиВзаиморасчетыПоУслугам И Не Объект.АР_ДепозитКт;
		Элементы.АР_ДепозитКт.Видимость		 = ВидимостьКт;
		
	КонецЕсли;
	
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкаДолга.ПереносЗадолженности") Тогда
		ВидимостьНовойДопАналитикиДт = Истина;
		ВидимостьНовойДопАналитикиКт = Истина;
	Иначе
		ВидимостьНовойДопАналитикиДт = Ложь;
		ВидимостьНовойДопАналитикиКт = Ложь;
	КонецЕсли;
	
	Элементы.ДополнительнаяАналитикаДтНовыйУслугаАренды.Видимость = ВидимостьНовойДопАналитикиДт;
	Элементы.ДополнительнаяАналитикаДтНовыйОбъектАренды.Видимость = ВидимостьНовойДопАналитикиДт;
	Элементы.ДополнительнаяАналитикаДтНовыйПериодАренды.Видимость = ВидимостьНовойДопАналитикиДт;
	Элементы.ДополнительнаяАналитикаДтНовыйСчетНаАренду.Видимость = ВидимостьНовойДопАналитикиДт;
	Элементы.ДополнительнаяАналитикаДтНовыйДепозит.Видимость = ВидимостьНовойДопАналитикиДт;
	Элементы.ДополнительнаяАналитикаДтНовыйПени.Видимость = ВидимостьНовойДопАналитикиДт;
	
	Элементы.ДополнительнаяАналитикаКтНовыйУслугаАренды.Видимость = ВидимостьНовойДопАналитикиКт;
	Элементы.ДополнительнаяАналитикаКтНовыйОбъектАренды.Видимость = ВидимостьНовойДопАналитикиКт;
	Элементы.ДополнительнаяАналитикаКтНовыйПериодАренды.Видимость = ВидимостьНовойДопАналитикиКт;
	Элементы.ДополнительнаяАналитикаКтНовыйСчетНаАренду.Видимость = ВидимостьНовойДопАналитикиКт;
	Элементы.ДополнительнаяАналитикаКтНовыйДепозит.Видимость = ВидимостьНовойДопАналитикиКт;
	Элементы.ДополнительнаяАналитикаКтНовыйПени.Видимость = ВидимостьНовойДопАналитикиКт;
	//--ЭЛИАС 20120622
...
Функция СформироватьПараметрыЗаполнения(ИмяКомандыЗаполнения)
	...
	//++ЭЛИАС 20160920 
	ПараметрыЗаполнения.Вставить("АР_КорректировкаДолгаПоАренде", Объект.АР_КорректировкаДолгаПоАренде);
	//--ЭЛИАС 20160920
...
Функция ЗагрузитьПодготовленныеДанные()
	...
	//++ЭЛИАС 20160920 
	Для каждого ЗаполняемаяТаблица Из СтруктураДанных.ТаблицыДопАналитики Цикл
		
		Если ЗаполняемаяТаблица.Ключ = Перечисления.ВидыЗадолженности.Дебиторская Тогда
			Объект.АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеДт.Загрузить(ЗаполняемаяТаблица.Значение);
		Иначе
			Объект.АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеКт.Загрузить(ЗаполняемаяТаблица.Значение);
		КонецЕсли;
...
#Область ПроцедурыИФункцииАрендыИУправленияНедвижимости

//++ЭЛИАС 20120622

&НаКлиенте
Процедура ДополнительнаяАналитикаПриНачалеРедактирования(Элемент, СтрокаТаблицы)
...
&НаКлиенте
Процедура ДополнительнаяАналитикаУстановитьОтборСтрок(СтрокаТаблицы, ЭлементДополнительнаяАналитика)

	Если СтрокаТаблицы <> Неопределено
		И ТипЗнч(СтрокаТаблицы.ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда

		ЭлементДополнительнаяАналитика.ОтборСтрок = Новый ФиксированнаяСтруктура("Ключ", СтрокаТаблицы.АР_Ключ);
		ЭлементДополнительнаяАналитика.Доступность = Истина;

	Иначе

		ЭлементДополнительнаяАналитика.ОтборСтрок = Новый ФиксированнаяСтруктура("Ключ", Новый УникальныйИдентификатор);
		ЭлементДополнительнаяАналитика.Доступность = Ложь;
...
&НаСервереБезКонтекста
Функция ПолучитьПризнакВидимостиАналитикиПоСчету(Счет, ВидОперации, ДоговорКонтрагента)
	
	СтрокаСубконто = Счет.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Договоры);
	ПризнакВидимости = (ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.Прочее) И
		(СтрокаСубконто <> Неопределено) И ДоговорКонтрагента.АР_АрендныйДоговор;
	
	Возврат ПризнакВидимости;
...
//++ЭЛИАС 20120622
&НаКлиенте
Процедура АР_КорректировкаДолгаПоАрендеПриИзменении(Элемент)
...
//++ЭЛИАС 20120705
&НаКлиенте
Процедура АР_ДепозитКтПриИзменении(Элемент)
...
&НаКлиенте
Процедура АР_ПериодАрендыДтПриИзменении(Элемент)
	
	ПараметрыДокумента = БухгалтерскийУчетКлиентСервер.ДанныеУстановкиПараметровСубконто(
		Объект, ПараметрыУстановкиСвойствСубконтоДт(ЭтотОбъект));
	
	ПривестиКНачалуПериода(Объект.АР_ПериодАрендыДт, ПараметрыДокумента.ДоговорКонтрагента);
		
КонецПроцедуры

&НаКлиенте
Процедура АР_ПериодАрендыКтПриИзменении(Элемент)
	
	ПараметрыДокумента = БухгалтерскийУчетКлиентСервер.ДанныеУстановкиПараметровСубконто(
		Объект, ПараметрыУстановкиСвойствСубконтоКт(ЭтотОбъект));
	ПривестиКНачалуПериода(Объект.АР_ПериодАрендыКт, ПараметрыДокумента.ДоговорКонтрагента);
...
&НаКлиенте
Процедура ТаблицаДебиторскаяЗадолженностьПриАктивизацииСтроки(Элемент)
	
	Если НЕ Элементы.ТаблицаДебиторскаяЗадолженность.ТекущиеДанные = Неопределено Тогда
		ДополнительнаяАналитикаУстановитьОтборСтрок(Элементы.ТаблицаДебиторскаяЗадолженность.ТекущиеДанные,
			Элементы.ДополнительнаяАналитикаДт);
	Иначе
		ДополнительнаяАналитикаУстановитьОтборСтрок(Элементы.ТаблицаДебиторскаяЗадолженностьВспомогательная.ТекущиеДанные,
			Элементы.ДополнительнаяАналитикаДтВспомогательная);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаДебиторскаяЗадолженностьПередУдалением(Элемент, Отказ)
	
	СтрокаТаблицы = Элементы.ТаблицаДебиторскаяЗадолженность.ТекущиеДанные;
	
	Если СтрокаТаблицы = Неопределено Тогда
		СтрокаТаблицы = Элементы.ТаблицаДебиторскаяЗадолженностьВспомогательная.ТекущиеДанные;	
	КонецЕсли;
	
	ПодчиненныеСтроки = Объект.АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеДт.НайтиСтроки(Новый Структура("Ключ", СтрокаТаблицы.АР_Ключ));
	...
	Для каждого Строка Из ПодчиненныеСтроки Цикл
		Объект.АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеДт.Удалить(Строка);
	КонецЦикла;
...
Процедура ТаблицаДебиторскаяЗадолженностьПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НЕ СтрокаТаблицы = Неопределено Тогда
		СтрокаТаблицы.АР_Ключ = Новый УникальныйИдентификатор;
		ДополнительнаяАналитикаУстановитьОтборСтрок(Элементы.ТаблицаДебиторскаяЗадолженность.ТекущиеДанные,
		Элементы.ДополнительнаяАналитикаДт);
	Иначе	
		СтрокаТаблицы = Элементы.ТаблицаДебиторскаяЗадолженностьВспомогательная.ТекущиеДанные;	
...
&НаКлиенте
Процедура ДополнительнаяАналитикаДтПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	
	Если НЕ НоваяСтрока Тогда
		Возврат;
	КонецЕсли;

	СтрокаТЧ = Элементы.ТаблицаДебиторскаяЗадолженность.ТекущиеДанные;
	Если НЕ СтрокаТЧ = Неопределено Тогда
		СтрокаТЧДопАналитика = Элементы.ДополнительнаяАналитикаДт.ТекущиеДанные;
	Иначе 
		СтрокаТЧ = Элементы.ТаблицаДебиторскаяЗадолженностьВспомогательная.ТекущиеДанные;
		СтрокаТЧДопАналитика = Элементы.ДополнительнаяАналитикаДтВспомогательная.ТекущиеДанные;
	КонецЕсли;
	СтрокаТЧДопАналитика.КурсВзаиморасчетов = СтрокаТЧ.КурсВзаиморасчетов;
	СтрокаТЧДопАналитика.КратностьВзаиморасчетов = СтрокаТЧ.КратностьВзаиморасчетов;
	
	ДополнительнаяАналитикаПриНачалеРедактирования(Элемент, СтрокаТЧДопАналитика);

КонецПроцедуры

&НаКлиенте
Процедура ДополнительнаяАналитикаДтПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	
	ДанныеКонтрагента = Элементы.ТаблицаДебиторскаяЗадолженность.ТекущиеДанные;
	ДанныеКонтрагентаВспомогательные = Элементы.ТаблицаДебиторскаяЗадолженностьВспомогательная.ТекущиеДанные;
	
	Если ДанныеКонтрагента = Неопределено И НЕ ДанныеКонтрагентаВспомогательные = Неопределено Тогда
		ДанныеКонтрагента = ДанныеКонтрагентаВспомогательные;
	КонецЕсли;
	
	Если ДанныеКонтрагента = Неопределено Тогда
		Отказ = Истина;
	ИначеЕсли Не ЗначениеЗаполнено(ДанныеКонтрагента.ДоговорКонтрагента) Тогда
		Отказ = Истина;
	ИначеЕсли НЕ АР_ОбщиеПроцедуры.ЗначениеРеквизитаОбъекта(ДанныеКонтрагента.ДоговорКонтрагента, "АР_АрендныйДоговор") Тогда
		Если НЕ (ЗначениеЗаполнено(ДанныеКонтрагента.КорДоговорКонтрагента) 
			И АР_ОбщиеПроцедуры.ЗначениеРеквизитаОбъекта(ДанныеКонтрагента.КорДоговорКонтрагента, "АР_АрендныйДоговор")) Тогда
			
			ТекстСообщения = "Добавление дополнительной аналитики возможно только для арендного договора";
			Если НЕ ДанныеКонтрагентаВспомогательные = Неопределено Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "ДополнительнаяАналитикаДтВспомогательная", "Объект", Отказ);
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "ДополнительнаяАналитикаДт", "Объект", Отказ);
			КонецЕсли;
...
&НаКлиенте
Процедура ДополнительнаяАналитикаДтПериодАрендыПриИзменении(Элемент)
	
	ТекущиеДанныеДебиторскаяЗадолженность = Элементы.ТаблицаДебиторскаяЗадолженность.ТекущиеДанные;
	ТекущиеДанныеДебиторскаяЗадолженностьВспомогательные = Элементы.ТаблицаДебиторскаяЗадолженность.ТекущиеДанные;
	Если ТекущиеДанныеДебиторскаяЗадолженность <> Неопределено Тогда
		ТекущиеДанные = Элементы.ДополнительнаяАналитикаДт.ТекущиеДанные;
		ПривестиКНачалуПериода(ТекущиеДанные.ПериодАренды, ТекущиеДанныеДебиторскаяЗадолженность.ДоговорКонтрагента);
	ИначеЕсли ТекущиеДанныеДебиторскаяЗадолженностьВспомогательные <> Неопределено Тогда
		ТекущиеДанные = Элементы.ДополнительнаяАналитикаДтВспомогательная.ТекущиеДанные;
		ПривестиКНачалуПериода(ТекущиеДанные.ПериодАренды, ТекущиеДанныеДебиторскаяЗадолженностьВспомогательные.ДоговорКонтрагента);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ДополнительнаяАналитикаДтНовыйПериодАрендыПриИзменении(Элемент)
	
	ТекущиеДанныеДебиторскаяЗадолженность = Элементы.ТаблицаДебиторскаяЗадолженность.ТекущиеДанные;
	ТекущиеДанныеДебиторскаяЗадолженностьВспомогательные = Элементы.ТаблицаДебиторскаяЗадолженность.ТекущиеДанные;
	Если ТекущиеДанныеДебиторскаяЗадолженность <> Неопределено Тогда
		ТекущиеДанные = Элементы.ДополнительнаяАналитикаДт.ТекущиеДанные;
		ПривестиКНачалуПериода(ТекущиеДанные.НовыйПериодАренды, ТекущиеДанныеДебиторскаяЗадолженность.ДоговорКонтрагента);
	ИначеЕсли ТекущиеДанныеДебиторскаяЗадолженностьВспомогательные <> Неопределено Тогда
		ТекущиеДанные = Элементы.ДополнительнаяАналитикаДтВспомогательная.ТекущиеДанные;
		ПривестиКНачалуПериода(ТекущиеДанные.НовыйПериодАренды, ТекущиеДанныеДебиторскаяЗадолженностьВспомогательные.ДоговорКонтрагента);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ДополнительнаяАналитикаДтКурсВзаиморасчетовПриИзменении(Элемент)
...
&НаКлиенте
Процедура ДополнительнаяАналитикаДтСуммаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ДополнительнаяАналитикаДтСуммаВзаиморасчетовПриИзменении(Элемент)
...
&НаКлиенте
Процедура ТаблицаКредиторскаяЗадолженностьПриАктивизацииСтроки(Элемент)
	
	ДополнительнаяАналитикаУстановитьОтборСтрок(Элементы.ТаблицаКредиторскаяЗадолженность.ТекущиеДанные,
	Элементы.ДополнительнаяАналитикаКт);
	
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаКредиторскаяЗадолженностьПередУдалением(Элемент, Отказ)
	
	СтрокаТаблицы = Элементы.ТаблицаКредиторскаяЗадолженность.ТекущиеДанные;
	
	ПодчиненныеСтроки = Объект.АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеКт.НайтиСтроки(Новый Структура("Ключ", СтрокаТаблицы.АР_Ключ));
	...
	Для каждого Строка Из ПодчиненныеСтроки Цикл
		Объект.АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеКт.Удалить(Строка);
	КонецЦикла;
...
&НаКлиенте
Процедура ДополнительнаяАналитикаКтПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	
	Если НЕ НоваяСтрока Тогда
		Возврат;
	КонецЕсли;

	СтрокаТЧ = Элементы.ТаблицаКредиторскаяЗадолженность.ТекущиеДанные;
	СтрокаТЧДопАналитика = Элементы.ДополнительнаяАналитикаКт.ТекущиеДанные;
	СтрокаТЧДопАналитика.КурсВзаиморасчетов = СтрокаТЧ.КурсВзаиморасчетов;
	СтрокаТЧДопАналитика.КратностьВзаиморасчетов = СтрокаТЧ.КратностьВзаиморасчетов;
...
&НаКлиенте
Процедура ДополнительнаяАналитикаКтПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	
	ДанныеКонтрагента = Элементы.ТаблицаКредиторскаяЗадолженность.ТекущиеДанные;
	Если ДанныеКонтрагента = Неопределено Тогда
		Отказ = Истина;
	ИначеЕсли Не ЗначениеЗаполнено(ДанныеКонтрагента.ДоговорКонтрагента) Тогда
		Отказ = Истина;
	ИначеЕсли НЕ АР_ОбщиеПроцедуры.ЗначениеРеквизитаОбъекта(ДанныеКонтрагента.ДоговорКонтрагента, "АР_АрендныйДоговор") Тогда
		Если НЕ (ЗначениеЗаполнено(ДанныеКонтрагента.КорДоговорКонтрагента) 
			И АР_ОбщиеПроцедуры.ЗначениеРеквизитаОбъекта(ДанныеКонтрагента.КорДоговорКонтрагента, "АР_АрендныйДоговор")) Тогда
			ТекстСообщения = "Добавление дополнительной аналитики возможно только для арендного договора";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "ДополнительнаяАналитикаКт", "Объект", Отказ);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ДополнительнаяАналитикаКтПериодАрендыПриИзменении(Элемент)
	
	ТекущиеДанныеКредиторскаяЗадолженность = Элементы.ТаблицаКредиторскаяЗадолженность.ТекущиеДанные;
	Если ТекущиеДанныеКредиторскаяЗадолженность <> Неопределено Тогда
		ТекущиеДанные = Элементы.ДополнительнаяАналитикаКт.ТекущиеДанные;
		ПривестиКНачалуПериода(ТекущиеДанные.ПериодАренды, ТекущиеДанныеКредиторскаяЗадолженность.ДоговорКонтрагента);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ДополнительнаяАналитикаКтКурсВзаиморасчетовПриИзменении(Элемент)
	
	ДанныеСтрокаТаблицы = ПолучитьСтруктуруРеквизитовСтроки();
	ЗаполнитьЗначенияСвойств(ДанныеСтрокаТаблицы, Элементы.ТаблицаКредиторскаяЗадолженность.ТекущиеДанные);
	ЗаполнитьЗначенияСвойств(ДанныеСтрокаТаблицы, Элемент.Родитель.ТекущиеДанные);
...
&НаКлиенте
Процедура ДополнительнаяАналитикаКтСуммаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ДополнительнаяАналитикаКтСуммаВзаиморасчетовПриИзменении(Элемент)
