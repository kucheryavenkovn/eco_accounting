					...
					Иначе
						...
						Иначе	
							...
							Если ВидДоговора = ВидДоговораСПокупателем Тогда
								...
								//++ЭЛИАС 20130516  
								СтруктураНовыхРеквизитовДокумента.Вставить("АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеКт", ПолучитьСтруктуруДополнительнойАналитикиАренды(Объект, Ложь));
								//--ЭЛИАС 20130516
					...
					Иначе
						...
						Иначе	
							...
							Если ВидДоговора = ВидДоговораСПокупателем Тогда
								...
								//++ЭЛИАС 20130516  
								СтруктураНовыхРеквизитовДокумента.Вставить("АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеДт", ПолучитьСтруктуруДополнительнойАналитикиАренды(Объект, Истина));
								//--ЭЛИАС 20130516
...
Процедура ЗаполнитьОстаткамиВзаиморасчетов(СтруктураПараметров, АдресХранилища) Экспорт
	...
	//++ЭЛИАС 20160920 
	СтруктураДанныхЗаполнения.Вставить("ТаблицыДопАналитики", Новый Соответствие);
	//--ЭЛИАС 20160920
	...
	Для каждого ЗаполняемаяТаблица Из СтруктураПараметров.ЗаполняемыеТаблицы Цикл
		...
		СтруктураТаблицЗадолженности = ПолучитьОстаткиВзаиморасчетовПоВидуЗадолженности(СтруктураПараметров, ЗаполняемаяТаблица);
		ТаблицаЗадолженности = СтруктураТаблицЗадолженности.ТаблицаЗадолженности;
		ТаблицаДопАналитика = СтруктураТаблицЗадолженности.ТаблицаДопАналитика;
		СтруктураДанныхЗаполнения.ТаблицыДопАналитики.Вставить(ЗаполняемаяТаблица.ВидЗадолженности, ТаблицаДопАналитика);
		//--ЭЛИАС 20160920
...
Функция ПолучитьОстаткиВзаиморасчетовПоВидуЗадолженности(СтруктураПараметров, ЗаполняемаяТаблица)
	...
	//++ЭЛИАС 20160920 
	Если ВидЗадолженности = Перечисления.ВидыЗадолженности.Дебиторская Тогда
		ИмяТаблицыДопАналитика = "АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеДт";
	Иначе		
		ИмяТаблицыДопАналитика = "АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеКт";
	КонецЕсли;
	ТаблицаДопАналитика = Документы.КорректировкаДолга.ПустаяСсылка()[ИмяТаблицыДопАналитика].ВыгрузитьКолонки();
	
	СтруктураПараметровАренды = Новый Структура("УслугаАренды, СчетНаАренду, ПериодАренды, ОбъектАренды, Депозит, СпособПогашенияПени, ГраницаОстатков,
	|Период, Регистратор, Организация, Контрагент, ДоговорКонтрагента, НаправлениеДвижения");
	ЗаполнитьЗначенияСвойств(СтруктураПараметровАренды, СтруктураПараметров);
	СтруктураПараметровАренды.Период = СтруктураПараметров.Дата;
	СтруктураПараметровАренды.Регистратор = СтруктураПараметров.Ссылка;
	
	СтруктураПараметровАренды.Вставить("НаправлениеДвижения", ?(ВидЗадолженности = Перечисления.ВидыЗадолженности.Кредиторская, "Выбытие", "Поступление"));
	//--ЭЛИАС 20160920
	...
	Пока ВыборкаОстатков.Следующий() Цикл
		...
		//++ЭЛИАС 20120706
		НоваяСтрока.АР_Ключ = Новый УникальныйИдентификатор;
		
		Если СтруктураПараметров.АР_КорректировкаДолгаПоАренде Тогда
			СтруктураПараметровАренды.Вставить("ДоговорКонтрагента", НоваяСтрока.ДоговорКонтрагента);
			СтруктураПараметровАренды.Вставить("Контрагент", НоваяСтрока.ДоговорКонтрагента.Владелец);
			ТаблицаОстатков = АР_УправлениеВзаиморасчетами.ПолучитьОстатокПоАренднымВзаиморасчетам(СтруктураПараметровАренды, Новый Структура());
			Сч = 1;
			Для Каждого СтрокаОстатков Из ТаблицаОстатков Цикл
				Если Не СтрокаОстатков.ДокументРасчетов = НоваяСтрока.Сделка Тогда
					Продолжить;
				КонецЕсли;
				НоваяСтрокаДопАналитика = ТаблицаДопАналитика.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаДопАналитика, СтрокаОстатков);
				НоваяСтрокаДопАналитика.Ключ = НоваяСтрока.АР_Ключ;
				
				Если СписокВалютыРеглУчета.НайтиПоЗначению(СтруктураПараметров.ВалютаДокумента) = Неопределено Тогда
					НоваяСтрокаДопАналитика.СуммаВзаиморасчетов = СтрокаОстатков.СуммаВзаиморасчетов;
					
					НоваяСтрокаДопАналитика.КурсВзаиморасчетов = СтруктураПараметров.КурсДокумента;
					НоваяСтрокаДопАналитика.КратностьВзаиморасчетов = СтруктураПараметров.КратностьДокумента;
					
					Если СтрокаОстатков.ДоговорКонтрагента.РасчетыВУсловныхЕдиницах Тогда
						НоваяСтрокаДопАналитика.Сумма = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(НоваяСтрокаДопАналитика.СуммаВзаиморасчетов,
						СтруктураПараметров.ВалютаДокумента, ВалютаРеглУчета,
						НоваяСтрокаДопАналитика.КурсВзаиморасчетов, 1,
						НоваяСтрокаДопАналитика.КратностьВзаиморасчетов, 1)
					Иначе
						НоваяСтрокаДопАналитика.Сумма = СтрокаОстатков.СуммаВзаиморасчетов;
					КонецЕсли;
					
					Если ЭтоАванс Тогда
						НоваяСтрокаДопАналитика.КурсВзаиморасчетов = ?(СтрокаОстатков.СуммаВзаиморасчетов = 0, НоваяСтрокаДопАналитика.КурсВзаиморасчетов, Окр(СтрокаОстатков.СуммаБУ / СтрокаОстатков.СуммаВзаиморасчетов, 4));
						НоваяСтрокаДопАналитика.КратностьВзаиморасчетов = 1;
					КонецЕсли;
					
				Иначе
					Если СписокВалютыРеглУчета.НайтиПоЗначению(СтрокаОстатков.ВалютаВзаиморасчетов) = Неопределено Тогда
						// Это расчеты в у.е.
						НоваяСтрокаДопАналитика.СуммаВзаиморасчетов = СтрокаОстатков.СуммаВзаиморасчетов;
						КурсИКратностьРасчетов = РаботаСКурсамиВалют.ПолучитьКурсВалюты(СтрокаОстатков.ВалютаВзаиморасчетов, СтруктураПараметров.Дата);
						
						НоваяСтрокаДопАналитика.КратностьВзаиморасчетов = КурсИКратностьРасчетов.Кратность;
						НоваяСтрокаДопАналитика.КурсВзаиморасчетов = ?(СтрокаОстатков.СуммаВзаиморасчетов = 0, 0,
						СтрокаОстатков.СуммаБУ / СтрокаОстатков.СуммаВзаиморасчетов) /
						?(КурсИКратностьРасчетов.Кратность = 0, 1, КурсИКратностьРасчетов.Кратность);
						
					Иначе
						НоваяСтрокаДопАналитика.СуммаВзаиморасчетов 	= СтрокаОстатков.СуммаБУ;
						НоваяСтрокаДопАналитика.КурсВзаиморасчетов 		= 1;
						НоваяСтрокаДопАналитика.КратностьВзаиморасчетов = 1;
					КонецЕсли;
					НоваяСтрокаДопАналитика.Сумма = СтрокаОстатков.СуммаБУ;
				КонецЕсли;
				
				Если СтруктураПараметров.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.СписаниеЗадолженности
					ИЛИ СтруктураПараметров.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.Прочее Тогда
					НоваяСтрокаДопАналитика.СуммаНУ = НоваяСтрокаДопАналитика.Сумма;
				КонецЕсли;
	...
	Возврат Новый Структура("ТаблицаЗадолженности, ТаблицаДопАналитика", ТаблицаЗадолженности, ТаблицаДопАналитика);
	//--ЭЛИАС 20160920
...
//++ЭЛИАС 20130516  
Функция ПолучитьСтруктуруДополнительнойАналитикиАренды(Объект, ЭтоДебиторскаяЗадолженность)
	
	Суффикс = ?(ЭтоДебиторскаяЗадолженность, "Дт", "Кт");
	Пени = Ложь;
	Депозит = Ложь;
	Если ЭтоДебиторскаяЗадолженность Тогда
		Пени = Объект.АР_ПениДт;
	Иначе
		Депозит = Объект.АР_ДепозитКт;
	КонецЕсли;
	
	СтруктураТабличнойЧасти = Новый Структура;
	СтруктураТабличнойЧасти.Вставить("НовыйУслугаАренды", Объект["АР_УслугаАренды" + Суффикс]);
	СтруктураТабличнойЧасти.Вставить("НовыйОбъектАренды", Объект["АР_ОбъектАренды" + Суффикс]);
	СтруктураТабличнойЧасти.Вставить("НовыйСчетНаАренду", Объект["АР_СчетНаАренду" + Суффикс]);
	СтруктураТабличнойЧасти.Вставить("НовыйПериодАренды", Объект["АР_ПериодАренды" + Суффикс]);
	СтруктураТабличнойЧасти.Вставить("НовыйПени", Пени);
	СтруктураТабличнойЧасти.Вставить("НовыйДепозит", Депозит);
	
	Возврат СтруктураТабличнойЧасти;
	
КонецФункции //--ЭЛИАС 20130516
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	"ВЫБРАТЬ
	|//++ЭЛИАС 20120706
	|	Реквизиты.АР_КорректировкаДолгаПоАренде КАК КорректировкаДолгаПоАренде,
	|//--ЭЛИАС 20120706
	...
	//++ЭЛИАС 20120706
	Запрос.УстановитьПараметр("КорректировкаДолгаПоАренде", Выборка.КорректировкаДолгаПоАренде);
	
	ТребуетсяСписатьДебиторскуюЗадолжность = Истина;
	ТребуетсяСписатьКредиторскуюЗадолжность = Истина;
	
	Если ДокументСсылка.ВидОперации = Перечисления.ВидыОперацийКорректировкаДолга.Прочее Тогда
		
		// 1 - сумма ДЗ и КЗ равны, то документ списывает только ту задолженность, по которой заполнен счет. Если не заполнены оба счета, то списывваем все
		Если ДокументСсылка.СуммаДтЗадолженности = ДокументСсылка.СуммаКтЗадолженности Тогда
			Если НЕ ЗначениеЗаполнено(ДокументСсылка.СчетДт) И НЕ ЗначениеЗаполнено(ДокументСсылка.СчетКт) Тогда 
				ТребуетсяСписатьДебиторскуюЗадолжность = Истина;
				ТребуетсяСписатьКредиторскуюЗадолжность = Истина;
			ИначеЕсли НЕ ЗначениеЗаполнено(ДокументСсылка.СчетДт) Тогда
				ТребуетсяСписатьДебиторскуюЗадолжность = Ложь;	
			ИначеЕсли НЕ ЗначениеЗаполнено(ДокументСсылка.СчетКт) Тогда
				ТребуетсяСписатьКредиторскуюЗадолжность = Ложь;	
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Запрос.УстановитьПараметр("АрендаТребуетсяСписатьДебиторскуюЗадолжность", ТребуетсяСписатьДебиторскуюЗадолжность);
	Запрос.УстановитьПараметр("АрендаТребуетсяСписатьКредиторскуюЗадолжность", ТребуетсяСписатьКредиторскуюЗадолжность);
	//--ЭЛИАС 20120706
	...
	Запрос.Текст = ТекстЗапросаВременныеТаблицы(НомераТаблиц)
		...
		+ ТекстЗапросаВзаиморасчетыПоАренде(НомераТаблиц); //++ЭЛИАС 20120705
...
//++ЭЛИАС 20120705
Функция ТекстЗапросаВзаиморасчетыПоАренде(НомераТаблиц)

	НомераТаблиц.Вставить("ВзаиморасчетыПоАрендеРеквизиты", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ВзаиморасчетыПоАрендеДебиторскаяЗадолженность", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ВзаиморасчетыПоАрендеКредиторскаяЗадолженность", НомераТаблиц.Количество()); 
	НомераТаблиц.Вставить("ВременнаяЭтоАрендаДт", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ВременнаяЭтоАрендаКт", НомераТаблиц.Количество());
	НомераТаблиц.Вставить("ВзаиморасчетыПоАренде", НомераТаблиц.Количество());
	
	
	// ВзаиморасчетыПоАрендеРеквизиты
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	Реквизиты.Ссылка,
	|	Реквизиты.Дата КАК Период,
	|	Реквизиты.Ссылка КАК Регистратор,
	|	Реквизиты.Организация КАК Организация,
	|	Реквизиты.КонтрагентДебитор,
	|	Реквизиты.КонтрагентКредитор,
	|	Реквизиты.ВидОперации,
	|	Реквизиты.АР_УслугаАрендыДт,
	|	Реквизиты.АР_ОбъектАрендыДт,
	|	Реквизиты.АР_СчетНаАрендуДт,
	|	Реквизиты.АР_ПериодАрендыДт,
	|	Реквизиты.АР_ПениДт КАК Пени,
	|	Реквизиты.АР_УслугаАрендыКт,
	|	Реквизиты.АР_ОбъектАрендыКт,
	|	Реквизиты.АР_СчетНаАрендуКт,
	|	Реквизиты.АР_ПериодАрендыКт,
	|	Реквизиты.АР_ДепозитКт КАК Депозит,
	|	&ДоговорДтСсылка КАК ДоговорДт,
	|	&ДоговорДтАР_АрендныйДоговор КАК АрендныйДоговорДт,
	|	&ДоговорДтАР_ВестиВзаиморасчетыПоПериодам КАК ВестиВзаиморасчетыПоПериодамДт,
	|	&ДоговорДтАР_ВестиВзаиморасчетыПоОбъектам КАК ВестиВзаиморасчетыПоОбъектамДт,
	|	&ДоговорДтАР_ВестиВзаиморасчетыПоУслугам КАК ВестиВзаиморасчетыПоУслугамДт,
	|	&ДоговорДтАР_ВестиВзаиморасчетыПоСчетамПост
	|		ИЛИ &ДоговорДтАР_ВестиВзаиморасчетыПоСчетамПерем КАК ВестиВзаиморасчетыПоСчетамДт,
	|	&ДоговорКтСсылка КАК ДоговорКт,
	|	&ДоговорКтАР_ВестиВзаиморасчетыПоПериодам КАК ВестиВзаиморасчетыПоПериодамКт,
	|	&ДоговорКтАР_ВестиВзаиморасчетыПоОбъектам КАК ВестиВзаиморасчетыПоОбъектамКт,
	|	&ДоговорКтАР_ВестиВзаиморасчетыПоУслугам КАК ВестиВзаиморасчетыПоУслугамКт,
	|	&ДоговорКтАР_ВестиВзаиморасчетыПоСчетамПост
	|		ИЛИ &ДоговорКтАР_ВестиВзаиморасчетыПоСчетамПерем КАК ВестиВзаиморасчетыПоСчетамКт,
	|	&ДоговорКтАР_АрендныйДоговор КАК АрендныйДоговорКт
	|ПОМЕСТИТЬ ВТ_ВзаиморасчетыПоАрендеРеквизиты
	|ИЗ
	|	Документ.КорректировкаДолга КАК Реквизиты
	|ГДЕ
	|	Реквизиты.Ссылка = &Ссылка"
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ДоговорДт", "Реквизиты.СубконтоДт2.");
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ДоговорКт", "Реквизиты.СубконтоКт2.");
	
	// ВзаиморасчетыПоАрендеДебиторскаяЗадолженность
	ТекстЗапроса = ТекстЗапроса + 
	"ВЫБРАТЬ
	|	КорректировкаДолгаДебиторскаяЗадолженность.Ссылка,
	|	КорректировкаДолгаДебиторскаяЗадолженность.НомерСтроки,
	...
	|	КорректировкаДолгаДебиторскаяЗадолженность.СуммаНУ,
	|	КорректировкаДолгаДебиторскаяЗадолженность.СуммаВзаиморасчетов,
	|	КорректировкаДолгаДебиторскаяЗадолженность.СчетУчетаРасчетов,
	|	КорректировкаДолгаДебиторскаяЗадолженность.АР_Ключ,
	|	КорректировкаДолгаДебиторскаяЗадолженность.КорДоговорКонтрагента,
	|	КорректировкаДолгаДебиторскаяЗадолженность.КорСчетУчетаРасчетов,
	|	КорректировкаДолгаДебиторскаяЗадолженность.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоПериодам КАК ВестиВзаиморасчетыПоПериодам,
	|	КорректировкаДолгаДебиторскаяЗадолженность.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоОбъектам КАК ВестиВзаиморасчетыПоОбъектам,
	|	КорректировкаДолгаДебиторскаяЗадолженность.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоУслугам КАК ВестиВзаиморасчетыПоУслугам,
	|	КорректировкаДолгаДебиторскаяЗадолженность.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПост
	|		ИЛИ КорректировкаДолгаДебиторскаяЗадолженность.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПерем КАК ВестиВзаиморасчетыПоСчетам,
	|	КорректировкаДолгаДебиторскаяЗадолженность.ДоговорКонтрагента.АР_АрендныйДоговор КАК АрендныйДоговор,
	|	КорректировкаДолгаДебиторскаяЗадолженность.КорДоговорКонтрагента.АР_ВестиВзаиморасчетыПоПериодам КАК ВестиВзаиморасчетыПоПериодамКор,
	|	КорректировкаДолгаДебиторскаяЗадолженность.КорДоговорКонтрагента.АР_ВестиВзаиморасчетыПоОбъектам КАК ВестиВзаиморасчетыПоОбъектамКор,
	|	КорректировкаДолгаДебиторскаяЗадолженность.КорДоговорКонтрагента.АР_ВестиВзаиморасчетыПоУслугам КАК ВестиВзаиморасчетыПоУслугамКор,
	|	КорректировкаДолгаДебиторскаяЗадолженность.КорДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПост
	|		ИЛИ КорректировкаДолгаДебиторскаяЗадолженность.КорДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПерем КАК ВестиВзаиморасчетыПоСчетамКор,
	|	КорректировкаДолгаДебиторскаяЗадолженность.КорДоговорКонтрагента.АР_АрендныйДоговор КАК АрендныйДоговорКор
	|ПОМЕСТИТЬ ВТ_КорректировкаДолгаДебиторскаяЗадолженность
	|ИЗ
	|	Документ.КорректировкаДолга.ДебиторскаяЗадолженность КАК КорректировкаДолгаДебиторскаяЗадолженность"
	+ ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
	
	// ВзаиморасчетыПоАрендеКредиторскаяЗадолженность
	ТекстЗапроса = ТекстЗапроса + 
	"ВЫБРАТЬ
	|	КорректировкаДолгаКредиторскаяЗадолженность.Ссылка,
	|	КорректировкаДолгаКредиторскаяЗадолженность.НомерСтроки,
	|	КорректировкаДолгаКредиторскаяЗадолженность.ДоговорКонтрагента,
	...
	|	КорректировкаДолгаКредиторскаяЗадолженность.СуммаНУ,
	|	КорректировкаДолгаКредиторскаяЗадолженность.СуммаВзаиморасчетов,
	|	КорректировкаДолгаКредиторскаяЗадолженность.СчетУчетаРасчетов,
	|	КорректировкаДолгаКредиторскаяЗадолженность.АР_Ключ,
	|	КорректировкаДолгаКредиторскаяЗадолженность.КорДоговорКонтрагента,
	|	КорректировкаДолгаКредиторскаяЗадолженность.КорСчетУчетаРасчетов,
	|	КорректировкаДолгаКредиторскаяЗадолженность.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоПериодам КАК ВестиВзаиморасчетыПоПериодам,
	|	КорректировкаДолгаКредиторскаяЗадолженность.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоОбъектам КАК ВестиВзаиморасчетыПоОбъектам,
	|	КорректировкаДолгаКредиторскаяЗадолженность.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоУслугам КАК ВестиВзаиморасчетыПоУслугам,
	|	КорректировкаДолгаКредиторскаяЗадолженность.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПост
	|		ИЛИ КорректировкаДолгаКредиторскаяЗадолженность.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПерем КАК ВестиВзаиморасчетыПоСчетам,
	|	КорректировкаДолгаКредиторскаяЗадолженность.ДоговорКонтрагента.АР_АрендныйДоговор КАК АрендныйДоговор,
	|	КорректировкаДолгаКредиторскаяЗадолженность.КорДоговорКонтрагента.АР_ВестиВзаиморасчетыПоПериодам КАК ВестиВзаиморасчетыПоПериодамКор,
	|	КорректировкаДолгаКредиторскаяЗадолженность.КорДоговорКонтрагента.АР_ВестиВзаиморасчетыПоОбъектам КАК ВестиВзаиморасчетыПоОбъектамКор,
	|	КорректировкаДолгаКредиторскаяЗадолженность.КорДоговорКонтрагента.АР_ВестиВзаиморасчетыПоУслугам КАК ВестиВзаиморасчетыПоУслугамКор,
	|	КорректировкаДолгаКредиторскаяЗадолженность.КорДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПост
	|		ИЛИ КорректировкаДолгаКредиторскаяЗадолженность.КорДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПерем КАК ВестиВзаиморасчетыПоСчетамКор,
	|	КорректировкаДолгаКредиторскаяЗадолженность.КорДоговорКонтрагента.АР_АрендныйДоговор КАК АрендныйДоговорКор
	|ПОМЕСТИТЬ ВТ_КорректировкаДолгаКредиторскаяЗадолженность
	|ИЗ
	|	Документ.КорректировкаДолга.КредиторскаяЗадолженность КАК КорректировкаДолгаКредиторскаяЗадолженность"
	+ ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
	
	// ВременнаяЭтоАрендаДт
	// Условие в запросе должно соответствовать условию в запросе ТекстЗапросаТаблицаПереносЗадолженностиДебиторскаяЗадолженность
	ТекстЗапроса = ТекстЗапроса + 
	"ВЫБРАТЬ
	|	Количество(*) КАК Количество
	|ПОМЕСТИТЬ ВТ_НаличиеДебиторскойЗадолженности
	|ИЗ
	|	ВТ_ДебиторскаяЗадолженность КАК ТаблицаДебиторскаяЗадолженность
	|ГДЕ
	|	(ТаблицаДебиторскаяЗадолженность.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКорректировкаДолга.ПереносЗадолженности)
	...
	|				И ТаблицаДебиторскаяЗадолженность.СубконтоДт2 = ТаблицаДебиторскаяЗадолженность.СубконтоКт2
	|				И ТаблицаДебиторскаяЗадолженность.СубконтоДт3 = ТаблицаДебиторскаяЗадолженность.СубконтоКт3
	|				И ТаблицаДебиторскаяЗадолженность.Ссылка.АР_УслугаАрендыДт = ТаблицаДебиторскаяЗадолженность.Ссылка.АР_УслугаАрендыКт
	|				И ТаблицаДебиторскаяЗадолженность.Ссылка.АР_ОбъектАрендыДт = ТаблицаДебиторскаяЗадолженность.Ссылка.АР_ОбъектАрендыКт
	|				И ТаблицаДебиторскаяЗадолженность.Ссылка.АР_СчетНаАрендуДт = ТаблицаДебиторскаяЗадолженность.Ссылка.АР_СчетНаАрендуКт
	|				И ТаблицаДебиторскаяЗадолженность.Ссылка.АР_ПериодАрендыДт = ТаблицаДебиторскаяЗадолженность.Ссылка.АР_ПериодАрендыКт
	|				И (ТаблицаДебиторскаяЗадолженность.СубконтоДт1 ССЫЛКА Справочник.Контрагенты
	|					ИЛИ ТаблицаДебиторскаяЗадолженность.СубконтоДт2 ССЫЛКА Справочник.Контрагенты
	|					ИЛИ ТаблицаДебиторскаяЗадолженность.СубконтоДт3 ССЫЛКА Справочник.Контрагенты)
	|				И (ТаблицаДебиторскаяЗадолженность.СубконтоДт1 ССЫЛКА Справочник.ДоговорыКонтрагентов
	...
	// ВременнаяЭтоАрендаКт
	// Условие в запросе должно соответствовать условию в запросе ТекстЗапросаТаблицаПереносЗадолженностиКредиторскаяЗадолженность
	ТекстЗапроса = ТекстЗапроса + 
	"ВЫБРАТЬ
	|	Количество(*) КАК Количество
	|ПОМЕСТИТЬ ВТ_НаличиеКредиторскойЗадолженности
	|ИЗ
	|	ВТ_КредиторскаяЗадолженность КАК ТаблицаКредиторскаяЗадолженность
	|ГДЕ
	|	(ТаблицаКредиторскаяЗадолженность.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКорректировкаДолга.ПереносЗадолженности)
	...
	|				И ТаблицаКредиторскаяЗадолженность.СубконтоДт2 = ТаблицаКредиторскаяЗадолженность.СубконтоКт2
	|				И ТаблицаКредиторскаяЗадолженность.СубконтоДт3 = ТаблицаКредиторскаяЗадолженность.СубконтоКт3
	|				И ТаблицаКредиторскаяЗадолженность.Ссылка.АР_УслугаАрендыДт = ТаблицаКредиторскаяЗадолженность.Ссылка.АР_УслугаАрендыКт
	|				И ТаблицаКредиторскаяЗадолженность.Ссылка.АР_ОбъектАрендыДт = ТаблицаКредиторскаяЗадолженность.Ссылка.АР_ОбъектАрендыКт
	|				И ТаблицаКредиторскаяЗадолженность.Ссылка.АР_СчетНаАрендуДт = ТаблицаКредиторскаяЗадолженность.Ссылка.АР_СчетНаАрендуКт
	|				И ТаблицаКредиторскаяЗадолженность.Ссылка.АР_ПериодАрендыДт = ТаблицаКредиторскаяЗадолженность.Ссылка.АР_ПериодАрендыКт
	|				И (ТаблицаКредиторскаяЗадолженность.СубконтоДт1 ССЫЛКА Справочник.Контрагенты
	|					ИЛИ ТаблицаКредиторскаяЗадолженность.СубконтоДт2 ССЫЛКА Справочник.Контрагенты
	|					ИЛИ ТаблицаКредиторскаяЗадолженность.СубконтоДт3 ССЫЛКА Справочник.Контрагенты)
	|				И (ТаблицаКредиторскаяЗадолженность.СубконтоДт1 ССЫЛКА Справочник.ДоговорыКонтрагентов
	...
	|	Реквизиты.Организация КАК Организация,
	|	ТабличнаяЧасть.Сделка КАК ДокументРасчетов,
	|	ТабличнаяЧасть.ДоговорКонтрагента.ВалютаВзаиморасчетов КАК Валюта,
	|	ТабличнаяЧасть.ДоговорКонтрагента.Владелец КАК Контрагент,
	...
	|	ДополнительнаяАналитика.СуммаВзаиморасчетов КАК СуммаВзаиморасчетов,
	|	ДополнительнаяАналитика.Сумма КАК Сумма,
	|	ДополнительнаяАналитика.СуммаНУ КАК СуммаНУ,
	...
	|ИЗ
	|	ВТ_КорректировкаДолгаДебиторскаяЗадолженность КАК ТабличнаяЧасть
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ВзаиморасчетыПоАрендеРеквизиты КАК Реквизиты
	|		ПО ТабличнаяЧасть.Ссылка = Реквизиты.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаДолга.АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеДт КАК ДополнительнаяАналитика
	|		ПО ТабличнаяЧасть.АР_Ключ = ДополнительнаяАналитика.Ключ
	|			И ТабличнаяЧасть.Ссылка = ДополнительнаяАналитика.Ссылка
	|ГДЕ
	|	ТабличнаяЧасть.Ссылка = &Ссылка
	|	И &КорректировкаДолгаПоАренде
	|	И &АрендаТребуетсяСписатьДебиторскуюЗадолжность
	|	И ТабличнаяЧасть.АрендныйДоговор
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ТабличнаяЧасть.Сделка,
	|	ТабличнаяЧасть.ДоговорКонтрагента.ВалютаВзаиморасчетов,
	|	ТабличнаяЧасть.ДоговорКонтрагента.Владелец,
	...
	|	-ДополнительнаяАналитика.СуммаНУ,
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
	|	ДополнительнаяАналитика.Депозит,
	|	ДополнительнаяАналитика.Пени,
	|	НЕОПРЕДЕЛЕНО
	|ИЗ
	|	ВТ_КорректировкаДолгаКредиторскаяЗадолженность КАК ТабличнаяЧасть
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ВзаиморасчетыПоАрендеРеквизиты КАК Реквизиты
	|		ПО ТабличнаяЧасть.Ссылка = Реквизиты.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.КорректировкаДолга.АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеКт КАК ДополнительнаяАналитика
	|		ПО ТабличнаяЧасть.АР_Ключ = ДополнительнаяАналитика.Ключ
	|			И ТабличнаяЧасть.Ссылка = ДополнительнаяАналитика.Ссылка
	|ГДЕ
	|	ТабличнаяЧасть.Ссылка = &Ссылка
	|	И &КорректировкаДолгаПоАренде
	|	И &АрендаТребуетсяСписатьКредиторскуюЗадолжность
	|	И ТабличнаяЧасть.АрендныйДоговор
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	Реквизиты.Организация,
	|	&Ссылка,
	|	ТабличнаяЧасть.КорДоговорКонтрагента.ВалютаВзаиморасчетов,
	|	ТабличнаяЧасть.КорДоговорКонтрагента.Владелец,
	|	ВЫБОР
	|		КОГДА ТабличнаяЧасть.ВестиВзаиморасчетыПоУслугамКор
	|			ТОГДА ДополнительнаяАналитика.НовыйУслугаАренды
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТабличнаяЧасть.ВестиВзаиморасчетыПоОбъектамКор
	|			ТОГДА ДополнительнаяАналитика.НовыйОбъектАренды
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.АР_ОбъектыАренды.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТабличнаяЧасть.ВестиВзаиморасчетыПоСчетамКор
	|			ТОГДА ДополнительнаяАналитика.НовыйСчетНаАренду
	|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.АР_СчетНаАренду.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ТабличнаяЧасть.ВестиВзаиморасчетыПоПериодамКор
	|			ТОГДА ДополнительнаяАналитика.НовыйПериодАренды
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|	КОНЕЦ,
	|	СУММА(ВЫБОР
	|			КОГДА ТабличнаяЧасть.КорДоговорКонтрагента.Владелец = ТабличнаяЧасть.ДоговорКонтрагента.Владелец
	|					И ТабличнаяЧасть.КорДоговорКонтрагента.ВалютаВзаиморасчетов = ТабличнаяЧасть.ДоговорКонтрагента.ВалютаВзаиморасчетов
	|				ТОГДА ДополнительнаяАналитика.СуммаВзаиморасчетов
	|			ИНАЧЕ ДополнительнаяАналитика.Сумма / (КурсыВалютСрезПоследних.Курс / ВЫБОР
	|					КОГДА КурсыВалютСрезПоследних.Кратность = 0
	|						ТОГДА 1
	|					ИНАЧЕ КурсыВалютСрезПоследних.Кратность
	|				КОНЕЦ)
	|		КОНЕЦ),
	|	СУММА(ДополнительнаяАналитика.Сумма),
	|	СУММА(ДополнительнаяАналитика.СуммаНУ),
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход),
	|	ЛОЖЬ,
	|	ДополнительнаяАналитика.НовыйПени,
	|	&Ссылка
	|ИЗ
	|	ВТ_КорректировкаДолгаДебиторскаяЗадолженность КАК ТабличнаяЧасть
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ВзаиморасчетыПоАрендеРеквизиты КАК Реквизиты
	...
	|		ПО ТабличнаяЧасть.АР_Ключ = ДополнительнаяАналитика.Ключ
	|			И ТабличнаяЧасть.Ссылка = ДополнительнаяАналитика.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_НаличиеДебиторскойЗадолженности КАК НаличиеДебиторскойЗадолженности
	|		ПО (НЕ НаличиеДебиторскойЗадолженности.Количество ЕСТЬ NULL)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Дата, ) КАК КурсыВалютСрезПоследних
	|		ПО (КурсыВалютСрезПоследних.Валюта = ТабличнаяЧасть.КорДоговорКонтрагента.ВалютаВзаиморасчетов)
	|ГДЕ
	|	ТабличнаяЧасть.Ссылка = &Ссылка
	|	И &КорректировкаДолгаПоАренде
	|	И Реквизиты.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКорректировкаДолга.ПереносЗадолженности)
	|	И ТабличнаяЧасть.АрендныйДоговорКор
	|
	|СГРУППИРОВАТЬ ПО
	...
	|		КОГДА ТабличнаяЧасть.КорДоговорКонтрагента.Владелец = ТабличнаяЧасть.ДоговорКонтрагента.Владелец
	|				И ТабличнаяЧасть.КорДоговорКонтрагента.ВалютаВзаиморасчетов = ТабличнаяЧасть.ДоговорКонтрагента.ВалютаВзаиморасчетов
	|			ТОГДА ТабличнаяЧасть.Сделка
	|		ИНАЧЕ &Ссылка
	|	КОНЕЦ,
	|	ТабличнаяЧасть.КорДоговорКонтрагента.ВалютаВзаиморасчетов,
	|	ТабличнаяЧасть.КорДоговорКонтрагента.Владелец,
	|	ВЫБОР
	|		КОГДА НЕ ДополнительнаяАналитика.НовыйДепозит
	|				И ТабличнаяЧасть.ВестиВзаиморасчетыПоУслугамКор
	|			ТОГДА ДополнительнаяАналитика.НовыйУслугаАренды
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА НЕ ДополнительнаяАналитика.НовыйДепозит
	|				И ТабличнаяЧасть.ВестиВзаиморасчетыПоОбъектамКор
	|			ТОГДА ДополнительнаяАналитика.НовыйОбъектАренды
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.АР_ОбъектыАренды.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА НЕ ДополнительнаяАналитика.НовыйДепозит
	|				И ТабличнаяЧасть.ВестиВзаиморасчетыПоСчетамКор
	|			ТОГДА ДополнительнаяАналитика.НовыйСчетНаАренду
	|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.АР_СчетНаАренду.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА НЕ ДополнительнаяАналитика.НовыйДепозит
	|				И ТабличнаяЧасть.ВестиВзаиморасчетыПоПериодамКор
	|			ТОГДА ДополнительнаяАналитика.НовыйПериодАренды
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|	КОНЕЦ,
	|	СУММА(ВЫБОР
	...
	|	СУММА(ДополнительнаяАналитика.СуммаНУ),
	|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход),
	|	ДополнительнаяАналитика.НовыйДепозит,
	|	ЛОЖЬ,
	|	&Ссылка
	|ИЗ
	|	ВТ_КорректировкаДолгаКредиторскаяЗадолженность КАК ТабличнаяЧасть
	...
	|		ПО ТабличнаяЧасть.АР_Ключ = ДополнительнаяАналитика.Ключ
	|			И ТабличнаяЧасть.Ссылка = ДополнительнаяАналитика.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_НаличиеКредиторскойЗадолженности КАК НаличиеКредиторскойЗадолженности
	|		ПО (НЕ НаличиеКредиторскойЗадолженности.Количество ЕСТЬ NULL)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Дата, ) КАК КурсыВалютСрезПоследних
	|		ПО (КурсыВалютСрезПоследних.Валюта = ТабличнаяЧасть.КорДоговорКонтрагента.ВалютаВзаиморасчетов)
	|ГДЕ
	|	ТабличнаяЧасть.Ссылка = &Ссылка
	...
	|	Реквизиты.Организация,
	|	&Ссылка,
	|	Реквизиты.ДоговорДт.ВалютаВзаиморасчетов,
	|	Реквизиты.ДоговорДт.Владелец,
	|	ВЫБОР
	|		КОГДА Реквизиты.ВестиВзаиморасчетыПоУслугамДт
	|			ТОГДА Реквизиты.АР_УслугаАрендыДт
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА Реквизиты.ВестиВзаиморасчетыПоОбъектамДт
	|			ТОГДА Реквизиты.АР_ОбъектАрендыДт
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.АР_ОбъектыАренды.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА Реквизиты.ВестиВзаиморасчетыПоСчетамДт
	|			ТОГДА Реквизиты.АР_СчетНаАрендуДт
	|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.АР_СчетНаАренду.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА Реквизиты.ВестиВзаиморасчетыПоПериодамДт
	|			ТОГДА Реквизиты.АР_ПериодАрендыДт
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА Реквизиты.ДоговорДт.Владелец = Реквизиты.ДоговорКт.Владелец
	|				И Реквизиты.ДоговорДт.ВалютаВзаиморасчетов = Реквизиты.ДоговорКт.ВалютаВзаиморасчетов
	|			ТОГДА ТабличнаяЧасть.СуммаВзаиморасчетов
	|		ИНАЧЕ ТабличнаяЧасть.Сумма / (КурсыВалютСрезПоследних.Курс / ВЫБОР
	|				КОГДА КурсыВалютСрезПоследних.Кратность = 0
	|					ТОГДА 1
	|				ИНАЧЕ КурсыВалютСрезПоследних.Кратность
	|			КОНЕЦ)
	...
	|		ПО (НЕ НаличиеДебиторскойЗадолженности.Количество ЕСТЬ NULL)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Дата, ) КАК КурсыВалютСрезПоследних
	|		ПО (КурсыВалютСрезПоследних.Валюта = Реквизиты.ДоговорДт.ВалютаВзаиморасчетов)
	|ГДЕ
	|	ТабличнаяЧасть.Ссылка = &Ссылка
	|	И &КорректировкаДолгаПоАренде
	|	И Реквизиты.АрендныйДоговорДт
	|	И Реквизиты.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКорректировкаДолга.Прочее)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	Реквизиты.Организация,
	|	&Ссылка,
	|	Реквизиты.ДоговорКт.ВалютаВзаиморасчетов,
	|	Реквизиты.ДоговорКт.Владелец,
	|	ВЫБОР
	|		КОГДА НЕ Реквизиты.Депозит
	|				И Реквизиты.ВестиВзаиморасчетыПоУслугамКт
	|			ТОГДА Реквизиты.АР_УслугаАрендыКт
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА НЕ Реквизиты.Депозит
	|				И Реквизиты.ВестиВзаиморасчетыПоОбъектамКт
	|			ТОГДА Реквизиты.АР_ОбъектАрендыКт
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.АР_ОбъектыАренды.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА НЕ Реквизиты.Депозит
	|				И Реквизиты.ВестиВзаиморасчетыПоСчетамКт
	|			ТОГДА Реквизиты.АР_СчетНаАрендуКт
	|		ИНАЧЕ ЗНАЧЕНИЕ(Документ.АР_СчетНаАренду.ПустаяСсылка)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА НЕ Реквизиты.Депозит
	|				И Реквизиты.ВестиВзаиморасчетыПоПериодамКт
	|			ТОГДА Реквизиты.АР_ПериодАрендыКт
	|		ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА Реквизиты.ДоговорДт.Владелец = Реквизиты.ДоговорКт.Владелец
	...
	|		ПО (НЕ НаличиеКредиторскойЗадолженности.Количество ЕСТЬ NULL)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют.СрезПоследних(&Дата, ) КАК КурсыВалютСрезПоследних
	|		ПО (КурсыВалютСрезПоследних.Валюта = Реквизиты.ДоговорКт.ВалютаВзаиморасчетов)
	|ГДЕ
	|	ТабличнаяЧасть.Ссылка = &Ссылка
	|	И &КорректировкаДолгаПоАренде
	|	И Реквизиты.АрендныйДоговорКт
	|	И Реквизиты.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКорректировкаДолга.Прочее)"
	+ ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();	

	Возврат ТекстЗапроса;
 
КонецФункции //--ЭЛИАС 20120705
