	...
	Для Каждого ТекущаяТаблица Из ПараметрыЗаполнения.Контрагенты Цикл
		...
		Иначе
			...
			Для Каждого ТекущаяСтрока Из ОстаткиЗадолженности Цикл
				...
				ИначеЕсли СчетаУчетПоНаименованию.Найти(ТекущаяСтрока.СчетРасчетов) <> Неопределено Тогда
...
Функция ТекстЗапросаОстаткиЗадолженностиРасчетыСБюджетом(СводныеДанные = Ложь)
	...
	Если СводныеДанные Тогда
		...
		|	ВЫРАЗИТЬ(ВТ_ОстаткиПоВидамПлатежей.Счет КАК ПланСчетов.Хозрасчетный).Наименование КАК НаименованиеСчета,
		...
		|ВЫБРАТЬ
		|	НЕОПРЕДЕЛЕНО,
		|	ВЫРАЗИТЬ(ВТ_ОстаткиПоВидамПлатежейУровнямБюджета.Счет КАК ПланСчетов.Хозрасчетный).Наименование,
