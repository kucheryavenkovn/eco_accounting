...
Функция ТаблицаУслугНПД(Документ) Экспорт
...
Функция ПодготовитьДанныеДляЗаполненияДокументов(МассивСсылокНаОбъект, СчетаФактурыРеализаций = Неопределено) Экспорт
	...
	Для Каждого СсылкаНаОбъект Из МассивСсылокНаОбъект Цикл
		...
		КонецЕсли;
		НастройкиОбмена.Вставить("ВерсияФормата", "ON_NSCHFDOPPR_1_997_01_05_01");
...
Функция СформироватьЭДПрисоединенныйФайлПакетаТакском(СтруктураОбмена, СчетаФактурыРеализаций) 
	...
	ОбменСКонтрагентамиВнутренний.СформироватьТранспортнуюИнформацию(СтруктураОбмена.СтруктураЭД,
		СтруктураФайловЭД, АдресКаталога, ТекстОшибки, Неопределено);
...
Функция ТекстЗапросаРеализованнаяНоменклатура(НомераТаблиц) Экспорт
	...
	|	ГДЕ
	|		(РеализованныеТовары.СчетНаОплатуПокупателю = &СчетНаОплату
	...
	|	ГДЕ
	|		(РеализованныеУслуги.СчетНаОплатуПокупателю = &СчетНаОплату
	|				ИЛИ ВТ_РеализацииПоСчету.СчетНаОплату = &СчетНаОплату)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|	ГДЕ
	|		(РеализованныеАгентскиеУслуги.СчетНаОплатуПокупателю = &СчетНаОплату
...
Функция ТекстЗапросаДанныеДляПечатиЧека(ИменаТаблиц) Экспорт
	...
	|	ЕСТЬNULL(ОплачиваемаяНоменклатура.Номенклатура.КодНоменклатурнойКлассификацииККТ.Код, НЕОПРЕДЕЛЕНО) КАК КодВидаНоменклатурнойКлассификации,
	...
	|	ЕСТЬNULL(ОплачиваемаяНоменклатура.Номенклатура.КодНоменклатурнойКлассификацииККТ.Код, НЕОПРЕДЕЛЕНО),
