...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Претензия") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "Претензия", "Претензия",
			ПечатьПретензии(МассивОбъектов, ОбъектыПечати),, "Документ.НачислениеПеней.ПФ_MXL_Претензия");
...
Функция ПечатьПретензии(МассивОбъектов, ОбъектыПечати)
	...
	ДанныеШапки = Новый Структура;
	ДанныеШапки1 = Новый Структура;
	ДанныеПодвал = Новый Структура;
	ДанныеСтроки = Новый Структура;
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.НачислениеПеней.ПФ_MXL_Претензия");
	...
	|	ПРЕДСТАВЛЕНИЕ(НачислениеПеней.ДоговорКонтрагента) КАК Основание,
	|	НачислениеПеней.СтавкаПени КАК СтавкаПени,
	|	ПРЕДСТАВЛЕНИЕ(НачислениеПеней.ПериодРасчета) КАК ПериодРасчета,
	|	НачислениеПеней.Дата КАК Дата,
	|	НачислениеПеней.ПодразделениеОрганизации КАК ПодразделениеОрганизации,
	|	СУММА(ЕСТЬNULL(НачислениеПенейЗадолженность.Сумма, 0)) КАК СуммаПени,
	|	СУММА(ЕСТЬNULL(НачислениеПенейЗадолженность.Задолженность, 0)) КАК Задолженность,
	|	НачислениеПеней.ВалютаДокумента КАК ВалютаДокумента,
	|	НачислениеПеней.ДоговорКонтрагента.Номер КАК НомерД,
	|	НачислениеПеней.ДоговорКонтрагента.Дата КАК ДатаД
	|ИЗ
	|	Документ.НачислениеПеней КАК НачислениеПеней
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.НачислениеПеней.Задолженность КАК НачислениеПенейЗадолженность
	|		ПО (НачислениеПенейЗадолженность.Ссылка = НачислениеПеней.Ссылка)
	...
	|	НачислениеПенейЗадолженность.Сделка КАК ДокументРасчетов,
	|	НачислениеПенейЗадолженность.Задолженность КАК Задолженность,
	|	НачислениеПенейЗадолженность.Просрочка КАК Просрочка,
	|	НачислениеПенейЗадолженность.Сумма КАК СуммаПени,
	|	НачислениеПенейЗадолженность.Представление КАК Представление,
	|	НачислениеПенейЗадолженность.НачПериода КАК НачПериода,
	|	НачислениеПенейЗадолженность.КонПериода КАК КонПериода,
	|	НачислениеПенейЗадолженность.Отсрочка КАК Отсрочка
	|ИЗ
	|	Документ.НачислениеПеней.Задолженность КАК НачислениеПенейЗадолженность
	|ГДЕ
	|	НачислениеПенейЗадолженность.Ссылка В(&МассивОбъектов)";
	...
	Для каждого СведенияОДокументе Из ТаблицаДокументов Цикл
		...
		СведенияОбОрганизации    = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(СведенияОДокументе.Организация, СведенияОДокументе.Дата);
		ПредставлениеОрганизации = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОбОрганизации, "НаименованиеДляПечатныхФорм,");
		ПредставлениеОрганизации1 = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОбОрганизации, "НаименованиеДляПечатныхФорм,ИНН,КПП,ЮридическийАдрес,Телефоны,КодПоОКПО");
		// Запомним номер строки, с которой начали выводить текущий документ.
		НомерСтрокиНачало = ТабличныйДокумент.ВысотаТаблицы + 1;
		...
		СведенияОКонтрагенте    = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(СведенияОДокументе.Контрагент, СведенияОДокументе.Дата);
		ПредставлениеКонтрагента = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОКонтрагенте, "НаименованиеДляПечатныхФорм,");
		АдресКонтрагента = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОКонтрагенте, "ЮридическийАдрес,");
		СтавкаПениПредставление = СтрШаблон(
			НСтр("ru='%1 %% в %2'"), 
			СведенияОДокументе.СтавкаПени, 
			НРег(СведенияОДокументе.ПериодРасчета));
		...
		ДанныеШапки1.Вставить("ИтогоПени", ОбщегоНазначенияБПВызовСервера.ФорматСумм(СведенияОДокументе.СуммаПени));
		ДанныеШапки1.Вставить("прописьИтогоПени", ОбщегоНазначенияБПВызовСервера.СформироватьСуммуПрописью(СведенияОДокументе.СуммаПени,СведенияОДокументе.ВалютаДокумента));
		ДанныеШапки1.Вставить("ДАТАД", Формат(СведенияОДокументе.датаД,"ДЛФ=DD"));
		ДанныеШапки1.Вставить("НомерД", СведенияОДокументе.НомерД);
		ДанныеШапки1.Вставить("ПредставлениеОрганизации", ПредставлениеОрганизации);
		ДанныеШапки1.Вставить("ПредставлениеКонтрагента", ПредставлениеКонтрагента);
		ДанныеШапки1.Вставить("АдресКонтрагента",		 АдресКонтрагента);
		ОблШапка.Параметры.Заполнить(ДанныеШапки1);
		...
		Для каждого СтрокаРасчета Из ТаблицаРасчетПениПоДокументу Цикл
			...
			ДанныеСтроки.Вставить("Задолженность", ОбщегоНазначенияБПВызовСервера.ФорматСумм(СтрокаРасчета.Задолженность));
			ДанныеСтроки.Вставить("Просрочка",           Формат(СтрокаРасчета.Просрочка, "ЧДЦ="));
			ДанныеСтроки.Вставить("НачПериода",           Формат(СтрокаРасчета.НачПериода,"ДЛФ=D"));
			ДанныеСтроки.Вставить("КонПериода",           Формат(СтрокаРасчета.КонПериода, "ДЛФ=D"));
			ДанныеСтроки.Вставить("Отсрочка",           Формат(СтрокаРасчета.Отсрочка, "ЧДЦ="));
			ДанныеСтроки.Вставить("СуммаПени",           ОбщегоНазначенияБПВызовСервера.ФорматСумм(СтрокаРасчета.СуммаПени));
			Если ЗначениеЗаполнено(СтрокаРасчета.Представление) Тогда
				ДанныеСтроки.Вставить("ДокументРасчетов", СтрокаРасчета.Представление);
			Иначе
