...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ИсточникВыбора.ВладелецФормы) = Тип("ФормаКлиентскогоПриложения") 
		И ЭтоТекущаяФорма(ИсточникВыбора.ВладелецФормы) И ЗначениеЗаполнено(ВыбранноеЗначение) 
...
Процедура ГиперссылкаВводДополнительныхДанныхНажатие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("ОбщаяФорма.ВводДанныхЛисткаНетрудоспособности", ПараметрыФормы, ЭтотОбъект, , , , ОбработчикВыбора, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура УстановитьВидимостьКарантинаПоКоронавирусу()
	...
	Элементы.ГруппаКарантинПоКоронавирусу.Видимость = ОбменЛисткамиНетрудоспособностиФСС.ЭтоКарантинныйЭЛН(
			Объект.ПричинаНетрудоспособности,
...
Процедура ОбновитьСведенияОПрямыхВыплатах()
	...
	Элементы.ГруппаДанныеДляФСС.Видимость = 
		ПрямыеВыплатыПособийСоциальногоСтрахования.ПособиеПлатитУчастникПилотногоПроекта(
			Объект.Организация,
			Объект.ПериодРегистрации,
...
Процедура ИзменитьСтаж(Форма)
	...
	ИначеЕсли Объект.ПрименятьЛьготыПриНачисленииПособия Тогда
		Если Не Объект.ФинансированиеФедеральнымБюджетом = ПредопределенноеЗначение("Перечисление.ОснованияФинансированияПособияФедеральнымБюджетом.ЗачетНестраховыхПериодов") 
			И Не Объект.ПричинаНетрудоспособности = ПредопределенноеЗначение("Перечисление.ПричиныНетрудоспособности.Профзаболевание") Тогда
			Если ЗначениеЗаполнено(Форма.ДатаУвольненияСотрудника) 
				И Объект.ДатаНачалаСобытия > Форма.ДатаУвольненияСотрудника Тогда
				// Сотрудник уволен, в течение 30 календарных дней, оплачивается 60%.
				Объект.ПроцентОплатыБезЛьгот = 60;
			Иначе
	...
	Иначе
		...
		Если Не Объект.ПричинаНетрудоспособности = ПредопределенноеЗначение("Перечисление.ПричиныНетрудоспособности.Профзаболевание") Тогда
			Если ЗначениеЗаполнено(Форма.ДатаУвольненияСотрудника) 
				И Объект.ДатаНачалаСобытия > Форма.ДатаУвольненияСотрудника Тогда
				// Сотрудник уволен, в течение 30 календарных дней, оплачивается 60%.
				Объект.ПроцентОплаты = 60;
