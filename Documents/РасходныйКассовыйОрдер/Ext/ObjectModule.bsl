...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СтруктураТаблицУСН        = Новый Структура("ТаблицаРасчетов, ТаблицаВыплатаЗарплаты, ТаблицаВыплатаДепонентов, ТаблицаВыплатаДоговоровПодряда",
		ТаблицаВзаиморасчеты, ТаблицаВыплатаЗарплаты, ТаблицаВыплатаДепонентов, ТаблицаВыплатаДоговоровПодряда);
...
Процедура ЗаполнитьПоПКО()
	...
	Если ВидОперацииДокументаОснования = Перечисления.ВидыОперацийПКО.ОплатаПокупателя Тогда
		...
		АР_ОплатаАренды = ДокументОснование.АР_ОплатаАренды; //++ЭЛИАС 20120705
		...
		Для каждого СтрокаОснование Из ДокументОснование.РасшифровкаПлатежа Цикл
			...
			СтруктураКурсаВзаиморасчетов         = АР_ПроцедурыНачисления.ПолучитьКурсВалюты(
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПлатеж.ДоговорКонтрагента, "ВалютаВзаиморасчетов"), Дата, СтрокаПлатеж.ДоговорКонтрагента);
			//--ЭЛИАС 20141118
			...
			//++ЭЛИАС 20120705
			ЗаполнитьЗначенияСвойств(СтрокаПлатеж, СтрокаОснование, "АР_ПериодАренды, АР_СчетНаАренду, АР_УслугаАренды, АР_ОбъектАренды, АР_Депозит, АР_Пени"); 
			//--ЭЛИАС 20120705
