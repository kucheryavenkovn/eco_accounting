...
//++ЭЛИАС 20120622
&НаКлиенте
Процедура АР_ВводОстатковВзаиморасчетовПоАрендеПриИзменении(Элемент)

	Для Каждого СтрокаТЧ Из Объект.РасчетыСКонтрагентами Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаТЧ.АР_Ключ) Тогда
			СтрокаТЧ.АР_Ключ = Новый УникальныйИдентификатор;	
		КонецЕсли;
	КонецЦикла;
...
Процедура РасчетыСКонтрагентамиКонтрагентПриИзменении(Элемент)
	...
	//++ЭЛИАС 20120622
	ДополнительнаяАналитикаУстановитьОтборСтрок(СтрокаТаблицы, Элементы.АР_ДополнительнаяАналитикаВзаиморасчетовПоАренде);
	//--ЭЛИАС 20120622
...
Процедура РасчетыСКонтрагентамиПриАктивизацииСтроки(Элемент)
	...
	ДополнительнаяАналитикаУстановитьОтборСтрок(Элементы.РасчетыСКонтрагентами.ТекущиеДанные,
	Элементы.АР_ДополнительнаяАналитикаВзаиморасчетовПоАренде);

КонецПроцедуры

&НаКлиенте
Процедура РасчетыСКонтрагентамиПередУдалением(Элемент, Отказ)
	
	СтрокаТаблицы = Элементы.РасчетыСКонтрагентами.ТекущиеДанные;
	
	ПодчиненныеСтроки = Объект.АР_ДополнительнаяАналитикаВзаиморасчетовПоАренде.НайтиСтроки(Новый Структура("Ключ", СтрокаТаблицы.АР_Ключ));
	...
	Для каждого Строка Из ПодчиненныеСтроки Цикл
		Объект.АР_ДополнительнаяАналитикаВзаиморасчетовПоАренде.Удалить(Строка);
	КонецЦикла;
...
Процедура УстановитьУсловноеОформление()
	...
	//++ЭЛИАС 20150225  
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеСумма");
	//--ЭЛИАС 20150225
	...
	//++ЭЛИАС 20150225  
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеСуммаКт");
	//--ЭЛИАС 20150225
	...
	//++ЭЛИАС 20150225  
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеВалютнаяСумма");
	//--ЭЛИАС 20150225
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "РасчетыСКонтрагентамиСуммаНУ");
	//++ЭЛИАС 20150225  
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, "АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеСуммаНУ");
	//--ЭЛИАС 20150225
...
Процедура УправлениеФормойСервер()
	...
	//++ЭЛИАС 20120622
	Элементы.ГруппаДополнительнаяАналитика.Видимость = Объект.АР_ВводОстатковВзаиморасчетовПоАренде;
	//--ЭЛИАС 20120622
...
Процедура ДополнительнаяАналитикаУстановитьОтборСтрок(СтрокаТаблицы, ЭлементДополнительнаяАналитика)
	...
	Если СтрокаТаблицы <> Неопределено
		...
		ТекстЗаголовка = НСтр("ru='Дополнительная аналитика взаиморасчетов по аренде по договору ""%1"":'");
		ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстЗаголовка, СтрокаТаблицы.ДоговорКонтрагента);
		ЗаголовокДополнительнаяАналитика = ТекстЗаголовка;
	...
	Иначе
		...
		ЗаголовокДополнительнаяАналитика = НСтр("ru='Дополнительная аналитика взаиморасчетов по аренде:'");
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ <АР_ДополнительнаяАналитикаВзаиморасчетовПоАренде>

&НаКлиенте
Процедура АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	
	Если НЕ НоваяСтрока Тогда
		Возврат;
	КонецЕсли;

	ДополнительнаяАналитикаПриНачалеРедактирования(Элемент, Элементы.АР_ДополнительнаяАналитикаВзаиморасчетовПоАренде.ТекущиеДанные);

КонецПроцедуры

&НаКлиенте
Процедура АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	
	ДанныеКонтрагента = Элементы.РасчетыСКонтрагентами.ТекущиеДанные;
	...
	ИначеЕсли НЕ АР_ОбщиеПроцедуры.ЗначениеРеквизитаОбъекта(ДанныеКонтрагента.ДоговорКонтрагента, "АР_АрендныйДоговор") Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "АР_ДополнительнаяАналитикаВзаиморасчетовПоАренде", "Объект", Отказ);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеПериодАрендыПриИзменении(Элемент)
	
	ДанныеКонтрагента = Элементы.РасчетыСКонтрагентами.ТекущиеДанные;
	Если ДанныеКонтрагента <> Неопределено Тогда
		ТекущиеДанные = Элементы.АР_ДополнительнаяАналитикаВзаиморасчетовПоАренде.ТекущиеДанные;
		ПривестиКНачалуПериода(ТекущиеДанные.ПериодАренды, ДанныеКонтрагента.ДоговорКонтрагента);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеСуммаПриИзменении(Элемент)

	СтрокаТаблицы = Элементы.АР_ДополнительнаяАналитикаВзаиморасчетовПоАренде.ТекущиеДанные;
	...
	ПараметрыСтроки		= ПоляСтрокиТабличнойЧасти();
	ЗаполнитьЗначенияСвойств(ПараметрыСтроки, СтрокаТаблицы, "Сумма, СуммаКт, ВалютнаяСумма");
	ПараметрыДокумента	= Новый Структура("ВалютаРегламентированногоУчета, РазделУчета, Организация",
		ВалютаРегламентированногоУчета, Объект.РазделУчета, Объект.Организация);
...
&НаКлиенте
Процедура АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеСуммаКтПриИзменении(Элемент)
