...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Реквизиты.АР_ВводОстатковВзаиморасчетовПоАренде, //++ЭЛИАС 20120705
	...
	Запрос.Текст =
		...
		+ ТекстЗапросаВзаиморасчетыПоАренде(НомераТаблиц, Реквизиты) //++ЭЛИАС 20120705
	;
...
//++ЭЛИАС 20120705
Функция ТекстЗапросаВзаиморасчетыПоАренде(НомераТаблиц, Реквизиты)

	Если Не Реквизиты.АР_ВводОстатковВзаиморасчетовПоАренде Тогда
		Возврат "";
	КонецЕсли;
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	""ВзаиморасчетыПоАренде"" КАК ИмяСписка,
	|	РасчетыСКонтрагентами.Ссылка КАК Регистратор,
	|	РасчетыСКонтрагентами.Ссылка.Дата КАК Период,
	|	РасчетыСКонтрагентами.НомерСтроки КАК НомерСтроки,
	|	РасчетыСКонтрагентами.Ссылка.Организация КАК Организация,
	|	РасчетыСКонтрагентами.Валюта,
	|	РасчетыСКонтрагентами.ДоговорКонтрагента,
	|	РасчетыСКонтрагентами.Документ КАК ДокументРасчетов,
	|	РасчетыСКонтрагентами.Контрагент,
	|	ВЫБОР
	|		КОГДА ДополнительнаяАналитика.Сумма ЕСТЬ NULL 
	|			ТОГДА ВЫБОР
	|					КОГДА РасчетыСКонтрагентами.Аванс
	|						ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|					ИНАЧЕ ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ДополнительнаяАналитика.Сумма <> 0
	|					ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|				ИНАЧЕ ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|			КОНЕЦ
	|	КОНЕЦ КАК ВидДвижения,
	|	ЕСТЬNULL(ДополнительнаяАналитика.Сумма + ДополнительнаяАналитика.СуммаКт, РасчетыСКонтрагентами.Сумма + РасчетыСКонтрагентами.СуммаКт) КАК Сумма,
	|	ЕСТЬNULL(ДополнительнаяАналитика.ВалютнаяСумма, РасчетыСКонтрагентами.ВалютнаяСумма) КАК СуммаВзаиморасчетов,
	|	ЕСТЬNULL(ДополнительнаяАналитика.Сумма + ДополнительнаяАналитика.СуммаКт, РасчетыСКонтрагентами.Сумма + РасчетыСКонтрагентами.СуммаКт) КАК СуммаНУ,
	...
	|	ДополнительнаяАналитика.ДатаРасчетаПени
	|ИЗ
	|	Документ.ВводНачальныхОстатков.РасчетыСКонтрагентами КАК РасчетыСКонтрагентами
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВводНачальныхОстатков.АР_ДополнительнаяАналитикаВзаиморасчетовПоАренде КАК ДополнительнаяАналитика
	|		ПО РасчетыСКонтрагентами.АР_Ключ = ДополнительнаяАналитика.Ключ
	|			И РасчетыСКонтрагентами.Ссылка = ДополнительнаяАналитика.Ссылка
	|ГДЕ
	|	РасчетыСКонтрагентами.Ссылка = &Ссылка
	|	И РасчетыСКонтрагентами.ДоговорКонтрагента.АР_АрендныйДоговор";
