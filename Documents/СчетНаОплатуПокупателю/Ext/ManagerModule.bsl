...
Функция ТаблицаУслугНПД(СчетНаОплату) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СчетНаОплату", СчетНаОплату);
	
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СчетНаОплатуПокупателюТовары.Номенклатура КАК УслугаНПД,
	...
	|	И СчетНаОплатуПокупателюТовары.Номенклатура.Услуга";
...
Функция ПолучитьТекстЗапросаДляФормированияТаблицыСведенийСчетаНаОплату()
	...
	|	СчетНаОплату.ДополнительныеУсловия КАК ДополнительныеУсловия
	...
	|	ДокументыДляПечати.Руководители КАК Руководители,
	...
	|	ДокументыДляПечати.НомерДоговора КАК НомерДоговора,
	|	ДокументыДляПечати.ГосударственныйКонтракт КАК ГосударственныйКонтракт,
	|	ДокументыДляПечати.ДатаДоговора КАК ДатаДоговора,
	|	ДокументыДляПечати.ФИОИсполнителя КАК ФИОИсполнителя,
	|	ДокументыДляПечати.ДолжностьИсполнителя КАК ДолжностьИсполнителя,
	|	ДокументыДляПечати.ФИОЗаказчика КАК ФИОЗаказчика,
	|	ДокументыДляПечати.ДолжностьЗаказчика КАК ДолжностьЗаказчика,
	|	ДокументыДляПечати.НДСИсчисляетсяНалоговымАгентом КАК НДСИсчисляетсяНалоговымАгентом,
	...
	|	ДокументыДляПечати.Руководитель КАК Руководитель,
	|	ДокументыДляПечати.ГлавныйБухгалтер КАК ГлавныйБухгалтер,
	|	ДокументыДляПечати.ЗаРуководителяНаОсновании КАК ЗаРуководителяНаОсновании,
	|	ДокументыДляПечати.ЗаГлавногоБухгалтераНаОсновании КАК ЗаГлавногоБухгалтераНаОсновании,
	...
	|	ДокументыДляПечати.СуммаСкидки КАК СуммаСкидки,
	...
	|	ДокументыДляПечати.ДополнительныеУсловия КАК ДополнительныеУсловия
	|ИЗ
	...
	|		ПО ДокументыДляПечати.Документ = СрокиОплатыДокументов.Документ
	|			И ДокументыДляПечати.Организация = СрокиОплатыДокументов.Организация
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДополнительныеУсловия КАК СправочникДополнительныеУсловия
	|		ПО ДокументыДляПечати.ДополнительныеУсловия = СправочникДополнительныеУсловия.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаДокумента,
	|	Документ
	...
	|		ПО СчетНаОплату.Ссылка = ДокументыДляПечати.Документ
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаДокумента,
	|	Документ,
...
Функция ПрочитатьЭлектронныйСчет(СтруктураЭД)
	...
	Если ТипЗнч(СтруктураРазбора) = Тип("Структура")
		...
		ДанныеСчета.Вставить("ДанныеРазбора", СтруктураРазбора);
		ДанныеСчета.Вставить("ФайлДанных", Новый ДвоичныеДанные(ИмяФайла));
	Иначе
		ОбщегоНазначения.УдалитьВременныйКаталог(ПапкаДляРаспаковки);
		Возврат Неопределено;
	КонецЕсли;
