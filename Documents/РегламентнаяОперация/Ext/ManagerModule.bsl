...
Функция ТекстЗапросаВалюнаяПереоценка(НомераТаблиц)
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	Истина КАК ЭтоАренда, //++ЭЛИАС 20120727
	|	РегламентнаяОперация.Ссылка КАК Регистратор,
	|	РегламентнаяОперация.Организация КАК Организация,
	|	РегламентнаяОперация.Дата КАК Период,
...
// Закрытие счетов 20, 23, 25, 26 и 29
...
Процедура ЗакрытиеСчетов20_23_25_26(Движения, Период, Организация, Ссылка, Отказ, Ошибки) Экспорт
	...
	//Кучерявенко - замена статей затрат при закрытии 25 и 26 счетов
	Попытка
	Для Каждого Стр из РезультатРасчета.Хозрасчетный Цикл
		
		Если  Стр.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("20") или Стр.СчетКт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("25") Тогда
			Стр.СубконтоДт2=Справочники.СтатьиЗатрат.НайтиПоКоду("000000056");
		ИначеЕсли Стр.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("20") или Стр.СчетКт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("26") Тогда
			Стр.СубконтоДт2=Справочники.СтатьиЗатрат.НайтиПоКоду("000000060");
		КонецЕсли;
	...
	Если РезультатРасчета.Свойство("Ошибки") Тогда
		...
		КонтрольЗатрат.ОписатьОшибкиВыявленныеПриРасчетеСебестоимости(Ошибки, РезультатРасчета.Ошибки, Период, Организация);
