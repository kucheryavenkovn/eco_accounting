...
Процедура ПереоценкаВалютныхСредств(СтруктураШапкиДокумента, Отказ, Ошибки)
	...
	ТаблицаСправкаРасчет = УчетДоходовРасходов.ПодготовитьТаблицуСправкаРасчетПереоценкаВалютныхОстатков(
		ТаблицаПереоценка, ПараметрыПроведения.ПереоценкаВалютныхОстатков);
	
	ЗакрытиеМесяца.ЗаписьВоВспомогательныеРегистрыСведений(
		Движения,
		ТаблицаСправкаРасчет,
