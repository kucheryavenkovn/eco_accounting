...
Функция ПечатьПеремещениеТоваров(МассивОбъектов, ОбъектыПечати)
	...
	|	ПеремещениеТоваров.Ссылка КАК Ссылка,
	|	ПеремещениеТоваров.СтоимостьЕдиницыДляТЦ КАК СтоимостьЕдиницыДляТЦ
	|ПОМЕСТИТЬ Товары
	|ИЗ
	...
	|	ПеремещениеТоваров.Ссылка,
	|	ПеремещениеТоваров.СтоимостьЕдиницыДляТЦ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура
	|;
	...
	|	Товары.СтоимостьЕдиницыДляТЦ КАК СтоимостьЕдиницыДляТЦ
	|ИЗ
	|	Товары КАК Товары
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	Пока Шапка.Следующий() Цикл
		...
		НС = 1;
		ДатаНач = ?(ЗначениеЗаполнено(Шапка.Ссылка.Дата), Шапка.Ссылка.Дата, '00010101');
		ДатаКон = ?(ЗначениеЗаполнено(Шапка.Ссылка.Дата), Шапка.Ссылка.Дата, '00010101');

		ТаблицаСуммСписания = БухгалтерскийУчетПереопределяемый.ПолучитьСуммуСписанияАктивов(Шапка.Ссылка,ДатаНач,ДатаКон);
		...
		Пока ВыборкаСтрокТовары.Следующий() Цикл
			...
			//+Кучяерявенко
			НайденныеСтроки = ТаблицаСуммСписания.НайтиСтроки(Новый Структура("Номенклатура", ВыборкаСтрокТовары.Номенклатура));
			Если НайденныеСтроки.Количество() > 0 Тогда
				Если (НайденныеСтроки[0].Сумма+НайденныеСтроки[0].Сумма*18/100)>=1000 Тогда
					ОбластьДанных.ТекущаяОбласть.Шрифт = Новый Шрифт (ОбластьДанных.ТекущаяОбласть.Шрифт,,,Истина);
				иначе
					ОбластьДанных.ТекущаяОбласть.Шрифт = Новый Шрифт (ОбластьДанных.ТекущаяОбласть.Шрифт,,,Ложь);
				КонецЕсли;
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(Шапка.СкладОтправитель) Тогда 
		...
		Если ЗначениеЗаполнено(Шапка.СкладПолучатель) Тогда 
		...
		Если ТаблицаСуммСписания.Итог("сумма")>0 Тогда
			ОбластьМакета = Макет.ПолучитьОбласть("Подписи1");
			ТабличныйДокумент.Вывести(ОбластьМакета);		
		КонецЕсли;
		
		ЕстьДорогостоящие=Ложь;
		
		Для Каждого стр из  ТаблицаСуммСписания Цикл
			Если (Стр.Сумма+Стр.Сумма*18/100)>1000 Тогда
				ЕстьДорогостоящие=Истина;
			КонецЕсли;
		КонецЦикла;
		
		Если ЕстьДорогостоящие Тогда
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Для Каждого Элемент из МассивОбъектов Цикл
		Если не Элемент.Проведен Тогда
			...
			Сообщить(Строка(Элемент)+"    ---  Документ нужно провести");
