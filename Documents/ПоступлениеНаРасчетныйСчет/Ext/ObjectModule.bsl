...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//++ЭЛИАС 20120622
	АР_ОбщиеПроцедуры.СформироватьДвиженияПогашениеЗадолженности(ТаблицаВзаиморасчетов,
		ПараметрыПроведения.Реквизиты, Движения, Отказ);
...
Процедура ЗаполнитьПоДокументуОснованию(Основание)
	...
	//++ЭЛИАС 20120704
	ТипСчетНаАренду                                = Тип("ДокументСсылка.АР_СчетНаАренду");
	ТипНачислениеАренднойПлаты                     = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты");
	ТипНачислениеПени                              = Тип("ДокументСсылка.АР_НачислениеПени");
	//--ЭЛИАС 20120704

	//++ЭЛИАС 20100602
	Если ТипДокументаОснования = ТипСчетНаАренду
		ИЛИ ТипДокументаОснования = ТипНачислениеАренднойПлаты
		ИЛИ ТипДокументаОснования = ТипНачислениеПени Тогда
		...
		Для Каждого СтрокаСчета Из Основание.Состав Цикл
			...
			СтрокаПлатеж.СуммаПлатежа = СтрокаСчета.Сумма + ?(Основание.СуммаВключаетНДС, 0, СтрокаСчета.СуммаНДС);
			СтрокаПлатеж.СуммаНДС = СтрокаСчета.СуммаНДС;
			СтрокаПлатеж.СтавкаНДС = СтрокаСчета.СтавкаНДС;
			
			Если ТипДокументаОснования = ТипНачислениеАренднойПлаты Тогда
				НачалоПериода = Основание.НачалоПериода;
			ИначеЕсли ТипДокументаОснования = ТипНачислениеПени Тогда
			...
			КонецЕсли;
			СтрокаПлатеж.АР_ПериодАренды = ?(ЗначениеЗаполнено(СтрокаСчета.ПериодАренды), СтрокаСчета.ПериодАренды, НачалоПериода);
			СтрокаПлатеж.АР_СчетНаАренду = ?(ТипДокументаОснования = ТипСчетНаАренду, Основание, СтрокаСчета.СчетНаАренду);
			СтрокаПлатеж.АР_УслугаАренды = СтрокаСчета.Номенклатура;
			СтрокаПлатеж.АР_ОбъектАренды = СтрокаСчета.ОбъектАренды;
			Если ТипДокументаОснования = ТипСчетНаАренду И Основание.ВидОперации = Перечисления.АР_ВидыОперацийСчетНаАренду.ОплатаДепозита Тогда
				СтрокаПлатеж.АР_Депозит = Истина;
				СтрокаПлатеж.АР_ПериодАренды = '00010101';
				СтрокаПлатеж.АР_УслугаАренды = Справочники.Номенклатура.ПустаяСсылка();
				СтрокаПлатеж.АР_ОбъектАренды = Справочники.АР_ОбъектыАренды.ПустаяСсылка();
			КонецЕсли;
			...
			Если ДоговорКонтрагента.РасчетыВУсловныхЕдиницах тогда
				Если ДанныеОснования.ВалютаДокумента = ВалютаРегламентированногоУчета Тогда
					...
					СтрокаПлатеж.СуммаНДС 			 = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаПлатеж.СуммаНДС,
						...
						1, ДанныеОснования.КурсВзаиморасчетов,
						1, ДанныеОснования.КратностьВзаиморасчетов);
					СуммаПлатежаДоПересчета = СтрокаПлатеж.СуммаПлатежа;
					СуммаНДСДоПересчета = СтрокаПлатеж.СуммаНДС;
				КонецЕсли;
				...
				Если ДанныеОснования.ВалютаДокумента = ВалютаРегламентированногоУчета Тогда
					СтрокаПлатеж.СуммаПлатежа  = СуммаПлатежаДоПересчета;
					СтрокаПлатеж.СуммаНДС  = СуммаНДСДоПересчета;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;

		Если Не ТипДокументаОснования = ТипСчетНаАренду Тогда
		...
		Если ТипДокументаОснования = ТипСчетНаАренду Тогда
	...
	ИначеЕсли ТипДокументаОснования = ТипДокументаРеализацияТоваровУслуг
		//--ЭЛИАС 20120704
		...
		Если УчетнаяПолитика.ПрименяетсяНалогНаПрофессиональныйДоход(Организация, Дата)
			...
			ТаблицаСуммДокументаОснования = МенеджерОснования.ТаблицаУслугНПД(Основание);
