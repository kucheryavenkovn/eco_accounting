...
Процедура ЗаполнитьПоДокументуОснованию(Основание, ВидОперацииЗаполнения = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		...
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Основание, "Организация, Грузоотправитель, Грузополучатель, ОтпускПроизвел, СопроводительныеДокументы");
	Иначе
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//++ЭЛИАС 20120710
	Если ТипЗнч(ДокументРеализации) = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты") Тогда
		МассивТЧ = Новый Массив;
		МассивТЧ.Добавить("Услуги");
		МассивТЧ.Добавить("АгентскиеУслуги");
		Для Каждого ИмяТЧ Из МассивТЧ Цикл
			ИмяСписка = ИмяТЧ;
			Для Каждого ВыборкаУслуг Из ЭтотОбъект[ИмяТЧ] Цикл
				Префикс = ИмяТч + "[" + Формат(ВыборкаУслуг.НомерСтроки - 1, "ЧН=0; ЧГ=") + "].";

				Если Не ЗначениеЗаполнено(ВыборкаУслуг.Номенклатура) И ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоУслугам Тогда
					ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения("Колонка",,
						НСтр("ru = 'Номенклатура'"), ВыборкаУслуг.НомерСтроки, ИмяСписка);
					Поле = Префикс + "Номенклатура";
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Поле, "Объект", Отказ);
				КонецЕсли;
				Если Не ЗначениеЗаполнено(ВыборкаУслуг.ОбъектАренды) И ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоОбъектам Тогда
					ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения("Колонка",,
						НСтр("ru = 'Объект аренды'"), ВыборкаУслуг.НомерСтроки, ИмяСписка);
					Поле = Префикс + "ОбъектАренды";
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, Поле, "Объект", Отказ);
				КонецЕсли;
				//Проверим, заполнены ли счета на оплату услуг аренды
				Если ((ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПост И ВыборкаУслуг.Номенклатура.АР_ПостояннаяЧастьОплаты)
					ИЛИ (ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПерем И Не ВыборкаУслуг.Номенклатура.АР_ПостояннаяЧастьОплаты))
					И Не ЗначениеЗаполнено(ВыборкаУслуг.СчетНаАренду) Тогда
					ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения("Колонка",,
						НСтр("ru = 'Счет на аренду'"), ВыборкаУслуг.НомерСтроки, ИмяСписка);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//++ЭЛИАС 20130918 
	//Документы.КорректировкаРеализации.ПодготовитьТаблицуДокументаДляВзаиморасчетовПоАренде(ПараметрыПроведения.ЗачетАвансовТаблицаДокумента, ПараметрыПроведения.ВзаиморасчетыПоАрендеРеквизиты, Истина);
	//--ЭЛИАС 20130918
	...
	//++ЭЛИАС 20130918
	//Документы.КорректировкаРеализации.ПодготовитьТаблицуДокументаДляВзаиморасчетовПоАренде(ТаблицаСобственныеТоварыУслуги, ПараметрыПроведения.ВзаиморасчетыПоАрендеРеквизиты, Истина);
	//Документы.КорректировкаРеализации.ПодготовитьТаблицуДокументаДляВзаиморасчетовПоАренде(ТаблицаТоварыУслугиКомитентов, ПараметрыПроведения.ВзаиморасчетыПоАрендеРеквизиты, Истина);
	//--ЭЛИАС 20130918
	...
	//++ЭЛИАС 20130918
	//Документы.КорректировкаРеализации.ПодготовитьТаблицуДокументаДляВзаиморасчетовПоАренде(ТаблицаСторноСобственныеТоварыУслуги, ПараметрыПроведения.ВзаиморасчетыПоАрендеРеквизиты, Истина);
	//Документы.КорректировкаРеализации.ПодготовитьТаблицуДокументаДляВзаиморасчетовПоАренде(ТаблицаСторноТоварыУслугиКомитентов, ПараметрыПроведения.ВзаиморасчетыПоАрендеРеквизиты, Истина);
	//--ЭЛИАС 20130918
	...
	//++ЭЛИАС 20130506  
	Если ПараметрыПроведения.ПродажиПоАренде = Неопределено Тогда
		ТаблицаПродажиПоАренде = Новый ТаблицаЗначений;
	Иначе
		ТаблицаПродажиПоАренде = ПараметрыПроведения.ПродажиПоАренде;
	КонецЕсли;
	Документы.КорректировкаРеализации.ПодготовитьТаблицуДокументаДляВзаиморасчетовПоАренде(ТаблицаПродажиПоАренде, ПараметрыПроведения.ПродажиПоАрендеРеквизиты);
	//--ЭЛИАС 20130506	
	...
	//++ЭЛИАС 20120710	
	//Если НЕ ПараметрыПроведения.РеализацияРеквизиты = Неопределено 
	//	И ПараметрыПроведения.РеализацияРеквизиты.Количество() > 0
	//	И ПараметрыПроведения.РеализацияРеквизиты[0].ЭтоАренда Тогда
	...
	//	АР_ОбщиеПроцедуры.СформироватьДвиженияВыделениеАвансов(ТаблицаВзаиморасчетыВозврат, ПараметрыПроведения.ВыделениеАвансаРеквизиты, Движения, Отказ);
	//	АР_ОбщиеПроцедуры.СформироватьДвиженияРеализацияВозврат(ТаблицаСторноСобственныеТоварыУслуги, ПараметрыПроведения.РеализацияРеквизиты, Движения, Отказ);
	//	АР_ОбщиеПроцедуры.СформироватьДвиженияРеализацияВозврат(ТаблицаСторноТоварыУслугиКомитентов, ПараметрыПроведения.РеализацияРеквизиты, Движения, Отказ);
	//КонецЕсли;
	////--ЭЛИАС 20120710
