...
Процедура ПеречислениеДепонентовПриИзменении(Элемент)
	...
	Объект.СуммаДокумента = ПеречислениеДепонентов.Итог("СуммаПлатежа");
...
Процедура ЗагрузитьПараметрыВРеквизитыФормы()
	...
	Объект.Вставить("СчетКонтрагента",   ПланыСчетов.Хозрасчетный.ПустаяСсылка());
	Объект.Вставить("НазначениеПлатежа", "");
...
&НаСервере
Процедура СформироватьНазначениеПлатежаСервер()
	
	Если НазначениеПлатежа <> СгенерированноеНазначенияПлатежа Тогда
		НазначениеПлатежа = Документы.СписаниеСРасчетногоСчета.ПолучитьНазначениеПлатежа(Объект);
	КонецЕсли;
