...
Процедура ЗаполнитьПоДокументуОснованию(Основание)
	...
	//++ЭЛИАС 20100602
	Если ВидДокументаОснования = Тип("ДокументСсылка.АР_СчетНаАренду")
		ИЛИ ВидДокументаОснования = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты")
		ИЛИ ВидДокументаОснования = Тип("ДокументСсылка.АР_НачислениеПени") Тогда
		...
		КонецЕсли;
		//++ЭЛИАС 20141118  исправление типовой ошибки
		...
		СтруктураКурсаВзаиморасчетов = АР_ПроцедурыНачисления.ПолучитьКурсВалюты(ВалютаВзаиморасчетов, Дата, ДоговорКонтрагента);

		СчетОрганизации = ДанныеОснования.Организация.ОсновнойБанковскийСчет;
		
		ТаблицаПлатежей = РасшифровкаПлатежа.Выгрузить();
		ТаблицаПлатежей.Очистить();
		Для Каждого СтрокаСчета Из Основание.Состав  Цикл
			
			СтрокаПлатежа = ТаблицаПлатежей.Добавить();
			
			СтрокаПлатежа.СуммаПлатежа = СтрокаСчета.Сумма + ?(Основание.СуммаВключаетНДС, 0, СтрокаСчета.СуммаНДС);
			СтрокаПлатежа.СуммаНДС = СтрокаСчета.СуммаНДС;
			СтрокаПлатежа.СтавкаНДС = СтрокаСчета.СтавкаНДС;
			
			Если ВидДокументаОснования = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты") Тогда
				НачалоПериода = Основание.НачалоПериода;
			ИначеЕсли ВидДокументаОснования = Тип("ДокументСсылка.АР_НачислениеПени") Тогда
				НачалоПериода = НачалоМесяца(Основание.Дата);
			Иначе	
				НачалоПериода = ?(Основание.ВидОперации = Перечисления.АР_ВидыОперацийСчетНаАренду.ОплатаАренды, Основание.НачалоПериода, НачалоМесяца(Основание.Дата));
			КонецЕсли;
			СтрокаПлатежа.АР_ПериодАренды = ?(ЗначениеЗаполнено(СтрокаСчета.ПериодАренды), СтрокаСчета.ПериодАренды, НачалоПериода);
			СтрокаПлатежа.АР_СчетНаАренду = ?(ВидДокументаОснования = Тип("ДокументСсылка.АР_СчетНаАренду"), Основание, СтрокаСчета.СчетНаАренду);
			СтрокаПлатежа.АР_УслугаАренды = СтрокаСчета.Номенклатура;
			СтрокаПлатежа.АР_ОбъектАренды = СтрокаСчета.ОбъектАренды;
			Если ВидДокументаОснования = Тип("ДокументСсылка.АР_СчетНаАренду") И Основание.ВидОперации = Перечисления.АР_ВидыОперацийСчетНаАренду.ОплатаДепозита Тогда
				СтрокаПлатежа.АР_Депозит = Истина;
		...
		Для каждого СтрокаПлатежа Из ТаблицаПлатежей Цикл
			...
			Если ДоговорКонтрагента.РасчетыВУсловныхЕдиницах тогда
				Если ДанныеОснования.ВалютаДокумента = ВалютаРегламентированногоУчета тогда
					СтрокаПлатежа.СуммаВзаиморасчетов = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаПлатежа.СуммаПлатежа,
					...
					СтрокаПлатежа.СуммаНДС 			 = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаПлатежа.СуммаНДС,
						ВалютаРегламентированногоУчета, ВалютаВзаиморасчетов,
						1, ДанныеОснования.КурсВзаиморасчетов,
						1, ДанныеОснования.КратностьВзаиморасчетов);
					СуммаПлатежаДоПересчета = СтрокаПлатежа.СуммаПлатежа;
					СуммаНДСДоПересчета = СтрокаПлатежа.СуммаНДС;
				КонецЕсли;

				СтрокаПлатежа.СуммаПлатежа = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту(СтрокаПлатежа.СуммаВзаиморасчетов,
				...
				СтрокаПлатежа.СуммаНДС = РаботаСКурсамиВалютБПКлиентСервер.ПересчитатьИзВалютыВВалюту	  (СтрокаПлатежа.СуммаНДС,
					...
					СтрокаПлатежа.КратностьВзаиморасчетов, 1);
				Если ДанныеОснования.ВалютаДокумента = ВалютаРегламентированногоУчета Тогда
					СтрокаПлатежа.СуммаПлатежа  = СуммаПлатежаДоПересчета;
					СтрокаПлатежа.СуммаНДС  = СуммаНДСДоПересчета;
				КонецЕсли;
			КонецЕсли;
			СтруктураСчетов = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаРасчетовСКонтрагентом(
			Организация, Контрагент, СтрокаПлатежа.ДоговорКонтрагента);
			
			СтрокаПлатежа.СчетУчетаРасчетовСКонтрагентом = СтруктураСчетов.СчетРасчетовПокупателя;
			СтрокаПлатежа.СчетУчетаРасчетовПоАвансам = СтруктураСчетов.СчетАвансовПокупателя;
		КонецЦикла;

		Если Не ВидДокументаОснования = Тип("ДокументСсылка.АР_СчетНаАренду") Тогда
		...
		Если ВидДокументаОснования = Тип("ДокументСсылка.АР_СчетНаАренду") Тогда
			НазначениеПлатежа = "Оплата по счету " + ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Основание.Номер, Истина, Истина);;
		КонецЕсли;
		
	ИначеЕсли ВидДокументаОснования = Тип("ДокументСсылка.РеализацияТоваровУслуг")
	//Если ВидДокументаОснования = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		...
		Если УчетнаяПолитика.ПрименяетсяНалогНаПрофессиональныйДоход(Организация, Дата)
			...
			ТаблицаСуммОснования = МенеджерОснования.ТаблицаУслугНПД(Основание);
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	//++ЭЛИАС 20120622
	АР_ОбщиеПроцедуры.СформироватьДвиженияПогашениеЗадолженности(ТаблицаВзаиморасчеты,
		ПараметрыПроведения.Реквизиты, Движения, Отказ);
	АР_ОбщиеПроцедуры.СвернутьСформированныеПроводки(Движения.Хозрасчетный);
	АР_ОбщиеПроцедуры.СвернутьСформированныеВзаиморасчетыПоАренде(Движения.АР_ВзаиморасчетыПоАренде);
	//--ЭЛИАС 20120622
