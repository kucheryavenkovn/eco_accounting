...
Функция ТекстЗапросаСчетФактураВыданныйНаАвансРасшифровкаПлатежа(НомераТаблиц) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.АР_СчетНаАренду = ЗНАЧЕНИЕ(Документ.АР_СчетНаАренду.ПустаяСсылка)
	|			ТОГДА ТаблицаДокумента.СчетНаОплату
	|		ИНАЧЕ ТаблицаДокумента.АР_СчетНаАренду
	|	КОНЕЦ КАК СчетНаОплату,
	|	ТаблицаДокумента.АР_ПериодАренды КАК АР_ПериодАренды,
	|	ТаблицаДокумента.АР_СчетНаАренду КАК АР_СчетНаАренду,
	|	ТаблицаДокумента.АР_УслугаАренды КАК АР_УслугаАренды,
	|	ТаблицаДокумента.АР_ОбъектАренды КАК АР_ОбъектАренды,
	|	ТаблицаДокумента.АР_Депозит КАК АР_Депозит,
	|	ТаблицаДокумента.АР_Пени КАК АР_Пени,
	...
	|		ИНАЧЕ ТаблицаДокумента.АР_СчетНаАренду
	|	КОНЕЦ,
	|	ТаблицаДокумента.АР_ПериодАренды,
	|	ТаблицаДокумента.АР_СчетНаАренду,
	|	ТаблицаДокумента.АР_УслугаАренды,
	|	ТаблицаДокумента.АР_ОбъектАренды,
	|	ТаблицаДокумента.АР_Депозит,
	|	ТаблицаДокумента.АР_Пени,
	//--ЭЛИАС 20120706
...
Функция ПодготовитьПараметрыПроведения(ДокументСсылка, Отказ, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ОбособленныйУчетПени", АР_ПроцедурыНачисления.ПоДоговорамЕстьОбособленныйУчетПени(ДокументСсылка)); //++ЭЛИАС 20190416
	...
	Запрос.Текст = ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
		...
		+ ТекстЗапросаРегистрацияОтложенныхРасчетовСКонтрагентами(НомераТаблиц, ПараметрыПроведения, Реквизиты)
 		+ ТекстЗапросаВзаиморасчетыПоАренде(НомераТаблиц) //++ЭЛИАС 20120622
		;
...
Функция ТекстЗапросаВзаиморасчетыПоАренде(НомераТаблиц)
	...
	|	ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента.ВалютаВзаиморасчетов КАК Валюта,
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА Реквизиты.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеДенежныхСредств.ПриобретениеИностраннойВалюты)
	...
	|	ТаблицаРасшифровкаПлатежа.АР_ПериодАренды КАК ПериодАренды,
	|	ТаблицаРасшифровкаПлатежа.АР_СчетНаАренду КАК СчетНаАренду,
	|	ТаблицаРасшифровкаПлатежа.АР_УслугаАренды КАК УслугаАренды,
	|	ТаблицаРасшифровкаПлатежа.АР_ОбъектАренды КАК ОбъектАренды,
	|	ТаблицаРасшифровкаПлатежа.АР_Депозит КАК Депозит,
	|	ТаблицаРасшифровкаПлатежа.АР_Пени КАК Пени
	|ИЗ
	|	Документ.ПриходныйКассовыйОрдер.РасшифровкаПлатежа КАК ТаблицаРасшифровкаПлатежа
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПриходныйКассовыйОрдер КАК Реквизиты
...
Функция ТекстЗапросаРеквизитыДокумента(НомераТаблиц)
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	Реквизиты.АР_ОплатаАренды КАК ЭтоАренда, //++ЭЛИАС 20120704
	|	&ОбособленныйУчетПени КАК ОбособленныйУчетПени, //++ЭЛИАС 20120704
	|	Реквизиты.Дата КАК Период,
...
Функция ТекстЗапросаТаблицыДокумента(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	|	ТаблицаРасшифровкаПлатежа.АР_Пени КАК Пени,
	|	ВЫБОР
	|		КОГДА ТаблицаРасшифровкаПлатежа.Ссылка.АР_ОплатаАренды
	|			И НЕ &ОбособленныйУчетПени 
	|			И НЕ ТаблицаРасшифровкаПлатежа.АР_СчетПеней = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)
	|			ТОГДА ТаблицаРасшифровкаПлатежа.АР_СчетПеней
	|		ИНАЧЕ ТаблицаРасшифровкаПлатежа.СчетУчетаРасчетовСКонтрагентом
	|	КОНЕЦ КАК СчетУчетаРасчетовПоПеням,
	//--ЭЛИАС 20120704
...
Функция ТекстЗапросаПогашениеЗадолженности(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя
		...
		//++ЭЛИАС 20120704
		|	ВЫБОР
		|		КОГДА ТаблицаРасшифровкаПлатежа.Депозит
		|				ИЛИ НЕ ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента КАК Справочник.ДоговорыКонтрагентов).АР_ВестиВзаиморасчетыПоОбъектам
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.АР_ОбъектыАренды.ПустаяСсылка)
		|		ИНАЧЕ ТаблицаРасшифровкаПлатежа.ОбъектАренды
		|	КОНЕЦ КАК ОбъектАренды,
		|	ВЫБОР
		|		КОГДА ТаблицаРасшифровкаПлатежа.Депозит
		|				ИЛИ НЕ ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента КАК Справочник.ДоговорыКонтрагентов).АР_ВестиВзаиморасчетыПоУслугам
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|		ИНАЧЕ ТаблицаРасшифровкаПлатежа.УслугаАренды
		|	КОНЕЦ КАК УслугаАренды,
		|	ВЫБОР
		|		КОГДА ТаблицаРасшифровкаПлатежа.Депозит
		|				ИЛИ НЕ ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента КАК Справочник.ДоговорыКонтрагентов).АР_ВестиВзаиморасчетыПоСчетамПост
		|					И НЕ ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента КАК Справочник.ДоговорыКонтрагентов).АР_ВестиВзаиморасчетыПоСчетамПерем
		|				ИЛИ НЕ ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента КАК Справочник.ДоговорыКонтрагентов).АР_ВестиВзаиморасчетыПоСчетамПерем
		|					И НЕ (ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.УслугаАренды КАК Справочник.Номенклатура)) = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|					И НЕ ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.УслугаАренды КАК Справочник.Номенклатура).АР_ПостояннаяЧастьОплаты
		|				ИЛИ НЕ ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента КАК Справочник.ДоговорыКонтрагентов).АР_ВестиВзаиморасчетыПоСчетамПост
		|					И ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.УслугаАренды КАК Справочник.Номенклатура).АР_ПостояннаяЧастьОплаты
		|			ТОГДА ЗНАЧЕНИЕ(Документ.АР_СчетНаАренду.ПустаяСсылка)
		|		ИНАЧЕ ТаблицаРасшифровкаПлатежа.СчетНаАренду
		|	КОНЕЦ КАК СчетНаАренду,
		|	ВЫБОР
		|		КОГДА ТаблицаРасшифровкаПлатежа.Депозит
		|				ИЛИ НЕ ВЫРАЗИТЬ(ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента КАК Справочник.ДоговорыКонтрагентов).АР_ВестиВзаиморасчетыПоПериодам
		|			ТОГДА ДАТАВРЕМЯ(1, 1, 1)
		|		ИНАЧЕ ТаблицаРасшифровкаПлатежа.ПериодАренды
		|	КОНЕЦ КАК ПериодАренды,
		|	ТаблицаРасшифровкаПлатежа.Депозит,
		|	ТаблицаРасшифровкаПлатежа.Пени,
		|	ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоПериодам КАК ВестиВзаиморасчетыПоПериодам,
		|	ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоОбъектам КАК ВестиВзаиморасчетыПоОбъектам,
		|	ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПост КАК ВестиВзаиморасчетыПоСчетамПост,
		|	ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоСчетамПерем КАК ВестиВзаиморасчетыПоСчетамПерем,
		|	ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента.АР_ВестиВзаиморасчетыПоУслугам КАК ВестиВзаиморасчетыПоУслугам,
		|	ТаблицаРасшифровкаПлатежа.СчетУчетаРасчетовПоПеням КАК СчетУчетаРасчетовПоПеням,
		//--ЭЛИАС 20120704
		|	ТаблицаРасшифровкаПлатежа.НомерСтроки КАК НомерСтроки,
...
Функция ТекстЗапросаОплатаСчетов(НомераТаблиц, ПараметрыПроведения, Реквизиты)
	...
	Если Реквизиты.ВидОперации = Перечисления.ВидыОперацийПКО.ОплатаПокупателя
		...
		//++ЭЛИАС 20160426 
		|	И НЕ ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента.АР_АрендныйДоговор
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ТаблицаРасшифровкаПлатежа.НомерСтроки КАК НомерСтроки,
		|	ТаблицаРасшифровкаПлатежа.СчетНаАренду КАК СчетНаОплату,
		|	ТаблицаРасшифровкаПлатежа.СтавкаНДС КАК СтавкаНДС,
		|	ВЫБОР
		|		КОГДА СчетНаОплатуПокупателю.ВалютаДокумента = &ВалютаРеглУчета
		|			ТОГДА ТаблицаРасшифровкаПлатежа.СуммаРуб
		...
		|ИЗ
		|	ТаблицаРасшифровкаПлатежа КАК ТаблицаРасшифровкаПлатежа
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.АР_СчетНаАренду КАК СчетНаОплатуПокупателю
		|		ПО ТаблицаРасшифровкаПлатежа.СчетНаАренду = СчетНаОплатуПокупателю.Ссылка
		|			И ТаблицаРасшифровкаПлатежа.ДоговорКонтрагента.АР_АрендныйДоговор
		|
		|ГДЕ
		|	ТаблицаРасшифровкаПлатежа.СчетНаАренду <> ЗНАЧЕНИЕ(Документ.АР_СчетНаАренду.ПустаяСсылка)
		//--ЭЛИАС 20160426
		|
		|УПОРЯДОЧИТЬ ПО
		|	ТаблицаРасшифровкаПлатежа.НомерСтроки";
