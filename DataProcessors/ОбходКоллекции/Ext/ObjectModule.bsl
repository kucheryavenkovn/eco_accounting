...
Процедура ЗаполнитьСоставКолонок()
	...
	ИначеЕсли ТипЗнч(Коллекция) = Тип("ДанныеФормыКоллекция") 
		Или ЭтоНаборЗаписей(Коллекция) Или ЭтоТабличнаяЧасть(Коллекция) Тогда
...
Функция ЭтоТабличнаяЧасть(ПроверяемаяКоллекция)
	...
	Если Не ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(ОбъектМетаданных)
		Или Не ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(ОбъектМетаданных.Родитель()) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ОбъектМетаданных.Родитель().ТабличныеЧасти.Содержит(ОбъектМетаданных);
