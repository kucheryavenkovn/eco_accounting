...
Процедура СформироватьПечатнуюФорму(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ИмяМакета, СинонимМакета, ОбъектыПечати, ПараметрыВывода)
	...
	Для каждого ОбъектыТипа Из ТипыОбъектов Цикл
		...
		Если ПараметрыПечати.Свойство("Сводный") Тогда
			СинонимМакета = СинонимМакета + " (сводный по услугам)";
			ТаблицаСведенийСчетНаОплату = МенеджерОбъекта.ПолучитьТаблицуСведенийСчетаНаОплату(ОбъектыТипа.Значение, СоответствиеСчетовРеализациям, ДокументыБезСчетовНаОплату, ПараметрыПечати.Сводный);
		Иначе
			ТаблицаСведенийСчетНаОплату = МенеджерОбъекта.ПолучитьТаблицуСведенийСчетаНаОплату(ОбъектыТипа.Значение, СоответствиеСчетовРеализациям, ДокументыБезСчетовНаОплату);
		КонецЕсли;
		//--ЭЛИАС 20180726
