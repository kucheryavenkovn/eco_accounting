...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаБаннерНастройкаОтменаЕНВД.Видимость = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ПомощникРасчетаЕНВД",
		"БаннерОтменаЕНВДВидимость", ОбщегоНазначенияБП.ТекущаяДатаНаСервере() < УчетЕНВДКлиентСервер.ДатаОтменыЕНВД());
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_НастройкиПродленияСроковНалоговОтчетов"
		...
		ОсвобождениеОтНалогаКлючАктуальности = "";
		УправлениеФормой(ЭтотОбъект);
...
Процедура УправлениеФормой(Форма)
	...
	ОсвобождениеОтУплатыНалога = НастроитьОсвобождениеОтНалога(Форма);
	...
	ПоказыватьДействиеУплата = ПоказыватьДействия И Не ОсвобождениеОтУплатыНалога;
	
	Элементы.ДокументУплатыПредставление.Видимость = ПоказыватьДействиеУплата И Форма.ЕстьДокументУплаты;
	Элементы.УплатитьНалогПодменю.Видимость        = ПоказыватьДействиеУплата И Не Форма.ЕстьДокументУплаты И НадоВыбратьСпособОплаты;
	Элементы.УплатитьНалог.Видимость               = ПоказыватьДействиеУплата И Не Форма.ЕстьДокументУплаты И Не НадоВыбратьСпособОплаты;
...
&НаСервереБезКонтекста
Функция НастройкиОсвобождениеОтНалога(Период, Организация)
	
	Настройки = Новый Структура;
	Настройки.Вставить("ТекстБаннера",  Неопределено); // ФорматированнаяСтрока, если баннер надо показывать
	Настройки.Вставить("Освобождение",  Ложь);
	
	Если НалоговыйУчет.ВидимостьБаннераАнтикризис(Организация, КодЗадачиУплатыНалога(), Период) Тогда
		Настройки.ТекстБаннера = НалоговыйУчет.ТекстБаннераНастройкиНалоговИОтчетов(Организация, КодЗадачиУплатыНалога());
	КонецЕсли;
	
	Настройки.Освобождение = НалоговыйУчет.ВозможноОсвобождениеОтНалога(КодЗадачиУплатыНалога(), Период)
		И НалоговыйУчет.ДеятельностьОтнесенаКПострадавшимОтКоронавируса(Организация);
...
Функция КодЗадачиУплатыНалога()
	Возврат ЗадачиБухгалтераКлиентСервер.КодЗадачиЕНВД();
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ТекстОсвобождениеОтУплатыНалога(Форма)
	
	Если Форма.ЭтоЮрЛицо Тогда
		ШаблонHTML = НСтр("ru = 'Организация освобождена от уплаты ЕНВД за 2 квартал 2020 года
                           |<a href = ""Антикризис"">Настройки налогов и отчетов</a>'");
	Иначе
		ШаблонHTML = НСтр("ru = 'Предприниматель освобожден от уплаты ЕНВД за 2 квартал 2020 года
                           |<a href = ""Антикризис"">Настройки налогов и отчетов</a>'");
	КонецЕсли;
	// Переход по гиперссылке - в ОсвобождениеОтНалогаПояснениеОбработкаНавигационнойСсылки
	
	Возврат СтроковыеФункцииКлиентСервер.ФорматированнаяСтрока(ШаблонHTML);
	
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция НастроитьОсвобождениеОтНалога(Форма)
	
	КлючАктуальности = Строка(Форма.Объект.Организация.УникальныйИдентификатор()) + Формат(Форма.Объект.Период, "ДФ=yyyyMM");
	Если Форма.ОсвобождениеОтНалогаКлючАктуальности = КлючАктуальности Тогда
		Возврат Форма.ОсвобождениеОтНалога;
	КонецЕсли;
	
	Форма.ОсвобождениеОтНалогаКлючАктуальности = КлючАктуальности;
	
	НастройкиОсвобождениеОтНалога = НастройкиОсвобождениеОтНалога(Форма.Объект.Период, Форма.Объект.Организация);
	
	Форма.ОсвобождениеОтНалога = НастройкиОсвобождениеОтНалога.Освобождение;
	
	Если НастройкиОсвобождениеОтНалога.ТекстБаннера = Неопределено Тогда
		Форма.Элементы.ГруппаБаннерНастройкаНалоговИОтчетов.Видимость = Ложь;
	Иначе
		Форма.Элементы.ГруппаБаннерНастройкаНалоговИОтчетов.Видимость = Истина;
		Форма.ОсвобождениеОтНалогаТекстБаннера = НастройкиОсвобождениеОтНалога.ТекстБаннера;
	КонецЕсли;
	
	Форма.Элементы.ГруппаОсвобождениеОтНалога.Видимость = НастройкиОсвобождениеОтНалога.Освобождение;
	
	ФормулаРасчета = Новый Структура;
	ФормулаРасчета.Вставить("Результат", НСтр("ru = 'Налог к уплате'"));
	ФормулаРасчета.Вставить("Расчет",    НСтр("ru = 'Налог за квартал - Расходы, уменьшающие налог'"));
	
	ПолужирныйШрифт = Истина;
	
	Если НастройкиОсвобождениеОтНалога.Освобождение Тогда
		// Размеры элементов подобраны так, чтобы суммы выстраивались в одну колонку
		Форма.Элементы.НалогКУплатеОсвобождение.Ширина = 42;
		ФормулаРасчета.Результат = НСтр("ru = 'Налог в декларации'");
		ПолужирныйШрифт = Ложь;
		
		Форма.Элементы.ОсвобождениеОтНалогаПояснение.Заголовок = ТекстОсвобождениеОтУплатыНалога(Форма);
		
	КонецЕсли;
	
	Форма.Элементы.НалогКУплате.Шрифт = Новый Шрифт(Форма.Элементы.НалогКУплате.Шрифт, , , ПолужирныйШрифт);
	Если ПолужирныйШрифт Тогда
		РезультатHTML = СтрШаблон(НСтр("ru = '<b>%1</b>'"), ФормулаРасчета.Результат);
	Иначе
		РезультатHTML = ФормулаРасчета.Результат;
	КонецЕсли;
		
	ШаблонФормулы = НСтр("ru = '%1 = %2'");
	ФормулаHTML   = СтрШаблон(ШаблонФормулы, РезультатHTML, ФормулаРасчета.Расчет);
	Форма.Элементы.ЗаголовокНалогКУплате.Заголовок = СтроковыеФункцииКлиентСервер.ФорматированнаяСтрока(ФормулаHTML);
	
	Возврат НастройкиОсвобождениеОтНалога.Освобождение;
...
&НаКлиенте
Процедура ОсвобождениеОтНалогаПояснениеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура КартинкаЗакрытьБаннерНажатие(Элемент)
	...
	СохранитьБаннерАнтикризисСкрыт(Объект.Организация);
	Элементы.ГруппаБаннерНастройкаНалоговИОтчетов.Видимость = Ложь;
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура СохранитьБаннерАнтикризисСкрыт(Организация)
	
	НалоговыйУчет.СохранитьНастройкуБаннераАнтикризис(Организация, КодЗадачиУплатыНалога(), Ложь);
	
КонецПроцедуры

#КонецОбласти

#Область БаннерОтменаЕНВД

&НаКлиенте
Процедура КартинкаЗакрытьБаннерОтменаЕНВДНажатие(Элемент)
	
	СохранитьБаннерОтменаЕНВДСкрыт(Объект.Организация);
	Элементы.ГруппаБаннерНастройкаОтменаЕНВД.Видимость = Ложь;
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура СохранитьБаннерОтменаЕНВДСкрыт(Организация)
	
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПомощникРасчетаЕНВД", "БаннерОтменаЕНВДВидимость", Ложь);
