...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДанныеФискальнойОперации = МенеджерОборудованияВызовСервера.ДанныеФискальнойОперации(Параметры.Ссылка,,,ПараметрыОперацииФискализацииЧека.ТипРасчета);
	Если ДанныеФискальнойОперации <> Неопределено Тогда
		...
		Объект.КорректируемыйДокумент               = Параметры.Ссылка;
	...
	Объект.ОтправлятьSMS   = Объект.ОтправлятьSMS   И ЗначениеЗаполнено(Объект.НомерТелефона);
	...
	Иначе
		...
		Если Значение.ПризнакАгента <> ПараметрыОперацииФискализацииЧека.ПризнакАгента
			...
			Если ПараметрыОперацииФискализацииЧека.ПозицииЧека.Количество() > 1 Тогда
				...
				ПараметрыОперацииФискализацииЧека.ДанныеАгента     = МенеджерОборудованияКлиентСервер.ПараметрыДанныеАгента();
				ПараметрыОперацииФискализацииЧека.ДанныеПоставщика = МенеджерОборудованияКлиентСервер.ПараметрыДанныеПоставщика();
