...
Процедура ОбработкаВыбораЗначения()
	...
	СуммаДокумента = ТекущиеДанные.СуммаДокумента;
	
	Если ТипЗнч(ТекущиеДанные.Документ) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах") Тогда
		СуммаДокумента = ПолучитьСуммуПоРозницеВОтчетеКомиссионеру(ТекущиеДанные.Документ, ВидСчетаФактуры,
			ТекущиеДанные.СуммаДокумента);
	КонецЕсли;

	СтруктураВыбора.Вставить("СуммаДокумента", СуммаДокумента);
...
Функция ПолучитьСуммуПоРозницеВОтчетеКомиссионеру(Документ, ВидСчетаФактуры, СуммаДокумента)
	
	Если (ВидСчетаФактуры <> Перечисления.ВидСчетаФактурыВыставленного.СводнаяСправка
		И ВидСчетаФактуры <> Перечисления.ВидСчетаФактурыВыставленного.КорректировочнаяСправка) Тогда
	...
	Если ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.СводнаяСправка Тогда
	...
	Иначе
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	-СУММА(ОтчетКомиссионераОПродажахТоварыВозвращенные.Сумма) КАК Сумма
		|ИЗ
		...
		|ГДЕ
		|	ОтчетКомиссионераОПродажахВозвраты.Ссылка = &Документ
