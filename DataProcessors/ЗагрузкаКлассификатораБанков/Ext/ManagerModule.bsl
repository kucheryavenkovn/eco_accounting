...
Процедура ПриДобавленииКлассификаторов(Классификаторы, Описатель) Экспорт
	...
	Описатель.ОбработкаРазделенныхДанных = Истина; // БП
...
Процедура ПриЗагрузкеКлассификатора(Идентификатор, Версия, Адрес, Обработан, ДополнительныеПараметры) Экспорт
	...
	Если АвтономнаяРаботаСлужебный.ЭтоАвтономноеРабочееМесто() Тогда // БП
		ЗаписьЖурналаРегистрации(
			ИмяСобытияВЖурналеРегистрации(),
			УровеньСобытия,
			,
			,
			НСтр("ru = 'Загрузка в автономном рабочем месте не предусмотрена'"));
		Возврат;
	КонецЕсли;
	...
	ЗагрузитьДанныеИзФайла(ИмяВременногоФайла, ДополнительныеПараметры); // БП
...
Процедура ЗагрузитьДанные(Параметры)
	...
	Для Каждого Строка Из СтрРазделить(ТекстКлассификатора, Символы.ПС, Ложь) Цикл 
		...
		ЗаписатьЭлементКлассификатораБанковРФ(СведенияОБанке, Параметры.СписокИзмененныхБанков); // БП
	КонецЦикла;
	...
	ОтметитьНедействующиеБанки(ДействующиеБанки, НедействующиеБанки, Параметры.СписокИзмененныхБанков); // БП
...
Процедура ЗагрузитьДанныеИзФайла(ИмяФайла, ДополнительныеПараметры) // БП
	...
	Если ФайлыКлассификатораПолучены(ПапкаСИзвлеченнымиФайлами) Тогда
		...
		Параметры.Вставить("СписокИзмененныхБанков", Новый Массив); // БП
		
		ЗагрузитьДанные(Параметры);
		УстановитьВерсиюКлассификатора();
		
		ДополнительныеПараметры.Вставить("СписокИзмененныхБанков", Параметры["СписокИзмененныхБанков"]); // БП
		
		Если Не ОбщегоНазначения.РазделениеВключено() Тогда
			РаботаСБанкамиБП.ОбновитьБанкиИзКлассификатора(Параметры["СписокИзмененныхБанков"]); // БП
		КонецЕсли;
	КонецЕсли;
...
Функция ОтметитьНедействующиеБанки(ДействующиеБанки, НедействующиеБанки, СписокИзмененныхБанков) // БП
	...
	Пока ВыборкаБанков.Следующий() Цикл
		...
		Попытка
			...
			БанкОбъект.Записать();
			СписокИзмененныхБанков.Добавить(БанкОбъект.Ссылка); // БП
...
//
Процедура ЗаписатьЭлементКлассификатораБанковРФ(СведенияОБанке, СписокИзмененныхБанков) // БП
