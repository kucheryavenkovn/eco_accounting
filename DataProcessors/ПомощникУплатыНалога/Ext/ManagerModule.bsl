...
Функция СведенияОРасчетеСуммы(ПараметрыЗадачи)
	...
	Иначе
		...
		ИначеЕсли СпособВыполненияЗадачи = "УплатаАвансовИмущественныеНалоги" Тогда
...
Процедура ЗаполнитьСведенияУплатаАвансовПоИмущественнымНалогам(ПараметрыЗадачи, СведенияОРасчете)
	...
	Если Платежи.Количество() > 0 Тогда	
		...
		Если ВидНалога = Перечисления.ВидыНалогов.НалогНаИмущество Тогда
		...
		ИначеЕсли ВидНалога = Перечисления.ВидыНалогов.ТранспортныйНалог Тогда
	...
	СведенияОРасчете.ИнформацияНетДанныхДляРасчета = НСтр("ru = 'Налог не рассчитан'");
	СведенияОРасчете.ТаблицаПлатежей        = Платежи;
	СведенияОРасчете.ТаблицаРасчета         = ТаблицаРасчета;
