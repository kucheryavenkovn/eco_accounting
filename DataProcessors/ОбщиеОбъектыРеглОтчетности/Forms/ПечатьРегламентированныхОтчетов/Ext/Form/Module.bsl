...
Процедура Инициализация()
	...
	// ПОД_ФК_ПроцессыОбработкиДокументов
	//
	Если ОбщегоНазначения.ПодсистемаСуществует("ПОД_ФК_ПроцессыОбработкиДокументов") Тогда
		
		Модуль_ПОД_ФК_РегламентированнаяОтчетностьСервер = ОбщегоНазначения.ОбщийМодуль(
			"ПОД_ФК_РегламентированнаяОтчетностьСервер");
		
		Модуль_ПОД_ФК_РегламентированнаяОтчетностьСервер.ПечатьРегламентированныхОтчетов_Инициализация(
			ЭтаФорма, ДеревоЛистов);
	...
	Если СписокПечатаемыхЛистов.Количество() > 0
	   И ВРег(СокрЛП(СписокПечатаемыхЛистов[0].Представление)) = ВРег(СокрЛП("СсылкаНаРегламентированныйОтчет")) Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ПОД_ФК_ПроцессыОбработкиДокументов") Тогда
			...
			Модуль_ПОД_ФК_РегламентированнаяОтчетностьСервер.ПечатьРегламентированныхОтчетов_ВывестиВыбранныйЛистВТабличныйДокумент(
				ЭтаФорма, АдресВоВременномХранилище, ТаблДок);
...
Процедура СохранениеМодификацийВПечатнойФорме()
	...
	Если СтраницаПечатнойФормыМодифицирована Тогда
		...
		Если СписокПечатаемыхЛистов.Количество() > 0
			...
			// ПОД_ФК_ПроцессыОбработкиДокументов
			//
			Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПОД_ФК_ПроцессыОбработкиДокументов") Тогда
				
				ОбщийМодуль_ПОД_ФК_РегламентированнаяОтчетностьКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль(
					"ПОД_ФК_РегламентированнаяОтчетностьКлиент");
				
				ОбщийМодуль_ПОД_ФК_РегламентированнаяОтчетностьКлиент.ПечатьРегламентированныхОтчетов_СохранениеМодификацийВПечатнойФорме(
					ЭтаФорма, ТекущиеДанныеОтчета);
