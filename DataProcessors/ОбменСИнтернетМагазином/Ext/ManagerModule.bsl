...
Функция ЗагрузитьЗаказыИнтернетМагазина(СтатистикаЗагрузки, ОписаниеОшибки) Экспорт
	...
	Если ПустаяСтрока(СтрокаXML) Тогда
		...
		ТекстСообщенияОбОшибке = СтрШаблон(НСтр("ru = 'Ответ сервера: %1'"), Лев(ОтветСервера,255));
	...
	Если ОтветСервера = Неопределено Тогда 
		...
		ДобавитьОписаниеОшибки(ОписаниеОшибки,
			НСтр("ru = 'Не удалось отправить подтверждение загрузки заказов.'"));
		Возврат Ложь;
	...
	Для каждого Заказ Из ЗаказыКЗагрузке Цикл
		...
		Попытка
			...
			Если НЕ ЭтоНовыйДокумент
				...
				СтатусыДокумента.Статус = Перечисления.СтатусОплатыСчета.Отменен;
...
Процедура ЗаполнитьТабличнуюЧастьЗаказа(ДокументОбъект, ДокументXDTO, Параметры)
	...
	Для каждого ТоварXDTO Из МассивТоваровXDTO Цикл
		...
		Если ЗначениеЗаполнено(Номенклатура) Тогда
			ЗаполнитьСодержание = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Номенклатура, "Услуга");
		Иначе
		...
		НоваяСтрока.Содержание   = ?(ЗаполнитьСодержание, НаименованиеНоменклатуры, "");
