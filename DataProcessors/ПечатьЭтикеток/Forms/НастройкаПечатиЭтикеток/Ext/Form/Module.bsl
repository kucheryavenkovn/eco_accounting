...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ДанныеПечати") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Параметры.ДанныеПечати, ШтрихкодыМаркируемойПродукции);
	КонецЕсли; 
...
Процедура ПодключениеПринтераЭтикетокЗавершение(Значение, ДополнительныеПараметры) Экспорт
	...
	РезультатПодготовкиДанных = ДанныеДляПечатиЭтикеток(СписокОбъектов.ВыгрузитьЗначения(), АдресХранилищаСКД, XMLОписаниеМакета, ТипЦен, Количество);
...
&НаСервереБезКонтекста
Функция ДанныеДляПечатиЭтикеток(СписокОбъектов, АдресХранилищаСКД, XMLОписаниеМакета, ТипЦен, КоличествоПоУмолчанию)
...
Функция ПараметрыШаблонаЭтикеток(ИмяШаблонаЭтикетки)
	Возврат Справочники.ХранилищеШаблонов.ПараметрыШаблонаЭтикеток(ИмяШаблонаЭтикетки);
КонецФункции 
