...
Процедура Конвертация_ПередЗагрузкойОбъекта(ФайлОбмена, Отказ, Нпп, Источник, ИмяПравила, Правило, ГенерироватьНовыйНомерИлиКодЕслиНеУказан, 
	...
	Если НЕ Параметры.ОрганизацияСуществует Тогда
			...
			Иначе	
				...
				Если СтрНайти(Метаданные.Имя,"Бухгалтерия") > 0 Или СтрНайти(Метаданные.Имя,"УправлениеХолдингом") > 0 Или Найти(Метаданные.Имя,"АрендаИУправлениеНедвижимостью") > 0 Тогда
				//--ЭЛИАС 20150422
