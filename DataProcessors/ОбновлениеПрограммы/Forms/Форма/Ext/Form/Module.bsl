...
&НаКлиенте
Процедура НадписьЕстьНеПрименныеИсправленияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура ОтобразитьИнформациюОДоступномОбновлении(ЗаполнитьНачальныеНастройки = Ложь)
	...
	Иначе
		...
		ЕстьНеПрименныеИсправления = Ложь;
		Если УстановленныеИсправления <> Неопределено Тогда
			Для Каждого ОписаниеИсправления Из УстановленныеИсправления Цикл
				Если Не ЗначениеЗаполнено(ОписаниеИсправления.Идентификатор) Тогда
					ЕстьНеПрименныеИсправления = Истина;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		...
		Элементы.НадписьЕстьНеПрименныеИсправления.Видимость = ЕстьНеПрименныеИсправления;
		Элементы.ПанельПредупреждений.Видимость = (Элементы.НадписьЕстьОшибкиУчета.Видимость
			...
			Или Элементы.НадписьЕстьНеПрименныеИсправления.Видимость);
