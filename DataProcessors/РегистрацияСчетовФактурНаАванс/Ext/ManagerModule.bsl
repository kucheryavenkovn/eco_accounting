...
Функция ЗаполнитьСтроки(СтруктураПараметров) Экспорт
	...
	Если ТаблицаАвансов <> Неопределено
		...
		Для Каждого Стр Из ТаблицаАвансов Цикл
			ДокАванса = Стр.ДокументОснование;
			ДокАвансаМетаданные = ДокАванса.Метаданные();
			Если НЕ ТипЗнч(ДокАванса) = Тип("ДокументСсылка.ОтчетКомиссионераОПродажах")
				И ДокАвансаМетаданные.ТабличныеЧасти.Найти("РасшифровкаПлатежа") <> Неопределено Тогда
				ЕстьСчетНаОплату =
				...
				Если ЕстьСчетНаОплату Тогда
					Для Каждого СтрПлатежа Из ДокАванса.РасшифровкаПлатежа Цикл
						Если ЗначениеЗаполнено(СтрПлатежа.СчетНаОплату) Тогда
							МассивСчетовНаОплату.Добавить(СтрПлатежа.СчетНаОплату);
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
				//++ЭЛИАС 20200331 
				ЕстьСчетНаАренду =
				ДокАвансаМетаданные.ТабличныеЧасти.РасшифровкаПлатежа.Реквизиты.Найти("АР_СчетНаАренду") <> Неопределено;
				Если ЕстьСчетНаАренду Тогда
					Для Каждого СтрПлатежа Из ДокАванса.РасшифровкаПлатежа Цикл
						Если ЗначениеЗаполнено(СтрПлатежа.АР_СчетНаАренду) Тогда
							МассивСчетовНаОплату.Добавить(СтрПлатежа.АР_СчетНаАренду);
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
				//--ЭЛИАС 20200331
			КонецЕсли;
		КонецЦикла;
		ТаблицаСчетовНаОплату = ПолучитьДанныеПоСчетамНаОплату(МассивСчетовНаОплату);
		...
		Для Каждого Выборка Из ТаблицаАвансов Цикл
			...
			Если ЗначениеЗаполнено(ДокАванса)
				...
				ТаблицаПоСтавкам.Колонки.Добавить("СчетНаОплату",
					...
					Новый ОписаниеТипов("ДокументСсылка.СчетНаОплатуПокупателю, ДокументСсылка.АР_СчетНаАренду"));
					//--ЭЛИАС 20180201
				...
				ТаблицаПоСтавкам.Колонки.Добавить("СуммаНДСПоСчетуНаОплату",
					ОбщегоНазначения.ОписаниеТипаЧисло(15, 2));
				ТаблицаПоСтавкам.Колонки.Добавить("ЕстьОбщаяСкидкаПоСчету", Новый ОписаниеТипов("Булево"));
					...
					Для Каждого СтрПлатежа Из ДокАванса.РасшифровкаПлатежа Цикл
						Если СтрПлатежа.ДоговорКонтрагента = Выборка.ДоговорКонтрагента 
							...
							Если ЕстьСчетНаОплату
								...
								ДанныеПоСчетуНаОплату = ТаблицасчетовНаОплату.Найти(СтрПлатежа.СчетНаОплату, "СчетНаОплату");
								Если ДанныеПоСчетуНаОплату <> Неопределено Тогда
									ЗаполнитьЗначенияСвойств(СтрокаПоСтавке, ДанныеПоСчетуНаОплату);
								КонецЕсли;
								Если СтрокаПоСтавке.ЕстьОбщаяСкидкаПоСчету Тогда
							...
							КонецЕсли;
							//++ЭЛИАС 20180201 
							Если ЕстьСчетНаАренду 
								И ЗначениеЗаполнено(СтрПлатежа.АР_СчетНаАренду)
								И НЕ ЗначениеЗаполнено(СтрПлатежа.СчетНаОплату) Тогда
								СтрокаПоСтавке.СчетНаОплату = СтрПлатежа.АР_СчетНаАренду;
								ДанныеПоСчетуНаОплату = ТаблицасчетовНаОплату.Найти(СтрПлатежа.АР_СчетНаАренду, "СчетНаОплату");
								Если ДанныеПоСчетуНаОплату <> Неопределено Тогда
									ЗаполнитьЗначенияСвойств(СтрокаПоСтавке, ДанныеПоСчетуНаОплату);
								КонецЕсли;
								Если СтрокаПоСтавке.ЕстьОбщаяСкидкаПоСчету Тогда
	...
	Для каждого СтрокаДокумента Из ТаблицаОснований Цикл
		...
		Для каждого СтрокаКлючевыхПолей Из ТаблицаКлючевыхПолей Цикл
			...
			Для каждого СтрокаСФ Из СтрокиСФ Цикл
				...
				ОплатаАренды = ЗначениеЗаполнено(СтрокаСФ.ДокументОснование)
					И ОбщегоНазначения.ЕстьРеквизитОбъекта("АР_ОплатаАренды", СтрокаСФ.ДокументОснование.Метаданные())
					И СтрокаСФ.ДокументОснование.АР_ОплатаАренды;
				Если ЗначениеЗаполнено(СтрокаСФ.СчетНаОплату) ИЛИ ОплатаАренды Тогда
					Если ОплатаАренды Тогда
						ПараметрыЗаполнения = Документы.СчетФактураВыданный.ПодготовитьПараметрыЗаполненияАванс(СтрокаСФ.ДокументОснование, Неопределено, СтрокаСФ.ДоговорКонтрагента, Ложь);
						Если ПараметрыЗаполнения.Свойство("Авансы") Тогда
							ТаблицаАвансов = ПараметрыЗаполнения.Авансы;
						Иначе
							ТаблицаАвансов = ПолучитьНоменклатуруСчетаНаОплату(
								СтрокаСФ.СчетНаОплату, СтрокаСФ, СчФ.Авансы.ВыгрузитьКолонки());
						КонецЕсли;
						Если СтрокиСФ.Количество() > 1 И ЗначениеЗаполнено(СтрокаСФ.СчетНаОплату) Тогда
							ТаблицаАвансов = ПолучитьНоменклатуруСчетаНаОплату(
								СтрокаСФ.СчетНаОплату, СтрокаСФ, СчФ.Авансы.ВыгрузитьКолонки());
						КонецЕсли;
					Иначе 	
			...
			//++ЭЛИАС 20170725 
			СтрокиПредвОплаты = СчФ.Авансы.НайтиСтроки(Новый Структура("Номенклатура", Справочники.Номенклатура.ПустаяСсылка()));
			Для Каждого Стр Из СтрокиПредвОплаты Цикл
				Индекс = СчФ.Авансы.Индекс(Стр);
				СчФ.Авансы.Сдвинуть(Стр, -1 * Индекс);
			КонецЦикла;
			//--ЭЛИАС 20170725
...
Функция ПолучитьНоменклатуруСчетаНаОплату(СчетНаОплату, СтрокаОтбора, ТаблицаРезультата)
	...
	|	АР_СчетНаАрендуСостав.Содержание,
	|	АР_СчетНаАрендуСостав.СтавкаНДС,
	|	АР_СчетНаАрендуСостав.СуммаНДС,
	|	ВЫБОР
	|		КОГДА АР_СчетНаАрендуСостав.Ссылка.СуммаВключаетНДС
	|			ТОГДА АР_СчетНаАрендуСостав.Сумма
	|		ИНАЧЕ АР_СчетНаАрендуСостав.Сумма + АР_СчетНаАрендуСостав.СуммаНДС
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА АР_СчетНаАрендуСостав.Ссылка.СуммаВключаетНДС
	|			ТОГДА АР_СчетНаАрендуСостав.Сумма - АР_СчетНаАрендуСостав.СуммаНДС
	|		ИНАЧЕ АР_СчетНаАрендуСостав.Сумма
	|	КОНЕЦ,
	...
	|ГДЕ
	|	АР_СчетНаАрендуСостав.Ссылка = &СчетНаОплату
	|	И АР_СчетНаАрендуСостав.СтавкаНДС В(&СтавкиНДС)
	|
	//--ЭЛИАС 20131126
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
...
//++ЭЛИАС 20180201 
//Функция ПолучитьДанныеПоСчетамНаОплату(МассивСчетов)
Функция ПолучитьДанныеПоСчетамНаОплату(МассивСчетов, СчетНаАренду = Ложь)
//--ЭЛИАС 20180201
	...
	|	МАКСИМУМ(СчетНаОплатуПокупателю.СуммаДокумента) КАК СуммаПоСчетуНаОплату,
	|	ВЫБОР
	|		КОГДА СчетНаОплатуПокупателю.СуммаСкидки <> 0
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЕстьОбщаяСкидкаПоСчету,
	|	СчетТовары.Ссылка КАК СчетНаОплату
	|ИЗ
	...
	|		ПО СчетТовары.Ссылка = СчетНаОплатуПокупателю.Ссылка
	|ГДЕ
	|	СчетТовары.Ссылка В(&МассивСчетов)
	|	И СчетНаОплатуПокупателю.Ссылка В(&МассивСчетов)
	|
	//++ЭЛИАС 20200331 
	|СГРУППИРОВАТЬ ПО
	|	ВЫБОР
	...
	|
	|ВЫБРАТЬ
	|	СУММА(СчетТовары.СуммаНДС),
	|	МАКСИМУМ(СчетНаОплатуПокупателю.СуммаДокумента),
	|	ВЫБОР
	|		КОГДА ЛОЖЬ
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|	СчетТовары.Ссылка
	|ИЗ
	|	Документ.АР_СчетНаАренду.Состав КАК СчетТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетНаОплатуПокупателю КАК СчетНаОплатуПокупателю
	|		ПО СчетТовары.Ссылка = СчетНаОплатуПокупателю.Ссылка
	|ГДЕ
	|	СчетТовары.Ссылка В(&МассивСчетов)
