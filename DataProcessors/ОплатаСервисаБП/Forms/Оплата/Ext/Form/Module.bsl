...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СрокПредупрежденияОЗавершенииПодписки = Обработки.ОплатаСервисаБП.СрокПредупрежденияОЗавершенииПодписки();
	...
	ДанныеВыставленногоСчета = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища("ОплатаСервиса",
		...
		|ПлатежнаяСсылка, ЗаполнятьДанныеАбонента, Сообщение");
	...
	ЗаполнятьДанныеАбонента            = ДанныеВыставленногоСчета.ЗаполнятьДанныеАбонента;
	
	СчетПоставщика = Обработки.ОплатаСервисаБП.НайтиСчетПоставщика(ИдентификаторСчетаПоставщика);
...
Процедура ПриПовторномОткрытии()
	...
	ИдетИнтерактивноеОбновлениеТарифа = Истина;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_Организации" Тогда
		Если НЕ ЗначениеЗаполнено(Организация) Тогда
			...
			УстановитьДанныеОрганизации();
			УправлениеФормой(ЭтотОбъект);
		ИначеЕсли Источник = Организация Тогда
			УстановитьДанныеОрганизации();
			УправлениеФормой(ЭтотОбъект);
		КонецЕсли;
	КонецЕсли;
...
Процедура ОплатитьПлатежнойКартой(Команда)
	...
	Если НЕ ЗаполнятьДанныеАбонента Тогда
		ПерейтиКОплате();
	Иначе
		ОткрытьФормуДанныеАбонента();
	КонецЕсли;
...
&НаКлиенте
Процедура ТребуютсяРеквизитыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура УправлениеФормой(Форма)
	...
	ДействуетТестовыйТариф = Элементы.ГруппаПродлениеТарифа.ПодчиненныеЭлементы.Количество() = 0;
	...
	ИдетИнтерактивноеОбновлениеТарифа = Форма.ИдетИнтерактивноеОбновлениеТарифа;
	
	Ожидание = НЕ ПолученСписокТарифов
		ИЛИ НЕ ПолученТекущийТариф
		ИЛИ ИдетПолучениеСчета
		ИЛИ ИдетИнтерактивноеОбновлениеТарифа;
	...
	Элементы.ТекстОшибки.Видимость = НЕ ПустаяСтрока(Форма.ТекстОшибки);
	Элементы.ТекстОшибки.Заголовок = Форма.ТекстОшибки;
	
	Элементы.ТребуютсяРеквизитыИП.Видимость = Форма.ТребуютсяРеквизиты И Форма.ЭтоИндивидуальныйПредприниматель;
	Элементы.ТребуютсяРеквизитыЮЛ.Видимость = Форма.ТребуютсяРеквизиты И НЕ Форма.ЭтоИндивидуальныйПредприниматель;
	
	Элементы.ГруппаПродлениеТарифа.Видимость = НЕ Форма.ТребуютсяРеквизиты И НЕ ОжиданиеОплаты;
	Элементы.ГруппаОжиданиеОплаты.Видимость = НЕ Форма.ТребуютсяРеквизиты И ОжиданиеОплатыВПлатежнойСистеме;
	Элементы.ГруппаОплата.Видимость = НЕ Форма.ТребуютсяРеквизиты;
	Элементы.ДругиеТарифы.Видимость = НЕ Форма.ТребуютсяРеквизиты;
	
	Элементы.Организация.Видимость = НЕ Форма.ТребуютсяРеквизиты;
	...
	Форма.ИнформацияОПродленииПодпискиПояснение = ?(ДействуетТестовыйТариф,
	...
	Элементы.ИнформацияПоОплатеКартой.Видимость = НЕ Продление И ОплатаПлатежнойКартой И НЕ ДействуетТестовыйТариф;
	Элементы.ИнформацияПоОплатеКартойТестовыйТариф.Видимость = НЕ Продление И ОплатаПлатежнойКартой И ДействуетТестовыйТариф;
...
Функция ФоновоеЗаданиеНачатьВыставлениеСчета(ПараметрыПроцедуры, УникальныйИдентификатор)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ОплатаСервисаБП.НачатьВыставлениеСчетаВФоне",
...
Процедура ОбработатьРезультатНачалаВыставленияСчета(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Если РезультатВыполнения.Ошибка Тогда
			...
			ОбработатьОшибкуСвязи(РезультатВыполнения.ОписаниеОшибки);
		Иначе
			ОшибкаСвязи = Ложь;
			ТекстОшибки = "";
			СчетчикОбращений = 0;
	...
	ИначеЕсли Результат.Статус = "Ошибка" Тогда
		...
		ИдентификаторЗаданияВыставлениеСчета = Неопределено;
		ОбработатьОшибкуСвязи(Результат.ПодробноеПредставлениеОшибки);
...
Процедура Подключаемый_ПроверитьВыставлениеСчета()
	...
	Если СчетчикОбращений >= 60 Тогда
		ТекстОписанияОшибки = НСтр("ru='Исчерпано допустимое число попыток получения результата из менеджера сервиса'");
		ЗаписатьОшибкуВЖурналРегистрации(ТекстОписанияОшибки);
		ОбработатьОшибкуСвязи(ТекстОписанияОшибки);
		ИдентификаторЗаданияВыставлениеСчета = Неопределено;
		УправлениеФормой(ЭтотОбъект);
		Возврат;
	КонецЕсли;
...
Функция ФоновоеЗаданиеПроверитьВыставлениеСчета(ПараметрыПроцедуры, УникальныйИдентификатор)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ОплатаСервисаБП.ПроверитьВыставлениеСчетаВФоне",
		ПараметрыПроцедуры, ПараметрыВыполнения);
...
Процедура ОбработатьРезультатПроверкиВыставленияСчета(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Если РезультатОбработки.Ошибка Тогда
			...
			ОбработатьОшибкуСвязи(РезультатОбработки.ОписаниеОшибки);
			ИдентификаторЗаданияВыставлениеСчета = Неопределено;
		...
		ИначеЕсли ЗначениеЗаполнено(РезультатОбработки.Ссылка) Тогда
			...
			ЗаполнятьДанныеАбонента      = РезультатОбработки.ЗаполнятьДанныеАбонента;
...
&НаКлиенте
Процедура ОбработатьПереходКОплате(Результат, ДополнительныеПараметры) Экспорт
...
&НаКлиенте
Процедура ОткрытьФормуДанныеАбонента()
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Организация", Организация);
	ОбработчикОповещения = Новый ОписаниеОповещения("ОбработатьПереходКОплате", ЭтотОбъект);
	ОткрытьФорму("Обработка.ОплатаСервисаБП.Форма.ДанныеАбонента", ПараметрыФормы, ЭтотОбъект,,,, ОбработчикОповещения);
...
Процедура СохранитьДанныеВыставленногоСчета(Форма)
	...
	ДанныеСчета.Вставить("НовыйТарифНаименование",             Форма.НовыйТарифНаименование);
	ДанныеСчета.Вставить("НовыйТарифДополнительнаяИнформация", Форма.НовыйТарифДополнительнаяИнформация);
	ДанныеСчета.Вставить("НовыйТарифИдентификатор",            Форма.НовыйТарифИдентификатор);
	ДанныеСчета.Вставить("ТекущийТарифИдентификатор",          Форма.ТекущийТарифИдентификатор);
	ДанныеСчета.Вставить("ТекущийТарифСрокДействия",           Форма.ТекущийТарифСрокДействия);
	ДанныеСчета.Вставить("ИдентификаторСчетаПокупателю",       Форма.ИдентификаторСчетаПокупателю);
	ДанныеСчета.Вставить("ИдентификаторСчетаПоставщика",       Форма.ИдентификаторСчетаПоставщика);
	ДанныеСчета.Вставить("ПлатежнаяСсылка",                    Форма.ПлатежнаяСсылка);
	ДанныеСчета.Вставить("Сообщение",                          Форма.Сообщение);
	ДанныеСчета.Вставить("ЗаполнятьДанныеАбонента",            Форма.ЗаполнятьДанныеАбонента);
	
	СохранитьДанныеВыставленногоСчетаНаСервере(ДанныеСчета);
...
Функция ФоновоеЗаданиеПолучениеТарифов(Знач УникальныйИдентификатор)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ОплатаСервисаБП.ПолучитьТарифыВФоне",
		ПараметрыПроцедуры, ПараметрыВыполнения);
...
Функция ФоновоеЗаданиеПолучениеТекущегоТарифа(УникальныйИдентификатор)
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("Обработки.ОплатаСервисаБП.ПолучитьТекущийТарифВФоне",
		ПараметрыПроцедуры, ПараметрыВыполнения);
...
Процедура ОбработатьРезультатПолученияТекущегоТарифа(Результат, ДополнительныеПараметры) Экспорт
	...
	ИдетИнтерактивноеОбновлениеТарифа = Ложь;
	...
	Если Результат.Статус = "Выполнено" Тогда
		...
		Если ТарифНеОпределен Тогда
			Если ВыполнятьОднократно Тогда
				ТекстОписанияОшибки = НСтр("ru='Менеджер сервиса не передал текущий тариф'");
				ЗаписатьОшибкуВЖурналРегистрации(ТекстОписанияОшибки);
				ОбработатьОшибкуСвязи(ТекстОписанияОшибки);
				УправлениеФормой(ЭтотОбъект);
			Иначе
...
&НаКлиенте
Процедура ОбработатьОшибкуСвязи(Описание)
...
Процедура ОткрытьФормуВыбораТарифа()
	...
	ОткрытьФорму("Обработка.ОплатаСервисаБП.Форма.ВыборТарифа", ПараметрыФормы, ЭтотОбъект,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура УстановитьДанныеОрганизации()
	...
	РеквизитыОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Организация, "ИНН, ЮридическоеФизическоеЛицо");
	
	ТребуютсяРеквизиты = НЕ ЗначениеЗаполнено(РеквизитыОрганизации.ИНН);
	ЭтоИндивидуальныйПредприниматель = РеквизитыОрганизации.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо;
