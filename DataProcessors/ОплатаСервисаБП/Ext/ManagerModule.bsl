...
Процедура НачатьВыставлениеСчетаВФоне(Параметры, АдресРезультата) Экспорт
	...
	Попытка
		...
		Если Ответ.КодСостояния <> 201 Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru='Менеджер сервиса: %1 - %2'"),
				Ответ.КодСостояния,
...
Процедура ПроверитьВыставлениеСчетаВФоне(Параметры, АдресРезультата) Экспорт
	...
	Попытка
		...
		Результат.Вставить("ИдентификаторСчетаПокупателю", "");
		Результат.Вставить("ИдентификаторСчетаПоставщика", "");
		Результат.Вставить("ПлатежнаяСсылка", "");
		Результат.Вставить("Сообщение", "");
		Результат.Вставить("ЗаполнятьДанныеАбонента", Ложь);
		Результат.Вставить("Ошибка", Ложь);
		Результат.Вставить("ОписаниеОшибки", "");
			...
			Если НЕ ПустаяСтрока(ДанныеОтвета.status) Тогда
				...
				Если КодСостоянияПрокси = "200" Тогда // Получен ответ учетной системы.
					...
					Иначе
						...
						Результат.ЗаполнятьДанныеАбонента = ?(ДанныеОтвета.Свойство("requireCustomerDetails"),
							ДанныеОтвета.requireCustomerDetails,
							Ложь);
...
Процедура ОтправитьДанныеАбонентаВФоне(ДанныеАбонента, АдресРезультата) Экспорт
	
	ИдентификаторОбъекта = Строка(Новый УникальныйИдентификатор);
	
	Результат = Новый Структура;
	Результат.Вставить("ИдентификаторОбъекта", ИдентификаторОбъекта);
	Результат.Вставить("Ошибка", Ложь);
	Результат.Вставить("ОписаниеОшибки", "");
	
	ДанныеЗапроса = Новый Структура;
	ДанныеЗапроса.Вставить("id",   ИдентификаторОбъекта);
	ДанныеЗапроса.Вставить("zone", РаботаВМоделиСервиса.ЗначениеРазделителяСеанса());
	ДанныеЗапроса.Вставить("data", ДанныеАбонента);
	
	Попытка
		
		Ответ = ОтправитьДанныеВМенеджерСервиса("/hs/ui/customer_details", ДанныеЗапроса);
		
		Если Ответ.КодСостояния <> 201 Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru='Менеджер сервиса: %1 - %2'"),
				Ответ.КодСостояния,
				Ответ.ПолучитьТелоКакСтроку());
		КонецЕсли;
