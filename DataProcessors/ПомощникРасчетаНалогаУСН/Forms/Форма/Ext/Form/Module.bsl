...
Процедура ЗаполнитьНачальныеПоказатели()
	...
	ТекстБаннера = НалоговыйУчет.ТекстБаннераНастройкиНалоговИОтчетов(Объект.Организация,
		ЗадачиБухгалтераКлиентСервер.КодЗадачиУСН());
...
&НаКлиенте
Процедура ОткрытьСправкуРасчетУСН()
	
	НастройкиОтчета = Новый ПользовательскиеНастройкиКомпоновкиДанных;
	
	НастройкиОтчета.ДополнительныеСвойства.Вставить("Организация", Объект.Организация);
	// Период в настройках не указываем, он будет инициализирован
	// при обработке параметра формы ОткрытьВПериодеОсвобожденияОтНалогов
