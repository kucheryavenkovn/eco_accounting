...
Процедура СформироватьДокументыОООВФоне(ПараметрыФормированияДокументов, АдресРезультата = Неопределено) Экспорт
	...
	Попытка
		...
		Если СервисКвитанцияНаОплатуГоспошлиныДоступен()
			И ЗначениеЗаполнено(ДанныеРуководителя.АдресПропискиЗначениеJSON) Тогда
...
Процедура СформироватьДокументыИПВФоне(ПараметрыФормированияДокументов, АдресРезультата = Неопределено) Экспорт
	...
	Попытка
		...
		ПараметрыЗаявленияОРегистрацииИП = ПараметрыЗаявленияОРегистрации(
			...
			ПараметрыФормированияДокументов.СпособРегистрации,
			ДанныеДокументов,
			ПараметрыФормированияДокументов.ОтчествоЛатиницей);
...
Функция ЗаписатьФизическоеЛицо(ДанныеФизическогоЛица) Экспорт
	...
	Если ДанныеФизическогоЛица.Свойство("ПаспортныеДанные") И ДанныеФизическогоЛица.Свойство("Гражданство") Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(ДанныеФизическогоЛица.Гражданство) Тогда
...
Функция ПараметрыЗаявленияОРегистрации(Организация, СпособРегистрации, ДанныеДокументов = Неопределено, ОтчествоЛатиницей = "")
	
	ПараметрыЗаполнения = Новый Структура;
	ПараметрыЗаполнения.Вставить("Организация", Организация);
	ПараметрыЗаполнения.Вставить("СпособРегистрации", СпособРегистрации);
	ПараметрыЗаполнения.Вставить("ОтчествоЛатиницей", ОтчествоЛатиницей);
	ПараметрыЗаполнения.Вставить("ДанныеДокументов", ДанныеДокументов);
...
Процедура ЗаполнитьУведомлениеОПостановкеНаУчетИП(ПараметрыОтчета, Контейнер) Экспорт
	...
	СведенияОбОрганизации = РегламентированнаяОтчетностьПереопределяемый.ПолучитьСведенияОбОрганизации(
		ПараметрыОтчета.Организация, ТекущаяДатаСеанса(), СписокПоказателей);
	ГражданинРФ = СведенияОбОрганизации.Гражд = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.СтраныМира.Россия"), "Код");
	...
	ДанныеДокументов = ПараметрыЗаполнения.ДанныеДокументов;
	Если ТипЗнч(ДанныеДокументов) = Тип("Структура") Тогда
		ДанныеДокументов.ДанныеПаспорта.Свойство("Серия", СерияПаспорта);
		ДанныеДокументов.ДанныеПаспорта.Свойство("Номер", НомерПаспорта);
		ДанныеДокументов.ДанныеОснованияДляПроживания.Свойство("Серия", СерияОснованияДляПроживания);
		ДанныеДокументов.ДанныеОснованияДляПроживания.Свойство("Номер", НомерОснованияДляПроживания);
	КонецЕсли;
	...
	ДанныеЛиста.Вставить("П01040100", СведенияОбОрганизации.ДатаРожд);
	...
	Иначе
		...
		ДокументыИП = КадровыйУчет.ДокументыФизическихЛиц(ПараметрыОтчета.Организация.ИндивидуальныйПредприниматель, Истина);
		Если ЗначениеЗаполнено(СведенияОбОрганизации.Гражд) Тогда
			// Иностранец
			...
			Если ДанныеИностранногоПаспорта <> Неопределено Тогда
				ДанныеЛиста.Вставить("П01010201", ДанныеИностранногоПаспорта.ФамилияЛатиницей);
				ДанныеЛиста.Вставить("П01010202", ДанныеИностранногоПаспорта.ИмяЛатиницей);
			КонецЕсли;
			ДанныеЛиста.Вставить("П01010203", ПараметрыЗаполнения.ОтчествоЛатиницей);
	...
	ДанныеЛиста.Вставить("П01070300", СведенияОбОрганизации.ДатаУдЛичн);
	...
	Если ГражданинРФ Тогда
		ДанныеОснованияДляПроживанияВРФ = Неопределено;
	Иначе
		...
		ДанныеОснованияДляПроживанияВРФ = ДокументыФизическихЛицБП.ДанныеДействующегоДокумента(
			...
			ЗначениеЗаполнено(СведенияОбОрганизации.Гражд));
	КонецЕсли;
	
	Если ДанныеОснованияДляПроживанияВРФ = Неопределено Тогда
		КодРазрешения = "";
	ИначеЕсли ДанныеОснованияДляПроживанияВРФ.ВидДокумента = ДокументыФизическихЛицБП.ВидДокументаПоКодуФНС(
		КодыВидовДокументов.ВидНаЖительство) Тогда
		КодРазрешения = "1";
	ИначеЕсли ДанныеОснованияДляПроживанияВРФ.ВидДокумента = ДокументыФизическихЛицБП.ВидДокументаПоКодуФНС(
		КодыВидовДокументов.РазрешениеНаВременноеПроживание) Тогда
		КодРазрешения = "2";
	КонецЕсли;
	...
	Если КодРазрешения <> "" Тогда
		...
		ДанныеЛиста.Вставить("П01080200", ?(ЗначениеЗаполнено(ДанныеОснованияДляПроживанияВРФ.Серия),
			ДанныеОснованияДляПроживанияВРФ.Серия + " ", "") + ДанныеОснованияДляПроживанияВРФ.Номер);
		ДанныеЛиста.Вставить("П01080300", ДанныеОснованияДляПроживанияВРФ.ДатаВыдачи);
		ДанныеЛиста.Вставить("П01080400", ДанныеОснованияДляПроживанияВРФ.КемВыдан);
		ДанныеЛиста.Вставить("П01080500", ?(ЗначениеЗаполнено(ДанныеОснованияДляПроживанияВРФ.СрокДействия),
			ДанныеОснованияДляПроживанияВРФ.СрокДействия, ""));
...
Процедура ЗаполнитьУведомлениеОПостановкеНаУчетООО(ПараметрыОтчета, Контейнер) Экспорт
	...
	Если УчредителиФизическиеЛица.Количество() > 0 Тогда
		Для каждого СтрокаТаблицы Из УчредителиФизическиеЛица Цикл
			...
			НовыйЛист.В01030100 = СведенияОФизическомЛице.ДатаРождения;
			...
			НовыйЛист.В01050300 = СведенияОФизическомЛице.ДокументДатаВыдачи;
			...
			КонецЕсли;
			Если ТипЗнч(РоссийскийАдрес) = Тип("Структура") Тогда
				НовыйЛист.Вставить("АдресXML", РоссийскийАдрес.ЗначенияПолей);
				РегламентированнаяОтчетностьВызовСервера.СформироватьАдрес(
					РоссийскийАдрес.ЗначениеJSON, РоссийскийАдрес);
				ОбработатьЭлементыАдреса(РоссийскийАдрес);
				ЗаполнитьЗначенияСвойств(НовыйЛист, РоссийскийАдрес);
			Иначе
				НовыйЛист.В01060201 = СведенияОФизическомЛице.АдресИнформированияСтруктурой.КодСтраны;
				НовыйЛист.В01060202 = СведенияОФизическомЛице.АдресИнформированияСтруктурой.Представление;
			КонецЕсли;
	...
	// Дата и место рождения
	НовыйЛист.Е03010000 = СведенияОФизическомЛице.ДатаРождения;
	...
	НовыйЛист.Е05030000 = СведенияОФизическомЛице.ДокументДатаВыдачи;
	...
	Иначе
		НовыйЛист.Е06020100 = СведенияОФизическомЛице.АдресИнформированияСтруктурой.КодСтраны;
		НовыйЛист.Е06020200 = СведенияОФизическомЛице.АдресИнформированияСтруктурой.Представление;
	КонецЕсли;
	// Телефон
	...
	Для каждого СтрокаТаблицы Из ДанныеУчредителей.Учредители Цикл
		...
		Если СтрокаТаблицы.ЭтоЮридическоеЛицо Или КоличествоУчредителей > 1 Тогда
			...
			// Дата и место рождения
			НовыйЛист.Н04030100 = СведенияОЗаявителе.ДатаРождения;
			...
			НовыйЛист.Н04040300 = СведенияОЗаявителе.ДокументДатаВыдачи;
			...
			КонецЕсли;
			// Адрес
			Если ЗначениеЗаполнено(СведенияОЗаявителе.АдресПрописки) Тогда
				РоссийскийАдрес = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(СведенияОЗаявителе.АдресСтруктурой);
			ИначеЕсли ЗначениеЗаполнено(СведенияОЗаявителе.АдресИнформирования)
				И СведенияОЗаявителе.АдресИнформированияСтруктурой.АдресРФ Тогда
				РоссийскийАдрес = ОбщегоНазначенияКлиентСервер.СкопироватьСтруктуру(
					СведенияОЗаявителе.АдресИнформированияСтруктурой);
			Иначе
				РоссийскийАдрес = Неопределено;
			КонецЕсли;
			Если ТипЗнч(РоссийскийАдрес) = Тип("Структура") Тогда
				...
				НовыйЛист.Вставить("АдресXML", АдресXML);
			Иначе
				НовыйЛист.Н04050201 = СведенияОЗаявителе.АдресИнформированияСтруктурой.КодСтраны;
				НовыйЛист.Н04050202 = СведенияОЗаявителе.АдресИнформированияСтруктурой.Представление;
			КонецЕсли;
...
//
Функция ИнструкцияРегистрацияИП(СпособПодачиДокументов, ВыборСпецрежимаНалогообложения, ДанныеРегистрирующегоОргана, ДанныеНалоговойИнспекции, СтранаГражданства) Экспорт
...
Функция ИнструкцияРегистрацииООО(КоличествоУчредителей, ОдинУчредительЮридическоеЛицо, СпособПодачиДокументов, ВыборСпецрежимаНалогообложения, ДанныеРегистрирующегоОргана, ДанныеНалоговойИнспекции, УникальныйИдентификатор, Гражданство, ЕстьИностранцы = Ложь) Экспорт
	...
	Если СпособПодачиДокументов <> СпособыПодачиДокументов.ИФНСлично Или Не ОдинУчредитель Тогда
		...
		ИмяОбласти = ?(ЕстьИностранцы, "ЗаверениеКопийУНотариусаИностранныеГраждане", "ЗаверениеКопийУНотариуса");
		Область = ПолучитьОбластьИнструкцииРегистрация(ИмяОбласти, ШаблонИнструкции, ПараметрыИнструкции);
		Параметры = Новый Структура("НомерШага, Паспорт", УвеличитьЗначениеНаЕдиницу(НомерШага), ПараметрыИнструкции.Паспорт);
		Область.Параметры.Заполнить(Параметры);
	...
	Параметры = Новый Структура();
	Параметры.Вставить("НомерШага", УвеличитьЗначениеНаЕдиницу(НомерШага));
	Параметры.Вставить("Паспорт", ПараметрыИнструкции.Паспорт);
	Если ВыборСпецрежимаНалогообложения.УСН Тогда
...
Функция ПроверитьРеквизитыУчредителя(Учредитель, ПроверяемыеРеквизиты, ПроверяемаяКонтактнаяИнформация = "", ПроверяемыеПаспортныеДанныеРФ = "", ПроверяемыеПаспортныеДанныеНеГражданРФ = "")
	...
	Если ТипЗнч(Учредитель) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		...
		Если Не Выборка.Следующий() Тогда
			Возврат Ложь;
		Иначе
			ПроверяемыеДанныеУдостоверенияЛичности = ?(Выборка.ВидДокумента =
				ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыДокументовФизическихЛиц.ПаспортРФ"),
				ПроверяемыеПаспортныеДанныеРФ,
				ПроверяемыеПаспортныеДанныеНеГражданРФ);
...
Процедура ЗаписатьСтатусЗаявленияОВнесенииИзмененийЕГР(Параметры, СтатусЗаявления)
	...
	Если Параметры.Свойство("ЗаявлениеОВнесенииИзмененийЕГР",
		...
		СохранитьСтатусОтправкиУведомления(ЗаявлениеОВнесенииИзменений, СтатусЗаявления);
	...
	Если Параметры.Свойство("ЗаявлениеОВнесенииИзмененийУчредительныеДокументы",
		...
		СохранитьСтатусОтправкиУведомления(ЗаявлениеОВнесенииИзмененийУчредительныеДокументы, СтатусЗаявления);
		
	КонецЕсли;
	
КонецПроцедуры

Процедура СохранитьСтатусОтправкиУведомления(Документ, Статус)
...
Процедура ЗаполнитьЗаявлениеРегистрацииИзмененийЕГРЮЛ(ПараметрыОтчета, Контейнер) Экспорт
		...
		Для Каждого СтрокаТаблицы Из УчастникиФизическиеЛица Цикл
			...
			Если СтрокаТаблицы.СуммаВзносаДоИзменения = 0 Тогда
				...
				Если ТипЗнч(РоссийскийАдрес) = Тип("Структура") Тогда
					...
					ЗаполнитьЗначенияСвойств(НовыйЛист, РоссийскийАдрес);
				Иначе
					НовыйЛист.Д03060201 = СведенияОФизическомЛице.АдресИнформированияСтруктурой.КодСтраны;
					НовыйЛист.Д03060202 = СведенияОФизическомЛице.АдресИнформированияСтруктурой.Представление;
				КонецЕсли;
	...
	Если Изменено.Руководитель Тогда
		...
		// Паспортные данные
		Если ЗначениеЗаполнено(ДанныеНовогоРуководителя.Гражданство) Тогда
			ДанныеУдостоверенияЛичности = ДанныеНовогоРуководителя.ПаспортныеДанные;
		Иначе
			ДанныеУдостоверенияЛичности = ДанныеНовогоРуководителя.ДанныеОснованияДляПроживанияВРФ;
		КонецЕсли;
		НовыйЛист.К03050100 = ДанныеУдостоверенияЛичности.ДокументКодМВД;
		НовыйЛист.К03050200 = СтрШаблон("%1 %2", ДанныеУдостоверенияЛичности.Серия, ДанныеУдостоверенияЛичности.Номер);
		НовыйЛист.К03050300 = ДанныеУдостоверенияЛичности.ДатаВыдачи;
		НовыйЛист.К03050400 = ДанныеУдостоверенияЛичности.КемВыдан;
		Если ДанныеУдостоверенияЛичности.Свойство("КодПодразделения") Тогда
		...
		Если ЗначениеЗаполнено(ДанныеНовогоРуководителя.АдресПропискиЗначениеJSON) Тогда
			РоссийскийАдрес = ДанныеНовогоРуководителя.АдресПропискиЗначениеJSON;
		ИначеЕсли ЗначениеЗаполнено(ДанныеНовогоРуководителя.АдресИнформированияЗначениеJSON)
			И УправлениеКонтактнойИнформацией.СтранаАдресаКонтактнойИнформации(
				ДанныеНовогоРуководителя.АдресИнформированияЗначениеJSON) =
				ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.СтраныМира.Россия") Тогда
			РоссийскийАдрес = ДанныеНовогоРуководителя.АдресИнформированияЗначениеJSON;
		Иначе
			РоссийскийАдрес = Неопределено;
		КонецЕсли;
		Если РоссийскийАдрес <> Неопределено Тогда
		...
		Иначе
			АдресСтруктурой = УправлениеКонтактнойИнформациейБП.СтруктураАдреса(
				ДанныеНовогоРуководителя.АдресИнформированияЗначениеJSON, ДополнительныеПараметрыКИ);
			НовыйЛист.К03060201 = АдресСтруктурой.КодСтраны;
			НовыйЛист.К03060202 = АдресСтруктурой.Представление;
		КонецЕсли;
		// Телефон
	...
	Если ПараметрыЗаполнения.Свойство("ДанныеЗаявителя", ДанныеЗаявителя) Тогда
		...
		// Паспортные данные
		Если ЗначениеЗаполнено(ДанныеЗаявителя.Гражданство) Тогда
			ДанныеУдостоверенияЛичности = ДанныеЗаявителя.ПаспортныеДанные;
		Иначе
			ДанныеУдостоверенияЛичности = ДанныеЗаявителя.ДанныеОснованияДляПроживанияВРФ;
		КонецЕсли;
		
		НовыйЛист.Р04040100 = ДанныеУдостоверенияЛичности.ДокументКодМВД;
		НовыйЛист.Р04040200 = СтрШаблон("%1 %2", ДанныеУдостоверенияЛичности.Серия, ДанныеУдостоверенияЛичности.Номер);
		НовыйЛист.Р04040300 = ДанныеУдостоверенияЛичности.ДатаВыдачи;
		НовыйЛист.Р04040400 = ДанныеУдостоверенияЛичности.КемВыдан;
		Если ДанныеУдостоверенияЛичности.Свойство("КодПодразделения") Тогда
			ЧастиКодаПодразделения = СтрРазделить(ДанныеУдостоверенияЛичности.КодПодразделения, "-", Ложь);
			Если ЧастиКодаПодразделения.Количество() = 2 Тогда
		...
		Иначе
			АдресСтруктурой = УправлениеКонтактнойИнформациейБП.СтруктураАдреса(
				ДанныеЗаявителя.АдресИнформированияЗначениеJSON, ДополнительныеПараметрыКИ);
			НовыйЛист.Р04050201 = АдресСтруктурой.КодСтраны;
			НовыйЛист.Р04050202 = АдресСтруктурой.Представление;
		КонецЕсли;
