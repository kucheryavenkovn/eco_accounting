	...
	Если ЭтоЮрЛицо Тогда
		...
		Если ЕдинственныйУчредительЮридическоеЛицо Тогда
			Элементы.ПротоколСобранияУчредителейЕдинственногоУчредителяДекорация.Заголовок =
				СтрШаблон(НСтр("ru = 'Образец протокола собрания учредителей %1'"), УчредителиПоТипам.ЮридическиеЛица[0]);
		КонецЕсли;
	...
	Если ЭтоЮрЛицо Тогда
		...
		Иначе
			Если ВидДокумента = РегистрацияОрганизацииКлиентСервер.ЗначениеРазрешенияНаВременноеПроживаниеВСпискеВыбора() Тогда
				ДанныеЗаявителя.ДанныеОснованияДляПроживанияВРФ.ВидДокумента =
...
Функция УчредителиПоТипам(Форма)
	...
	Для Каждого СтрокаТаблицы Из Форма.Учредители Цикл
		Если СтрокаТаблицы.ТипУчредителя = ПредопределенноеЗначение("Перечисление.ЮридическоеФизическоеЛицо.ФизическоеЛицо") Тогда
			НаименованияФизическихЛиц.Добавить(СтрокаТаблицы.Наименование);
		ИначеЕсли СтрокаТаблицы.ТипУчредителя = ПредопределенноеЗначение("Перечисление.ЮридическоеФизическоеЛицо.ЮридическоеЛицо") Тогда
			НаименованияЮридическихЛиц.Добавить(СтрокаТаблицы.Наименование);
		КонецЕсли;
	КонецЦикла;
