...
Процедура ПриИзмененииПоляНаСервере(ИмяПоля)
	...
	ИначеЕсли ИмяПоля = "Продавец" ИЛИ ИмяПоля = "ПокупательОрганизация" Тогда
		...
		ИзменитьИсточникиДляВыбораДоговора(Истина, ИмяПоля);
	ИначеЕсли ИмяПоля = "Покупатель" ИЛИ ИмяПоля = "Исполнитель" Тогда // ИЛИ ИмяПоля = "ПродавецОрганизация" 
		...
		ВыполнитьПоискСозданныхДокументов(ИмяПоля);
		ИзменитьИсточникиДляВыбораДоговора(Истина, ИмяПоля);
	ИначеЕсли ИмяПоля = "НомерДокумента" ИЛИ ИмяПоля = "ДатаДокумента" Тогда
		ВыполнитьПоискСозданныхДокументов(ИмяПоля);
	КонецЕсли;
...
Процедура ВыполнитьПоискСозданныхДокументов(ИмяПоля)
	...
	Если ИмяПоля = "НомерДокумента" ИЛИ ИмяПоля = "ДатаДокумента" Тогда
		Если НЕ СозданныеДокументы.ДокументВходящий.СозданПоРаспознанномуДокументу Тогда
			СозданныеДокументы.ДокументВходящий.Ссылка = Неопределено;
		КонецЕсли;
		Если НЕ СозданныеДокументы.ДокументИсходящий.СозданПоРаспознанномуДокументу Тогда
...
Процедура Подключаемый_ПриИзмененииКолонки(Элемент)
	...
	Если ИмяЭлемента = "ПроцентСкидки" Тогда
		ТекущиеДанные.СуммаСкидки = ТекущиеДанные.Сумма * ТекущиеДанные.ПроцентСкидки / 100;
	ИначеЕсли ИмяЭлемента = "СуммаСкидки" Тогда
		ТекущиеДанные.ПроцентСкидки =
			?(ТекущиеДанные.Сумма = 0,
				0,
				ТекущиеДанные.СуммаСкидки / ТекущиеДанные.Сумма * 100
			);
	КонецЕсли;
	...
	Для Каждого РеквизитШапки Из Объект.РеквизитыДокумента Цикл
		...
		Если НЕ РеквизитШапки.ИмяЭлемента = "Договор" Тогда
			...
			Если РеквизитШапки.ИмяЭлемента = "БанковскийСчетОрганизации" Тогда
				// FIXME: нечеткий поиск не отбирает банковские счета по владельцу, поэтому ПодходящиеЗначения
...
&НаСервере
Процедура ИзменитьИсточникиДляВыбораДоговора(ПерезаполнитьДоговор = Истина, ИмяПоля = "")
...
Процедура ОбновитьЗаголовокКнопкиСозданныйДокумент()
	...
	Элементы.КнопкаСозданныйДокументВходящий.Заголовок = Строка(СозданныеДокументы.ДокументВходящий.Ссылка);
	Элементы.КнопкаСозданныйДокументИсходящий.Заголовок = Строка(СозданныеДокументы.ДокументИсходящий.Ссылка);
	
	Элементы.ВсегоНайденные.Заголовок = СтрШаблон(НСтр("ru = 'Всего (%1)'"), КоличествоЕщеНайденные);
	
	Элементы.КнопкаСозданныйДокументВходящий.Видимость =
		ЗначениеЗаполнено(СозданныеДокументы.ДокументВходящий.Ссылка)
		И Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Входящий;
	Элементы.КнопкаСозданныйДокументИсходящий.Видимость =
		ЗначениеЗаполнено(СозданныеДокументы.ДокументИсходящий.Ссылка)
		И Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Исходящий;
	Элементы.ДекорацияНайден.Видимость = Элементы.КнопкаСозданныйДокументВходящий.Видимость ИЛИ Элементы.КнопкаСозданныйДокументИсходящий.Видимость;
	
	Элементы.ВсегоНайденные.Видимость = Элементы.ДекорацияНайден.Видимость И КоличествоЕщеНайденные > 1;
	
	СканПрикреплен = ЗначениеЗаполнено(СозданныеДокументы.ДокументВходящий.Ссылка)
		И Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Входящий
		И СозданныеДокументы.ДокументВходящий.СозданПоРаспознанномуДокументу
		ИЛИ
		ЗначениеЗаполнено(СозданныеДокументы.ДокументИсходящий.Ссылка)
		И Объект.Направление = Перечисления.НаправленияРаспознанногоДокумента.Исходящий
		И СозданныеДокументы.ДокументИсходящий.СозданПоРаспознанномуДокументу;
...
Процедура ОткрытьДокументВходящий(Команда)
	ОткрытьСозданныйДокумент(СозданныеДокументы.ДокументВходящий.Ссылка);
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьДокументИсходящий(Команда)
	ОткрытьСозданныйДокумент(СозданныеДокументы.ДокументИсходящий.Ссылка);
КонецПроцедуры
