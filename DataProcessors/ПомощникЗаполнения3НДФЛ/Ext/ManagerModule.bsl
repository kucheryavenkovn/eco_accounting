...
Функция СформироватьАвтоматическиРеглОтчет(ПараметрыФормирования) Экспорт
	...
	Если ФормаЗаполняетсяПомощником(ПараметрыФормирования.ИмяФормыОтчета) Тогда
		...
		СтруктураПараметров.Вставить("Период",                        ПараметрыФормирования.ДатаОкончания);
		СтруктураПараметров.Вставить("Организация",                   ПараметрыФормирования.Организация);
		СтруктураПараметров.Вставить("НомерКорректировки",            0);
		СтруктураПараметров.Вставить("НомерКорректировки4НДФЛ",       0);
		СтруктураПараметров.Вставить("ДоходыВычеты",                  ДоходыВычеты);
		СтруктураПараметров.Вставить("ДанныеПомощникаЗаполнения",     ДанныеПомощникаЗаполнения);
		СтруктураПараметров.Вставить("Декларация3НДФЛ",               Неопределено);
		СтруктураПараметров.Вставить("Декларация3НДФЛВыбраннаяФорма", ПараметрыФормирования.ИмяФормыОтчета);
		СтруктураПараметров.Вставить("СформироватьДекларацию4НДФЛ",   Ложь);
		СтруктураПараметров.Вставить("ПредполагаемаяСуммаДоходов",    0);
		СтруктураПараметров.Вставить("Декларация4НДФЛ",               Неопределено);
		СтруктураПараметров.Вставить("Декларация4НДФЛВыбраннаяФорма", "");
...
Функция ЗаполнитьДоходыРасходыОтПредпринимательскойДеятельности(ПараметрыЗаполнения)
	...
	РезультатВыполнения.ТорговыйСбор = УплаченныйТорговыйСбор;
...
//
// Параметры:
//   СтруктураПараметров - Структура - см. НовыйДанныеПомощникаЗаполнения()
//
// Возвращаемое значение:
...
Функция СоздатьЗаполнитьДекларацию(СтруктураПараметров)
	...
	Для Каждого СтрокаТаблицы Из ДоходыВычеты Цикл
		...
		ИначеЕсли СтрокаТаблицы.Вид = Перечисления.ВычетыФизическихЛиц.СтандартныйНаНалогоплательщика Тогда
			...
			НоваяСтрока.НалоговаяСтавка = НалоговыеСтавки.ПоУмолчанию;
			НоваяСтрока.Вид = СтрокаТаблицы.Вид;
			НоваяСтрока.СтруктураДанных = ДанныеЗаполнения_СтандартныйВычет(
				СтрокаТаблицы.СтруктураДоходовВычетов, ВыбраннаяФорма3НДФЛ);
...
Функция НовыйСтруктураТаблицыДоходыИВычеты()
	...
	РезультатВыполнения.Вставить("ДоходыПредпринимателя", Неопределено); // ТаблицаЗначений - Таблица доходов предпринимателя
	РезультатВыполнения.Вставить("АвансовыеПлатежи", 0);                 // Число - Сумма авансовых платежей
	РезультатВыполнения.Вставить("ТорговыйСбор", 0);                     // Число - Сумма торгового сбора
...
Функция ТаблицаДоходовОтПредпринимательскойДеятельности(Организация, НачалоПериода, КонецПериода) Экспорт
	...
	Для Каждого СтрокаДоходов Из ТаблицаДоходов Цикл
		...
		Если ВключатьАвансы Тогда
			...
			СтрокаДоходов.АвансыПокупателей = Обработки.ПомощникЗаполнения3НДФЛ.АвансыВключенныеВПрочиеДоходы(
				Организация,
				НачалоПериода,
				КонецПериода,
...
Функция АвансовыеПлатежиПоНДФЛЗаПериод(Организация, НачалоПериода, КонецПериода) Экспорт
	...
	Если КонецПериода > УчетНДФЛПредпринимателя.ДатаНачалаРасчетаАвансовыхПлатежейНДФЛ() Тогда
		Возврат УчетНДФЛПредпринимателя.УплаченныйНалогЗаПериод(
			Организация,
			НачалоПериода,
			НачалоКвартала(КонецПериода) - 1);
	КонецЕсли;
...
Функция СтатусУплаты(ПараметрыЗадачи)
	...
	Иначе
		...
		Начислено = УчетНДФЛПредпринимателя.НачисленныйНалогЗаПериод(
			Организация, НачалоГода(ПериодСобытия), КонецДня(ПериодСобытия));
		
		ОстатокРасчетов = УчетНДФЛПредпринимателя.ОстатокРасчетовПоНалогуЗаПредыдущиеПериоды(
			Организация, НачалоГода(ПериодСобытия));
...
Функция ДанныеСтандартныхВычетов(ПараметрыЗаполнения)
	...
	Если ЗначениеЗаполнено(ВычетНаНалогоплательщика) Тогда
		...
		ДанныеФормы.Вставить("РазмерВычета", 500);
	...
	Если ЗначениеЗаполнено(ВычетНаДетей) Тогда
		...
		СтруктураВычета.СтруктураДоходовВычетов.Вставить("ВидДохода",
			Перечисления.ИсточникиДоходовФизическихЛиц.ПредпринимательскаяДеятельность);
...
Процедура СохранитьДанныеСтандартныхВычетовВРегистр(Знач ДоходыВычеты, Организация, Период, ВыбраннаяФорма3НДФЛ)
	...
	Если ЗначениеЗаполнено(ВычетНаНалогоплательщика) Тогда
		...
		РазмерыВычетов = Отчеты.РегламентированныйОтчет3НДФЛ.РазмерыВычетов(ВыбраннаяФорма3НДФЛ);
...
Функция ПоясненияКРасчету(Показатели) Экспорт
	...
	НалогВсего                    = Формат(Показатели.НалогИсчисленныйВсего,   ФорматЧиселВПояснении);
	...
	Если НомерКвартала > 1 Тогда
		...
		ШаблонОсновногоТекста = НСтр("ru = 'Налог за %1 = Налоговая база * Ставка налога'");
		...
		ДополнительныйТекст = СтрШаблон("%1 * %2%%", Показатели.НалоговаяБаза, СтавкаНалога);
	...
	Иначе
		...
		Если ДобавитьЭтапПереплаты(ТаблицаЭтапы, Показатели.Переплата, Показатели, НомерЭтапа) Тогда
			ТекстРасчета = НСтр("ru = 'Налоговая база * Ставка налога - Переплата'");
			ДополнительныйТекст = СтрШаблон("%1 * %2%% - %3", Показатели.НалоговаяБаза, СтавкаНалога, Показатели.Переплата);
		Иначе
			ТекстРасчета = НСтр("ru = 'Налоговая база * Ставка налога'");
			ДополнительныйТекст = СтрШаблон("%1 * %2%%", Показатели.НалоговаяБаза, СтавкаНалога);
		КонецЕсли;
		
		СодержимоеСтроки.Добавить(ТекстРасчета);
...
Процедура ЗаполнитьНачальныеПоказатели(Показатели)
	...
	Показатели.ПрофессиональныйВычет = ДоходыРасходы.Расходы;
	...
	Показатели.НалогИсчисленныйВсего = Показатели.НалоговаяБаза * Показатели.СтавкаНалога / 100;
	...
	Если Показатели.КонецПериода > Показатели.ДатаНачалаПримененияРасчетаАвансов Тогда
		...
		Показатели.НалогКУплате = Макс(0, Окр(Показатели.НалогИсчисленныйВсего
			...
			УчетНДФЛПредпринимателя.РазрядностьОкругленияПоказателейПриРасчетеНалога(Показатели.Период)));
...
Функция ФорматЧиселУплатаНалога(Период)
	...
	Возврат СтрШаблон(НСтр("ru = 'ЧДЦ=%1; ЧН=0; ЧГ=3,0'"),
		УчетНДФЛПредпринимателя.РазрядностьОкругленияПоказателейПриРасчетеНалога(Период));
...
Функция НовыеПоказатели(Организация, Период)
	...
	Показатели.Вставить("СтавкаНалога",   Отчеты.РегламентированныйОтчет3НДФЛ.НалоговыеСтавки(
		Показатели.ИмяФормыОтчета).ПоУмолчанию);
	
	Показатели.Вставить("СтавкаНалогаПредставление", Формат(Показатели.СтавкаНалога, НСтр("ru = 'ЧН=0; ЧФ=Ч%; БЛ='")));
