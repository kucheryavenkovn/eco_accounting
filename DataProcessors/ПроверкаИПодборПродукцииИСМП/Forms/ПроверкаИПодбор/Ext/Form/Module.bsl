...
Процедура ЗаполнитьСтрокуДереваПоДаннымШтрихкода(Форма, НайденнаяСтрокаДерева, ДанныеШтрихкода, ТаблицаИзмененийПодобраннойПродукции)
	...
	ПроверкаИПодборПродукцииИСМПКлиентСервер.ПодобраннаяПродукцияПриУточненииСопоставления(Форма, СтрокаИзмененийДо, СтрокаИзмененийПосле, 1);
...
Процедура ЗаполнитьСтрокуДереваПоДаннымВыбора(НайденнаяСтрокаДерева, ДанныеВыбора, ЗаполнитьИзмененияПодобраннойПродукции = Ложь)
	...
	СвойстваСтроки = Новый Массив;
	СвойстваСтроки.Добавить("GTIN");
	СвойстваСтроки.Добавить("Номенклатура");
	СвойстваСтроки.Добавить("Характеристика");
	СвойстваСтроки.Добавить("Серия");
	СвойстваСтроки.Добавить("СтатусПроверки");
	
	Если ВидМаркируемойПродукции = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.МолочнаяПродукция") Тогда
		СвойстваСтроки.Добавить("ГоденДо");
		СвойстваСтроки.Добавить("Скоропортящаяся");
		СвойстваСтроки.Добавить("ИдентификаторПроисхожденияВЕТИС");
	КонецЕсли;
	
	СвойстваСтроки = СтрСоединить(СвойстваСтроки, ",");
	
	СтрокаИзмененийДо    = Новый Структура(СвойстваСтроки);
	СтрокаИзмененийПосле = Новый Структура(СвойстваСтроки);
...
Функция ОбработатьПолученныеДанныеТСДНаСервере(ДанныеДляТСД)
	...
	// Восстановление данных по штрихкодам из дерева маркированной продукции
