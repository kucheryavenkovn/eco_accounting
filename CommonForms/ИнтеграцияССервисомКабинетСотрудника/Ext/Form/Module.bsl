...
Процедура СозданиеАдминистратораСервисаЗавершение(АдресРезультата)
	...
	НаборЗаписей[0].Идентификатор = ОписаниеИнформационнойСистемы.Идентификатор;
	НаборЗаписей[0].Наименование  = ОписаниеИнформационнойСистемы.Наименование;
...
Процедура УстановитьЗаголовокДекорацияПереходДляАдминистратора()
	...
	ШаблонЗаголовка = НСтр("ru = '<body>Если вы - %1, тогда перейдите сюда: <a href=""%2"">%3</a> создайте личные кабинеты и разошлите приглашения своим сотрудникам</body>'");
	СтрокаЗаголовка = СтрШаблон(ШаблонЗаголовка, Строка(Ответственный), СсылкаДляАдминистратора,СтрокаАдресПриложения());
