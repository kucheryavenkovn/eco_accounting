...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Договор") Тогда
		АР_Договор = Параметры.Договор;
	Иначе
		АР_Договор = Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
	КонецЕсли;
	СписокКурсовВалют = ПолучитьСписокКурсовВалют(АР_Договор);
	//--ЭЛИАС 20141114
...
Процедура ПолучитьЗначенияПараметровФормы()
	...
	//++ЭЛИАС 20170302 
	Если Параметры.Свойство("ПениПоАрендеБезНДС") И Параметры.ПениПоАрендеБезНДС Тогда
		ПлательщикНДС = Ложь;	
	КонецЕсли;
	//--ЭЛИАС 20170302
...
Процедура ЗаполнитьТаблицуКурсовВалют()
	...
	Если Параметры.Свойство("Договор") И ЗначениеЗаполнено(Параметры.Договор) Тогда
		СпецКурс = АР_ПроцедурыНачисления.ПолучитьКурсВалюты(Параметры.Договор.ВалютаВзаиморасчетов, Параметры.ДатаДокумента, Параметры.Договор);
		СтрокаВалюты = ТаблицаРезультатаЗапроса.Найти(Параметры.Договор.ВалютаВзаиморасчетов, "Валюта");
		Если НЕ СтрокаВалюты = Неопределено Тогда
			СтрокаВалюты.Курс = СпецКурс.Курс;	
			СтрокаВалюты.Кратность = СпецКурс.Кратность;	
		КонецЕсли;
	КонецЕсли;
	//--ЭЛИАС 20141114
...
&НаСервере
Функция ПолучитьСписокКурсовВалют(Договор) //++ЭЛИАС 20141114  
	...
	Если ВалютаУчета <> ВалютаДляОпределенияКурса Тогда 
		...
		//++ЭЛИАС 20141114  
		СпецКурс = АР_ПроцедурыНачисления.ПолучитьКурсВалюты(ВалютаДляОпределенияКурса, ДатаДокумента, Договор);
		Если НЕ СпецКурс.Курс = КурсНаДату.Курс Тогда
			СписокКурсовВалют.Добавить(СпецКурс.Курс, Строка(СпецКурс.Курс) + " (спец. курс на " + Формат(ДатаДокумента, "ДФ = дд.ММ.гг") + ")");
		КонецЕсли;
		//--ЭЛИАС 20141114
