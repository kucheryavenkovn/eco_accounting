...
Процедура ПерейтиВСписокПатентов(Команда)
	...
	ПараметрыФормы  = Новый Структура("Отбор, Период", Новый Структура("Владелец", Организация), ПериодПереносаСроков.Начало);
...
Процедура _ОК(Команда)
	...
	Если ИмяРегистра = "НастройкиСистемыНалогообложения" Тогда
		...
		ПараметрыОповещения = Новый Структура("Организация, Период", НастройкиСистемыНалогообложения.Организация, НастройкиСистемыНалогообложения.Период);
...
Процедура УправлениеФормой(Форма, ИмяРаздела = Неопределено)
	...
	Если ИмяРаздела = "НастройкиУчетаСтраховыхВзносовИП" Тогда
...
Процедура НастроитьПодразделАнтикризисРегиональныеНПА()
	...
	Если Не ЭтоЮрЛицо И НастройкиСистемыНалогообложения.ПрименяетсяУСНПатент Тогда
		Патенты = Справочники.Патенты.ПатентыОрганизацииУплачиваемыеЗаПериод(
			Организация, 
