...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ЭЛИАС 20171129 
	Если Параметры.Свойство("АР_БыстраяОтправка") И Параметры.АР_БыстраяОтправка = Истина Тогда                                                                                                          
		НовыеПараметры = Новый Структура("АР_БыстраяОтправка, АР_ПодписиИПечати, АР_УчетнаяЗапись, АдресИсходногоВложения, АдресПолучателяСкрытойКопии, Вложения, ВыбиратьПолучателей, Кому, Тело, Тема, Текст, УдалятьФайлыПослеОтправки, УчетнаяЗапись");
		ЗаполнитьЗначенияСвойств(НовыеПараметры, Параметры);
		Если Параметры.Свойство("Получатель") Тогда
			НовыеПараметры.Вставить("Кому", Параметры.Получатель);
		КонецЕсли;
		Если Параметры.Свойство("АР_ПодписиИПечати") И Параметры.АР_ПодписиИПечати Тогда
			Для Каждого Стр Из НовыеПараметры.Вложения Цикл
				Для Каждого Вложение Из ВложенияПодписьИПечать Цикл
					Если Вложение.Представление = Стр.Представление Тогда
						Стр.АдресВоВременномХранилище = Вложение.АдресВоВременномХранилище;
						Прервать;
					КонецЕсли;
				КонецЦикла;
			КонецЦикла; 
		КонецЕсли;
		АР_ОбщиеПроцедуры.ОтправитьПочтовоеСообщение(НовыеПараметры);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	//--ЭЛИАС 20171129
		...
		Если ЗначениеЗаполнено(АдресИсходногоВложения) Тогда
			...
			Если ПараметрыФаксимиле.Свойство("Организация") Тогда
				...
				Если НастройкиПодписиИПечати.ВыводитьФаксимиле = Истина
					...
					ПодписьИПечатьПриИзмененииНаСервере();
					//++ЭЛИАС 20180907 
				ИначеЕсли Параметры.Свойство("АР_БыстраяОтправка") 
					И Параметры.АР_БыстраяОтправка = Истина 
					И ПараметрыФаксимиле.ВыводитьПодписьИПечать
					И НЕ ПараметрыФаксимиле.ПредложитьНастроить Тогда
					ПодписьИПечатьБП = Параметры.Свойство("АР_ПодписиИПечати") И Параметры.АР_ПодписиИПечати;
					ПодписьИПечатьПриИзмененииНаСервере();
					//--ЭЛИАС 20180907
				КонецЕсли;
			КонецЕсли;
