...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ЭЛИАС 20191010 
	Если Параметры.ИмяФормы = "Документ.АР_НачислениеАренднойПлаты.Форма.ФормаДокумента" Тогда
		Если Объекты[0].Значение.ВалютаДокумента = Константы.ВалютаРегламентированногоУчета.Получить() Тогда
			...
			Для Каждого Элемент Из СписокОтключаемыхКоманд Цикл
				ОтключаемаяКоманда = КомандыПечати.Найти(Элемент, "Идентификатор");
				Если НЕ ОтключаемаяКоманда = Неопределено Тогда
					ОтключаемаяКоманда.СкрытаФункциональнымиОпциями = Истина;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
