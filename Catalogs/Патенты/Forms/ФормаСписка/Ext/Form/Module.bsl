...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтборВладелец = ОбщегоНазначенияБПВызовСервера.УстановитьОтборПоОсновнойОрганизации(
		ЭтотОбъект, , "Владелец", ОсновнаяОрганизация);
...
Процедура УстановитьОтборПоПериоду(Форма, НачалоПериода, КонецПериода, Использование)
	...
	Форма.Элементы.ГруппаИнформацияПереносСроков.Видимость = Использование 
		И ЗначениеЗаполнено(НачалоПериода) 
		И НачалоПериода >= Форма.ПериодПереносаСроков.Начало 
		И НачалоПериода <= Форма.ПериодПереносаСроков.Окончание;
