...
Процедура СоздатьОснованияПраваПодписи() Экспорт
	...
	|
	|ВЫБРАТЬ
	|	НачислениеАренднойПлаты.Организация,
	|	НачислениеАренднойПлаты.Руководитель,
	|	НачислениеАренднойПлаты.УдалитьЗаРуководителяПоПриказу
	|ИЗ
	|	Документ.АР_НачислениеАренднойПлаты КАК НачислениеАренднойПлаты
	|ГДЕ
	|	НачислениеАренднойПлаты.Руководитель <> &ПустоеФизическоеЛицо
	|	И НачислениеАренднойПлаты.Организация <> &ПустаяОрганизация
	|	И НачислениеАренднойПлаты.УдалитьЗаРуководителяПоПриказу <> """"
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	|	НачислениеАренднойПлаты.Организация,
	|	НачислениеАренднойПлаты.ГлавныйБухгалтер,
	|	НачислениеАренднойПлаты.УдалитьЗаГлавногоБухгалтераПоПриказу
	|ИЗ
	|	Документ.АР_НачислениеАренднойПлаты КАК НачислениеАренднойПлаты
	|ГДЕ
	|	НачислениеАренднойПлаты.ГлавныйБухгалтер <> &ПустоеФизическоеЛицо
	|	И НачислениеАренднойПлаты.Организация <> &ПустаяОрганизация
	|	И НачислениеАренднойПлаты.УдалитьЗаГлавногоБухгалтераПоПриказу <> """"
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	|	НачислениеПени.Организация,
	|	НачислениеПени.Руководитель,
	|	НачислениеПени.УдалитьЗаРуководителяПоПриказу
	|ИЗ
	|	Документ.АР_НачислениеПени КАК НачислениеПени
	|ГДЕ
	|	НачислениеПени.Руководитель <> &ПустоеФизическоеЛицо
	|	И НачислениеПени.Организация <> &ПустаяОрганизация
	|	И НачислениеПени.УдалитьЗаРуководителяПоПриказу <> """"
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	|	НачислениеПени.Организация,
	|	НачислениеПени.ГлавныйБухгалтер,
	|	НачислениеПени.УдалитьЗаГлавногоБухгалтераПоПриказу
	|ИЗ
	|	Документ.АР_НачислениеПени КАК НачислениеПени
	|ГДЕ
	|	НачислениеПени.ГлавныйБухгалтер <> &ПустоеФизическоеЛицо
	|	И НачислениеПени.Организация <> &ПустаяОрганизация
	|	И НачислениеПени.УдалитьЗаГлавногоБухгалтераПоПриказу <> """"
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	|	СчетНаАренду.Организация,
	|	СчетНаАренду.Руководитель,
	|	СчетНаАренду.УдалитьЗаРуководителяПоПриказу
	|ИЗ
	|	Документ.АР_СчетНаАренду КАК СчетНаАренду
	|ГДЕ
	|	СчетНаАренду.Руководитель <> &ПустоеФизическоеЛицо
	|	И СчетНаАренду.Организация <> &ПустаяОрганизация
	|	И СчетНаАренду.УдалитьЗаРуководителяПоПриказу <> """"
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
	|	СчетНаАренду.Организация,
	|	СчетНаАренду.ГлавныйБухгалтер,
	|	СчетНаАренду.УдалитьЗаГлавногоБухгалтераПоПриказу
	|ИЗ
	|	Документ.АР_СчетНаАренду КАК СчетНаАренду
	|ГДЕ
	|	СчетНаАренду.ГлавныйБухгалтер <> &ПустоеФизическоеЛицо
	|	И СчетНаАренду.Организация <> &ПустаяОрганизация
	|	И СчетНаАренду.УдалитьЗаГлавногоБухгалтераПоПриказу <> """"
	//--ЭЛИАС 20151023
	|
	|ОБЪЕДИНИТЬ
	|
...
Процедура ОбновитьОснованияПраваПодписиВДокументах() Экспорт
	...
	|
	|ВЫБРАТЬ
	|	НачислениеАренднойПлаты.Ссылка,
	|	НачислениеАренднойПлаты.Организация,
	|	НачислениеАренднойПлаты.Руководитель,
	|	НачислениеАренднойПлаты.УдалитьЗаРуководителяПоПриказу,
	|	""УдалитьЗаРуководителяПоПриказу"",
