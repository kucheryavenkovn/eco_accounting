...
Процедура ПередЗаписью(Отказ)
	...
	Если ВыводитьНомерИДатуДоговораВСчетФактуре и (Не ЗначениеЗаполнено(Дата) или Не ЗначениеЗаполнено(Номер)) Тогда
		Сообщить("Не заполнены номер и дата в договоре");
		Отказ = Истина;
	КонецЕсли;	
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//++ЭЛИАС 20171208 
	Если НЕ ЭтоГруппа И (НЕ АР_АрендныйДоговор ИЛИ НЕ АР_ВестиВзаиморасчетыПоПериодам) Тогда
		МассивНепроверяемыхРеквизитов.Добавить("АР_Периодичность");
	КонецЕсли;
	//--ЭЛИАС 20171208
...
Процедура ВыполнитьПроверкуКлючевыхРеквизитов(Отказ)
	...
	Иначе
		...
		Если ОпределитьНеобходимостьПроверкиАренды(ЕстьПроведенныеДокументыПоДоговору) Тогда
			...
			Если ЕстьПроведенныеДокументыПоДоговору Тогда
				//СписокРеквизитов = Новый Структура("АР_АрендныйДоговор, АР_ВестиВзаиморасчетыПоПериодам, АР_ВестиВзаиморасчетыПоОбъектам, АР_ВестиВзаиморасчетыПоУслугам, АР_ВестиВзаиморасчетыПоСчетамПост, АР_ВестиВзаиморасчетыПоСчетамПерем", //СВД
				СписокРеквизитов = Новый Структура("АР_АрендныйДоговор, АР_ВестиВзаиморасчетыПоПериодам, АР_ВестиВзаиморасчетыПоОбъектам, АР_ВестиВзаиморасчетыПоСчетамПост, АР_ВестиВзаиморасчетыПоСчетамПерем", 
				...
				ТекстСообщения = НСтр("ru = 'Существуют платежные документы, проведенные по договору %1.
					|Реквизит %2 не может быть изменен.'");
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Наименование);
				Для Каждого КлючИЗначение Из СписокРеквизитов Цикл
					СообщитьОНекорректномРеквизите(КлючИЗначение.Ключ, КлючИЗначение.Значение, ТекстСообщения, Отказ);
...
Функция ОпределитьНеобходимостьПроверкиАренды(ЕстьПроведенныеДокументыПоДоговору)
	...
	Если АР_ВестиВзаиморасчетыПоПериодам <> Ссылка.АР_ВестиВзаиморасчетыПоПериодам
		...
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.Ссылка КАК ДокументПоДоговору
		|ПОМЕСТИТЬ ВТ_ДокументыПоДоговору
		|ИЗ
		|	Документ.ПоступлениеНаРасчетныйСчет.РасшифровкаПлатежа КАК ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа
		|ГДЕ
		|	ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.ДоговорКонтрагента = &ДоговорКонтрагента
		|	И ПоступлениеНаРасчетныйСчетРасшифровкаПлатежа.Ссылка.АР_ОплатаАренды
		...
		|ГДЕ
		|	СписаниеСРасчетногоСчетаРасшифровкаПлатежа.ДоговорКонтрагента = &ДоговорКонтрагента
		|	И СписаниеСРасчетногоСчетаРасшифровкаПлатежа.Ссылка.АР_ОплатаАренды
		...
		|ГДЕ
		|	ПриходныйКассовыйОрдерРасшифровкаПлатежа.ДоговорКонтрагента = &ДоговорКонтрагента
		|	И ПриходныйКассовыйОрдерРасшифровкаПлатежа.Ссылка.АР_ОплатаАренды
		...
		|ГДЕ
		|	РасходныйКассовыйОрдерРасшифровкаПлатежа.ДоговорКонтрагента = &ДоговорКонтрагента
		|	И РасходныйКассовыйОрдерРасшифровкаПлатежа.Ссылка.АР_ОплатаАренды
		...
		|ИЗ
		|	ВТ_ДокументыПоДоговору КАК ВТ_ДокументыПоДоговору";
