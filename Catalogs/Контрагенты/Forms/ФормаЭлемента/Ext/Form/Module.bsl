...
Процедура ЗаполнитьРеквизитыКонтрагента(РеквизитыКонтрагента, ПроверитьИНН)
	...
	Если ЭтоЮрЛицо Тогда
		...
		Если РеквизитыКонтрагента.Руководитель <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(РеквизитыКонтрагента.Руководитель.Должность) Тогда
				Объект.АР_ДолжностьРуководителя = РеквизитыКонтрагента.Руководитель.Должность;
				АР_ДолжностьРуководителяРодительныйПадеж = "";
				Объект.АР_ДолжностьРуководителяРодительныйПадеж = СклонениеПредставленийОбъектов.ПросклонятьПредставление(Объект.АР_ДолжностьРуководителя, 2, АР_ДолжностьРуководителяРодительныйПадеж);
			КонецЕсли;
			Если ЗначениеЗаполнено(РеквизитыКонтрагента.Руководитель.Фамилия) Тогда
				Объект.АР_ГенеральныйДиректор = РеквизитыКонтрагента.Руководитель.Представление;
				Объект.АР_ГенеральныйДиректорКратко = РеквизитыКонтрагента.Руководитель.Фамилия
					+ " " + Лев(РеквизитыКонтрагента.Руководитель.Имя, 1) + "."
					+ " " + Лев(РеквизитыКонтрагента.Руководитель.Отчество, 1) + ".";
				Объект.АР_ГенДиректорРодительныйПадеж = СклонениеПредставленийОбъектов.ПросклонятьПредставление(РеквизитыКонтрагента.Руководитель.Представление, 2);
				Объект.АР_ГенДиректорКраткоРодительныйПадеж = Лев(Объект.АР_ГенДиректорРодительныйПадеж, СтрНайти(Объект.АР_ГенДиректорРодительныйПадеж, " ") - 1) 
					+ " " + Лев(РеквизитыКонтрагента.Руководитель.Имя, 1) + "."
					+ " " + Лев(РеквизитыКонтрагента.Руководитель.Отчество, 1) + ".";
			КонецЕсли;
