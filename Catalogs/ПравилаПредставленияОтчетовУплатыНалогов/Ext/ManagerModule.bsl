...
Процедура ЗаполнитьПравилаОрганизации(Правила, Отбор, МенеджерВременныхТаблиц = Неопределено) Экспорт
	...
	Если Отбор.ПроверятьУсловияТолькоНаГраницахОбзора Тогда
		ПроверяемыеНаГраницах.Добавить(Перечисления.УсловияПримененияТребованийЗаконодательства.ВыплачиваетсяЗарплата);
	КонецЕсли;
		...
		Пока ВсеПравила.Следующий() Цикл
			...
			Если НеВыполненноеУсловие = Неопределено И ПроверяемыеНаГраницах.Количество() > 0 Тогда
				...
				Для Каждого ВыполненноеУсловие Из ВыполненныеУсловия Цикл
					...
					НеВыполненноеУсловиеНаКонецИнтервала = Перечисления.УсловияПримененияТребованийЗаконодательства.ПроверитьУсловия(
...
Процедура СоздатьАктуальныеПравила(МенеджерВременныхТаблиц, Отбор)
	...
	УсловияИсключения = Перечисления.УсловияПримененияТребованийЗаконодательства.УсловияИсключенияПоТипуОрганизации(Отбор.Организация);
