...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ТекущийОбъект.ПодконтрольнаяПродукцияВЕТИС = (ГосударственныйКонтроль = "ВЕТИС") ИЛИ (ГосударственныйКонтроль = "МолочнаяПродукция");
	...
	ТекущийОбъект.МолочнаяПродукция            = ГосударственныйКонтроль = "МолочнаяПродукция";
...
Процедура УстановитьУчетПодконтрольнойПродукции()
	...
	Если Объект.МолочнаяПродукция
		ИЛИ ПолучитьФункциональнуюОпцию("ВестиУчетПодконтрольныхТоваровВЕТИС")
		ИЛИ ПолучитьФункциональнуюОпцию("ВестиУчетПодконтрольныхТоваровВЕТИСБП") 
		ИЛИ ПолучитьФункциональнуюОпцию("ВестиУчетМаркируемойПродукцииИСМП") Тогда
	
		Элементы.ГосударственныйКонтроль.СписокВыбора.Добавить("МолочнаяПродукция", НСтр("ru = 'Молоко и молочная продукция'"));
	КонецЕсли; 
	...
	Если Объект.МолочнаяПродукция Тогда
		ГосударственныйКонтроль = "МолочнаяПродукция";
...
Процедура УправлениеФормой(Форма)
	...
	ШаблонПодсказки = НСтр("ru='Содержание услуги в документе будет дополнено выбранной периодичностью.
	    //++ЭЛИАС 20190916 
		|Для документов по аренде недвижимости будет указан период аренды.
		//--ЭЛИАС 20190916
		|Например, ""%1""'");
	...
	//++ЭЛИАС 20171108 
	Элементы.АР_ОплачиваетсяАвансом.Доступность = НЕ Объект.АР_ПостояннаяЧастьОплаты;
	//--ЭЛИАС 20171108
...
Процедура ЗаполнитьПодсказкуПечатиАртикула()
	...
	Если ПравоДоступа("Редактирование", Метаданные.Константы.ДополнительнаяКолонкаПечатныхФормДокументов) Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИнтерфейсТаксиПростой") Тогда
			...
			,
			,
			УправлениеПечатьюБП.ГиперссылкаФормыНастройкиПечатиАртикулов()));
		Иначе
			ЭлементыСтроки.Добавить(Новый ФорматированнаяСтрока(
...
//++ЭЛИАС 20170329 
&НаКлиенте
Процедура АР_ПостояннаяЧастьОплатыПриИзменении(Элемент)
	
	Если Объект.АР_ПостояннаяЧастьОплаты Тогда
		Объект.АР_ОплачиваетсяАвансом = Ложь;
	КонецЕсли;
	УправлениеФормой(ЭтаФорма);
	
КонецПроцедуры //--ЭЛИАС 20170329
