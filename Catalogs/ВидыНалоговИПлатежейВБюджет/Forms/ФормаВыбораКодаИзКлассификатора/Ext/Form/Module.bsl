...
Процедура НастроитьСписокВыбора()
	...
	СтарыйПериод = НачалоГода(ДатаАктуальностиКлассификатора) - 1;
...
Процедура ОпределитьНомерПриказа()
	...
	Если ГодНачалаДействияПриказа = "2020" Тогда
		...
		НадписьПриказа = НСтр("ru = 'по приказу Минфина от 6 июня 2019 г. № 85н
			|(в редакции приказа Минфина от 12.05.2020 № 86н)'");
	Иначе
		ИмяМакета      = "КлассификацияДоходовБюджетов2019";
		НадписьПриказа = НСтр("ru = 'по приказу Минфина от 8 июня 2018 г. № 132н
			|(в редакции приказа Минфина от 25.11.2019 № 197н)'");
	КонецЕсли;
