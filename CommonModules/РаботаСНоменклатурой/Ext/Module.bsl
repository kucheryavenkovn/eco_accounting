...
Процедура ОбновитьСодержаниеУслуг(ТабличнаяЧасть, Знач ТекущаяДата, Знач ПредыдущаяДата = Неопределено, ВидПериода = Неопределено, ПредыдущийВидПериода = Неопределено) Экспорт
//--ЭЛИАС 20190916
	...
	Для Каждого СтрокаТаблицы Из ТабличнаяЧасть Цикл
		...
		Если ЗначениеЗаполнено(ПредыдущаяДата) Тогда
			...
			ПредыдущееСодержаниеУслуги = РаботаСНоменклатуройКлиентСервер.СодержаниеУслуги(
				...
				//++ЭЛИАС 20190916 
				//ПредыдущаяДата);
				ПредыдущаяДата,, ПредыдущийВидПериода);
				//--ЭЛИАС 20190916
