...
Процедура ЗаполнитьЗначенияСвойствДляТаблицыЗначенийПРО(ТаблицаПриемник, ТаблицаИсточник)
	...
	Для Каждого СтрокаИсточника Из ТаблицаИсточник Цикл
		...
		//++ЭЛИАС 20191211 
		Если СтрокаИсточника.ИмяПланаОбмена = "ПоОрганизации" И СтрокаИсточника.ОбъектМетаданныхИмя = "Документ.РеализацияОтгруженныхТоваров" Тогда
			// используем данную строку как шаблон для документов без присоединенных файлов
			МассивДокументов = Новый Массив;
			МассивДокументов.Добавить("АР_ВводПараметровЭксплуатацииОбъектов");
			МассивДокументов.Добавить("АР_ВыполнениеРаботПоОбслуживанию");
			МассивДокументов.Добавить("АР_ЗакрытиеЗаявкиНаОбслуживание");
			МассивДокументов.Добавить("АР_ИзменениеСтатусовОбъектов");
			МассивДокументов.Добавить("АР_ПланированиеЗатратНаЭксплуатацию");
			МассивДокументов.Добавить("АР_ПланируемыеПоступленияДенежныхСредств");
			МассивДокументов.Добавить("АР_УстановкаЦенНаАренду");
			Для Каждого Документ Из МассивДокументов Цикл
				ДобавитьПравилоРегистрацииОбъектаПоШаблону(ТаблицаПриемник, СтрокаИсточника, Документ, "РеализацияОтгруженныхТоваров"); 
			КонецЦикла;
		ИначеЕсли СтрокаИсточника.ИмяПланаОбмена = "ПоОрганизации" И СтрокаИсточника.ОбъектМетаданныхИмя = "Документ.РеализацияТоваровУслуг" Тогда
			// используем данную строку как шаблон для документов с присоединенными файлами
			МассивДокументов = Новый Массив;
			МассивДокументов.Добавить("АР_АктПриемаВозвратаАрендуемогоПомещения");
			МассивДокументов.Добавить("АР_АктПриемаПередачиПомещенияВАренду");
			МассивДокументов.Добавить("АР_АктСнятияПоказанийСчетчика");
			МассивДокументов.Добавить("АР_ДополнительноеСоглашение");
			МассивДокументов.Добавить("АР_ЗаключениеДоговораАренды");
			МассивДокументов.Добавить("АР_ЗаявкаНаОбслуживание");
			МассивДокументов.Добавить("АР_НачислениеАренднойПлаты");
			МассивДокументов.Добавить("АР_НачислениеПени");
			МассивДокументов.Добавить("АР_РасторжениеДоговораАренды");
			МассивДокументов.Добавить("АР_СчетНаАренду");
			Для Каждого Документ Из МассивДокументов Цикл
				ДобавитьПравилоРегистрацииОбъектаПоШаблону(ТаблицаПриемник, СтрокаИсточника, Документ, "РеализацияТоваровУслуг"); 
			КонецЦикла;
		КонецЕсли;
		//--ЭЛИАС 20191211
...
//++ЭЛИАС 20191211 
Процедура ДобавитьПравилоРегистрацииОбъектаПоШаблону(ТаблицаПриемник, СтрокаШаблон, ИмяОбъекта, ИмяШаблона)
	
	НоваяСтрока = ТаблицаПриемник.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаШаблон);
	СтруктураСвойств = Новый Структура("ОбъектМетаданныхИмя, ПередОбработкой, ПослеОбработки, ПриОбработке, ПриОбработкеДополнительный");
	Для Каждого Свойство Из СтруктураСвойств Цикл
		НоваяСтрока[Свойство.Ключ] = СтрЗаменить(НоваяСтрока[Свойство.Ключ], ИмяШаблона, ИмяОбъекта);
	КонецЦикла;
	
КонецПроцедуры //--ЭЛИАС 20191211
