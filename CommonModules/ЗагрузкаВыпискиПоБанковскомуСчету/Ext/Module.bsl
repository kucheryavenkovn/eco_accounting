...
Функция ВыпискиПоБанковскимСчетам(ДанныеИзБанка, УчастникиОпераций, ИдентификаторыВсехДокументов, Протокол = Неопределено)
	...
	Для Каждого Документ Из ДанныеИзБанка.Документы Цикл
		...
		Для Каждого КлючИЗначение Из Стороны Цикл
			...
			Если ЭтоОперацияПоКорпоративнойКарте(Операция) Тогда
...
Функция ХозяйственнаяОперацияПоступлениеНаличныхВБанк(Параметры)
	...
	Если Параметры.ПроверитьПрименимостьПравила Тогда
		...
		Если ЭтоОбязательстваПоПрочимОперациям Тогда
			ВидОперации =
...
Функция СтавкаНДС(ЗначениеСтавкиНДС)
	...
	Если Метаданные.Перечисления.СтавкиНДС.ЗначенияПеречисления.Найти(ТекстСтавки) <> Неопределено Тогда
...
Функция ЗаписатьДокумент(Документ, Проводить)
	...
	Попытка
		Если МетаданныеДокумента.Имя = "ПоступлениеНаРасчетныйСчет"
			ИЛИ МетаданныеДокумента.Имя = "ПриходныйКассовыйОрдер"
			ИЛИ МетаданныеДокумента.Имя = "СписаниеСРасчетногоСчета"
			ИЛИ МетаданныеДокумента.Имя = "РасходныйКассовыйОрдер" Тогда
			
			Если ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("ДоговорКонтрагента", МетаданныеДокумента, "РасшифровкаПлатежа") 
				И ОбщегоНазначенияБП.ЕстьРеквизитДокумента("АР_ОплатаАренды", МетаданныеДокумента) Тогда
				
				ЭтоАрендныйДоговор = Ложь;
				Для Каждого Стр Из Документ.РасшифровкаПлатежа Цикл
					ЭтоАрендныйДоговор = ЭтоАрендныйДоговор ИЛИ Стр.ДоговорКонтрагента.АР_АрендныйДоговор;
				КонецЦикла;
				
				Если НЕ Документ.АР_ОплатаАренды = ЭтоАрендныйДоговор Тогда
					Документ.АР_ОплатаАренды = ЭтоАрендныйДоговор;
				КонецЕсли;
