...
Функция СтраховыеВзносыСДоходовКУплате(Организация, Период, СтруктураДоходов, СообщатьОбОшибках = Истина) Экспорт
	...
	Если Период >= ДатаНачалаУчетаРасходовПриРасчетеСтраховыхВзносовИП()
		...
		Если Период >= УчетНДФЛПредпринимателя.ДатаНачалаРасчетаАвансовыхПлатежейНДФЛ() Тогда
			ФиксированныеСтраховыеВзносы = РассчитатьФиксированныеСтраховыеВзносы(Период, Организация);
			ФиксированнаяЧастьВзносов = ФиксированныеСтраховыеВзносы.ПФР + ФиксированныеСтраховыеВзносы.ФФОМС;
		Иначе
...
Функция ДанныеДляРасчетаСтраховыхВзносовСДоходов(Период, Организация = Неопределено) Экспорт
	...
	ПредельнаяСуммаВзноса = 8 * РазмерыСтраховыхВзносов(Период, Организация).ПФР;
...
Функция РассчитатьСтраховыеВзносыСДоходов(Организация, Период)
	...
	Если Период >= УчетСтраховыхВзносовИП.ДатаНачалаУчетаРасходовПриРасчетеСтраховыхВзносовИП()
		...
		СтруктураДоходов.СуммаДоходаИП = Макс(СтруктураДоходов.СуммаДоходаИП - СтраховыеВзносыУплаченныеЗаПериод.Итог("Сумма"), 0);
	КонецЕсли;
