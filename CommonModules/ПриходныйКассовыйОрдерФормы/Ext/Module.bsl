...
Процедура РассчитатьКурсИСуммуВзаиморасчетов(Форма, СтрокаПлатежа) Экспорт //++ЭЛИАС 20150924  
	...
	СтруктураКурсаВзаиморасчетов = АР_ПроцедурыНачисления.ПолучитьКурсВалюты(
		СтрокаПлатежа.ДоговорКонтрагентаВалютаВзаиморасчетов, Объект.Дата, СтрокаПлатежа.ДоговорКонтрагента);
	//--ЭЛИАС 20141118
...
Функция НовыеПараметрыПриИзмененииВалютыДокумента(ПараметрыОбъекта) 
	...
	Если ПараметрыОбъекта.Свойство("ДоговорКонтрагента") Тогда
		СтруктураКурса = АР_ПроцедурыНачисления.ПолучитьКурсВалюты(ПараметрыОбъекта.ВалютаДокумента, ПараметрыОбъекта.Дата, ПараметрыОбъекта.ДоговорКонтрагента);
	КонецЕсли;
	//--ЭЛИАС 20141118
