...
Функция ДобавитьОснованиеВСчетФактуруПолученный(Основание, НомерСчетаФактурыПолученного, ДатаСчетаФактурыПолученного,
	ОбновлятьСтатусСчетаФактурыПоДокументу = Истина, ОригиналСчетаФактуры = Неопределено) Экспорт
...
Функция ПараметрыСчетаФактуры(СчетФактура) Экспорт
	...
	Если ЭтоПолученныйСФ Тогда
		Если ТипыОснований.Получить(Тип("ДокументСсылка.АвансовыйОтчет")) <> Неопределено
			И СчетФактура.БланкСтрогойОтчетности = Истина Тогда
...
Функция ТекстЗапросаНаОснованииМетаданных(ДокументыОснования, ЭтоПолученныйСФ, ПараметрыЗапроса, ТипДокументаОснования)
	...
	ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		//++ЭЛИАС 20150415  
	ИначеЕсли ТипДокументаОснования = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты")
		ИЛИ ТипДокументаОснования = Тип("ДокументСсылка.АР_НачислениеПени") Тогда
		
		ТекстЗапросаКомиссияПоСчету = 
		" СУММА(ВЫБОР КОГДА НЕ Таблица.Контрагент = Значение(Справочник.Контрагенты.ПустаяСсылка)
		|						И НЕ Таблица.ДоговорКонтрагента = Значение(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
		|					ТОГДА Таблица.Сумма + ВЫБОР
		|						КОГДА Таблица.Ссылка.СуммаВключаетНДС
		|					ТОГДА 0
		|					ИНАЧЕ Таблица.СуммаНДС КОНЕЦ
...
Функция ТекстЗапросаКорректировочныеСправки()
	...
	|ИЗ
	|	ВозвратыПоОтчетуКомиссионера КАК ВозвратыПоОтчетуКомиссионера
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
