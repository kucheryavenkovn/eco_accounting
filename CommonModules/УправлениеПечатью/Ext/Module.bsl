...
Функция СформироватьПечатныеФормы(Знач ИмяМенеджераПечати, Знач ИменаМакетов, Знач МассивОбъектов, Знач ПараметрыПечати, 
	...
	//++ЭЛИАС 20191009 
	ПечатьСводныхФормВКомплекте = Ложь;
	//--ЭЛИАС 20191009
	...
	Для Каждого ИмяМакета Из ИменаМакетов Цикл
		...
		//++ЭЛИАС 20191009 
		ЭтоСводныйМакет = Ложь;
		Если Идентификатор = "СчетФактураСводныйКомплект" Тогда
			Идентификатор = "СчетФактураКомплект";
			Если ТипЗнч(ПараметрыПечати) = Тип("Структура") Тогда
				ПараметрыПечати.Вставить("СчетФактураСводный");
				ЭтоСводныйМакет = Истина;
				ПечатьСводныхФормВКомплекте = Истина;
			КонецЕсли;
		ИначеЕсли Идентификатор = "УниверсальныйПередаточныйДокументСводныйКомплект" Тогда
			Идентификатор = "УниверсальныйПередаточныйДокументКомплект";
			Если ТипЗнч(ПараметрыПечати) = Тип("Структура") Тогда
				ПараметрыПечати.Вставить("СчетФактураСводный");
				ЭтоСводныйМакет = Истина;
				ПечатьСводныхФормВКомплекте = Истина;
			КонецЕсли;
		ИначеЕсли Идентификатор = "СчетЗаказСводныйКомплект" Тогда
			Идентификатор = "СчетЗаказКомплект";
			Если ТипЗнч(ПараметрыПечати) = Тип("Структура") Тогда
				ПараметрыПечати.Вставить("Сводный", Истина);
				ЭтоСводныйМакет = Истина;
				ПечатьСводныхФормВКомплекте = Истина;
			КонецЕсли;
		ИначеЕсли Идентификатор = "СчетФактураСводный" 
			ИЛИ	Идентификатор = "УниверсальныйПередаточныйДокументСводный" Тогда
			ЭтоСводныйМакет = Истина;
		КонецЕсли;
		Если ПечатьСводныхФормВКомплекте 
			И ТипЗнч(ПараметрыПечати) = Тип("Структура") 
			И ПараметрыПечати.Свойство("СчетФактураСводный")
			И НЕ ЭтоСводныйМакет Тогда
			ПараметрыПечати.Удалить("СчетФактураСводный");
		ИначеЕсли ПечатьСводныхФормВКомплекте 
			И ТипЗнч(ПараметрыПечати) = Тип("Структура") 
			И ПараметрыПечати.Свойство("Сводный")
			И НЕ ЭтоСводныйМакет Тогда
			ПараметрыПечати.Удалить("Сводный");
		КонецЕсли;
		//--ЭЛИАС 20191009
...
Функция СформироватьПечатныеФормыДляБыстройПечати(ИмяМенеджераПечати, ИменаМакетов, МассивОбъектов, ПараметрыПечати) Экспорт
	...
	Для Каждого ПечатнаяФорма Из ПечатныеФормы.КоллекцияПечатныхФорм Цикл
		Если (ТипЗнч(ПечатнаяФорма.ТабличныйДокумент) = Тип("ТабличныйДокумент")) И (ПечатнаяФорма.ТабличныйДокумент.ВысотаТаблицы <> 0) Тогда
			//++ЭЛИАС 20181003 
			Если НЕ ПараметрыПечати = Неопределено И ПараметрыПечати.Свойство("СчетФактураСводный") Тогда
				Приписка = " (сводный по услугам)";
				Если СтрНайти(ПечатнаяФорма.СинонимМакета, Приписка) = 0 Тогда
					ПечатнаяФорма.СинонимМакета = ПечатнаяФорма.СинонимМакета + Приписка;
				КонецЕсли;
			КонецЕсли;
			Если НЕ ПараметрыПечати = Неопределено И ПараметрыПечати.Свойство("Сводный") Тогда
				Приписка = ?(ПараметрыПечати.Сводный = Истина, " (сводный по услугам)", " (по объектам)");
				Если СтрНайти(ПечатнаяФорма.СинонимМакета, Приписка) = 0 Тогда
					ПечатнаяФорма.СинонимМакета = ПечатнаяФорма.СинонимМакета + Приписка;
				КонецЕсли;
			КонецЕсли;
