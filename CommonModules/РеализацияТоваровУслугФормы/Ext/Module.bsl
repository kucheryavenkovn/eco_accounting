...
Процедура УстановитьПараметрыВыбора(Форма) Экспорт
	...
	////++ЭЛИАС 20140818  
	//НовыйПараметр = Новый ПараметрВыбора("Отбор.АР_АрендныйДоговор", Ложь);
	//НовыйМассивПараметров.Добавить(НовыйПараметр);
	////--ЭЛИАС 20140818
...
Процедура УстановитьЗаголовокФормы(Форма) Экспорт
	...
	ТекстЗаголовка = НСтр("ru = 'Реализация: '") + НРег(Строка(Объект.ВидОперации));
...
Процедура ПроверитьУстановитьДоговорКонтрагента(Форма, ПричиныИзмененияСчетовУчета) Экспорт
	...
	//++ЭЛИАС 20140828  
	Если ЗначениеЗаполнено(Объект.ДоговорКонтрагента) И Объект.ДоговорКонтрагента.АР_АрендныйДоговор Тогда
...
Процедура ОбработатьИзменениеДеятельностьНаПатенте(Форма) Экспорт
	...
	Если Объект.ДеятельностьНаПатенте Тогда
		...
		//++ЭЛИАС 20170919 
		Если Форма.ИмяФормы = "Документ.АР_НачислениеАренднойПлаты.Форма.ФормаДокумента"
			Или Форма.ИмяФормы = "Документ.АР_НачислениеПени.Форма.ФормаДокумента" Тогда
			Возврат
		КонецЕсли;
		//--ЭЛИАС 20170919
