...
Функция ПодготовитьТаблицуСтатусовОплатыСчетов(ТаблицаОплатыСчетов, ТаблицаРеквизитов) Экспорт
	...
	|	ТаблицаДокументов.СчетНаОплату,
	|	ТаблицаДокументов.Сумма КАК СуммаОплаты,
	|	СчетаНаОплату.Ссылка.Дата КАК Дата,
	|	СчетаНаОплату.СуммаДокумента КАК СуммаСчета,
	|	ЕСТЬNULL(СтатусыДокументов.Статус, &СтатусОплатыПоУмолчанию) КАК Статус,
	|	ЕСТЬNULL(СтатусыДокументов.ДополнительныйСтатус, &ДополнительныйСтатусПоУмолчанию) КАК ДополнительныйСтатус
	|ИЗ
	|	ВТ_ТаблицаДокументов КАК ТаблицаДокументов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.АР_СчетНаАренду КАК СчетаНаОплату
	|		ПО (СчетаНаОплату.Организация = ТаблицаДокументов.Организация)
	|			И (СчетаНаОплату.Ссылка = ТаблицаДокументов.СчетНаОплату)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыДокументов КАК СтатусыДокументов
	|		ПО (СтатусыДокументов.Организация = ТаблицаДокументов.Организация)
...
Функция ПодготовитьТаблицуСтатусовОтгрузкиПоСчетам(ТаблицаСчетовНаОплату, ТаблицаТоварыУслуги, ТаблицаВозвратнаяТара, ТаблицаРеквизитов) Экспорт
	...
	//++ЭЛИАС 20200424 
	НачисленияАрендыПоСчетам = Документы.АР_НачислениеАренднойПлаты.РеализацииПоСчетам(МассивСчетовНаОплату);
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(НачисленияАрендыПоСчетам, РеализацииПоСчетам);
	НачисленияПениПоСчетам = Документы.АР_НачислениеПени.РеализацииПоСчетам(МассивСчетовНаОплату);
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(НачисленияПениПоСчетам, РеализацииПоСчетам);
	//--ЭЛИАС 20200424
	...
	Для каждого СчетНаОплатуПокупателю Из МассивСчетовНаОплату Цикл
		...
		//++ЭЛИАС 20200425 
		Если ТипЗнч(СчетНаОплатуПокупателю) = Тип("ДокументСсылка.АР_СчетНаАренду")
			И НЕ СчетНаОплатуПокупателю.ВидОперации = Перечисления.АР_ВидыОперацийСчетНаАренду.ОплатаПени Тогда // оплата аренды или депозита
			ТоварыУслугиКРеализации = Документы.АР_НачислениеАренднойПлаты.ТоварыУслугиКРеализацииПоСчету(
				СчетНаОплатуПокупателю, РеализацииПоСчету);
		ИначеЕсли ТипЗнч(СчетНаОплатуПокупателю) = Тип("ДокументСсылка.АР_СчетНаАренду") Тогда // оплата пени
			ТоварыУслугиКРеализации = Документы.АР_НачислениеПени.ТоварыУслугиКРеализацииПоСчету(
				СчетНаОплатуПокупателю, РеализацииПоСчету);
		КонецЕсли;
		//--ЭЛИАС 20200425
...
Процедура ВходящийДокументПередЗаписьюНаСервере(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	Если Не Форма.ТребуетсяСчетФактура
		Или Объект.ЭтоУниверсальныйДокумент Тогда
