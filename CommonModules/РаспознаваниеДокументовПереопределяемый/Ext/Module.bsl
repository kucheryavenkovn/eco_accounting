...
Процедура ПриЗаполненииРаспознанногоДокумента(ДокументОбъект, ДокументСсылка) Экспорт
	...
	Для Каждого РеквизитШапки Из ДокументОбъект.РеквизитыДокумента Цикл
		...
		ИначеЕсли РеквизитШапки.ИмяРеквизита = "Грузополучатель" Тогда
			...
			Если НайденнаяСтрока <> Неопределено И РеквизитШапки.Значение = НайденнаяСтрока.Значение Тогда
				РеквизитШапки.Значение = Справочники.Контрагенты.ПустаяСсылка();
			КонецЕсли;
		ИначеЕсли РеквизитШапки.ИмяРеквизита = "Склад" Тогда
			РеквизитШапки.Значение = ПолучитьСкладПоУмолчанию();
		...
		Для Каждого СтрокаТаблицы Из ТаблицаДокумента Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.СтавкаНДС) Тогда
				...
				Если ЗначениеЗаполнено(СтрокаТаблицы.СтавкаНДС) Тогда
					СтрокаТаблицы.СуммаНДС = РаспознаваниеДокументовСлужебный.РассчитатьСуммуНДС(СтрокаТаблицы.Сумма, ПроверяемыеРеквизиты.СуммаВключаетНДС, СтавкаНДСЧислом);
	...
	ИначеЕсли ДокументОбъект.ТипДокумента = Перечисления.ТипыДокументовРаспознаваниеДокументов.УПД
		...
		Пока НеобходимоПроверять И Счетчик < 5 Цикл
			// FIXME: проверить и избавиться от счетчика
			Счетчик = Счетчик + 1;
...
Процедура ПриЗаполненииБанковскогоСчета(Счет, ДокументОбъект, ПараметрыСчета) Экспорт
	...
	Если ЗначениеЗаполнено(Счет) Тогда
		// FIXME: проверим соответствие счета и владельца, т.к. нечеткий поиск этого пока не делает
...
Процедура ПриСозданииСчетФактуры(ПараметрыСоздания, СчетФактураСсылка = Неопределено) Экспорт
	...
	Иначе
		ДанныеСчетаФактуры = УчетНДСВызовСервера.СоздатьСчетФактуруВыданныйНаОсновании(Основание);
...
Процедура ПриЗаполненииПараметровСозданияНовогоЭлемента(СвязанныеКолонки, ИмяЭлемента, ТипДокумента, Направление) Экспорт
	...
	ИначеЕсли ИмяЭлемента = "Продавец" Тогда
		...
		КонецЕсли;
		СвязанныеКолонки.Добавить(Новый Структура("Реквизит, Синоним, ИскатьПодходящие, ПустаяСсылка, Обязательный", "ЮридическоеФизическоеЛицо", "Юридическое / физическое лицо", Ложь, ПредопределенноеЗначение("Перечисление.ЮридическоеФизическоеЛицо.ПустаяСсылка"), Истина));
...
Процедура ПриИзмененииКолонкиНаСервере(Объект, СтрокаТаблицы, ТекИмя , НомерСтрокиТаблицы, ВыбранноеЗначение) Экспорт
	
	Если ТекИмя = "Номенклатура" Тогда
		Если ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
			СтрокаТаблицы.Услуга = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВыбранноеЗначение, "Услуга");
		КонецЕсли;
	Иначе
	
		СуммаВключаетНДС = РаспознаваниеДокументовСлужебныйКлиентСервер.ЗначениеРеквизитаДокумента(Объект, "СуммаВключаетНДС", Ложь);
		
		Если ТекИмя = "СуммаНДС" Тогда
			РаспознаваниеДокументовСлужебный.ПриИзмененииСуммаНДС(СтрокаТаблицы, СуммаВключаетНДС);
		ИначеЕсли ТекИмя = "Количество" ИЛИ ТекИмя = "Цена" ИЛИ ТекИмя = "Сумма" Тогда
			...
			РаспознаваниеДокументовСлужебный.ПриИзмененииСумма(СтрокаТаблицы, СуммаВключаетНДС);
		ИначеЕсли ТекИмя = "СтавкаНДС" Тогда
...
Функция ПолучитьСкладПоУмолчанию(ИмяПользователя = Неопределено) Экспорт
	
	СкладПоУмолчанию = Справочники.Склады.ПолучитьСкладПоУмолчанию(ИмяПользователя);
