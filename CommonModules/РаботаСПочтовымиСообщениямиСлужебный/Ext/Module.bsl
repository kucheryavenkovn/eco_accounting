...
Функция ПодготовитьПисьмо(УчетнаяЗапись, ПараметрыПисьма) Экспорт
	...
	//++ЭЛИАС 20190117 
	Если ПараметрыПисьма.Свойство("АР_БыстраяОтправка") И ПараметрыПисьма.Свойство("Текст") И ЗначениеЗаполнено(ПараметрыПисьма.Текст) Тогда
		Тело = Новый ФорматированныйДокумент();
		Тело.УстановитьHTML(ПараметрыПисьма.Текст, Новый Структура); // второй параметр означает, что нет вложенных изображений
