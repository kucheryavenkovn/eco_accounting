...
Функция РабочихДнейЧасовВПериоде(Календарь, НачалоПериода, ОкончаниеПериода) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ РасписанияРаботы.ДатаГрафика) КАК Дней,
		|	СУММА(РАЗНОСТЬДАТ(ЕСТЬNULL(РасписанияРаботы.ВремяНачала, ДАТАВРЕМЯ(1, 1, 1, 10, 0, 0)), ЕСТЬNULL(РасписанияРаботы.ВремяОкончания, ДАТАВРЕМЯ(1, 1, 1, 18, 0, 0)), МИНУТА)) / 60 КАК Часов
		|ИЗ
