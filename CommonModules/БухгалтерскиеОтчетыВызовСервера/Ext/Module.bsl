...
Функция ПолучитьПараметрыИсполненияОтчета(ПараметрыОтчета, МенеджерОтчета)
	...
	ДопустимыеПараметры = Новый Массив;
	ДопустимыеПараметры.Добавить("ПриВыводеЗаголовка");
	ДопустимыеПараметры.Добавить("ПередКомпоновкойМакета");
	ДопустимыеПараметры.Добавить("ДанныеРасшифровки");
	ДопустимыеПараметры.Добавить("ПослеКомпоновкиМакета");
	ДопустимыеПараметры.Добавить("ВнешниеНаборыДанных");
	ДопустимыеПараметры.Добавить("ПриВыводеПодвала");
	ДопустимыеПараметры.Добавить("ПередВыводомЭлементаРезультата");
	ДопустимыеПараметры.Добавить("ПослеВыводаРезультата");
	ДопустимыеПараметры.Добавить("ПривилегированныйРежим");

	Для каждого Параметр Из ДопустимыеПараметры Цикл
		
		Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
				ПараметрыИсполненияОтчета, "Использовать" + Параметр, Неопределено) = Неопределено Тогда
			ПараметрыИсполненияОтчета.Вставить("Использовать" + Параметр, Ложь);
		КонецЕсли;
