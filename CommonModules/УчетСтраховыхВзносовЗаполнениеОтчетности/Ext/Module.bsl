...
Процедура ЗначенияПоказателей_4ФСС_2017Кв3(ПараметрыОтчета, Контейнер) Экспорт
	...
	|	НАЧАЛОПЕРИОДА(Расчеты.Период, МЕСЯЦ) КАК Период,
	|	Расчеты.ПлатежноеПоручениеНомер,
	|	Расчеты.ПлатежноеПоручениеДата
...
Процедура РасчетПоказателей_4ФСС_2019(НачалоПериода, ОкончаниеПериода, Организация, ОписаниеТаблиц, ПодробныеДанные = Ложь)
	...
	Если ОписаниеТаблиц.Свойство("Доходы2017") Или ОписаниеТаблиц.Свойство("СКЕ") Тогда
		...
		// Получаем временные таблицы, используя общие алгоритмы.
		Запрос.МенеджерВременныхТаблиц = УчетСтраховыхВзносов.СформироватьВТРасширенныеСведенияОДоходахИВзносах(ДобавитьМесяц(НачалоПериода, -12), ОкончаниеПериода, Организация);
