...
Функция ПечатьСчетовФактур(МассивОбъектов, ОбъектыПечати, ТекстЗапросаПоСчетамФактурам,
	...
	Пока ВыборкаСФ.Следующий() Цикл
		...
		Пока ВыборкаПоОснованиям.Следующий() Цикл
			...
			ПараметрыОснования = УчетНДС.ПодготовитьДанныеДляПечатиСчетовФактур(ВыборкаПоОснованиям.ДокументОснование, ДанныеСчетаФактуры, ПараметрыПечати); //++ЭЛИАС 20131105  
	...
	Пока ВыборкаСФ.Следующий() Цикл
		...
		Пока ВыборкаПоОснованиям.Следующий() Цикл
			...
			ПараметрыОснования = УчетНДС.ПодготовитьДанныеДляПечатиКорректировочныхСчетовФактур(
				ВыборкаПоОснованиям.ДокументОснование, ВыборкаСФ.СчетФактура, ВыборкаСФ.ВидСчетаФактуры, Истина, ПараметрыПечати); //++ЭЛИАС 20120706
...
Функция ПолучитьТаблицуДокументаНДС(ДокументСсылка, Ошибка = Ложь, ДляКнигиПродаж = Ложь) Экспорт
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
		...
		//++ЭЛИАС 20130531  
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты") Тогда
		Возврат ПолучитьТаблицуНачислениеАренднойПлаты(ДокументСсылка, Ошибка);
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.АР_НачислениеПени") Тогда
		Возврат ПолучитьТаблицуНачислениеПени(ДокументСсылка, Ошибка);
		//--ЭЛИАС 20130531	
...
//++ЭЛИАС 20130531  
Функция ПолучитьТаблицуНачислениеАренднойПлаты(ДокументСсылка, Ошибка)
	...
	|	ВЫБОР
	|		КОГДА АР_НачислениеАренднойПлатыСостав.Ссылка.СуммаВключаетНДС
	|			ТОГДА АР_НачислениеАренднойПлатыСостав.Сумма - АР_НачислениеАренднойПлатыСостав.СуммаНДС
	|		ИНАЧЕ АР_НачислениеАренднойПлатыСостав.Сумма
	|	КОНЕЦ КАК СуммаБезНДС,
	|	АР_НачислениеАренднойПлатыСостав.СтавкаНДС,
	|	АР_НачислениеАренднойПлатыСостав.СуммаНДС КАК НДС,
	|	NULL КАК СчетУчетаБУ,
	|	АР_НачислениеАренднойПлатыСостав.СчетДоходов КАК СчетДоходовБУ,
	|	АР_НачислениеАренднойПлатыСостав.Субконто КАК СубконтоБУ,
...
КонецФункции //--ЭЛИАС 20130531

//++ЭЛИАС 20130531  
Функция ПолучитьТаблицуНачислениеПени(ДокументСсылка, Ошибка)

	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Ссылка", ДокументСсылка);
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	АР_НачислениеПениСостав.Номенклатура,
	|	1 КАК Количество,
	|	АР_НачислениеПениСостав.Сумма КАК Цена,
	|	АР_НачислениеПениСостав.Сумма,
	|	ВЫБОР
	|		КОГДА АР_НачислениеПениСостав.Ссылка.СуммаВключаетНДС
	|			ТОГДА АР_НачислениеПениСостав.Сумма - АР_НачислениеПениСостав.СуммаНДС
	|		ИНАЧЕ АР_НачислениеПениСостав.Сумма
	|	КОНЕЦ КАК СуммаБезНДС,
	|	АР_НачислениеПениСостав.СтавкаНДС,
	|	АР_НачислениеПениСостав.СуммаНДС КАК НДС,
	|	NULL КАК СчетУчетаБУ,
	|	АР_НачислениеПениСостав.СчетДоходов КАК СчетДоходовБУ,
	|	АР_НачислениеПениСостав.Субконто КАК СубконтоБУ,
	|	АР_НачислениеПениСостав.СчетУчетаНДСПоРеализации,
	|	АР_НачислениеПениСостав.СчетРасходов,
	|	NULL КАК НомерГТД,
	|	NULL КАК СтранаПроисхождения,
	|	АР_НачислениеПениСостав.НомерСтроки КАК НомерСтроки,
	|	ЛОЖЬ КАК ЭтоКомиссия
	|ИЗ
	|	Документ.АР_НачислениеПени.Состав КАК АР_НачислениеПениСостав
	|ГДЕ
	|	АР_НачислениеПениСостав.Ссылка = &Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки
	|";
