...
Процедура Заполнить(ДокументОбъект, ДанныеЗаполнения = Неопределено, ЗаполнятьСчетаУчета = Ложь) Экспорт
	...
	//++ЭЛИАС 20131231  
	ДокументАренды = МетаданныеДокумента.Имя = "АР_ЗаключениеДоговораАренды"
		ИЛИ МетаданныеДокумента.Имя = "АР_ДополнительноеСоглашение"
		ИЛИ МетаданныеДокумента.Имя = "АР_РасторжениеДоговораАренды"
		ИЛИ МетаданныеДокумента.Имя = "АР_НачислениеАренднойПлаты"
		ИЛИ МетаданныеДокумента.Имя = "АР_НачислениеПени"
		ИЛИ МетаданныеДокумента.Имя = "АР_СчетНаАренду";
	Если МетаданныеДокумента.Имя = "АР_НачислениеАренднойПлаты"
		ИЛИ МетаданныеДокумента.Имя = "АР_НачислениеПени" Тогда
		ЭтоАренда = Истина;
	Иначе	
		ЭтоАренда = Ложь;
	КонецЕсли;
	Если МетаданныеДокумента.Имя = "АР_СчетНаАренду" Тогда
		ЭтоСчетНаАренду = Истина;
	Иначе	
		ЭтоСчетНаАренду = Ложь;
	КонецЕсли;
	//--ЭЛИАС 20131231
	...
	//++ЭЛИАС 20131231  
	Если ЭтоАренда Или ЭтоСчетНаАренду Тогда
		ЕстьРеквизитСчетОрганизации = Ложь;	
	КонецЕсли;
	//--ЭЛИАС 20131231
	...
	Если ОбщегоНазначенияБП.ЕстьНезаполненныйРеквизитДокумента("СтруктурнаяЕдиница", ДокументОбъект, МетаданныеДокумента)
		...
		//++ЭЛИАС 20131231  
	ИначеЕсли ЭтоСчетНаАренду И ОбщегоНазначенияБП.ЕстьНезаполненныйРеквизитДокумента("СчетОрганизации", ДокументОбъект, МетаданныеДокумента)	
		И ЕстьРеквизитОрганизация Тогда
	...
	Если ОбщегоНазначенияБП.ЕстьНезаполненныйРеквизитДокумента("СчетОрганизации", ДокументОбъект, МетаданныеДокумента) И НЕ ЭтоАренда И НЕ ЭтоСчетНаАренду Тогда  
	//--ЭЛИАС 20180201
...
Процедура ЗаполнитьПоОснованию(ДокументОбъект, ДокументОснование, КопироватьПодразделение = Истина) Экспорт
	...
	//++ЭЛИАС 20160302
	Если МетаданныеДокументаОснования.Имя = "АР_НачислениеАренднойПлаты"
		ИЛИ МетаданныеДокументаОснования.Имя = "АР_НачислениеПени" Тогда
		НаОснованииАренды = Истина;
	Иначе	
		НаОснованииАренды = Ложь;
	КонецЕсли;
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("АР_ОплатаАренды", МетаданныеДокумента) Тогда
	   ...
	   Если НЕ ОбщегоНазначения.ЕстьРеквизитОбъекта("Организация", МетаданныеДокумента)
	      ИЛИ ОбщегоНазначенияБПВызовСервераПовтИсп.ГоловнаяОрганизация(ДокументОбъект.Организация) =
			...
			Если ОбщегоНазначения.ЕстьРеквизитОбъекта("КурсВзаиморасчетов", МетаданныеДокумента) Тогда
				...
				СтруктураКурсаВзаиморасчетов = АР_ПроцедурыНачисления.ПолучитьКурсВалюты(РеквизитыДоговора.ВалютаВзаиморасчетов, ДокументОбъект.Дата, ДокументОбъект.ДоговорКонтрагента);
				//--ЭЛИАС 20160920
	...
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("СчетОрганизации", МетаданныеДокумента) Тогда
		...
		ИначеЕсли ОбщегоНазначения.ЕстьРеквизитОбъекта("СчетОрганизации", МетаданныеДокументаОснования) И НЕ (НаОснованииАренды И ЭтоПлатежныйДокумент) Тогда
		//--ЭЛИАС 20160302	
	...
	//++ЭЛИАС 20120707
	// НачалоПериода.
	Если ОбщегоНазначенияБП.ЕстьРеквизитДокумента("НачалоПериода", МетаданныеДокумента)
	   И ОбщегоНазначенияБП.ЕстьРеквизитДокумента("НачалоПериода", МетаданныеДокументаОснования) Тогда
		ДокументОбъект.НачалоПериода = ДокументОснование.НачалоПериода;
	КонецЕсли;
	
	// ОкончаниеПериода.
	Если ОбщегоНазначенияБП.ЕстьРеквизитДокумента("ОкончаниеПериода", МетаданныеДокумента)
	   И ОбщегоНазначенияБП.ЕстьРеквизитДокумента("ОкончаниеПериода", МетаданныеДокументаОснования) Тогда
		ДокументОбъект.ОкончаниеПериода = ДокументОснование.ОкончаниеПериода;
	КонецЕсли;
	//--ЭЛИАС 20120707
