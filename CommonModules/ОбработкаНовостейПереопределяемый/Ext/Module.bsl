			...
			ИначеЕсли ИспользуетсяСервисЭлектронныхТрудовыхКнижек Тогда
				Если Не ПустаяСтрока(ЭлементР.ТекстовоеСодержимое) Тогда
					...
					Если НайденоНачалоБлока <> 0 Тогда
						...
						//ТекстХТМЛ = СтрЗаменить(ТекстХТМЛ, ">" + ОткрывающийТег, " class=""FO СкрытаяНовость"">" + ОткрывающийТег);
...
Процедура ДополнительноОбработатьНовостьПослеПолученияПередЗаписью(ОбъектНовости, НовостьХДТО) Экспорт
	...
	Если СервисЭлектронныхТрудовыхКнижек.ИспользуетсяСервисЭлектронныхТрудовыхКнижек() Тогда
		Если ОбъектНовости.ЛентаНовостей.Код = "Version_Changes" Тогда
			Если СтрНайти(ОбъектНовости.ТекстНовости, "<!--СервисЭлектронныхТрудовыхКнижек-->") = 0 Тогда
				ОбъектНовости.ПометкаУдаления = Истина;
			КонецЕсли;
		Иначе
			Если СтрНайти(ОбъектНовости.ТекстНовости, "<!--НЕ_СервисЭлектронныхТрудовыхКнижек-->") <> 0 Тогда
				ОбъектНовости.ПометкаУдаления = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
