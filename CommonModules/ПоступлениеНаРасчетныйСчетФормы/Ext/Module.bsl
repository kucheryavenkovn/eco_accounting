...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Форма.Параметры.Ключ.Пустая() Тогда
		//++ЭЛИАС 20150518  
		АР_МодификацияКонфигурации.ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка);
		//--ЭЛИАС 20150518
	...
	Если НЕ ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаДокументаОднострочная(Форма) Тогда
		АР_МодификацияКонфигурации.ПлатежныеДокументыУстановитьУсловноеОформление(Форма, Найти(Форма.ИмяФормы, "ФормаРасшифровкаПлатежа") > 0);
	КонецЕсли;
	//--ЭЛИАС 20141127
...
Процедура ПриЧтенииНаСервере(Форма, ТекущийОбъект) Экспорт
	...
	//++ЭЛИАС 20141127  
	АР_МодификацияКонфигурации.ПриСозданииНаСервере(Форма, Ложь, Истина);
	//--ЭЛИАС 20141127
...
Процедура УстановитьКурсДокумента(Форма) Экспорт
	...
	Если Объект.Ссылка.РасшифровкаПлатежа.Количество() > 0 Тогда
		СтруктураКурса = АР_ПроцедурыНачисления.ПолучитьКурсВалюты(Объект.ВалютаДокумента, Объект.Дата, Объект.Ссылка.РасшифровкаПлатежа[0].ДоговорКонтрагента);
	КонецЕсли;
	//--ЭЛИАС 20141118
...
Процедура РассчитатьКурсИСуммуВзаиморасчетов(СтрокаПлатеж, Форма) Экспорт //++ЭЛИАС 20150924  
	...
	//++ЭЛИАС 20141118  
	СтруктураКурсаВзаиморасчетов = АР_ПроцедурыНачисления.ПолучитьКурсВалюты(
		СтрокаПлатеж.ДоговорКонтрагентаВалютаВзаиморасчетов, Объект.Дата, СтрокаПлатеж.ДоговорКонтрагента);
	//--ЭЛИАС 20141118
...
Функция НовыеПараметрыПриИзмененииСчетаОрганизации(Знач ПараметрыОбъекта)
	...
	//++ЭЛИАС 20141118  
	Если ПараметрыОбъекта.Свойство("ДоговорКонтрагента") Тогда
		СтруктураКурса = АР_ПроцедурыНачисления.ПолучитьКурсВалюты(НоваяВалюта, ПараметрыОбъекта.Дата, ПараметрыОбъекта.ДоговорКонтрагента);
	КонецЕсли;
	//--ЭЛИАС 20141118
...
Функция ТекущиеПараметрыОбъекта(Форма) Экспорт //++ЭЛИАС 20150518  
	...
	Если Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ОплатаПокупателя Тогда
		...
		Если ПоступлениеНаРасчетныйСчетФормыКлиентСервер.ФормаДокументаОднострочная(Форма)
			...
			Если ЗначениеЗаполнено(СтрокаРасшифровки.СчетНаОплату) Тогда
				...
				//++ЭЛИАС 20160428 
			ИначеЕсли ЗначениеЗаполнено(СтрокаРасшифровки.АР_СчетНаАренду) Тогда
				ОплаченныеСчета.Добавить(СтрокаРасшифровки.АР_СчетНаАренду);
				//--ЭЛИАС 20160428
			КонецЕсли;
		Иначе
			Для каждого СтрокаРасшифровки Из РасшифровкаПлатежа Цикл
				...
				Если ЗначениеЗаполнено(СчетНаОплату) И УникальныеЗначения[СчетНаОплату] = Неопределено Тогда
					...
					//++ЭЛИАС 20160428 
				ИначеЕсли ЗначениеЗаполнено(СтрокаРасшифровки.АР_СчетНаАренду) И УникальныеЗначения[СтрокаРасшифровки.АР_СчетНаАренду] = Неопределено Тогда
					ОплаченныеСчета.Добавить(СтрокаРасшифровки.АР_СчетНаАренду);
					УникальныеЗначения.Вставить(СтрокаРасшифровки.АР_СчетНаАренду, Истина);
					//--ЭЛИАС 20160428
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Функция СвойстваСтрокРасшифровкиПлатежаСервер(Знач ПараметрыПлатежа, Знач ПолучатьДоговор) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыПлатежа.ДоговорКонтрагента) Тогда
		...
		Если ПараметрыПлатежа.ВидОперации <> Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПриобретениеИностраннойВалюты Тогда
			...
			//++ЭЛИАС 20141118  
			СтруктураКурса = АР_ПроцедурыНачисления.ПолучитьКурсВалюты(СвойстваДоговора.ВалютаВзаиморасчетов, ПараметрыПлатежа.Дата, СвойстваПлатежа.ДоговорКонтрагента);
			СчетУчетаПоПретензиям = АР_ОбщиеПроцедуры.ПолучитьСчетУчетаПеней(ПараметрыПлатежа.Организация, ПараметрыПлатежа.Контрагент, ПараметрыПлатежа.ДоговорКонтрагента, ПараметрыПлатежа.Дата);
			Если СвойстваПлатежа.Свойство("АР_СчетПеней") Тогда
				СвойстваПлатежа.АР_СчетПеней = СчетУчетаПоПретензиям;	
			КонецЕсли;
			//--ЭЛИАС 20141118
		Иначе
...
Процедура ИнициализироватьСвойстваПлатежа(Форма, ТолькоСоздатьКлючи = Ложь, ПолучатьДоговор = Ложь, ИнициализироватьСтатьюДДС = Ложь) Экспорт //++ЭЛИАС 20151023  
