...
Процедура ЗаполнитьДанныеДляДОПИнформацияПродавцаФНС(СсылкаНаОбъект, СтруктураЭД, ДеревоДанных, Отказ) Экспорт
	...
	//++ЭЛИАС 20190712 
	Если ТипЗнч(СсылкаНаОбъект) = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты") Тогда
		ТаблицаДанных = УчетНДС.ПолучитьДанныеДляПечатиУниверсальногоПередаточногоДокумента(
			МассивДокументов, Документы.АР_НачислениеАренднойПлаты.ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов(), Истина);
	ИначеЕсли ТипЗнч(СсылкаНаОбъект) = Тип("ДокументСсылка.АР_НачислениеПени") Тогда
		ТаблицаДанных = УчетНДС.ПолучитьДанныеДляПечатиУниверсальногоПередаточногоДокумента(
			МассивДокументов, Документы.АР_НачислениеПени.ТекстЗапросаПечатьУниверсальныхПередаточныхДокументов(), Истина);
	КонецЕсли;
	//--ЭЛИАС 20190712	
...
Процедура ЗаполнитьДанныеПоСчету(СсылкаНаОбъект, СтруктураЭД, ДеревоДокумента, Отказ) Экспорт
	...
	//++ЭЛИАС 20160802 
	Если ТипЗнч(СсылкаНаОбъект) = Тип("ДокументСсылка.АР_СчетНаАренду") Тогда
		ДанныеДляФормированияЭД = Документы.АР_СчетНаАренду.ПолучитьТаблицуСведенийСчетаНаОплату(МассивДокументов);
	КонецЕсли;
	//--ЭЛИАС 20160802
...
Процедура ЗаполнитьДанныеДляАктОРасхождениях(СсылкаНаОбъект, СтруктураЭД, ДеревоДанных, Отказ) Экспорт
	...
	// Номер документа.
	ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
		ДеревоДанных, "НомерДокумента", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ВыборкаШапка.НомерДокумента, Ложь, Истина));
	...
	ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
		...
		ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ВыборкаШапка.НомерВходящегоДокумента, Ложь, Истина), ТекстОшибки);
...
Процедура ЗаполнитьДанныеДляУПДИнформацииПокупателяФНС_2019(СсылкаНаЭД, СтруктураЭД, ДеревоДанных, Отказ) Экспорт
	...
	Если Не РезультатЗапроса[3].Пустой() Тогда
		...
		ДанныеОтветногоТитула.АктОРасхожденияхНомер = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ВыборкаАктОРасхождениях.Номер, Ложь, Истина);
...
Процедура НазначенСтатусПодписан(ЭлектронныйДокумент) Экспорт
	...
	ИсходящийЭД = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлектронныйДокумент, "ВладелецФайла");
	Если Не ЗначениеЗаполнено(ИсходящийЭД) Тогда
		Возврат;
	КонецЕсли;
	
	ОснованияЭД = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИсходящийЭД, "ДокументыОснования").Выгрузить();
	Для Каждого ОснованиеЭД Из ОснованияЭД Цикл
		ДокументОснование = ОснованиеЭД.ДокументОснование;
		Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			УстановитьСтатусРеализации(ДокументОснование, Истина);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьПараметрыЭДПоИсточнику(Источник, ПараметрыЭД, ФорматCML) Экспорт
	...
	//++ЭЛИАС 20200416 
	Если ТипИсточника = Тип("ДокументСсылка.АР_СчетНаАренду") Тогда
		ТипИсточника = Тип("ДокументСсылка.СчетНаОплатуПокупателю");
	ИначеЕсли ТипИсточника = Тип("ДокументОбъект.АР_СчетНаАренду") Тогда
		ТипИсточника = Тип("ДокументОбъект.СчетНаОплатуПокупателю");
	ИначеЕсли ТипИсточника = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты") Тогда
		ТипИсточника = Тип("ДокументСсылка.РеализацияТоваровУслуг");
	ИначеЕсли ТипИсточника = Тип("ДокументОбъект.АР_НачислениеАренднойПлаты") Тогда
		ТипИсточника = Тип("ДокументОбъект.РеализацияТоваровУслуг");
	ИначеЕсли ТипИсточника = Тип("ДокументСсылка.АР_НачислениеПени") Тогда
		ТипИсточника = Тип("ДокументСсылка.РеализацияТоваровУслуг");
	ИначеЕсли ТипИсточника = Тип("ДокументОбъект.АР_НачислениеПени") Тогда
		ТипИсточника = Тип("ДокументОбъект.РеализацияТоваровУслуг");
	КонецЕсли;
	//--ЭЛИАС 20200416
...
Процедура ЗаполнитьОсновнуюЧастьУКД(ДанныеДляФормированияЭД, СтруктураЭД, ДеревоДанных)
	...
	Если СтруктураЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ДИСУКД Тогда
		...
		ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДеревоДанных, "НомерИсходногоДокумента",
			ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеОснования.Номер, Ложь, Истина));
...
Процедура ЗаполнитьРасширеннуюЧастьУПД_2019(ДанныеДляФормированияЭД, СтруктураЭД, ДеревоДанных)
	...
	СтрокаДокументыПодтвержденияОтгрузки.Наименование = "Реализация (акт, накладная)";
...
Функция ПодготовитьСтруктуруДляСчетаФактурыУПД_2019(ДеревоДанных)
	...
	Если ЭлектронноеВзаимодействие.ЗначениеРеквизитаВДереве(ДеревоДанных, "СведенияОКомиссионере") = Истина Тогда
...
Функция СтруктураДляСопоставленияНоменклатурыЭД(Номенклатура, СтавкаНДС, Содержание = "", Владелец = Неопределено)
	...
	Если ЗначениеЗаполнено(Владелец) Тогда
		...
		НоменклатураИБ = ОбменСКонтрагентамиКлиентСервер.НоваяНоменклатураИнформационнойБазы(Номенклатура,, ЗначенияРеквизитовНоменклатуры.ЕдиницаИзмерения);
		Отбор.Вставить("НоменклатураИБ", НоменклатураИБ);
		Отбор.Вставить("Владелец"      , Владелец);
		СоответствиеНоменклатуры = ОбменСКонтрагентами.НайтиСоответствиеНоменклатуры(Отбор, Истина);
...
Функция СчетФактураНеТребуется(СсылкаНаДокумент)
	...
	МассивДокументов = Документы[СсылкаНаДокумент.Метаданные().Имя].ПолучитьДокументыСчетФактураНеТребуются(
...
Функция ДанныеПервичногоДокументаУПД(Знач ДокументСсылка)
	...
	Результат.Номер = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Результат.Номер, Ложь, Истина);
...
Процедура ПроверитьГотовностьОснованияУКД(СсылкаНаОбъект, Отказ)
	...
	Если ПокупателюВыставляетсяКорректировочныйСчетФактура <> Истина Тогда
		ШаблонСообщения = НСтр("ru = 'Для ""%1"" не удалось сформировать электронный документ. Во вкладке ""НДС"" формы документа должен быть установлен вариант ""Корректировочный счет-фактура выданный покупателю (рекомендуется)"".'");
...
Процедура УстановитьСтатусРеализации(ДокументСсылка, ДокументПодписан)
	
	СтатусДокумента = ?(ДокументПодписан,
