		...
		Если Строка.ЭтоРоль Тогда
			Если СкрытьРольПолныеПрава
			   И (    ВРег(Строка.Имя) = ВРег("ПолныеПрава")
				...
				//++ЭЛИАС 20120711 Скроем дополнительные права, которые будем добавлять всем
			ИначеЕсли ВРег(Строка.Имя) = ВРег("АР_ДополнительныеПраваНаЧтениеДанныхАренды") Тогда
				Коллекция.Удалить(Индекс);
				//--ЭЛИАС 20120711
			Иначе
