...
Процедура УстановитьОпциюУстановкаМобильногоПриложения(Включить = Истина) Экспорт
	...
	Включить = Ложь; // До публикации мобильного приложения на всех площадках
...
Процедура ПодготовитьСодержимоеКассовыхЧековПодотчетныхЛиц(Параметры, АдресХранилища) Экспорт
	...
	РезультатЗапроса = РезультатЗапросаЗагрузкиЧеков(Параметры.ЗагружаемыеЧеки, Параметры.ОписаниеДокумента.Номенклатура);
	...
	Пока ВыборкаЧек.Следующий() Цикл
		...
		Если Параметры.ОписаниеДокумента.Поставщик Тогда
