...
//Функция ПустаяТаблицаВзаиморасчетовЗачетАвансов(ЭтоАренда = Ложь) Экспорт //++ЭЛИАС 20120704
//--ЭЛИАС 20171229
	...
	//Если ЭтоАренда Тогда
		...
		ПустаяТаблица.Колонки.Добавить("СчетНаАренду",            Новый ОписаниеТипов("ДокументСсылка.АР_СчетНаАренду"));
		...
		ПустаяТаблица.Колонки.Добавить("ПериодАренды",            ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата));
		...
		ПустаяТаблица.Колонки.Добавить("СчетНаАрендуДокумент",            Новый ОписаниеТипов("ДокументСсылка.АР_СчетНаАренду"));
		...
		ПустаяТаблица.Колонки.Добавить("ПериодАрендыДокумент",            ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата));
	//КонецЕсли;
	//--ЭЛИАС 20120704 При арендных взаиморасчетах
...
Функция ПодготовитьТаблицуВзаиморасчетовЗачетАвансов(ТаблицаДокумента, ТаблицаДокументыАвансов, ТаблицаРеквизиты, Отказ) Экспорт
	...
	//++ЭЛИАС 20130502  
	ЭтоАренда = (Параметры.Реквизиты.Количество() <> 0 И Параметры.Реквизиты[0].ЭтоАренда) ИЛИ (ТаблицаРеквизиты = Неопределено);
	...
	Для каждого СтрокаДокумента Из ТаблицаДокумента Цикл
		...
		Если Реквизиты.СпособЗачетаАвансов <> Перечисления.СпособыЗачетаАвансов.ПоДокументу Тогда
			...
			Если ТипЗнч(Реквизиты.Регистратор) = Тип("ДокументСсылка.КорректировкаРеализации")
				И Не СтрокаДокумента.СчетАвансов = Реквизиты.СчетУчетаРасчетовПоАвансам Тогда
				ОстаткиАвансовПоДоговору.Очистить();
			КонецЕсли;
			//--ЭЛИАС 20130918
			
			ДобавитьСтрокиЗачтенныхАвансов_ЭЛИАС(       //++ЭЛИАС 20130502  
				ТаблицаВзаиморасчетов,
...
//++ЭЛИАС 20171229 
Функция ПустаяТаблицаВзаиморасчетовПогашениеЗадолженности(ВидДокумента = Неопределено, ЭтоАренда =  Ложь) Экспорт //++ЭЛИАС 20120704
//Функция ПустаяТаблицаВзаиморасчетовПогашениеЗадолженности(ВидДокумента = Неопределено, ЭтоАренда =  Ложь) Экспорт //++ЭЛИАС 20120704
//--ЭЛИАС 20171229
	...
	//Если ЭтоАренда Тогда
		...
		ПустаяТаблица.Колонки.Добавить("Пени",                    Новый ОписаниеТипов("Булево"));  
		ПустаяТаблица.Колонки.Добавить("ПериодАренды",            ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата));
		ПустаяТаблица.Колонки.Добавить("СчетУчетаРасчетовПоПеням",Новый ОписаниеТипов("ПланСчетовСсылка.Хозрасчетный"));
	//КонецЕсли;
	//--ЭЛИАС 20120704 При арендных взаиморасчетах
...
Процедура ЗаполнитьТаблицуВзаиморасчетовПогашениеЗадолженности(РасшифровкаПлатежа, Реквизиты, ТаблицаВзаиморасчетов, ОстаткиЗадолженности, ЭтоОтложенноеПроведение, Отказ) Экспорт
	...
	//++ЭЛИАС 20120704 При арендных взаиморасчетах
	Если Реквизиты.ЭтоАренда Тогда
		ТаблицаВзаиморасчетов.Индексы.Добавить("СчетРасчетов, Контрагент, ДоговорКонтрагента, ДокументРасчетов, ОбъектАренды, УслугаАренды, СчетНаАренду, ПериодАренды, Депозит, Пени");
	Иначе //--ЭЛИАС 20120704 При арендных взаиморасчетах
		ТаблицаВзаиморасчетов.Индексы.Добавить("СчетРасчетов, Контрагент, ДоговорКонтрагента, ДокументРасчетов");
	КонецЕсли; //++ЭЛИАС 20120704 При арендных взаиморасчетах
	...
	Для Каждого СтрокаОстатка Из ОстаткиЗадолженности Цикл
		...
		//++ЭЛИАС 20120704 При арендных взаиморасчетах
		Если Реквизиты.ЭтоАренда И СтрокаОстатка.ДоговорКонтрагента.АР_АрендныйДоговор Тогда
			СтруктураОтбора.Вставить("ПериодАренды", СтрокаОстатка.ПериодАренды);
			СтруктураОтбора.Вставить("ОбъектАренды", СтрокаОстатка.ОбъектАренды);
			СтруктураОтбора.Вставить("УслугаАренды", СтрокаОстатка.УслугаАренды);
			СтруктураОтбора.Вставить("СчетНаАренду", СтрокаОстатка.СчетНаАренду);
			Если Не СтрокаОстатка.Пени Тогда
				СтруктураОтбора.Вставить("Пени", Ложь);
			Иначе
				Если АР_ПроцедурыНачисления.ПоДоговоруЕстьОбособленныйУчетПени(СтрокаОстатка.ДоговорКонтрагента, Реквизиты.Период) Тогда
					СтруктураОтбора.Вставить("Пени", Истина);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		//--ЭЛИАС 20120704 При арендных взаиморасчетах
...
Функция ПодготовитьПараметрыТаблицыВзаиморасчетовЗачетАвансов(ТаблицаДокумента, ТаблицаДокументыАвансов, ТаблицаРеквизиты) Экспорт
	...
	//ТаблицаРеквизиты[0].ЭтоАренда = Ложь;

	ЭтоАренда = ТаблицаРеквизиты <> Неопределено И ТаблицаРеквизиты.Колонки.Найти("ЭтоАренда") <> Неопределено И ТаблицаРеквизиты[0].ЭтоАренда; //++ЭЛИАС 20120724
	...
	//++ЭЛИАС 20120704
	СписокОбязательныхКолонок = СписокОбязательныхКолонок + ?(ТаблицаРеквизиты <> Неопределено И ТаблицаРеквизиты.Количество() > 0 
		И ТипЗнч(ТаблицаРеквизиты[0].Регистратор) = Тип("ДокументСсылка.КорректировкаРеализации"), ", СчетУчетаРасчетовПоАвансам", "") ;
...
Функция ПодготовитьПараметрыЗачетАвансовКомитентов(ТаблицаТоварыУслугиКомитентов, ТаблицаРеквизиты)
	...
	//++ЭЛИАС 20130211
	Если ТаблицаРеквизиты <> Неопределено И ТаблицаРеквизиты.Колонки.Найти("ЭтоАренда") = Неопределено Тогда
		// Добавим колонку "ЭтоАренда" со значением Ложь
		ТаблицаРеквизиты.Колонки.Добавить("ЭтоАренда", Новый ОписаниеТипов("Булево"));
	КонецЕсли;
	//--ЭЛИАС 20130211
...
Функция ПолучитьОстаткиАвансов(ТаблицаДокумента, Реквизиты, Отказ)
	...
	Если ТипЗнч(Реквизиты) = Тип("СтрокаТаблицыЗначений")
		И НЕ Реквизиты.Владелец().Колонки.Найти("ЭтоАренда") = Неопределено 
		И Реквизиты.ЭтоАренда Тогда
		...
		// Анализируем есть ли остатки по авансам УСН и патентам
		СтрокиАвансов = ОстаткиАвансов.НайтиСтроки(Новый Структура("ДоговорКонтрагента", Реквизиты.Регистратор.ДоговорКонтрагента));
		
		ВозможенЕНВД 			= НЕ ОстаткиАвансов.Колонки.Найти("СуммаВзаиморасчетовЕНВД") = Неопределено;
		ВозможенПатент 			= НЕ ОстаткиАвансов.Колонки.Найти("СуммаВзаиморасчетовПатент") = Неопределено;
		ВозможенКомитент		= НЕ ОстаткиАвансов.Колонки.Найти("СуммаВзаиморасчетовКомитента") = Неопределено;
		ВозможенТорговыйСбор 	= НЕ ОстаткиАвансов.Колонки.Найти("СуммаВзаиморасчетовТорговыйСбор") = Неопределено;
		
		ДокументыРасчетовУСН = Новый Соответствие;
		Для Каждого Строк Из СтрокиАвансов Цикл
			
			СтруктураУСН = ДокументыРасчетовУСН.Получить(Строк.ДокументРасчетов);
			Если СтруктураУСН = Неопределено Тогда
				СтруктураУСН = Новый Структура("ЕстьОстаткиЕНВД, ЕстьОстаткиПатент, ЕстьОстаткиКомитента, ЕстьОстаткиТорговыйСбор", Ложь, Ложь, Ложь, Ложь);
			КонецЕсли;
			
			Если ВозможенЕНВД И ЗначениеЗаполнено(Строк.СуммаВзаиморасчетовЕНВД) Тогда
				СтруктураУСН.Вставить("ЕстьОстаткиЕНВД", Истина);	
			КонецЕсли;
			Если ВозможенПатент И ЗначениеЗаполнено(Строк.СуммаВзаиморасчетовПатент) Тогда
				СтруктураУСН.Вставить("ЕстьОстаткиПатент", Истина);	
			КонецЕсли;
			Если ВозможенКомитент И ЗначениеЗаполнено(Строк.СуммаВзаиморасчетовКомитента) Тогда
				СтруктураУСН.Вставить("ЕстьОстаткиКомитента", Истина);	
			КонецЕсли;
			Если ВозможенТорговыйСбор И ЗначениеЗаполнено(Строк.СуммаВзаиморасчетовТорговыйСбор) Тогда
				СтруктураУСН.Вставить("ЕстьОстаткиТорговыйСбор", Истина);	
			КонецЕсли;
			
			ДокументыРасчетовУСН.Вставить(Строк.ДокументРасчетов, СтруктураУСН);
			
		КонецЦикла;
		
		ДополнительныеПараметры.Вставить("ДокументыРасчетовУСН", ДокументыРасчетовУСН);
...
Процедура ДобавитьСтрокиЗачтенныхАвансов(ТаблицаВзаиморасчетов, НераспределеннаяСумма, ОстаткиАвансов, СтрокаДокумента, Реквизиты, НеТиповоеИспользование) //++ЭЛИАС 20130502
	...
	Для каждого СтрокаОстатка Из ОстаткиАвансов Цикл
		...
		Если Реквизиты.ЭтоАренда Тогда
			СтрокаВзаиморасчетов.ОбъектАренды           = СтрокаОстатка.ОбъектАренды;
			СтрокаВзаиморасчетов.УслугаАренды           = СтрокаОстатка.УслугаАренды;
			СтрокаВзаиморасчетов.СчетНаАренду           = СтрокаОстатка.СчетНаАренду;
			СтрокаВзаиморасчетов.Депозит                = СтрокаОстатка.Депозит;
			СтрокаВзаиморасчетов.Пени                   = СтрокаОстатка.Пени;
			СтрокаВзаиморасчетов.ПериодАренды           = СтрокаОстатка.ПериодАренды;
			...
			Если СтрокаОстатка.Депозит Тогда
				СтрокаВзаиморасчетов.СчетАвансов = Реквизиты.СчетДепозита;
			КонецЕсли;
		КонецЕсли;
		//--ЭЛИАС 20120704 Добавим арендные реквизиты
...
//++ЭЛИАС 20130502  
Процедура ДобавитьСтрокиЗачтенныхАвансов_ЭЛИАС(ТаблицаВзаиморасчетов, НераспределеннаяСумма, ОстаткиАвансов, СтрокаДокумента, Реквизиты)
	//++ЭЛИАС 20120704 Сначала закрываем авансы, которые проходят по конкретной услуге и объекту аренды (Сч = 1)
	// если аванс с указанными объектами и услугами не покрыл сумму начисления, ищем строку с авансом без привязки к конкретному объекту (Сч = 2)
	// если аванс с указанными объектами и услугами не покрыл сумму начисления, ищем строку с авансом без привязки к конкретной услуге (Сч = 3)
	// если аванс с указанными объектами и услугами не покрыл сумму начисления, ищем строку с авансом без привязки к конкретному объекту и услуге (Сч = 4)
	// если зачитываем залог, необходимо закрыть авансы с пустыми аналитиками (Сч = 5)
	
	Если НераспределеннаяСумма <= 0 Тогда
		Возврат;
	КонецЕсли;
	
	ЭтоАренда = Реквизиты.ЭтоАренда;
	Для Сч = 1 По 5 Цикл
		Если ЭтоАренда И ОстаткиАвансов.Количество() > 0 Тогда
			
			КлючАванса = Новый Структура();
			
			Если Реквизиты.ОбособленныйУчетПени Тогда
				КлючАванса.Вставить("Пени", Реквизиты.ЭтоПени);
			КонецЕсли;
			
			Если Сч = 5 И НЕ Реквизиты.Депозит Тогда
				Продолжить;
			КонецЕсли;
			
			Если (СтрокаДокумента.УслугаАренды.АР_ПостояннаяЧастьОплаты И Реквизиты.ВестиВзаиморасчетыПоСчетамПост 
				ИЛИ	Не СтрокаДокумента.УслугаАренды.АР_ПостояннаяЧастьОплаты И Реквизиты.ВестиВзаиморасчетыПоСчетамПерем)
				И ЗначениеЗаполнено(СтрокаДокумента.СчетНаАренду) Тогда
				Если Сч = 5 Тогда
					КлючАванса.Вставить("СчетНаАренду", Документы.АР_СчетНаАренду.ПустаяСсылка());
				Иначе
					КлючАванса.Вставить("СчетНаАренду", СтрокаДокумента.СчетНаАренду);
				КонецЕсли;
			КонецЕсли;
			
			Если Реквизиты.ВестиВзаиморасчетыПоПериодам Тогда
				Если Сч = 5 Тогда
					КлючАванса.Вставить("ПериодАренды", Дата(1,1,1));
				Иначе
					КлючАванса.Вставить("ПериодАренды", СтрокаДокумента.ПериодАренды);
				КонецЕсли;
			КонецЕсли;
			
			Если Реквизиты.ВестиВзаиморасчетыПоОбъектам Тогда
				Если Сч = 1 ИЛИ Сч = 3 Тогда
					КлючАванса.Вставить("ОбъектАренды", СтрокаДокумента.ОбъектАренды);
				Иначе
					КлючАванса.Вставить("ОбъектАренды", Справочники.АР_ОбъектыАренды.ПустаяСсылка());
				КонецЕсли;
			КонецЕсли;
			
			Если Реквизиты.ВестиВзаиморасчетыПоУслугам Тогда
				Если Сч = 1 ИЛИ Сч = 2 Тогда
					КлючАванса.Вставить("УслугаАренды", СтрокаДокумента.УслугаАренды);
				Иначе
					КлючАванса.Вставить("УслугаАренды", Справочники.Номенклатура.ПустаяСсылка());
				КонецЕсли;
			КонецЕсли;
			
			Если Сч = 5 Тогда
				КлючАванса.Вставить("Депозит", Истина);
			Иначе
				Если НЕ Реквизиты.Депозит Тогда
					КлючАванса.Вставить("Депозит", Ложь);
				КонецЕсли;
			КонецЕсли;
			
			Если КлючАванса.Количество() = 0 Тогда
				СтрокиОстатков = ОстаткиАвансов;
			Иначе
				Если ТипЗнч(ОстаткиАвансов) = Тип("ТаблицаЗначений") Тогда
					СтрокиОстатков = ОстаткиАвансов.НайтиСтроки(КлючАванса);
				Иначе
					СтрокиОстатков = Новый Массив;
					Для Каждого СтрокаАванса Из ОстаткиАвансов Цикл
						СтрокиСовпадают = Истина;
						Для Каждого Элемент Из КлючАванса Цикл
							Если Не СтрокаАванса[Элемент.Ключ] = Элемент.Значение Тогда
								СтрокиСовпадают = Ложь;
								Прервать;
							КонецЕсли;
						КонецЦикла;
						Если СтрокиСовпадают Тогда
							СтрокиОстатков.Добавить(СтрокаАванса);
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
			
			ДобавитьСтрокиЗачтенныхАвансов(ТаблицаВзаиморасчетов, НераспределеннаяСумма, СтрокиОстатков, СтрокаДокумента, Реквизиты, Истина)
			
		Иначе // типовое выполнение
			
			Если Сч > 1 Тогда
				Прервать;
			КонецЕсли;
			
			ДобавитьСтрокиЗачтенныхАвансов(ТаблицаВзаиморасчетов, НераспределеннаяСумма, ОстаткиАвансов, СтрокаДокумента, Реквизиты, Ложь)
			
		КонецЕсли;
		
	КонецЦикла;
	
КонецПроцедуры //--ЭЛИАС 20130502
...
Функция ПодготовитьПараметрыТаблицыВзаиморасчетовПогашениеЗадолженности(РасшифровкаПлатежа, ТаблицаРеквизиты, ВидДокумента) Экспорт
	...
	ТаблицаРеквизиты[0].ЭтоАренда = Ложь; //СВД
	...
	Если ЭтоАренда Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ","
		+ "ОбъектАренды,"              // <СправочникСсылка.АР_ОбъектыАренды> - объект аренды, по которому погашается задолженность
		+ "УслугаАренды,"              // <СправочникСсылка.Номенклатура> - услуга, по которому погашается задолженность
		+ "СчетНаАренду,"              // <ДокументСсылка.АР_СчетНаАренду> - счет на аренду, по которому погашается задолженность
		+ "ПериодАренды,"               // <Дата> - период, за который погашается задолженность
		+ "Пени,"                      // <Булево> - Истина при оплате пени
		+ "Депозит,"                   // <Булево> - Истина, если оплачивается депозит
		+ "ВестиВзаиморасчетыПоПериодам,"    // <Булево> - Истина при ведении взаиморасчетов по периодам
		+ "ВестиВзаиморасчетыПоОбъектам,"    // <Булево> - Истина при ведении взаиморасчетов по объектам аренды
		+ "ВестиВзаиморасчетыПоУслугам,"     // <Булево> - Истина при ведении взаиморасчетов по услугам
		+ "ВестиВзаиморасчетыПоСчетамПост,"  // <Булево> - Истина при ведении взаиморасчетов по счетам на постоянную часть
		+ "ВестиВзаиморасчетыПоСчетамПерем"; // <Булево> - Истина при ведении взаиморасчетов по счетам на переменную часть
		Если ТипЗнч(ТаблицаРеквизиты[0].Регистратор) = Тип("ДокументСсылка.ПоступлениеНаРасчетныйСчет") 
			ИЛИ ТипЗнч(ТаблицаРеквизиты[0].Регистратор) = Тип("ДокументСсылка.ПриходныйКассовыйОрдер") Тогда
			СписокОбязательныхКолонок = СписокОбязательныхКолонок + ", СчетУчетаРасчетовПоПеням";
		КонецЕсли;
	КонецЕсли;
	//--ЭЛИАС 20120704
...
Функция ПодготовитьПараметрыПогашениеЗадолженности(ТаблицаВзаиморасчетов, ТаблицаРеквизиты)
	...
	//++ЭЛИАС 20170302 
	ОплатаПени = ТаблицаВзаиморасчетов <> Неопределено 
		И ТаблицаВзаиморасчетов.Колонки.Найти("Пени") <> Неопределено 
		И ТаблицаВзаиморасчетов.Колонки.Найти("СчетУчетаРасчетовПоПеням") <> Неопределено;
		
	Если ОплатаПени Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ","
		+ "СчетУчетаРасчетовПоПеням,"                    // <ПланСчетовСсылка.Хозрасчетный> - счет расчетов, задолженность по которому погашается
		+ "Пени" // <Булево>
	КонецЕсли;
	//--ЭЛИАС 20170302
...
Функция ПолучитьОстаткиЗадолженности(РасшифровкаПлатежа, Реквизиты, Отказ)
	...
	ИспользуетсяОтложенноеПроведение = ПроведениеСервер.ИспользуетсяОтложенноеПроведение(Реквизиты.Организация, Реквизиты.Период);
	//++ЭЛИАС 20181218 Для арендных документов будем выполнять полное проведение
	Если АР_ОбщиеПроцедуры.ЭтоАрендныйДокумент(Реквизиты.Регистратор) Тогда
	...
	Если Реквизиты.ЭтоАренда Тогда
		...
		// Анализируем есть ли остатки по УСН и патентам
		СтрокиЗадолженности = ОстаткиЗадолженности.НайтиСтроки(Новый Структура("ДоговорКонтрагента", Реквизиты.Регистратор.ДоговорКонтрагента));
		
		ВозможенЕНВД 			= НЕ ОстаткиЗадолженности.Колонки.Найти("СуммаВзаиморасчетовЕНВД") = Неопределено;
		ВозможенПатент 			= НЕ ОстаткиЗадолженности.Колонки.Найти("СуммаВзаиморасчетовПатент") = Неопределено;
		ВозможенКомитент		= НЕ ОстаткиЗадолженности.Колонки.Найти("СуммаВзаиморасчетовКомитента") = Неопределено;
		ВозможенТорговыйСбор 	= НЕ ОстаткиЗадолженности.Колонки.Найти("СуммаВзаиморасчетовТорговыйСбор") = Неопределено;
		
		ДокументыРасчетовУСН = Новый Соответствие;
		Для Каждого Строк Из СтрокиЗадолженности Цикл
		...
		МассивДоговоров = РазличныеЗначенияКолонкиТаблицы(ТаблицаДоговоров, "ДоговорКонтрагента");
		ЭтоПервыйДоговор = Истина;
		Для Каждого ДоговорКонтрагента Из МассивДоговоров Цикл
			Если ДоговорКонтрагента.АР_АрендныйДоговор Тогда
				СтруктураПараметров = Новый Структура();
				СтруктураПараметров.Вставить("Контрагент", ДоговорКонтрагента.Владелец);
				СтруктураПараметров.Вставить("ДоговорКонтрагента", ДоговорКонтрагента);
				ПараметрыНачисленияПени = АР_ПроцедурыНачисления.ПолучитьПараметрыНачисленияПени(ДоговорКонтрагента, Реквизиты.Период);
				СтруктураПараметров.Вставить("СпособПогашенияПени", ?(ПараметрыНачисленияПени = Неопределено, Перечисления.АР_СпособыПогашенияПени.СперваПениПотомОсновнойДолг, ПараметрыНачисленияПени.СпособПогашенияПени));
				СтруктураПараметров.Вставить("ДокументыРасчетовУСН", ДокументыРасчетовУСН);
				Если ЭтоПервыйДоговор Тогда
					НачТаблицаОстатков = АР_УправлениеВзаиморасчетами.ПолучитьОстатокПоАренднымВзаиморасчетам(Реквизиты, СтруктураПараметров);
					ЭтоПервыйДоговор = Ложь;
				Иначе
					ВременнаяТаблицаОстатков = АР_УправлениеВзаиморасчетами.ПолучитьОстатокПоАренднымВзаиморасчетам(Реквизиты, СтруктураПараметров);
					Для Каждого СтрокаОстатка Из ВременнаяТаблицаОстатков Цикл
						НоваяСтрока = НачТаблицаОстатков.Добавить();
			...
			Иначе
				Если ЭтоПервыйДоговор Тогда
					...
					СтруктураПараметров.Вставить("ДоговорКонтрагента", ДоговорКонтрагента);
					ПараметрыНачисленияПени = АР_ПроцедурыНачисления.ПолучитьПараметрыНачисленияПени(ДоговорКонтрагента, Реквизиты.Период);
					СтруктураПараметров.Вставить("СпособПогашенияПени", Перечисления.АР_СпособыПогашенияПени.СперваПениПотомОсновнойДолг);
					СтруктураПараметров.Вставить("ДокументыРасчетовУСН", ДокументыРасчетовУСН);
					НачТаблицаОстатков = АР_УправлениеВзаиморасчетами.ПолучитьОстатокПоАренднымВзаиморасчетам(Реквизиты, СтруктураПараметров);
					НачТаблицаОстатков.Очистить();
					ЭтоПервыйДоговор = Ложь;
				КонецЕсли;
				СтрокиЗадолженности = ОстаткиЗадолженности.НайтиСтроки(Новый Структура("ДоговорКонтрагента", ДоговорКонтрагента));	
				Для Каждого СтрокаОстатка Из СтрокиЗадолженности Цикл
					НоваяСтрока = НачТаблицаОстатков.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОстатка);
					НоваяСтрока.Счет = СтрокаОстатка.СчетРасчетов;
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
		НачТаблицаОстатков.Колонки.Счет.Имя = "СчетРасчетов";
		ОстаткиЗадолженности = НачТаблицаОстатков.Скопировать();
		ОстаткиЗадолженности.Индексы.Добавить("Контрагент, ДоговорКонтрагента, ДокументРасчетов, ОбъектАренды, УслугаАренды, СчетНаАренду, ПериодАренды, Депозит, Пени");
	КонецЕсли;
	//--ЭЛИАС 20100514 При арендных взаиморасчетах
...
Процедура ДобавитьСтрокиПогашенияЗадолженности(ТаблицаВзаиморасчетов, НераспределеннаяСумма, НераспределеннаяСуммаБУ, КратностьВзаиморасчетов, ОстаткиЗадолженности, СтрокаПлатежа, Реквизиты, Отказ)
	...
	Для Каждого СтрокаТаблицы Из МассивСтрокОтбор Цикл
		Если ЭтоСчетРасчетовПоОтгрузке(СтрокаТаблицы.СчетРасчетов)
		 //++ЭЛИАС 20160415 
		 ИЛИ СтрокаТаблицы.СчетРасчетов = Неопределено
		 //--ЭЛИАС 20160415	
	...
	Сортировка = "";
	Если Реквизиты.ЭтоАренда Тогда
		ПараметрыНачисленияПени = АР_ПроцедурыНачисления.ПолучитьПараметрыНачисленияПени(СтрокаПлатежа.ДоговорКонтрагента, Реквизиты.Период);
		Если ПараметрыНачисленияПени = Неопределено Тогда
		ИначеЕсли ПараметрыНачисленияПени.СпособПогашенияПени = Перечисления.АР_СпособыПогашенияПени.СперваПениПотомОсновнойДолг Тогда
			Сортировка = "Пени Убыв,";
		ИначеЕсли ПараметрыНачисленияПени.СпособПогашенияПени = Перечисления.АР_СпособыПогашенияПени.СперваОсновнойДолгПотомПени Тогда
			Сортировка = "Пени Возр,";
		КонецЕсли;
		Если СтрокаПлатежа.ВестиВзаиморасчетыПоПериодам И ЗначениеЗаполнено(СтрокаПлатежа.ПериодАренды) Тогда
			Отбор.Вставить("ПериодАренды", СтрокаПлатежа.ПериодАренды);
		КонецЕсли;
		Если СтрокаПлатежа.ВестиВзаиморасчетыПоОбъектам И ЗначениеЗаполнено(СтрокаПлатежа.ОбъектАренды) Тогда
			Отбор.Вставить("ОбъектАренды", СтрокаПлатежа.ОбъектАренды);
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаПлатежа.УслугаАренды) Тогда
			Если СтрокаПлатежа.ВестиВзаиморасчетыПоУслугам Тогда
				Отбор.Вставить("УслугаАренды", СтрокаПлатежа.УслугаАренды);
			КонецЕсли;
			Если (СтрокаПлатежа.УслугаАренды.АР_ПостояннаяЧастьОплаты И СтрокаПлатежа.ВестиВзаиморасчетыПоСчетамПост 
				ИЛИ	Не СтрокаПлатежа.УслугаАренды.АР_ПостояннаяЧастьОплаты И СтрокаПлатежа.ВестиВзаиморасчетыПоСчетамПерем)
				И ЗначениеЗаполнено(СтрокаПлатежа.СчетНаАренду) Тогда
				Отбор.Вставить("СчетНаАренду", СтрокаПлатежа.СчетНаАренду);
			КонецЕсли;
		ИначеЕсли (СтрокаПлатежа.ВестиВзаиморасчетыПоСчетамПост ИЛИ СтрокаПлатежа.ВестиВзаиморасчетыПоСчетамПерем) Тогда
			Отбор.Вставить("СчетНаАренду", СтрокаПлатежа.СчетНаАренду);
		КонецЕсли;
		Если СтрокаПлатежа.Пени Тогда
			Отбор.Вставить("Пени", Истина);
		Иначе
			Если ПараметрыНачисленияПени <> Неопределено 
				И ПараметрыНачисленияПени.СпособПогашенияПени = Перечисления.АР_СпособыПогашенияПени.ВестиОбособленныйУчетПени Тогда
				Отбор.Вставить("Пени", Ложь);
			КонецЕсли;
		КонецЕсли;
	...
	//КонецЕсли;
	ЭтоСчетАванса = (СтрокаПлатежа.СчетРасчетов = ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученным)
	ИЛИ (СтрокаПлатежа.СчетРасчетов = ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученнымВал)
	ИЛИ (СтрокаПлатежа.СчетРасчетов = ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученнымУЕ);
	Если Реквизиты.ЭтоАренда И (СтрокаПлатежа.Депозит ИЛИ ЭтоСчетАванса) И (Реквизиты.НаправлениеДвижения = "Поступление") Тогда
		ОстаткиЗадолженностиСОтбором = Новый ТаблицаЗначений;
	Иначе //--ЭЛИАС 20120704 При арендных взаиморасчетах
		Если БухгалтерскийУчет.НаСчетеВедетсяУчетПоДокументамРасчетов(СтрокаПлатежа.СчетРасчетов) Тогда
			//++ЭЛИАС 20170411
			//ОстаткиЗадолженностиСОтбором.Сортировать("ДатаДокументаРасчетов, ДокументРасчетов", Новый СравнениеЗначений);
			ОстаткиЗадолженностиСОтбором.Сортировать(Сортировка + "ДатаДокументаРасчетов, ДокументРасчетов", Новый СравнениеЗначений);
			//--ЭЛИАС 20170411
		Иначе
			//++ЭЛИАС 20170411
			//ОстаткиЗадолженностиСОтбором.Сортировать("НомерСтроки");
			ОстаткиЗадолженностиСОтбором.Сортировать(Сортировка + "НомерСтроки");
			//--ЭЛИАС 20170411
		КонецЕсли;
	...
	//++ЭЛИАС 20120706
	Если Реквизиты.ЭтоАренда И ОстаткиЗадолженностиСОтбором.Колонки.Найти("СчетРасчетов") <> Неопределено Тогда
		ОстаткиЗадолженностиСОтбором.Колонки.Удалить(ОстаткиЗадолженностиСОтбором.Колонки.СчетРасчетов);
	КонецЕсли;
	//--ЭЛИАС 20120706
	...
	Для каждого СтрокаОстатка Из ОстаткиЗадолженностиСОтбором Цикл
		...
		Если РасчетыВВалюте Тогда
			...
			Иначе
				...
				Если НЕ Реквизиты.ЭтоАренда И ЭтоСчетРасчетовПоОтгрузке(СтрокаОстатка.СчетРасчетов) Тогда
				//--ЭЛИАС 20160420
...
Функция ПодготовитьПараметрыТаблицыВзаиморасчетовСписаниеЗадолженности(РасшифровкаПлатежа, ТаблицаРеквизиты) Экспорт
	...
	//++ЭЛИАС 20120706
	Если ТаблицаРеквизиты.Колонки.Найти("ЭтоАренда") = Неопределено Тогда
		// Добавим колонку "ЭтоАренда" со значением Ложь
		ТаблицаРеквизиты.Колонки.Добавить("ЭтоАренда", Новый ОписаниеТипов("Булево"));
	КонецЕсли;
	//--ЭЛИАС 20120706
