...
Функция ПодготовитьПараметрыВыручкиОтРеализации(ТаблицаТоварыУслуги, ТаблицаВзаиморасчеты, ТаблицаСписанныеТовары, ТаблицаРеквизиты)
	...
	Если ЭтоАренда Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ","
		+ "ОбъектАренды,"              // <СправочникСсылка.АР_ОбъектыАренды> - объект аренды, по которому происходит начисление
		+ "УслугаАренды,"              // <СправочникСсылка.Номенклатура> - услуга, по которому происходит начисление
		+ "СчетНаАренду,"              // <ДокументСсылка.АР_СчетНаАренду> - счет на аренду, по которому происходит начисление
		+ "ПериодАренды";              // <Дата> - период, за который происходит начисление
	...
	Если ЭтоАренда Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ","
		+ "ОбъектАрендыДокумент,"              // <СправочникСсылка.АР_ОбъектыАренды> - объект аренды, по которому происходит начисление
		+ "УслугаАрендыДокумент,"              // <СправочникСсылка.Номенклатура> - услуга, по которому происходит начисление
		+ "СчетНаАрендуДокумент,"              // <ДокументСсылка.АР_СчетНаАренду> - счет на аренду, по которому происходит начисление
		+ "ПериодАрендыДокумент,"              // <Дата> - период, за который происходит начисление
	КонецЕсли;
	//--ЭЛИАС 20120724
	...
	//++ЭЛИАС 20120726
	Если ЭтоАренда Тогда
		Параметры.Взаиморасчеты.Колонки.ОбъектАрендыДокумент.Имя = "ОбъектАренды";
		Параметры.Взаиморасчеты.Колонки.УслугаАрендыДокумент.Имя = "УслугаАренды";
		Параметры.Взаиморасчеты.Колонки.СчетНаАрендуДокумент.Имя = "СчетНаАренду";
		Параметры.Взаиморасчеты.Колонки.ПериодАрендыДокумент.Имя = "ПериодАренды";
	КонецЕсли;
	//--ЭЛИАС 20120726
...
Процедура СформироватьДвиженияРеализацияСобственныхТоваровУслуг(ТаблицаВыручкиОтРеализации, ПараметрыРеализации, ЭтоВозврат, Движения, Отказ)
	...
	Иначе
		...
		//++ЭЛИАС 20180420 
		Если ТипЗнч(ПараметрыРеализации.Регистратор) = Тип("ДокументСсылка.АР_НачислениеПени")
			И (ПараметрыРеализации.Регистратор.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.РасчетыПоПретензиям
			ИЛИ ПараметрыРеализации.Регистратор.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.РасчетыПоПретензиямВал
			ИЛИ ПараметрыРеализации.Регистратор.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.РасчетыПоПретензиямУЕ) Тогда
			ТекстСодержанияПоУслугам = "Начисление пеней";
		КонецЕсли;
		//--ЭЛИАС 20180420
	КонецЕслИ;
...
Процедура СформироватьДвиженияРеализацияТоваровУслугКомитентов(ТаблицаВыручкиОтРеализации, ПараметрыРеализации, ЭтоВозврат, Движения, Отказ)
	...
	//++ЭЛИАС 20180420 
	Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.АР_НачислениеПени")
		И (Регистратор.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.РасчетыПоПретензиям
		ИЛИ Регистратор.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.РасчетыПоПретензиямВал
		ИЛИ Регистратор.СчетУчетаРасчетовСКонтрагентом = ПланыСчетов.Хозрасчетный.РасчетыПоПретензиямУЕ) Тогда
		ТекстСодержанияПоУслугам = "Начисление пеней";
	КонецЕсли;
	//--ЭЛИАС 20180420
...
Функция ПолучитьТаблицуРеализацииПоКурсуАвансов(Параметры, Отказ) Экспорт
	...
	//++ЭЛИАС 20120724
	Если ТипЗнч(Реквизиты) = Тип("СтрокаТаблицыЗначений") Тогда
		ЭтоАренда = Реквизиты.Владелец().Колонки.Найти("ЭтоАренда") <> Неопределено  И Реквизиты.ЭтоАренда;
	ИначеЕсли ТипЗнч(Реквизиты) = Тип("Структура") Тогда
		ЭтоАренда = Реквизиты.Свойство("ЭтоАренда") И Реквизиты.ЭтоАренда;
	Иначе 
		ЭтоАренда = Реквизиты.ЭтоАренда;
	КонецЕсли;
	
	Если ЭтоАренда Тогда
		СписокКолонок = "ДоговорКонтрагента, ВалютаВзаиморасчетов, РасчетыВУсловныхЕдиницах, ОбъектАренды, УслугаАренды, СчетНаАренду, ПериодАренды";
	Иначе
		СписокКолонок = "ДоговорКонтрагента, ВалютаВзаиморасчетов, РасчетыВУсловныхЕдиницах";
	КонецЕсли;
	//--ЭЛИАС 20120724
	...
	ТаблицаДоговоров = ТаблицаВзаиморасчеты.Скопировать(, СписокКолонок);
	ТаблицаДоговоров.Свернуть(СписокКолонок);
	//--ЭЛИАС 20130502
	...
	Для каждого СтрокаДоговора Из ТаблицаДоговоров Цикл
		...
		Если ЭтоАренда Тогда
			СписокКолонокОтбора = "ДоговорКонтрагента, ОбъектАренды, УслугаАренды, СчетНаАренду, ПериодАренды";
		Иначе
			СписокКолонокОтбора = "ДоговорКонтрагента";
		КонецЕсли;
		Отбор = Новый Структура(СписокКолонокОтбора);
		ЗаполнитьЗначенияСвойств(Отбор, СтрокаДоговора);
		//--ЭЛИАС 20120724
...
Функция ПодготовитьПараметрыВыручкиОтКорректировкиРеализации(ТаблицаТоварыУслуги, ТаблицаСписанныеТовары, ТаблицаТоварыКомитента, ТаблицаРеквизиты)
	...
	ЭтоАренда = ТаблицаРеквизиты <> Неопределено И ТаблицаРеквизиты.Колонки.Найти("ЭтоАренда") <> Неопределено И ТаблицаРеквизиты[0].ЭтоАренда; //++ЭЛИАС 20130614
...
Процедура СформироватьДвиженияПереоценкаВалютныхОстатков(ТаблицаВалютныхОстатков, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	ЭтоАренда		  = Реквизиты.ЭтоАренда; //++ЭЛИАС 20120710
	
	//++ЭЛИАС 20120710
	Если ЭтоАренда Тогда
		ДвиженияВзаиморасчетыПоАренде = Движения.АР_ВзаиморасчетыПоАренде;
	КонецЕсли;
	//--ЭЛИАС 20120710
	...
	Для каждого СтрокаПереоценка Из ТаблицаПереоценка Цикл
		...
		//++ЭЛИАС 20120710
		Если ЭтоАренда И ТипЗнч(СтрокаПереоценка.Субконто2) = Тип("СправочникСсылка.ДоговорыКонтрагентов") 
			И СтрокаПереоценка.Субконто2.АР_АрендныйДоговор Тогда
			Запись = ДвиженияВзаиморасчетыПоАренде.ДобавитьПриход();
			ЗаполнитьЗначенияСвойств(Запись, СтрокаПереоценка);
			Запись.Период = Дата;
			Запись.Организация = Организация;
			Запись.Контрагент = СтрокаПереоценка.Субконто1;
			Запись.ДоговорКонтрагента = СтрокаПереоценка.Субконто2;
			Запись.ДокументРасчетов = СтрокаПереоценка.Субконто3;
			Запись.Сумма = СтрокаПереоценка.СуммаПереоценки;
			Если УчетнаяПолитика.ПлательщикНалогаНаПрибыль(Организация, Дата) Тогда
				Запись.СуммаНУ = СтрокаПереоценка.СуммаПереоценкиНУ;
			КонецЕсли;
		КонецЕсли;
		//--ЭЛИАС 20120710
...
Функция ПодготовитьПараметрыПереоценкаВалютныхОстатков(ТаблицаВалютныхОстатков, ТаблицаРеквизиты)
	...
	Если ЭтоАренда Тогда
		СписокОбязательныхКолонок = СписокОбязательныхКолонок + ","
		+ "ОбъектАренды,"            // <СправочникСсылка.АР_ОбъектыАренды> - 
		+ "УслугаАренды,"            // <СправочникСсылка.Номенклатура> - 
		+ "СчетНаАренду,"            // <ДокументСсылка.АР_СчетНаАренду> - 
		+ "ПериодАренды,"           // <Дата> - 
		+ "Пени,"               	 // <Булево> - 
		+ "Депозит";                  // <Булево> - 
	КонецЕсли;
...
Функция СоздатьПустуюТаблицуДляПереоценки()
	...
	//++ЭЛИАС 20120710
	ТаблицаДляВозврата.Колонки.Добавить("УслугаАренды",         Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаДляВозврата.Колонки.Добавить("ОбъектАренды",         Новый ОписаниеТипов("СправочникСсылка.АР_ОбъектыАренды"));
	ТаблицаДляВозврата.Колонки.Добавить("СчетНаАренду",         Новый ОписаниеТипов("ДокументСсылка.АР_СчетНаАренду"));
	ТаблицаДляВозврата.Колонки.Добавить("ПериодАренды",         ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата));
	ТаблицаДляВозврата.Колонки.Добавить("Депозит",              Новый ОписаниеТипов("Булево"));
	ТаблицаДляВозврата.Колонки.Добавить("Пени",                 Новый ОписаниеТипов("Булево"));
	//--ЭЛИАС 20120710
...
Функция ПодготовитьТаблицуВалютныхОстатковПоДокументу(СтруктураПараметров, Движения, ЕстьВалютныеСчета)
	...
	//++ЭЛИАС 20120710
	ТаблицаДляПереоценки.Колонки.Добавить("УслугаАренды",         Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаДляПереоценки.Колонки.Добавить("ОбъектАренды",         Новый ОписаниеТипов("СправочникСсылка.АР_ОбъектыАренды"));
	ТаблицаДляПереоценки.Колонки.Добавить("СчетНаАренду",         Новый ОписаниеТипов("ДокументСсылка.АР_СчетНаАренду"));
	ТаблицаДляПереоценки.Колонки.Добавить("ПериодАренды",         ОбщегоНазначенияБПКлиентСервер.ПолучитьОписаниеТиповДаты(ЧастиДаты.Дата));
	ТаблицаДляПереоценки.Колонки.Добавить("Депозит",              Новый ОписаниеТипов("Булево"));
	ТаблицаДляПереоценки.Колонки.Добавить("Пени",                 Новый ОписаниеТипов("Булево"));
	//--ЭЛИАС 20120710
	...
	Для каждого СтрокаДляПереоценки Из ТаблицаДляПереоценки Цикл
		...
		Пока ВыборкаПоСтрокамРезультата.Следующий() Цикл
			...
			//++ЭЛИАС 20120710 для арендных договоров сумма переоцениваемого остатка определяется по регистру "Взаиморасчеты по аренде"
			//НоваяСтрока = ТаблицаВалютныхОстатков.Добавить();
			//
			//ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаПоСтрокамРезультата);
			ЭтоАренда = Ложь;
			Если ТипЗнч(ВыборкаПоСтрокамРезультата.Субконто2) = Тип("СправочникСсылка.ДоговорыКонтрагентов") И ВыборкаПоСтрокамРезультата.Субконто2.АР_АрендныйДоговор Тогда
				
				Запрос.Текст = 
				"ВЫБРАТЬ
				|	АР_ВзаиморасчетыПоАрендеОстатки.Организация,
				|	АР_ВзаиморасчетыПоАрендеОстатки.Контрагент,
				|	АР_ВзаиморасчетыПоАрендеОстатки.ДоговорКонтрагента,
				...
				|	АР_ВзаиморасчетыПоАрендеОстатки.ПериодАренды,
				|	АР_ВзаиморасчетыПоАрендеОстатки.Депозит,
				|	АР_ВзаиморасчетыПоАрендеОстатки.ДокументРасчетов,
				|	АР_ВзаиморасчетыПоАрендеОстатки.Валюта,
				|	АР_ВзаиморасчетыПоАрендеОстатки.СуммаОстаток,
				|	АР_ВзаиморасчетыПоАрендеОстатки.СуммаВзаиморасчетовОстаток,
				|	АР_ВзаиморасчетыПоАрендеОстатки.Пени,
				|	АР_ВзаиморасчетыПоАрендеОстатки.СуммаНУОстаток
				|ИЗ
				...
				|				И ДокументРасчетов = &ДокументРасчетов) КАК АР_ВзаиморасчетыПоАрендеОстатки";
				
				Запрос.УстановитьПараметр("Организация", Организация);
				Запрос.УстановитьПараметр("Контрагент", ВыборкаПоСтрокамРезультата.Субконто1);
				Запрос.УстановитьПараметр("ДоговорКонтрагента", ВыборкаПоСтрокамРезультата.Субконто2);
				Запрос.УстановитьПараметр("ДокументРасчетов", ВыборкаПоСтрокамРезультата.Субконто3);
				...
				Если Не Результат.Пустой() Тогда
					...
					Пока Выборка.Следующий() Цикл
						...
						НоваяСтрока = ТаблицаВалютныхОстатков.Добавить();
						ЗаполнитьЗначенияСвойств(НоваяСтрока, ВыборкаПоСтрокамРезультата);
						НоваяСтрока.СуммаВВалюте             = Выборка.СуммаВзаиморасчетовОстаток;
						НоваяСтрока.СуммаВРублях             = Выборка.СуммаОстаток;
						НоваяСтрока.СуммаВРубляхНУ           = Выборка.СуммаНУОстаток;
						
						// Арендные реквизиты
						ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка, "УслугаАренды,ОбъектАренды,СчетНаАренду,ПериодАренды,Депозит,Пени");
						
					КонецЦикла;
				КонецЕсли;
			КонецЕсли; 
			
			Если Не ЭтоАренда Тогда
				НоваяСтрока = ТаблицаВалютныхОстатков.Добавить();
...
Функция ПодготовитьТаблицуВалютныхДвиженийПоДокументу(СтруктураПараметров, ТаблицаВалютныхОстатков, Движения, ЕстьВалютныеСчета)
	...
	Иначе
		...
		ТаблицаДляПереоценки.Очистить(); //++ЭЛИАС 20140718
	...
	Если Движения.Найти("АР_ВзаиморасчетыПоАренде") <> Неопределено И Движения.АР_ВзаиморасчетыПоАренде.Количество() > 0 Тогда
		ВзаиморасчетыПоАренде = Движения.АР_ВзаиморасчетыПоАренде.Выгрузить();
		Если ТаблицаВалютныхОстатков = Неопределено ИЛИ ТаблицаВалютныхОстатков.Колонки.Количество() = 0 Тогда
			ТаблицаДляПереоценкиАренда = СоздатьПустуюТаблицуДляПереоценки();
		Иначе
			ТаблицаДляПереоценкиАренда = ТаблицаВалютныхОстатков.Скопировать();
		КонецЕсли;
		ТаблицаДляПереоценки = СвернутьТаблицуДляПереоценки(ТаблицаДляПереоценки);
		Для Каждого СтрокаДляПереоценки Из ТаблицаДляПереоценки Цикл
			СтруктураОтбора = Новый Структура("ДоговорКонтрагента, ДокументРасчетов", СтрокаДляПереоценки.Субконто2, СтрокаДляПереоценки.Субконто3);
			ДвиженияПоАренде = ВзаиморасчетыПоАренде.НайтиСтроки(СтруктураОтбора);
			Для Каждого Запись Из ДвиженияПоАренде Цикл
				СтрокаДляПереоценкиАренда = ТаблицаДляПереоценкиАренда.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаДляПереоценкиАренда, СтрокаДляПереоценки);
				ЗаполнитьЗначенияСвойств(СтрокаДляПереоценкиАренда, Запись, "УслугаАренды, ОбъектАренды, СчетНаАренду, ПериодАренды, Депозит, Пени");
				...
				СтрокаДляПереоценкиАренда.СуммаВВалюте = Запись.СуммаВзаиморасчетов*Знак;
				СтрокаДляПереоценкиАренда.СуммаВРублях = Запись.Сумма*Знак;
				
				Если СтрокаДляПереоценкиАренда.Счет.НалоговыйУчет Тогда
					СтрокаДляПереоценкиАренда.СуммаВРубляхНУ       = Запись.СуммаНУ*Знак;
				Иначе
					СтрокаДляПереоценкиАренда.СуммаВРубляхНУ       = СтрокаДляПереоценки.СуммаВРублях;
				КонецЕсли;
				
			КонецЦикла;
		КонецЦикла;	
		Возврат СвернутьТаблицуДляПереоценки(ТаблицаДляПереоценкиАренда);
	Иначе
		Если ТаблицаВалютныхОстатков <> Неопределено И ТаблицаВалютныхОстатков.Колонки.Количество() > 0 Тогда
			Для Каждого СтрокаОстаток Из ТаблицаВалютныхОстатков Цикл
				СтрокаДляПереоценки = ТаблицаДляПереоценки.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаДляПереоценки, СтрокаОстаток);
			КонецЦикла;
		КонецЕсли;	
		Возврат СвернутьТаблицуДляПереоценки(ТаблицаДляПереоценки);
	КонецЕсли;
...
Функция ПодготовитьТаблицуПереоценкаВалютныхОстатков(СтруктураПараметров, Отказ, Ошибки, ЭтоЭкспрессПроверка)
	...
	Для каждого СтрокаОстатка Из ВалютныеОстатки Цикл
		...
		Иначе	
			...
			//++ЭЛИАС 20141114  
			Если ТипЗнч(СтрокаОстатка.Субконто2) = Тип("СправочникСсылка.ДоговорыКонтрагентов") 
				И СтрокаОстатка.Субконто2.АР_АрендныйДоговор Тогда
				СтруктураКурса = АР_ПроцедурыНачисления.ПолучитьКурсВалюты(СтрокаОстатка.Валюта, Дата, СтрокаОстатка.Субконто2);
			КонецЕсли;
			//--ЭЛИАС 20141114
...
Функция ПодготовитьТаблицуВалютныхОстатковВсе(СтруктураПараметров, Движения)
	...
	//++ЭЛИАС 20140718
	ТаблицаВалютныхОстатков.Колонки.Добавить("УслугаАренды",         Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	ТаблицаВалютныхОстатков.Колонки.Добавить("ОбъектАренды",         Новый ОписаниеТипов("СправочникСсылка.АР_ОбъектыАренды"));
	ТаблицаВалютныхОстатков.Колонки.Добавить("СчетНаАренду",         Новый ОписаниеТипов("ДокументСсылка.АР_СчетНаАренду"));
	ТаблицаВалютныхОстатков.Колонки.Добавить("ПериодАренды",         ОбщегоНазначения.ОписаниеТипаДата(ЧастиДаты.Дата));
	ТаблицаВалютныхОстатков.Колонки.Добавить("Депозит",              Новый ОписаниеТипов("Булево"));
	ТаблицаВалютныхОстатков.Колонки.Добавить("Пени",                 Новый ОписаниеТипов("Булево"));
	
	// Для арендных договоров сумма переоцениваемого остатка определяется по регистру "Взаиморасчеты по аренде"
	ТаблицаВалютныхОстатковКопия = ТаблицаВалютныхОстатков.Скопировать();
	ТаблицаВалютныхОстатков.Очистить();
	Для Каждого СтрокаОстаток Из ТаблицаВалютныхОстатковКопия Цикл
		Если ТипЗнч(СтрокаОстаток.Субконто2) = Тип("СправочникСсылка.ДоговорыКонтрагентов") И СтрокаОстаток.Субконто2.АР_АрендныйДоговор Тогда
			...
			|				И ДокументРасчетов = &ДокументРасчетов) КАК АР_ВзаиморасчетыПоАрендеОстатки
			|ГДЕ
			|	АР_ВзаиморасчетыПоАрендеОстатки.СуммаВзаиморасчетовОстаток >= 0";
			
			Запрос.УстановитьПараметр("Организация", Организация);
			Запрос.УстановитьПараметр("Контрагент", СтрокаОстаток.Субконто1);
			Запрос.УстановитьПараметр("ДоговорКонтрагента", СтрокаОстаток.Субконто2);
			Запрос.УстановитьПараметр("ДокументРасчетов", СтрокаОстаток.Субконто3);
			...
			Пока Выборка.Следующий() Цикл
				...
				НоваяСтрока.СуммаВВалюте             = Выборка.СуммаВзаиморасчетовОстаток;
				НоваяСтрока.СуммаВРублях             = Выборка.СуммаОстаток;
				Если СтрокаОстаток.Счет.НалоговыйУчет Тогда
					НоваяСтрока.СуммаВРубляхНУ = Выборка.СуммаНУОстаток;
				Иначе
					НоваяСтрока.СуммаВРубляхНУ = Выборка.СуммаОстаток;
				КонецЕсли;
