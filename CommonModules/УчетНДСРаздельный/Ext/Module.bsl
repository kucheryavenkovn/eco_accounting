	...
	Для каждого СтрокаТаблицы Из ТаблицаУменьшениеВычета Цикл
		...
		Если НДС > 0 
		 ИЛИ СуммаБезНДС > 0 Тогда
			...
			НоваяСтрока.СчетФактура = Реквизиты.ДокументОснование;
...
Процедура СформироватьВозвратОтсторнированныхДвиженийКорректировкаПоступления(Реквизиты, СтруктураТаблицДокумента,
	...
	ТаблицаНДСПредъявленный = СтруктураТаблицДокумента.НДСПредъявленный;
	...
	ТаблицаНДСПредъявленный.ЗаполнитьЗначения(Реквизиты.Регистратор, "ИсправленныйСчетФактура");
	ТаблицаНДСПредъявленный.ЗаполнитьЗначения(Реквизиты.Период, "ДатаСобытия");
	ТаблицаНДСПредъявленный.ЗаполнитьЗначения(Перечисления.СобытияПоНДСПокупки.ПредъявленНДСКВычету, "Событие");
...
Функция ПолучитьОстаткиДляРаспределенияНДСПредъявленныйРеализация0(Реквизиты)
	...
	|				И Состояние = ЗНАЧЕНИЕ(Перечисление.НДССостоянияРеализация0.ПодтвержденаРеализация0)) КАК НДСПредъявленныйРеализация0Обороты";
...
Функция ПодготовитьТаблицуДвиженийНДСПредъявленныйРеализация0(Реквизиты, НДСДляОпераций0, НДСПредъявленныйРеализация0)
	...
	НДСКРаспределению         = НДСДляОпераций0.Итог("НДС");
	СуммаБезНДСКРаспределению = НДСДляОпераций0.Итог("СуммаБезНДС");
	
	НДСОстаток = НДСПредъявленныйРеализация0.Итог("НДС");
	СуммаБезНДСОстаток = НДСПредъявленныйРеализация0.Итог("СуммаБезНДС");
	ОбщееКоличество = НДСПредъявленныйРеализация0.Количество();
	Сч = 1;
	СчНДС = 0;
	СчСуммаБезНДС = 0;
		
	Для Каждого Стр Из НДСПредъявленныйРеализация0 Цикл
	...
	Для Каждого СтрокаТаблицы Из ВычетНДСНаОснованииДокумента Цикл
		...
		Если СтрокаТаблицы.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.Распределяется Тогда
			...
			Если СтрокаТаблицы.НДСДляОперацийПо0 <> 0 
				ИЛИ СтрокаТаблицы.СуммаБезНДСДляОперацийПо0 <> 0 Тогда
		...
		Иначе
			...
			Иначе
				...
				НДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(СуммаБезНДС, Реквизиты.НДСВключенВСтоимость, ЗначениеСтавкиНДС);
				Если СуммаБезНДС <> 0 И НДС <> 0 Тогда
					ДобавитьЗаписьВНДСРаздельныйУчет(ДвиженияНДСРаздельныйУчет, СтрокаТаблицы, Реквизиты,
					НДС, СуммаБезНДС, 0);
				КонецЕсли;
...
Процедура ДобавитьЗаписьВНДСРаздельныйУчет(ДвиженияНДСРаздельныйУчет, СтрокаТаблицы, Реквизиты, НДС, СуммаБезНДС, Количество)
	...
	АналитикаУчетаЗатрат = Новый Структура("Организация,СчетЗатрат,Подразделение,Субконто1,Субконто2,Субконто3");
		...
		НоваяСтрока.АналитикаУчетаНДС		=
			Справочники.КлючиАналитикиУчетаНДС.КлючиАналитикиУчетаНДСДокумента(АналитикаУчетаНДС);
		НоваяСтрока.Партия              	=
			?(НДС < 0, Реквизиты.ДокументПоступленияСсылка, Реквизиты.Регистратор);
