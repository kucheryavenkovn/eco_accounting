...
Функция ПараметрыЭлектронногоПисьма(НаборПечатныхФорм, МенеджерПечати, ДополнительныеПараметры = Неопределено) Экспорт
	...
	//++ЭЛИАС 20171201 
	Если ДополнительныеПараметры.Свойство("АР_БыстраяОтправка") Тогда
		ПараметрыПисьма.Вставить("АР_БыстраяОтправка", ДополнительныеПараметры.АР_БыстраяОтправка);
	КонецЕсли;
	Если ДополнительныеПараметры.Свойство("АР_УчетнаяЗапись") Тогда
		ПараметрыПисьма.Вставить("АР_УчетнаяЗапись", ДополнительныеПараметры.АР_УчетнаяЗапись);
	КонецЕсли;
	Если ДополнительныеПараметры.Свойство("АР_ПодписиИПечати") Тогда
		ПараметрыПисьма.Вставить("АР_ПодписиИПечати", ДополнительныеПараметры.АР_ПодписиИПечати);
	КонецЕсли;
	//--ЭЛИАС 20171201
