...
#КонецОбласти
	...
	Обработчик.Процедура           = "ПомеченныеНаУдаление.УстановитьОтображениеПомеченныхНаУдаление";
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "3.0.79.1";
	Обработчик.Процедура           = "УчетЗарплаты.НастроитьПорядокЗаполненияСТДР";
	Обработчик.НачальноеЗаполнение = Истина;
	...
	Обработчик.РежимВыполнения     = "Отложенно";
	Обработчик.Идентификатор       = Новый УникальныйИдентификатор("76890e19-a04a-4fdf-a41b-eb718cd97fc8");
...
Процедура РазделитьЗадолженностьВДокументах() Экспорт
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Для Каждого СтрокаКредиторки Из СтрокиКредиторки Цикл
			...
			//++ЭЛИАС 20120712 Перенесем подчиченные строки в ТЧ АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеКт
			МассивПодчиненныхСтрок = ДокументОбъект.АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеДт.НайтиСтроки(Новый Структура("УдалитьКлюч", СтрокаКредиторки.УдалитьАР_Ключ));
			Для Каждого ТекущаяПодчиненнаяСтрока Из МассивПодчиненныхСтрок Цикл
				НоваяСтрокаДоп = ДокументОбъект.АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеКт.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаДоп, ТекущаяПодчиненнаяСтрока);
				ДокументОбъект.АР_ДополнительнаяАналитикаВзаиморасчетовПоАрендеДт.Удалить(ТекущаяПодчиненнаяСтрока);
			КонецЦикла;
			//--ЭЛИАС 20120712
...
Процедура УстановитьДатуТекущейВерсии() Экспорт
	...
	Константы.ДатаТекущейВерсии.Установить(Дата(2020,09,24));
