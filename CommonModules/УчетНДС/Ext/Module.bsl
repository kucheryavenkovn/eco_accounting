...
Процедура СформироватьКорректировочныеДвиженияКорректировкаРеализации(Реквизиты, ТаблицаНДСВычет, ТаблицаНДСВосстановление, Движения, Отказ)
	...
	СформироватьДвиженияПредъявленНДС(ТаблицаНДСПредъявленный, Движения, Отказ, Истина);
...
Функция ТабличныйДокументКорректировочногоСчетаФактуры(Макет, МассивОбъектов, ОбъектыПечати, ТабДокумент, ТекстЗапросаПоСчетамФактурам, ПараметрыПечати)
	...
	ДанныеСчетовФактур = ПолучитьДанныеДляПечатиКорректировочныхСчетовФактур1137(МассивОбъектов, ТекстЗапросаПоСчетамФактурам,, ПараметрыПечати); //++ЭЛИАС 20120706
...
Функция ПодготовитьДанныеШапкиСчетаФактуры1137(ВыборкаСФ, СтрокаТаблицыРеквизиты, ПлатежноРасчетныеДокументы, ДополнитьДаннымиУПД)
	...
	Если ЗначениеЗаполнено(Реквизиты.Валюта) Тогда
		...
		Если Не ЗначениеЗаполнено(Реквизиты.ДатаОснования) Тогда
		СтруктураКурса  = РаботаСКурсамиВалют.ПолучитьКурсВалюты(Реквизиты.Валюта, Реквизиты.ДокументОснование.Дата);
		Иначе
...
Функция ПодготовитьДанныеШапкиКорректировочногоСчетаФактуры1137(ВыборкаСФ, СтрокаТаблицыРеквизиты, ДополнитьДаннымиУПД) Экспорт
	...
	Если Не ДополнитьДаннымиУПД
		...
		И ЗначениеЗаполнено(ПараметрыСводногоСФПоКомиссии) Тогда
...
//
Функция ПодготовитьДанныеДляПечатиСчетовФактур(ДокументОснование, ДанныеСчетаФактуры, 
	...
	//++ЭЛИАС 20120706
	СчетФактураСводный = ТипЗнч(ПараметрыПечати) = Тип("Структура") И ПараметрыПечати.Свойство("СчетФактураСводный");
	Запрос.УстановитьПараметр("СчетФактураСводный", СчетФактураСводный);
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты")
		ИЛИ (ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации") 
		И ТипЗнч(ДокументОснование.ДокументРеализации) = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты")) Тогда
		Запрос.УстановитьПараметр("ЭтоАренда", Истина);
		Запрос.УстановитьПараметр("ЭтоПени", Ложь);
		Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты") Тогда
			Запрос.УстановитьПараметр("ПредставлениеПериода", ПредставлениеПериода(ДокументОснование.НачалоПериода, КонецДня(ДокументОснование.ОкончаниеПериода)));
		Иначе
			Запрос.УстановитьПараметр("ПредставлениеПериода", ПредставлениеПериода(ДокументОснование.ДокументРеализации.НачалоПериода, КонецДня(ДокументОснование.ДокументРеализации.ОкончаниеПериода)));
		КонецЕсли;
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.АР_НачислениеПени")
		ИЛИ (ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации") 
		И ТипЗнч(ДокументОснование.ДокументРеализации) = Тип("ДокументСсылка.АР_НачислениеПени")) Тогда
		Запрос.УстановитьПараметр("ЭтоАренда", Истина);
		Запрос.УстановитьПараметр("ЭтоПени", Истина);
		Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.АР_НачислениеПени") Тогда
			Запрос.УстановитьПараметр("ПредставлениеПериода", Формат(ДокументОснование.Дата,"ДФ='дд.ММ.гггг'"));
		Иначе
			Запрос.УстановитьПараметр("ПредставлениеПериода", Формат(ДокументОснование.ДокументРеализации.Дата,"ДФ='дд.ММ.гггг'"));
		КонецЕсли;
	Иначе
		Запрос.УстановитьПараметр("ЭтоАренда", Ложь);
		Запрос.УстановитьПараметр("ЭтоПени", Ложь);
		Запрос.УстановитьПараметр("ПредставлениеПериода", "");
	КонецЕсли;
	//--ЭЛИАС 20120706
...
Функция ПодготовитьДанныеДляПечатиКорректировочныхСчетовФактур(ДокументОснование, СчетФактура, ВидСчетаФактуры, ИспользуетсяПостановлениеНДС1137, 
	ПараметрыПечати = Неопределено) Экспорт //++ЭЛИАС 20120706
...
Функция ПолучитьДанныеДляПечатиСчетаФактуры1137(МассивОбъектов, ТекстЗапросаПоСчетамФактурам, ДополнитьДаннымиУПД = Ложь,
	...
	Пока ВыборкаСФ.Следующий() Цикл
		...
		Пока ВыборкаПоОснованиям.Следующий() Цикл
			...
			ПараметрыОснования = ПодготовитьДанныеДляПечатиСчетовФактур(ВыборкаПоОснованиям.ДокументОснование, ДанныеСчетаФактуры, ПараметрыПечати); //++ЭЛИАС 20120706
...
Функция ПолучитьДанныеДляПечатиКорректировочныхСчетовФактур1137(МассивОбъектов, ТекстЗапросаПоСчетамФактурам, ДополнитьДаннымиУПД = Ложь,
	ПараметрыПечати = Неопределено) Экспорт //++ЭЛИАС 20120706
	...
	Пока ВыборкаСФ.Следующий() Цикл
		...
		Пока ВыборкаПоОснованиям.СледующийПоЗначениюПоля("ДокументОснование") Цикл
			...
			ПараметрыОснования = ПодготовитьДанныеДляПечатиКорректировочныхСчетовФактур(
				ВыборкаПоОснованиям.ДокументОснование, ВыборкаСФ.СчетФактура, ВыборкаСФ.ВидСчетаФактуры, Истина, ПараметрыПечати); //++ЭЛИАС 20130114
...
Процедура ПечатьКомплектаСчетовФактур(КоллекцияПечатныхФорм, МассивОбъектов, ОбъектыПечати, ПараметрыПечати, ПараметрыВывода = Неопределено) Экспорт
	...
	Для Каждого МакетСчетаФактуры Из МакетыСчетовФактур Цикл
		...
		Если ВыводитьМакет И СтрокаКоллекции <> Неопределено Тогда
			...
			//++ЭЛИАС 20191009 
			Если ТипЗнч(ПараметрыПечати) = Тип("Структура") И ПараметрыПечати.Свойство("СчетФактураСводный") Тогда
				СтрокаКоллекции.СинонимМакета = СтрокаКоллекции.СинонимМакета + " (сводный по услугам)";
			КонецЕсли;
			//--ЭЛИАС 20191009
...
Функция ПолучитьДанныеДляПечатиУниверсальногоПередаточногоДокумента(МассивОбъектов, ТекстЗапросаДокументам, ФормированиеЭД = Ложь, ПараметрыПечати = Неопределено) Экспорт
//--ЭЛИАС 20171020
	...
	Пока ВыборкаПоОснованиям.Следующий() Цикл
		...
		ПараметрыОснования = ПодготовитьДанныеДляПечатиУниверсальныхПередаточныхДокументов(ВыборкаПоОснованиям.ДокументОснование, ДанныеСчетаФактуры, ФормированиеЭД, ПараметрыПечати); //++ЭЛИАС 20171020 
...
Функция ПодготовитьДанныеДляПечатиУниверсальныхПередаточныхДокументов(ДокументОснование, ДанныеСчетаФактуры, ФормированиеЭД = Ложь, ПараметрыПечати = Неопределено)
//--ЭЛИАС 20171020
	...
	Иначе
		...
		ДанныеДляПечати = ПодготовитьДанныеДляПечатиСчетовФактур(ДокументОснование, ДанныеСчетаФактуры, ПараметрыПечати);
		//--ЭЛИАС 20171020
	КонецЕсли;
...
Функция ТабличныйДокументУПД(Макет, МассивОбъектов, ОбъектыПечати, ТабДокумент, ТекстЗапросаДокументам, ТолькоПередаточныйДокумент, ПараметрыПечати)
	...
	ИначеЕсли ТолькоПередаточныйДокумент Тогда
		ДанныеУниверсальныхПередаточныхДокументов = ПолучитьДанныеДляПечатиУниверсальногоПередаточногоДокумента(
			...
			МассивОбъектов, ТекстЗапросаДокументам,, ПараметрыПечати);
			//--ЭЛИАС 20171020
	Иначе
		ДанныеУниверсальныхПередаточныхДокументов = ПолучитьДанныеДляПечатиСчетаФактуры1137(
			...
			МассивОбъектов, ТекстЗапросаДокументам, Истина, ПараметрыПечати);
			//--ЭЛИАС 20171016
	КонецЕсли;
