...
Функция ПолучитьСведенияОСпискеНоменклатуры(Знач СписокНоменклатуры, ПараметрыОбъекта, ОпределятьСчета = Истина, ОпределятьЦены = Истина) Экспорт
	...
	ИменаРеквизитов =
		...
		|ПродукцияМаркируемаяДляГИСМ, ТабачнаяПродукция, ОбувнаяПродукция, ПериодичностьУслуги";
...
// 
Функция ПолучитьСчетаУчетаНоменклатуры(Организация, Номенклатура, Склад = Неопределено, Дата = Неопределено, ОсобыйПорядокНалогообложения = Ложь, ОбъектАренды = Неопределено) Экспорт //++ЭЛИАС 20121220  
	...
	СведенияОНоменклатуре = ПолучитьСчетаУчетаСпискаНоменклатуры(
		Организация, СписокНоменклатуры, Склад, Дата, ОсобыйПорядокНалогообложения, ОбъектАренды).Получить(Номенклатура); //++ЭЛИАС 20121220
...
//
Функция ПолучитьСчетаУчетаСпискаНоменклатуры(Организация, СписокНоменклатуры, Склад = Неопределено, Знач Дата = Неопределено, Знач ОсобыйПорядокНалогообложения = Ложь, ОбъектАренды = Неопределено) Экспорт //++ЭЛИАС 20130502  
	...
	//++ЭЛИАС 20121220  
	МассивТиповОбъектовАренды = Новый Массив();
	МассивТиповОбъектовАренды.Добавить(Справочники.АР_ТипыОбъектовАренды.ПустаяСсылка());
	Если ЗначениеЗаполнено(ОбъектАренды) Тогда
		МассивТиповОбъектовАренды.Добавить(ОбъектАренды.ТипОбъекта);
	КонецЕсли;
	//--ЭЛИАС 20121220
	...
	//++ЭЛИАС 20121220  
	Запрос.УстановитьПараметр("ТипОбъектаАренды",	МассивТиповОбъектовАренды);
	//--ЭЛИАС 20121220	
	Если ОсобыйПорядокНалогообложения = Неопределено Тогда
	...
	|ВЫБРАТЬ
	//++ЭЛИАС 20121220  
	|	СчетаУчетаНоменклатуры.АР_ТипОбъектаАренды КАК ТипОбъектаАренды,
	|	ВЫБОР
	|		КОГДА СчетаУчетаНоменклатуры.АР_ТипОбъектаАренды = ЗНАЧЕНИЕ(Справочник.АР_ТипыОбъектовАренды.ПустаяСсылка)
	|			ТОГДА 0
	|		ИНАЧЕ 1
	|	КОНЕЦ КАК УровеньАренды,
	//--ЭЛИАС 20121220
	...
	//++ЭЛИАС 20121220  
	|	И СчетаУчетаНоменклатуры.АР_ТипОбъектаАренды В (&ТипОбъектаАренды)
	//--ЭЛИАС 20121220
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
