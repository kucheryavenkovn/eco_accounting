...
Процедура ПриСозданииНоменклатуры(Владелец, ДанныеСтроки, СтандартнаяОбработка, ВидПродукцииИС) Экспорт
	...
	ИначеЕсли ВидПродукцииИС = ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.МолочнаяПродукция") Тогда
		ПараметрыФормы.Вставить("МолочнаяПродукция", Истина);
...
Функция ПараметрыОтбора(ВидПродукцииИС)
	...
	Если ВидПродукцииИС.Найти(ПредопределенноеЗначение("Перечисление.ВидыПродукцииИС.МолочнаяПродукция")) <> Неопределено Тогда
		ПараметрыОтбора.Добавить("МолочнаяПродукция");
	КонецЕсли;
