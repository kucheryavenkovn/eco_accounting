...
Функция ПолучитьСсылкуAppStore() Экспорт
	
	КомандаСервиса = НоваяКомандаСервиса();
	КомандаСервиса.URLЗапроса = "https://ocr.1c.ai/mobile/appstore";
	...
	Возврат РезультатКоманды.Ответ;
	   ...
	   И Результат.КодСостояния <> КодСостояния_402_PaymentRequired() Тогда
		...
		ТекстОшибки = СтрШаблон(
			...
			КомандаСервиса.Метод + ?(
				КомандаСервиса.Свойство("Данные"),
				НСтр("ru = '
				           |Тело запроса:
				           |'") + Лев(КомандаСервиса.Данные, 800),
				""
			),
			Лев(Результат.ПолучитьТелоКакСтроку(), 800));
