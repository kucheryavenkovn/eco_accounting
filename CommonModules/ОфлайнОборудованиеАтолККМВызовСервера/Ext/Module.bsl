...
Процедура ДобавитьТранзакциюВЧек(Смена, Смены, Транзакция, ФорматОбмена, Оплата = Ложь)
	...
	Если Оплата Тогда
		...
		Иначе
			...
			НоваяОплата.Вставить("ТипОплаты",		?(Транзакция.Поле10 = 0 ИЛИ ПустаяСтрока(Транзакция.Поле10) ИЛИ Транзакция.Поле10 = 1, 0, 1)); // 0 - предопределенный тип оплаты "наличные".
