...
Функция ДоляЕНВДиКомиссии(СтруктураШапкиДокумента, ТаблицаСписанныхПартий = Неопределено, ДвиженияРегистров = Неопределено, ВидОперации = "Оплата", ДоговорДляОтложенногоПроведения = Неопределено)
	...
	//++ЭЛИАС 20150721  
	//В услугах
	Если ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("СчетДоходов", МетаданныеДокумента, "Состав") Тогда
		...
		Запрос.Текст = СтрЗаменить(Текст, "АР_НачислениеАренднойПлаты", ИмяВидаДокумента);
	...
	Если ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("СчетДоходов", МетаданныеДокумента, "Состав") Тогда
		...
		|ГДЕ
		|	ТаблицаУслуги.Ссылка = &Ссылка
		|	И НЕ ТаблицаУслуги.Контрагент = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаУслуги.СчетДоходов";
		...
		СуммаАгентскихУслуг = ТаблицаРезультата.Итог("Сумма");
...
Процедура СформироватьДвиженияУСН(ДокументОбъект, СтруктураДопПараметров = Неопределено, ЭтоОбъект = Истина, ДоговорДляОтложенногоПроведения = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		//++ЭЛИАС 20150721  
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты")
		ИЛИ ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.АР_НачислениеПени") Тогда

		Если (ДокументСсылка.ДоговорКонтрагента.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем) Тогда
...
Функция ОписаниеСобытияДляКУДиР(СтруктураШапкиДокумента, ВидОперации, МетаданныеДокумента, ДоговорКонтрагента = Неопределено)
	...
	ИначеЕсли ВидОперации = "Реализация" Тогда
		...
		//++ЭЛИАС 20151124  
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты") Тогда
			СтрокаСодержание = "Начислена арендная плата арендатору """ + НаименованиеКонтрагентаНаПечать(ДокументСсылка.Контрагент, СтруктураШапкиДокумента.Дата)
				+ """ по договору """ +	ДокументСсылка.ДоговорКонтрагента +	""". ";
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.АР_НачислениеПени") Тогда
			СтрокаСодержание = "Начислены пени арендатору """ + НаименованиеКонтрагентаНаПечать(ДокументСсылка.Контрагент, СтруктураШапкиДокумента.Дата)
				+ """ по договору """ +	ДокументСсылка.ДоговорКонтрагента +	""". ";
		//--ЭЛИАС 20151124	
		КонецЕсли;
