...
Функция ДополнительныеСведенияИзОтветаБанкаПоУмолчанию(Банк, РеквизитыТранзакции, ДатаАктуальности)
	...
	ОтветБанка = ЗаявкиНаКредит.ОтветБанкаПоУмолчанию(
		Банк, РеквизитыТранзакции.ТипТранзакции, ДатаАктуальности);
...
Функция СведенияОбАкцепте(ЗаявкаНаКредит, Банк)
	...
	Если НЕ ЗначениеЗаполнено(ЗаявкаНаКредит) Тогда
		ВызватьИсключение СтрШаблон(ОписаниеОшибки, НСтр("ru='Не указана заявка 1С:Кредит'"));
	КонецЕсли;
