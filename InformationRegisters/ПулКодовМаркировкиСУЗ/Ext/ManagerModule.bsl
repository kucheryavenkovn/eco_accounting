...
Функция КодыМаркировкиПоОснованию(Основания, Напечатанные = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА
		|			ПулКодовМаркировкиСУЗ.ШтрихкодУпаковки = ЗНАЧЕНИЕ(Справочник.ШтрихкодыУпаковокТоваров.ПустаяСсылка)
		|		ТОГДА
		|			ПулКодовМаркировкиСУЗ.КодМаркировки
		|		ИНАЧЕ
		|			Представление(ПулКодовМаркировкиСУЗ.ШтрихкодУпаковки)
...
Процедура ЗаписатьДанныеКодаМаркировки(ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	
	Если Не ЗначениеЗаполнено(ДанныеШтрихкода.ШтрихкодBase64)
		Или (ШтрихкодированиеИСКлиентСервер.Base64ВШтрихкод(ДанныеШтрихкода.Штрихкод) = ДанныеШтрихкода.Штрихкод) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ДанныеШтрихкода.СпособВводаВОборот = Перечисления.СпособыВводаВОборотСУЗ.МаркировкаОстатков Тогда
