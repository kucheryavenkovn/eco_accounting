...
Процедура ОтборПериодНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	КонецПериодаВыбор  = ?(ЗначениеЗаполнено(КонецПериода), КонецПериода, КонецМесяца(ОбщегоНазначенияКлиент.ДатаСеанса()));
...
Процедура ПоказатьПериод()
	...
	ПериодОтчета = ЗаполнениеФинОтчетностиВБанкиКлиентСервер.ПредставлениеПериодаОтчета(НачалоПериода, КонецДня(КонецПериода));
...
Процедура ОткрытьФормуНовогоОтчета()
	...
	ОткрытьФормуНовогоОтчетаЗавершение(Истина, Неопределено);
...
Процедура УстановитьОтборы(Форма)
	...
	Иначе
		...
		Иначе
			...
			ОтборТипБанка.ПравоеЗначение = Тип("СправочникСсылка.Банки");
		КонецЕсли;
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.СписокНастроитьОтправку.Видимость = БанкВыбран И НЕ ЭтоБанкУниверсальногоОбмена И Форма.ЕстьПравоРедактированияОтчета;
...
&НаКлиенте
Процедура ОжидатьЗавершениеОбновленияДанныхСервиса(ВыводитьОкноОжидания = Ложь, ОбработчикПослеЗавершения = Неопределено)
...
Процедура ОбновлениеДанныхСервисаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(Результат, "Статус", "") = "Ошибка" Тогда
		...
		Подстроки.Добавить(НСтр("ru = 'В процессе обновления данных сервиса 1С:ФинОтчетность возникла ошибка.'"));
