...
Функция УстановитьСтатусыДокументов(МассивДокументов, СтатусыДокумента) Экспорт
	...
	Если Не ИзменитьСтатус 
		...
		И НЕ ИзменитьСтатусОригиналаСФ Тогда
		Возврат СтатусИзменен;
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если (Не ИзменитьСтатус Или Выборка.Статус = СтатусыДокумента.Статус)
			...
			И (Не ИзменитьСтатусОригиналаСФ Или Выборка.СтатусОригиналаСчетаФактуры = СтатусыДокумента.СтатусОригиналаСФ) Тогда
			Продолжить;
		КонецЕсли;
		...
		// в которых дополнительно отображается статус счета-фактуры, например в списке "Поступления (акты, накладные)"
...
Функция СтатусНовогоДокумента(Документ)
	...
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.ПоступлениеТоваровУслуг")
		...
		//++ЭЛИАС 20171107 
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.АР_СчетНаАренду") Тогда
		
		Результат.Статус = Перечисления.СтатусОплатыСчета.СтатусНовогоДокумента();
		Результат.ДополнительныйСтатус = Перечисления.СтатусыОтгрузки.СтатусНовогоДокумента();
		
	ИначеЕсли ТипДокумента = Тип("ДокументСсылка.АР_НачислениеАренднойПлаты")
		Или ТипДокумента = Тип("ДокументСсылка.АР_НачислениеПени") Тогда
