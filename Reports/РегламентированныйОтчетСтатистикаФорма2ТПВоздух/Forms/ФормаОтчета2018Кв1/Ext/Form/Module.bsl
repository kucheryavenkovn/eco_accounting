...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Область.Имя = "ORG_SCALE" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ORG_SCALE");
		КодПоказателя = ПараметрВводаПоля.Показатель;
		КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе,
			ТЗВыбора, СтандартнаяОбработка, Неопределено);
	ИначеЕсли Область.Имя = "FISH_CAT" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "FISH_CAT");
		КодПоказателя = ПараметрВводаПоля.Показатель;
		КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе,
			ТЗВыбора, СтандартнаяОбработка, Неопределено);
	ИначеЕсли Область.Имя = "OBJ_EXPLOITATION" Тогда
		ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "OBJ_EXPLOITATION");
		КодПоказателя = ПараметрВводаПоля.Показатель;
		КопироватьДанныеФормы(ПараметрВводаПоля.ТаблицаЗначений, ТЗВыбора);
		ВвестиПоказатель(Элемент, Область, КодПоказателя, Ложь, ПараметрВводаПоля.ТекстПриВыборе,
			ТЗВыбора, СтандартнаяОбработка, Неопределено);
	...
	Для Каждого Элт Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СпПарам, ",", Истина) Цикл 
		Если Не ЗначениеЗаполнено(ПараметрыВыгрузки[Элт]) Тогда 
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект,
				"Не заполнен обязательный показатель!", "ПолеТабличногоДокументаФормаОтчета", Элт);
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция СформироватьСтруктуруПараметров()
	...
	|ОргИндекс, ОргСубъект, ОргРайон, ОргГород, ОргУлица, ОргДом, ОргКорпус, ОргКвартира,
	|БанкСчетНомер, БанкСчетНаимБанка, БанкСчетКоррСчетБанка, БанкСчетБИКБанка, ОргКодСубъект,
	|ТелОрганизации, ФаксОрганизации, ЭлектроннаяПочта, ФИОРук, ТелРук, ФИОБух, ТелБух";
	...
	АдрЮРНасПункт = АдрЮРНасПункт
		+ ?(ПустаяСтрока(АдрЮРНасПункт) ИЛИ ПустаяСтрока(СведенияОбОрганизации.ОргРайон), "", ", ")
		+ СокрЛП(СведенияОбОрганизации.ОргРайон);
	АдрЮРНасПункт = АдрЮРНасПункт
		+ ?(ПустаяСтрока(АдрЮРНасПункт) ИЛИ ПустаяСтрока(СведенияОбОрганизации.ОргГород), "", ", ")
		+ СокрЛП(СведенияОбОрганизации.ОргГород);
	ПараметрыВыгрузки.Вставить("АдрЮРНасПункт", АдрЮРНасПункт);
	
	АдрЮРУлицаДом = СокрЛП(СведенияОбОрганизации.ОргУлица);
	АдрЮРУлицаДом = АдрЮРУлицаДом
		+ ?(ПустаяСтрока(АдрЮРУлицаДом) ИЛИ ПустаяСтрока(СведенияОбОрганизации.ОргДом), "", ", ")
		+ СокрЛП(СведенияОбОрганизации.ОргДом);
	АдрЮРУлицаДом = АдрЮРУлицаДом
		+ ?(ПустаяСтрока(АдрЮРУлицаДом) ИЛИ ПустаяСтрока(СведенияОбОрганизации.ОргКорпус), "", ", ")
		+ СокрЛП(СведенияОбОрганизации.ОргКорпус);
	АдрЮРУлицаДом = АдрЮРУлицаДом
		+ ?(ПустаяСтрока(АдрЮРУлицаДом) ИЛИ ПустаяСтрока(СведенияОбОрганизации.ОргКвартира), "", ", ")
		+ СокрЛП(СведенияОбОрганизации.ОргКвартира);
