			...
			Для НомСтр = ВерхОбласти По НизОбласти Цикл
				...
				Если КодПоказателя <> "###" Тогда
					...
					Если КодПоказателя = "0" И Область.Имя = "ФормыРеорганизации"
						...
						И ТекущаяДатаСеанса() >= '20210101' Тогда
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	// ФНС России от 31.10.2019 № БС-4-21/22311@ и от 10.06.2020 № БС-4-21/9576@.
	//
	Если НЕ (Год(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) = 2020
		И ТекущаяДатаСеанса() >= '20210101') Тогда
	...
	Если Год(СтруктураРеквизитовФормы.мСохраненныйДок.ДатаОкончания) = 2020 Тогда
		...
		Иначе
			...
			Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.ФормРеорг) Тогда
				СообщитьВТС("В декларации за 2020 год должен быть заполнен код формы реорганизации (ликвидации). Основание: письмо ФНС России от 10.06.2020 № БС-4-21/9576@",
				"Титульный", "КодФормыРеорганизации");
				СтатусПроверки = Ложь;
			Иначе
				Если ВозможныеКодыФормыРеорганизции.Найти(ПараметрыВыгрузки.ФормРеорг) = Неопределено Тогда
					СообщитьВТС("В декларации за 2020 год код формы реорганизации (ликвидации) должен быть равен одному из следующих значений: "
					+ СтрСоединить(ВозможныеКодыФормыРеорганизции, ", ") + ". Основание: письмо ФНС России от 10.06.2020 № БС-4-21/9576@",
					"Титульный", "КодФормыРеорганизации");
					СтатусПроверки = Ложь;
...
Процедура Печать(ВидПечати, НеИзФормыОтчета = Ложь) Экспорт
	...
	Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 2020 Тогда
		...
		Иначе
			...
			Если НЕ ЗначениеЗаполнено(КодФормыРеорганизации) Тогда
				ТекстСообщения
				= "В декларации за 2020 год должен быть заполнен код формы реорганизации (ликвидации). Основание: письмо ФНС России от 10.06.2020 № БС-4-21/9576@";
			Иначе
				ДопустимыеКодыФормыРеорганизации = "0, 1, 2, 3, 5, 6";
				Если СтрРазделить(ДопустимыеКодыФормыРеорганизации, ", ", Ложь).Найти(КодФормыРеорганизации) = Неопределено Тогда
					ТекстСообщения
					= "В декларации за 2020 год код формы реорганизации (ликвидации) должен быть равен одному из следующих значений: "
					+ ДопустимыеКодыФормыРеорганизации + ". Основание: письмо ФНС России от 10.06.2020 № БС-4-21/9576@";
				КонецЕсли;
			КонецЕсли;
