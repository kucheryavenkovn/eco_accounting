...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Если ПараметрыОтчета.Период < '20200101' Тогда
		// До 2020 года
		ИмяГруппировки = "РасчетЗемельногоНалога2019";
	ИначеЕсли КонецКвартала(ПараметрыОтчета.Период) = КонецДня('20200630') И ПрименяетсяОсвобождениеОтУплатыНалога Тогда
		...
		ИмяГруппировки = "РасчетЗемельногоНалога2020_Аванс_Освобождение";
	ИначеЕсли КонецКвартала(ПараметрыОтчета.Период) <> КонецГода(ПараметрыОтчета.Период) Тогда
		...
		ИмяГруппировки = "РасчетЗемельногоНалога2020_Аванс";
