...
Процедура ДополнитьРеквизитыФормы()
	...
	Для Каждого Стр Из КатегорииКлассификатора Цикл
		...
		ИмяДобавляемогоРеквизита = "П0000101" + Стр;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Не СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда
		...
		Если СтрНачинаетсяС(Область.Имя, "П00001") И СтрНайти(Область.Имя, "09_") > 0 Тогда
			...
			КонецЕсли;
			Возврат;
		ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00002") И Прав(Область.Имя, 2) = "05" Тогда
		...
		ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00001") И СтрНайти(Область.Имя, "03_") > 0 Тогда
			...
			КонецЕсли;
			Возврат;
		ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00002") И Прав(Область.Имя, 2) = "04" Тогда
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтотОбъект, "ВсеОКОФ");
			Если ПараметрВводаПоля <> Неопределено И ПараметрВводаПоля.ТаблицаЗначений <> Неопределено Тогда
				СтандартнаяОбработка = Ложь;
				КолонкаПоиска = "Код";
				КодПоказателя = ПараметрВводаПоля.Показатель;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПечатнаяФорма.Вывести(ПолеТабличногоДокументаФормаОтчета.ПолучитьОбласть("ОсновнаяЧасть1_1"));
	ПечатнаяФорма.Вывести(ПолеТабличногоДокументаФормаОтчета.ПолучитьОбласть("ОсновнаяЧасть1_2"));
	...
	СекцияПодвал = ПолеТабличногоДокументаФормаОтчета.ПолучитьОбласть("Подвал");
	Если ПечатнаяФорма.ПроверитьВывод(СекцияПодвал) Тогда
		ПечатнаяФорма.Вывести(СекцияПодвал);
	...
	СекцияОсновнаяЧасть2 = ПолеТабличногоДокументаФормаОтчета.ПолучитьОбласть("ОсновнаяЧасть2");
	Если ПечатнаяФорма.ПроверитьВывод(СекцияОсновнаяЧасть2) Тогда
		ПечатнаяФорма.Вывести(СекцияОсновнаяЧасть2);
	Иначе
		СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 11(сделка)");
		ПечатнаяФорма = РегламентированнаяОтчетность.НоваяСтраница();
		ПечатнаяФорма.Вывести(СекцияОсновнаяЧасть2);
...
Процедура СформироватьСпискиВыбора()
	...
	Для Каждого КЗ Из СпискиВыбораПоКатегориям Цикл 
		РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(
			СвойстваПоказателей, "П0000101" + КЗ.Ключ + "03", , , , СпискиВыбораПоКатегориям[КЗ.Ключ]);
	КонецЦикла;
...
Процедура ВвестиПоказательЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если СтрНачинаетсяС(Область.Имя, "П00001") Тогда
		...
		Если ОСм.Имя <> Область.Имя Тогда
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли СтрНачинаетсяС(Область.Имя, "П00002") Тогда
		ОСм = ПолеТабличногоДокументаФормаОтчета.Области.Найти(СтрЗаменить(Область.Имя, "04", "01"));
		Если ОСм.Имя <> Область.Имя Тогда 
			Если ЗначениеЗаполнено(Область.Значение) Тогда
				ОСм.Значение = РезультатВыбора["Название"];
			Иначе
